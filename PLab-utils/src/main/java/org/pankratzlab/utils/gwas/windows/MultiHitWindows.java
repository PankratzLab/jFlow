...
public class MultiHitWindows {
  ...
  private static class MultiHitWindow implements Comparable<MultiHitWindow> {
    ...
    private static final String[] HEADER = new String[] {"MARKER", "CHR", "START", "END",
                                                         "NUM_INDEX", "NUM_SUGGESTIVE", "NUM_TOTAL",
                                                         "P-VAL"};
   ...
   */
  public static void generateHitWindows(String outSuffix, PFileStructure pFile,
                                        WindowThreshold hitParams, int genomeBuild) {
  ...
  public static void generateHitWindows(String outSuffix, PFileStructure pFile,
                                        WindowThreshold hitParams, int genomeBuild,
                                        String markerSetFile, Logger log) {

    Multimap<String, MultiHitWindow> hitWindows = findHitWindows(pFile, hitParams, genomeBuild,
                                                                 markerSetFile, log);
  ...
  public static void main(String... args) {
    ...
    c.addArgWithDefault(INDEX_PVAL, "p-value threshold for defining a window",
                        Double.toString(WindowThreshold.DEFAULT_INDEX_PVAL), CLI.Arg.NUMBER);
    c.addArgWithDefault(SUGGESTIVE_PVAL, "p-value threshold for extending a window",
                        Double.toString(WindowThreshold.DEFAULT_SUGGESTIVE_PVAL), CLI.Arg.NUMBER);
    ...
    pFile.markers(c.get(MARKER_COLUMNS).split(",")).chrs(c.get(CHR_COLUMNS).split(","))
         .pos(c.get(POS_COLUMNS).split(",")).pvals(c.get(PVAL_COLUMNS).split(","));
