package org.pankratzlab.utils.stats;
...
public class WeightedSumStatistic {
  ...
  public WeightedSumStatistic(String plinkRoot) {
    ...
    subset = ArrayUtils.arrayOfIndices(markerNames.length);
    valid_subset = subset;
    trait = ArrayUtils.toDoubleArray(affectionStatus);
    binary = true;
    mafThreshold = 1;
    reset();
  }
  ...
  public String[] getMarkerNames() {
    return ArrayUtils.subArray(markerNames, valid_subset);
  }
  ...
  public double[] getPermutations() {
    ...
    if (perms == null) {
      ...
      for (int i = 0; i < NUM_REPS; i++) {
        ...
        if (binary) {
          perms[i] = sumOfRanks(orderedTraits, permScores);
        } else {
          perms[i] = Correlation.Spearman(new double[][] {orderedTraits, permScores})[0];
  ...
  public void setTrait(double[] trait) {
    ...
    this.trait = trait;
    type = ArrayUtils.determineType(trait, true);
    if (type == -1) {
      System.exit(1);
    }
    binary = type == 0;
  ...
  public void validateSubset() {
    ...
    if (mafThreshold < 1) {
      ...
      valid_subset = new int[ArrayUtils.booleanArraySum(passes)];
  ...
  public static void demo(String dir, String root) {
    ...
    try {
      ...
      for (int i = 0; i < scores.length; i++) {
        writer.println(scores[i] + "\t" + ArrayUtils.toStr(markerCounts[i]));
      }
      writer.close();
    } catch (Exception e) {
    ...
    mean = ArrayUtils.mean(repXs);
    stdev = ArrayUtils.stdev(repXs);
  ...
  public static double sumOfRanks(double[] affectionStatus, double[] scores) {
    ...
    ranks = Sort.getRanks(scores, true);
    sum = 0;
