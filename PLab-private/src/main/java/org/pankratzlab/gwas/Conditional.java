...
public class Conditional {
  ...
  // newCovariatesFile, String allPossibleSNPs, Logger log) {
  // addCountsAsCovariate(baseDir, "../plink", originalCovariatesFile, newCovariatesFile,
  // allPossibleSNPs, log);
  // }
  ...
  public static void addCountsAsCovariate(String baseDir, String subDir,
    ...
    if (allPossibleSNPs == null) {
      ...
      // dir);
      // CmdLine.run("C:/plink/plink --bfile "+plinkFiles+" --extract
      // ../"+(baseDir.equals("")?"":"../")+ext.removeDirectoryInfo(allPossibleSNPs)+" --make-bed",
      // dir);
    ...
    if (!ArrayUtils.equals(markerNames, HashVec.loadFileToStringArray(dir + "plink.frq", true,
                                                                      new int[] {1}, false),
                           true)) {
  ...
  // public static void addCountsAsCovariate(String baseDir, String plinkFiles, String
  // originalCovariatesFile, String newCovariatesFile, String allPossibleSNPs, Logger log) {
  // BufferedReader reader;
  // PrintWriter writer;
  // String[] line;
  // Hashtable<String,String> hash;
  ...
  // new File(dir).mkdirs();
  //
  // if (!new File(plinkFiles+".bed").exists() || !new File(plinkFiles+".bim").exists() || !new
  // File(plinkFiles+".fam").exists()) {
  // log.reportError("Error - a full complement of binary PLINK files are required to perform this
  // algorithm");
  // return;
  // }
  // if (allPossibleSNPs == null) {
  // log.report("All markers in the dataset will be added to the covars file, since no subset was
  // pre-defined");
  ...
  // }
  // CreateDatabaseFromPlink.createDatabase(dir, dir+"plink.crf", log);
  // if (!Array.equals(markerNames, HashVec.loadFileToStringArray(dir+"plink.frq", true, new int[]
  // {1}, false), true)) {
  // log.reportError("Error - the freq file does not match the map file");
  // return;
  // }
  ...
  // v.add(line[i]);
  // }
  // hash = HashVec.loadFileToHashString(baseDir+originalCovariatesFile, new int[] {0,1}, indices,
  ...
  // } else {
  // log.reportError("Error - no covariate information for individual "+line[0]+"-"+line[1], true,
  // true, 8);
  // }
  // }
  // reader.close();
  ...
  public static void addDosageAsCovariate(String dir, String infoFile, String doseFile,
    ...
    if (v.size() > 0) {
      if (Mach.extractSpecificMarkers(dir, dir + "condis.txt", dir + doseFile, dir + infoFile,
        ...
        for (String markerName : markerNames) {
          SerialHash.createSerializedStringHash(dir + markerName + ".ser", hashes.get(markerName));
        }
      } else {
  ...
  public static void run(String[] dirs, String filenames, String[] snps, String annotationFile,
    ...
    for (int k = 0; k < dirs.length; k++) {
      if (preferGenotyped && new File(dirs[k] + "plink.bed").exists()
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      while (!done) {
          ...
          if (dosageSets[k]) {
            ...
            if (new File(dirs[k] + pheno).exists()) {
              ...
              addDosageAsCovariate(dirs[k], infoFile, doseFile, pheno, justIID, outfile,
                                   ArrayUtils.toStringArray(v), allowMissingConditional,
                                   useResiduals, log);
            } else {
        ...
        if ((run || iterate) && dirs.length > 1) {
          try {
            writer = Files.openAppropriateWriter(ext.rootOf(outfile, false) + ".Nweighted.batch");
        ...
        if (iterate) {
          ...
          for (int j = 0; j < pvals.length; j++) {
            if (Double.parseDouble(pvals[j][1]) < 0.05
                && Math.abs(Double.parseDouble(pvals[j][4])) < 5) {
              countSig++;
      ...
      if (iterate) {
        ...
        try {
          ...
          writer.print("Marker\tChr\tPosition"
                       + (annotationHeader.length > 0 ? "\t"
                                                        + ArrayUtils.toStr(annotationHeader,
          ...
          for (int j = 0; j < keys.length; j++) {
            ...
            if (annotationHeader.length > 0) {
              writer.print("\t" + ArrayUtils.toStr(annotation
                                                             .containsKey(keys[order[j]]) ? annotation.get(keys[order[j]]).split(PSF.Regex.GREEDY_WHITESPACE) : ArrayUtils.stringArray(annotationHeader.length, "."),
                                                   annotationHeaderMask, "\t", "."));
            }
        ...
        try {
          ...
          writer = Files.openAppropriateWriter(ext.parseDirectoryOfFile(outfile) + "All_tables.xln",
                                               true);
          if (header) {
  ...
  public static void runModels(String[] dirs, String fileWithModels, String annotationFile,
    ...
    try {
      ...
      for (String[] model : models) {
        ...
        snpFile = model[1];
        target = model[2];
        snps = model[3].trim().split(",");

        // not using probabel
        run(dirs, snpFile, snps, annotationFile, useResiduals, true, false, false, genomiccontrol,
        ...
        for (int j = 0; j < pvals.length; j++) {
          ...
          if (Math.abs(Double.parseDouble(pvals[j][4])) < 5
              && Double.parseDouble(pvals[j][1]) < minP) {
            minP = Double.parseDouble(pvals[j][1]);
            minIndex = j;
          }
        }
        ...
        if (reportMinimums || !target.equals("minimum")) {
          w2.print((pheno == null ? "plink" : pheno) + (snpFile.equals("null") ? "" : "_" + snpFile)
                   ...
                   + (index == -1 ? ".\t.\t." : pvals[index][0] + "\t" + pvals[index][4] + "\t"
                                                + pvals[index][1]));
        }
        ...
        for (int i = 0; i < markersAndChrs.length; i++) {
          ...
          if (!Files.exists("metas/" + markersAndChrs[i][0] + "_SE1.tbl")) {
            ...
            for (String[] result : results) {
              if (minNumValids < 0
                  || ext.replaceAllWith(result[1], "?", "").length() >= minNumValids) {
                if (Double.parseDouble(result[2]) < minValue) {
                  minValue = Double.parseDouble(result[2]);
                  minMarker = result[0];
                  minInfo = result[3] + "\t" + result[4] + "\t" + result[5] + "\t" + result[6];
                } else if (Double.parseDouble(result[2]) == minValue) {
                  minMarker += ";" + result[0];
                }
  ...
  public static void main(String[] args) {
    ...
    // filenames =
    // "GBA.txt,GAK.txt,BST1.txt,SNCA.txt,chr5.txt,HLA.txt,chr12.txt,chr16.txt,MAPT.txt,RIT2.txt";
    //// filenames = "SNCA.txt";
    //// filenames = "RIT2.txt";
    //
    ...
    //// "blacks/MESA/", "asians/MESA/", "hispanics/MESA/"};
    ...
    for (String arg : args) {
      if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
        ...
        filenames = ext.parseStringArg(arg, "null");
        numArgs--;
      } else if (arg.startsWith("residuals=")) {
        useResiduals = ext.parseBooleanArg(arg);
        numArgs--;
      } else if (arg.startsWith("snps=")) {
        snps = arg.split("=")[1].split(",");
        ...
        run = true;
        numArgs--;
      } else if (arg.startsWith("-iterate")) {
        iterate = true;
        numArgs--;
      } else if (arg.startsWith("annotation=")) {
        annotation = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("covariates=")) {
        covariates = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("sex=")) {
        sexAsCovariate = ext.parseBooleanArg(arg);
        numArgs--;
      } else if (arg.startsWith("pheno=")) {
        pheno = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("missingPheno=")) {
        phenoMissingValue = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("-allowmissingConditional")) {
        allowMissingConditional = true;
        numArgs--;
      } else if (arg.startsWith("out=")) {
        outfile = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("dose=")) {
        dose = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("info=")) {
        info = ext.parseStringArg(arg, null);
        numArgs--;
        ...
        genomiccontrol = ext.parseBooleanArg(arg);
        numArgs--;
      } else if (arg.startsWith("models=")) {
        models = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("indexMarkers=")) {
        indexMarkers = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("markerList=")) {
        markerList = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("addlMarkers=")) {
        columnIndexContainingAllMarkersForTheModel = ext.parseIntArg(arg);
        numArgs--;
      } else if (arg.startsWith("markerNameIndex=")) {
        markerNameIndex = ext.parseIntArg(arg);
        numArgs--;
      } else if (arg.startsWith("parse=")) {
        parse = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("-probabel")) {
        probabel = true;
        numArgs--;
      } else if (arg.startsWith("metaAll=")) {
        metaAll = ext.parseStringArg(arg, null);
        numArgs--;
      } else {
