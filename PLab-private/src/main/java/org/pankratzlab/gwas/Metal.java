...
public class Metal {
  ...
  public static final String[] TEXT = {"NADA", "STRAND_CONFIG_SAME", "STRAND_CONFIG_SAME_FLIPPED",
                                       ...
                                       "DIFFERENT_ALLELES", "BOTH_NULL", "SPECIAL_CASE"};
  ...
  public static void convertPlinkResults(String dir, String results, String test, String method,
    ...
    if (new File(dir + "ref." + results + ".metal").exists()) {
      ...
      // System.err.println("METAL batch file was not made because "+"ref."+results+".metal did not
      // exist in "+dir);
    }
  }
  ...
  public static void reformatResults(String filename, String[] unitOfAnlaysis, int defaultN,
    ...
    try {
      ...
      while (reader.ready()) {
        ...
        if (nSNPsIndex == -1
            || (!ext.isMissingValue(line[nSNPsIndex]) && Integer.parseInt(line[nSNPsIndex]) >= 5)) {
  ...
  public static void makeBatch(String dir, String file1, String file2, String weight1,
    ...
    try {
      writer = Files.openAppropriateWriter(dir + ext.rootRootOf(file1) + "_" + ext.rootRootOf(file2)
                                           + "_metal.batch");
  ...
  public static void metaAnalyze(String dir, String[] filenames, String[] unitOfAnalysis,
    ...
    // filename = ext.rootOf(outputFile, false)+"_metal"+(anlaysisType ==
    // SE_ANALYSIS?"_se":(anlaysisType == WEIGHTED_SE_ANALYSIS?"_wse":""))+"_input.txt";
      ...
      while (reader.ready()) {
        ...
        if (values.length == 1) {
          ...
          for (int i = 1; i < values.length; i++) {
            writer.print("\t"
                         + ArrayUtils.toStr(ArrayUtils.toStringArray(hash.get(values[order[i]])),
                                            ",")
                         + "\t" + values[order[i]]);
          }
          writer.println();
  ...
  public static void fromParameters(String filename, Logger log) {
    ...
    params = Files.parseControlFile(filename, "metal",
                                    new String[] {"outfile_root", "build=37",
                                                  ...
                                                  "se<=5", "maf>=0.001",
                                                  "customCommand=file1.metal:aCommand",
                                                  "file1.metal", "file2.txt",
    ...
    if (params != null) {
      ...
      // metaAnalyze(dir, inputFiles, Aliases.MARKER_NAMES, outputFile+"_InvVar", SE_ANALYSIS, null,
      // gcControlOn, log);
      ...
      // metaAnalyze(dir, inputFiles, Aliases.MARKER_NAMES, outputFile+"_NWeighted", PVAL_ANALYSIS,
      // null, gcControlOn, log);

      // check to see if file exists, report error otherwise
      // Files.backup(filename, sourceDir, backupDir);
      // if (!Files.isFileReady(outputFile+"_InvVar.out", 5000, 2000) ||
      // !Files.isFileReady(outputFile+"_NWeighted.out", 5000, 2000)) {
      ...
      try {
        ...
        if (markerPositionHash.size() > 0) {
          ...
          for (int i = 0; i < genes.length; i++) {
            writer.println(hitList[i] + "\t"
                           + ArrayUtils.toStr(markerPositionHash.get(hitList[i]), "\t") + "\t"
                           + genes[i][1]);
          }
        }
        writer.close();
  ...
  public static void generateInputFile(String filename, Logger log) {
    ...
    if (params != null) {
      ...
      // Files.writeList(new String[] {"MARKER MARKER", "ALLELE REF OTHER", "EFFECT beta", "STDERR
      ...
      Files.writeArray(new String[] {"MARKER MarkerName", "ALLELE Allele1 Allele2", "WEIGHT Weight",
                                     "EFFECT Direction", "PVALUE P-value", "",
  ...
  public static void calculateWeightedAlleleFrequency(String filename, double thresholdForRsq,
    ...
    try {
      ...
      if (thresholdForMAFfile < 0) {
        ...
        w2 = Files.openAppropriateWriter(ext.rootOf(filename, false) + "_exceedingThreshold.dat");
      ...
      while (reader.ready()) {
        ...
        for (int i = 0; i < roots.length; i++) {
          // includes all instances where the freq is not missing and either the Rsq column is not
          // present or the Rsq value is valid
      ...
      while (reader.ready()) {
        ...
        for (int w = 0; w < 3; w++) {
          ...
          if (w == 0 || (w == 1 && flipped) || (w == 2 && ambiguous)) {
            ...
            for (int i = 0; i < roots.length; i++) {
              writer.print("\t" + (Double.isNaN(freqs[i]) ? "." : ext.formDeci(freqs[i], 4, true)));
            }
            if (numNull > 0) {
              writer.print("\t.\t.\t.\t.\t.");
            } else {
  ...
  public static void main(String[] args) {
    ...
    // String dir = "C:/Documents and Settings/npankrat/My
    // Documents/tWork/Consortium/analyses/Sing550/";
    ...
    // strandFile = "C:/Documents and Settings/npankrat/My
    // Documents/UMN/Folson/VTE_meta_analysis/finalAnalysis/16 assessing
    // strand/snplist1_described3.xln";
    // fileToSort = "C:/Documents and Settings/npankrat/My
    // Documents/UMN/Folson/VTE_meta_analysis/finalAnalysis/16 assessing strand/all_consensus.xln";
    // calcFreq = "C:/Documents and Settings/npankrat/My
    // Documents/UMN/Folson/VTE_meta_analysis/finalAnalysis/16 assessing
    // strand/compFreqs/snplist_described2.xln";
    // calcFreq = "C:/Documents and Settings/npankrat/My
    // Documents/UMN/Folson/VTE_meta_analysis/finalAnalysis/16 assessing
    // strand/compFreqs/snplist_described2_noARIC_noCHS.xln";
    // calcFreq = "C:/Documents and Settings/npankrat/My
    // Documents/UMN/Folson/VTE_meta_analysis/finalAnalysis/16 assessing
    // strand/compFreqs/snplist_described2_noARIC_noCHS.xln";
    // compFreq = "C:/Documents and Settings/npankrat/My
    // Documents/UMN/Folson/VTE_meta_analysis/finalAnalysis/16 assessing
    // strand/compFreqs/ARIC_comp.xln";
    // compFreq = "C:/Documents and Settings/npankrat/My
    // Documents/UMN/Folson/VTE_meta_analysis/finalAnalysis/16 assessing
    // strand/compFreqs/CHS_comp.xln";
    // calcFreq = "C:/Documents and Settings/npankrat/My
    // Documents/UMN/Folson/VTE_meta_analysis/finalAnalysis/11 X
    // chromosome/00src/snplist_described.xln";
    // calcFreq = "C:/Documents and Settings/npankrat/My
    // Documents/tWork/Consortium/analysisOfImputation/alleleFreq/snplist_described.xln";
    ...
    for (String arg : args) {
      if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
        ...
        results = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("dir=")) {
        dir = ext.parseStringArg(arg, "");
        numArgs--;
      } else if (arg.startsWith("test=")) {
        test = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("method=")) {
        method = ext.parseStringArg(arg, "logistic");
        numArgs--;
      } else if (arg.startsWith("freq=")) {
        freq = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("-batch")) {
        batch = true;
        numArgs--;
      } else if (arg.startsWith("file1=")) {
        file1 = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("weight1=")) {
        weight1 = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("file2=")) {
        file2 = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("weight2=")) {
        weight2 = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.toLowerCase().startsWith("-se")) {
        useSE = true;
        numArgs--;
      } else if (arg.toLowerCase().startsWith("metalnom=")) {
        useMetalNomenclature = ext.parseBooleanArg(arg);
        numArgs--;
      } else if (arg.startsWith("out=")) {
        outfile = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("check=")) {
        strandFile = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("rsq=")) {
        rsqThreshold = Double.parseDouble(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("maf>")) {
        mafThreshold = Double.parseDouble(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("split=")) {
        fileToSplit = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("numSplits=")) {
        numSplits = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("sort=")) {
        fileToSort = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("calcFreq=")) {
        calcFreq = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("compFreq=")) {
        compFreq = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("diff=")) {
        diffThreshold = Double.parseDouble(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("analyze=")) {
        analyze = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("gcControl=")) {
        String[] ctrl = ext.parseStringArg(arg, null).split(",");
        ...
        numArgs--;
      } else if (arg.startsWith("compResults=")) {
        compResults = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("countMissing=")) {
        countMissing = ext.parseStringArg(arg, null);
        numArgs--;
      } else {
