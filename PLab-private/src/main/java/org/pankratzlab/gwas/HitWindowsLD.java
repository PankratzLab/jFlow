...
public class HitWindowsLD {
  ...
  public void setLoc(ReferenceMap referenceMap) {
    chr = referenceMap.getChr(target);
    pos = referenceMap.getPos(target);
    minHitPos = pos;
    maxHitPos = pos;
  }
  ...
  public static void analyzeTargetLDCombo(String targetFile, ReferenceMap referenceMap, int window,
    ...
    HitWindowsLD[] hitWindowsLD = prepWindows(targetFile, referenceMap, window, region, filter,
                                              log);
  ...
  public static class ReferenceMap implements Serializable {
    ...
    private final Hashtable<String, Integer> trackpos;
    private final Hashtable<String, Integer> trackchr;
    private final Hashtable<String, Integer> trackindex;
    private final Hashtable<Integer, String> lookup;
    ...
    public int getPreviousAvailableIndex(String marker) {
      ...
      if (currentIndex >= 1
          && trackchr.get(lookup.get(currentIndex)) == trackchr.get(lookup.get(currentIndex - 1))) {
        ...
        for (HitWindowsLD element : hitWindowsLD) {
          if (fileType == 0) {
            ...
            try {
              ...
              if (referenceMap.getChr(line[0]) == referenceMap.getChr(line[1])
  ...
  public static void main(String[] args) {
    ...
    // " OPTIONAL: "+
    // " (4) file extension of LD input files (options are " +Array.toStr(FILE_TYPES) +")(i.e.
    ...
    for (String arg : args) {
      if (arg.startsWith("targets=")) {
        targetFiles = arg.split("=")[1].split(",");
        numArgs--;
      } else if (arg.startsWith("refMap=")) {
        refMap = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("dir=")) {
        dir = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("fileExt=")) {
        fileExt = arg.split("=")[1];
        numArgs--;
        ...
        filtered = false;
        numArgs--;
      } else if (arg.startsWith("-restrictive")) {
        liberal = false;
        numArgs--;
      } else if (arg.startsWith("-noRegion")) {
        region = false;
        numArgs--;
      } else {
...
// public static Hashtable<String, String> getFilteredLDMap(String filteredLdFile, Hashtable<String,
// String> ldMap, Logger log) {
// if (ldMap == null) {
