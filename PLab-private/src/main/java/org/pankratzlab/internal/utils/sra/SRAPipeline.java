...
import org.pankratzlab.internal.utils.sra.Pipeline.PIPELINE_PARTS;
import org.pankratzlab.internal.utils.sra.Pipeline.PipelinePart;
import org.pankratzlab.internal.utils.sra.SRAUtils.SRABamWorker;
import org.pankratzlab.internal.utils.sra.SRAUtils.SRAConversionResult;
...
public class SRAPipeline implements Callable<List<PipelinePart>> {
   ...
   * @param log
   */
  public SRAPipeline(SRASample sraSample, String inputSRA, String rootOutDir, String refGenome,
                     String captureBed, String binBed, String vcf, String computelLocation,
                     int numThreads, boolean cleanup, List<PIPELINE_PARTS> parts, Logger log) {
    super();
  ...
  public List<PipelinePart> call() throws Exception {
    ...
    if (!Files.exists(getCompleteFile(rootOutDir, inputSRA))) {
      ...
      List<PipelinePart> parts = Pipeline.pipeline(bam, rootOutDir, captureBed, binBed, vcfFile,
                                                   sraSample, refGenome, partsToRun,
  ...
  private static void runCompile(List<SRASample> samples, String rootOutDir, String captureBed,
                                 String binBed, String vcf, String referenceGenomeFasta,
  ...
  private static void compilePrep(String sraInput, String sraRunTableFile, String rootOutDir,
                                  ...
                                  PLATFORM platform, int numThreads) {
      ...
      switch (sample.getaType()) {// create the required markerSets for
        ...
        case WXS:
          if (!prelimGenvisisWXS && !c.has(SKIP_PRELIM)) {
            generatePrelim(rootOutDir, refGenome, captureBed, binBed, vcf, log, sample.getaType());
  ...
  private static void fullPipeline(CLI c) {
    ...
    if (c.has(GENVISIS_PART) || c.has(ALL_PART)) {
      generatePrelim(c.get(CLI.ARG_OUTDIR), c.get(CLI.ARG_REFERENCE_GENOME), c.get(CAPTURE_BED),
                     c.get(BIN_BED), c.get(VCF), log, ASSAY_TYPE.WXS);
  ...
  public static void main(String[] args) {
    ...
    if (c.has(COMPILE)) {
      compilePrep(c.get(SRA_INPUT), c.get(SRA_RUN_TABLE), c.get(CLI.ARG_OUTDIR),
                  c.get(CLI.ARG_REFERENCE_GENOME), c.get(CAPTURE_BED), c.get(BIN_BED), c.get(VCF),
      ...
      runAll(c.get(SRA_INPUT), c.get(SRA_RUN_TABLE), c.get(CLI.ARG_OUTDIR),
             c.get(CLI.ARG_REFERENCE_GENOME), c.get(CAPTURE_BED), c.get(BIN_BED), c.get(VCF),
             c.get(COMPUTEL_LOCATION), c.getI(NUM_THREADS), c.getI(NUM_THREADS_PIPELINE),
             c.getI(NUM_BATCHES), c, c.has(CLEANUP));
    }
  }
}
