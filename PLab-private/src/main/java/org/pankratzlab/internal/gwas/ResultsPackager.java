...
import org.pankratzlab.common.bioinformatics.Alleles;
import org.pankratzlab.common.stats.ProbDist;
import org.pankratzlab.internal.gwas.parse.EasyQCParser;
...
public class ResultsPackager {
  ...
  private static final String[][] EASY_QC_ALIASES_IN_ORDER = {Aliases.MARKER_NAMES, Aliases.CHRS,
                                                              Aliases.POSITIONS, {"strand"},
                                                              Aliases.ALLELES[0],
                                                              ArrayUtils.addStrToArray("base_allele",
                                                                                       Aliases.ALLELES[1],
                                                                                       0),
                                                              Aliases.NS, Aliases.ALLELE_FREQS,
                                                              ArrayUtils.addStrToArray("beta_int",
                                                                                       Aliases.EFFECTS,
                                                                                       0),
                                                              ArrayUtils.addStrToArray("se_int",
                                                                                       Aliases.STD_ERRS,
                                                                                       0),
                                                              ArrayUtils.addStrToArray("chi_P_2df",
                                                                                       Aliases.PVALUES,
                                                                                       0),
  ...
  private static final String[] EASY_QC_OUTPUT_HEADER = {"SNP", "CHR", "POS", "STRAND",
                                                         "EFFECT_ALLELE", "OTHER_ALLELE", "N",
                                                         "EAF", "BETA", "SE", "PVAL", "IMPUTATION"};
  ...
  public static void parseEasyQCOld(String inputFile, String outputFile) {
    ...
    try {
      ...
      indices = ext.indexFactors(EASY_QC_ALIASES_IN_ORDER, parts, false, true, false, log);

      int miss;
      if ((miss = ext.indexOfInt(-1, indices)) != -1) {
        log.reportError("Missing header element: "
                        + ArrayUtils.toStr(EASY_QC_ALIASES_IN_ORDER[miss], ","));
        return;
      }

      writer.println(ArrayUtils.toStr(EASY_QC_OUTPUT_HEADER, " "));
  ...
  private static String[] getPvalEquations(String logLikilihood_null_C, String logLikilihood_full_C,
    ...
    return new String[] {"=1-CHISQ.DIST(2 * (" + logLikilihood_full_C + "-" + logLikilihood_null_C
                         + "),2,TRUE)",
   ...
   *          columnNamesOfAnalyses with desired display text (ignored for [i][0] and [i][1], null
   *          to use column names
   * @param fullPathOutFile Path to generate output file to
   * @param log a log Examples of using this method: getForestPlotParameterFile(new String[][]
   *          {{"extragonadal", "D:/temp/Poynter_emim/testing/allFinalPoynter_results_pvals_1.xln"},
   *          {"germinoma", "D:/temp/Poynter_emim/testing/allFinalPoynter_results_pvals_2.xln"}},
   ...
   *          false, null, false), "/home/pankrat2/shared/Poynter_emim/markerList.txt",
   *          "MarkerName", new String[] {"TDT", "EMIM Maternal Effect"}, new String[][] {{"tdt_OR",
   *          "tdt_U95", "tdt_P"}, {"M_lnS1", "M_se_lnS1", "pVal_M_df1", "pVal_CM-C_df1"}}, new
   *          String[][] {{"","","p"},{"","","p","removing Child Effect p"}},
