    ...
    for (int chr = 1; chr <= 22; chr++) {
      if (new File("chr" + chr + "/").exists()) {
        if (new File("chr" + chr + "/plink.map").exists()
          ...
          markerSet = new SnpMarkerSet(new File("chr"
                                                + chr + "/plink.map").exists()
                                                                               ? "chr" + chr
                                                                                 + "/plink.map"
                                                                               : "chr" + chr
                                                                                 + "/plink.bim",
  ...
  public static void filter(String filename) {
    ...
    for (int element : FILTER_CM_THRESHOLDS) {
      for (int strict = 0; strict < 2; strict++) {
        try {
          reader = new BufferedReader(new FileReader(filename));
          writer = Files.openAppropriateWriter(element + "cM"
                                               + (strict == 1 ? "_max"
                                                                + (int) (FILTER_MAX_THRESHOLD * 100)
                                                              : "")
                                               + ".segment");
          line = reader.readLine().trim().split(PSF.Regex.GREEDY_WHITESPACE);
