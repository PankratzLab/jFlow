  ...
  public static Segment[][][][] loadSegments(String dir) {
      ...
      for (int i = 0; i < files.length; i++) {
        ...
        if (Files.exists(dir + files[i] + "_" + CLASSES[0] + ".ser")) {
          ...
          for (int j = 0; j < CLASSES.length; j++) {
            segs[i][j] = SegmentLists.load(dir + files[i] + "_" + CLASSES[j] + ".ser").getLists();
          }
    ...
    for (int j = 0; j < 2; j++) {
      ...
      System.out.print("\t" + CLASSES[j] + "\t"
                       + ArrayUtils.toStr(coverage(segs[files.length][j])));
    }
    System.out.println();
        ...
        for (int j = 0; j < CLASSES.length; j++) {
          ...
          if (j < 2) {
            ...
            pvals = ArrayUtils.toDoubleArray(HashVec.loadFileToStringArray(file_dir
                                                                           ...
                                                                           false));
            System.out.print("\t"
                             + ext.formDeci(ProbDist.ChiDistReverse(ArrayUtils.median(ArrayUtils.removeNaN(pvals)),
                                                                    1)
                                            / ProbDist.ChiDistReverse(0.50, 1), 4));
            System.out.print("\t"
                             + ext.formDeci(ProbDist.ChiDistReverse(ArrayUtils.quantExclusive(ArrayUtils.removeNaN(pvals),
                                                                                              0.10),
                                                                    1)
