...
public class crfDB {
	...
	public crfDB(String dir, String keyFilename, boolean logging) throws Elision { // dir is necessary for UpdateCRFdb
		...
		double[][][] bounds;
		Vector<String> fileDescriptions = new Vector<String>(), idCollection = new Vector<String>(), hashDescriptions = new Vector<String>(), specials = new Vector<String>();
		String[] data;
		...
		try {
							...
							for (int i = 2; i<line.length; i++) {
								for (int j = 0; j<DELIMITERS.length(); j++) {
									if (line[i].startsWith(DELIMITERS.charAt(j)+"")) {
										log.reportError("Error - a variable name cannot start with a reserved delimiter character ('"+line[i]+"' for "+line[0]+")");
										problem = true;
									}
								}
							}
			...
			for (int i = 0; i<specials.size(); i++) {
				...
				if (colHash.containsKey(subline[0])) {
					log.reportError("Error - duplicate variable name ("+subline[0]+") for final database (detailed in "+keyFilename+")");
                	throw new Elision();
				} else {
					colHash.put(subline[0], "1");
				}
				if (delimiters.split(subline[0]).length>1) {
					log.reportError("Error - Final variable names may not contain characters used as operators (i.e. "+DELIMITERS+")");
					log.reportError("        "+subline[0]+" has "+(delimiters.split(subline[0]).length-1)+" of them");
                	throw new Elision();
				}
			...
			for (int fn = 0; fn<numFiles; fn++) {
				...
				if (!subline[0].equals("KEY")) {
					log.reportError("Error - "+line[1]+" is in the second position but does not use proper key syntax");
				...
				for (int i = 0; i<expecteds[fn].length; i++) {
					if (colHash.containsKey(expecteds[fn][i])) {
						log.reportError("Error - duplicate column header ("+expecteds[fn][i]+") in file '"+filenameKey[fn][0]+"'");
                    	throw new Elision();
					} else {
				...
				reader = new BufferedReader(new FileReader((new File(line[0]).exists()?"":POSS_DIR)+filenameKey[fn][0]));
				subline = reader.readLine().split(delimiter);

				for (int i = 0; i<expecteds[fn].length; i++) {
					if (i>=subline.length) {
						log.reportError("Error - too many column headers in "+keyFilename+" for file "+filenameKey[fn][0]);
				...
				if (subline.length>expecteds[fn].length) {
					log.reportError("Error - too few column headers in "+keyFilename+" for file "+filenameKey[fn][0]);
				...
				while (reader.ready()) {
					...
					if (subline.length==1) {
						...
						unique_id = data[ext.indexOfStr(subline[0], expecteds[fn])]+ext.formNum(Integer.valueOf(data[ext.indexOfStr(subline[1], expecteds[fn])]).intValue(), 3);
					}
					if (fn==0) {
						...
						if (hash.containsKey(unique_id)) {
							...
							if (!filenameKey[fn][0].equals("ninfo1.csv")&&!filenameKey[fn][0].equals("ninfo2.csv")&&!filenameKey[fn][0].equals("ninfo5.csv")&&!filenameKey[fn][0].equals("ninfo6.csv")) { // these
								// have
								// additional
								// unrequired
								// indviduals
								log.reportError("Error - Unique ID '"+unique_id+"' was in file '"+filenameKey[fn][0]+"' but not in previous file(s)");
							}
							allData = new String[numFiles+1][];
							allData[numFiles] = new String[specials.size()];
							hash.put(unique_id, allData);
					...
					for (int i = 0; i<expecteds[fn].length; i++) {
						checkBounds(filenameKey[fn][0]+":"+expecteds[fn][i], subline.length==1?unique_id:formatUniqueID(unique_id), data[i], bounds[fn][i][0], bounds[fn][i][1]);
					}
				...
				for (int fn = 0; fn<numFiles; fn++) {
					if (allData[fn]==null) {
						if (allData[0]!=null&&!filenameKey[fn][0].equals("G2019S.csv")&&SHOW_ABSENTS) {
							log.reportError("Error - "+unique_id+" was absent from file '"+filenameKey[fn][0]+"'");
						}
					}
				}
				for (int fn = 0; fn<numFiles; fn++) {
			...
			for (int i = 0; i<specials.size(); i++) {
				...
				for (int j = 0; j<indices.length; j++) {
					...
					for (int k = 0; k<line.length; k++) {
						if (line[k].indexOf(":")>=0) {
							indices[j] = getVector(line[k], expecteds, filenameKey);
							if (indices[j][0]==numFiles&&indices[j][1]>=i) {
								log.reportError("Error - cannot reference a variable (final:"+expecteds[numFiles][indices[j][1]]+") before it is rendered. Reorder the final variables.");
                            	throw new Elision();
							}
						}
					}
				...
				if (temp.startsWith("$RESIDUAL")||temp.startsWith("$PREDICTED")) {
					...
					for (int j = 0; j<idCollection.size(); j++) {
						...
						for (int k = 0; k<indices.length; k++) {
							if (allData[indices[k][0]][indices[k][1]].equals(".")) {
								used[j] = false;
							}
						}
					...
					if (v.size()==1) {
						log.reportError("Error - dependent variable has zero variation ("+v.elementAt(0)+"); "+temp+" will not be calculated");
						used = new boolean[used.length];
					} else {
						logistic = (v.size()==2);

						model = logistic?new LogisticRegression(deps, indeps):new LeastSquares(deps, indeps);
					...
					for (int j = 0; j<idCollection.size(); j++) {
						...
						if (indices.length==1&&operator[0]=='#') {
							...
							allData[numFiles][i] = dateIt(idCollection.elementAt(j), subline[1], allData[indices[0][0]][indices[0][1]]);
						} else if (indices.length==1&&operator[0]=='$'&&subline[1].startsWith("RECODE")) {
							...
							if (!recodeFrom.contains(".")&&allData[functionIndicies[0][0]][functionIndicies[0][1]].equals(".")) {
								allData[numFiles][i] = ".";
							} else {
								allData[numFiles][i] = recodeTo[recodeFrom.indexOf(allData[functionIndicies[0][0]][functionIndicies[0][1]])+1];
							}
							...
							if (allData[functionIndicies[0][0]][functionIndicies[0][1]].equals(".")) {
								allData[numFiles][i] = ".";
							} else if (!hashes.elementAt(index).containsKey(allData[functionIndicies[0][0]][functionIndicies[0][1]])) {
								allData[numFiles][i] = "Hashtable did not have key '"+allData[functionIndicies[0][0]][functionIndicies[0][1]]+"'";
							} else {
								allData[numFiles][i] = hashes.elementAt(index).get(allData[functionIndicies[0][0]][functionIndicies[0][1]]);
							}
						} else {
							score = 0;
							missing = 0;
							for (int k = 0; k<indices.length; k++) {
								...
								if (operator[k]!='$'&&procDouble(allData[indices[k][0]][indices[k][1]])-procInt(allData[indices[k][0]][indices[k][1]])>0) {
									doubleFun = true;
								}
								switch (operator[k]) {
								case '+':
								...
								case '!':
									if (allData[indices[k][0]][indices[k][1]].equals("0")) {
										score += 1;
									} else if (procInt(allData[indices[k][0]][indices[k][1]])!=0&&procInt(allData[indices[k][0]][indices[k][1]])!=1) {
										log.reportError("Error - '"+allData[indices[k][0]][indices[k][1]]+"' cannot be inversed for "+subline[k+1]);
									}
									break;
								case '$':
									...
									if (line[0].equals("EQUALS")) {
										...
										functionIndicies = new int[1][];
										functionIndicies[0] = getVector(line[1], expecteds, filenameKey);
										score += allData[functionIndicies[0][0]][functionIndicies[0][1]].equals(line[2])?1:0;
										...
										if (line[2].indexOf(":")>0) {
											...
											if (functionIndicies[1][0]==numFiles&&functionIndicies[1][1]>=i) {
												log.reportError("Error - cannot reference a variable (final:"+expecteds[numFiles][functionIndicies[1][1]]+") before it is rendered. Reorder the final variables.");
		                                    	throw new Elision();
											}
										}
										if (allData[functionIndicies[0][0]][functionIndicies[0][1]].equals(".")) {
											missing++;
										} else if (functionIndicies[1]!=null&&allData[functionIndicies[1][0]][functionIndicies[1][1]].equals(".")) {
											missing++;
										} else {
											score += procDouble(allData[functionIndicies[0][0]][functionIndicies[0][1]])>=procDouble(line[2].indexOf(":")>0?allData[functionIndicies[1][0]][functionIndicies[1][1]]:line[2])?1:0;
										}
										...
										if (allData[functionIndicies[0][0]][functionIndicies[0][1]].equals(".")) {
											...
											score += procDouble(allData[functionIndicies[0][0]][functionIndicies[0][1]].indexOf(".")>0?allData[functionIndicies[0][0]][functionIndicies[0][1]].substring(0, allData[functionIndicies[0][0]][functionIndicies[0][1]].indexOf(".")):allData[functionIndicies[0][0]][functionIndicies[0][1]]);
										}
									} else {
							...
							if (missingThreshold==0?missing==indices.length:(double)missing/(double)indices.length>=missingThreshold) {
								allData[numFiles][i] = ".";
							} else {
								allData[numFiles][i] = (doubleFun?ext.formDeci(score, 2, true):ext.formDeci(score, 0));
							}
						}
						checkBounds(filenameKey[numFiles][0]+":"+expecteds[numFiles][i], idCollection.elementAt(j).length()==8?formatUniqueID(idCollection.elementAt(j)):idCollection.elementAt(j), allData[numFiles][i], bounds[numFiles][i][0], bounds[numFiles][i][1]);
					}
				}
			...
			for (int i = 0; i<specials.size(); i++) {
				...
				temp += dropFromFinal[i]?"":(ext.indexOfStr(expecteds[numFiles][i], DONT_COMP)==-1&&ext.indexOfStr(expecteds[numFiles][i], DONT_COMP_DEP)==-1?"\t"+expecteds[numFiles][i]:"");
			}
			writer.println();
			...
			for (int i = 0; i<idCollection.size(); i++) {
				unique_id = idCollection.elementAt(keys[i]);
				writer.print(unique_id+(filenameKey[0][1].split(":").length==2?"\t"+unique_id.substring(0, 5)+"\t"+Integer.valueOf(unique_id.substring(5)).intValue():""));
	...
	public double[] parseBounds(String[] subline) {
		...
		if (subline.length==1) {
			...
			if (bounds[1]<0) {
				log.reportError("Warning - upper bound for variable "+subline[0]+" is below the default lower (zero); define new lower or else all will be flagged as invalid");
			}
		} else if (subline.length==3) {
			bounds[0] = Double.parseDouble(subline[1]);
			bounds[1] = Double.parseDouble(subline[2]);
			if (bounds[1]<bounds[0]) {
				log.reportError("Error - upper bound "+bounds[1]+" for variable "+subline[0]+" is less than the lower bound "+bounds[0]+"; all will be flagged as invalid");
			}
		} else {
			log.reportError("Error - Don't know what to do with "+subline.length+" arguments for variable '"+subline[0]+"'");
		}
		...
		if (!value.equals(".")&&lower<upper) {
			try {
				if (Double.parseDouble(value)<lower) {
					log.reportError("Error - "+variable+" for "+id+" was below the lower bound ("+lower+"): "+value);
				}
				if (Double.parseDouble(value)>upper) {
					log.reportError("Error - "+variable+" for "+id+" was above the upper bound ("+upper+"): "+value);
				}
	...
	public String dateIt(String unique_id, String variable, String date) {
		...
		if (date.length()==4) {
			log.reportError("Error - "+variable+" for "+unique_id+" is lacking missing data characters: '"+date+"'; should be 'UUUU"+date+"'");
			date = "UUUU"+date;
		}
		if (date.length()==6) {
			log.reportError("Error - "+variable+" for "+unique_id+" is lacking missing data characters: '"+date+"'; should be 'UU"+date+"'");
			date = "UU"+date;
		}
		if (date.length()!=8) {
			day = 0;
			...
			if (date.length()==day) {
				log.reportError("Error - "+variable+" for "+unique_id+" has the wrong number of characters: '"+date+"'; should be 8 characters, not "+date.length());
				return ".";
			}

			log.reportError("Error - "+variable+" for "+unique_id+" has a malformed date and is currently considered missing data: '"+date+"'");
			return ".";
		}
		for (int i = 0; i<errorChars.length; i++) {
			if (date.indexOf(errorChars[i])!=-1) {
				log.reportError("Error - "+variable+" for "+unique_id+" has an invalid character: '"+date+"'"+" -> replace letter '"+errorChars[i]+"' with "+(i==0?"number '0'":"letter 'U'"));
		...
		try {
			...
			log.reportError("Error - "+variable+" for "+unique_id+" has a malformed data and is currently considered missing data: '"+date+"'");
			return ".";
		}
		if (month==0) {
			month = 7;
		...
		return ext.formDeci(((double)((new GregorianCalendar(year, month, day).getTimeInMillis()-new GregorianCalendar(year, 0, 0).getTimeInMillis())/1000/60/60/24))/365+year, 2, true);
	}
	...
	public int[] getVector(String str, String[][] list, String[][] filenames) {
		...
		if (vec[0]==-1) {
			log.reportError("Error - malformed variable: "+subline[0]+" is not a filename delineated at the top of the file");
			return null;
		}
	...
	public static String fixMarks(String str) {
		...
		for (int i = 1; i<str.length(); i++) {
			if (str.charAt(i-1)==','&&str.charAt(i)=='\"'&&on!=1) {
				on = 1;
			} else if (str.charAt(i)=='\"'&&str.length()>(i+1)&&str.charAt(i+1)==','&&on==1) {
				on = 2;
	...
	public static String checkData(String str) {
		if (str.equals(".")||str.equals("N")||str.equals("U")||str.equals("N.N")||str.equals("U.U")||str.equals("NN")||str.equals("UU")||str.equals("NN.NN")||str.equals("UU.UU")||str.equals("NNNN")||str.equals("UUUU")||str.equals("UUU")||str.equals("NNN")||str.equals("N.A")||str.equals("NA")) {
			return ".";
		}
		if (str.equals(". 0")) {
			return "0";
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				logging = args[i].split("=")[1].equalsIgnoreCase("true");
				numArgs--;
