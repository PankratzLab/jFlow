...
//	        			count++;
	        			...
	        			for (int j = 0; j<2; j++) {
		        			...
		        			try {
		        				if (ladder[i] == -1) {
		        					...
		        					System.err.println("Error - mismatched ladder sizes ('"+line[i*2+1+0]+"' and '"+line[i*2+1+1]+"'); expecting the same size with X and Y suffixes");
		        				}
		...
		for (int i = 0; i<ladder.length; i++) {
			for (int j = 0; j<2; j++) {
    			try {
                    ...
                    System.err.println("Error parsing "+SUFFIXES[j]+" coordinate for "+ladder[i]+" bp ladder rung : "+values[1+i*2+j]);
                    e.printStackTrace();
                }
            }
        }
	...
	public static String[] translateAllelesIntoRepeats(String[] alleles) {
		...
		for (int i = 0; i<alleles.length; i++) {
			if (alleles[i].equals(".")) {
				...
				repeats[i] = ext.formDeci((double)(Integer.parseInt(alleles[i]) - (REFERENCE_FRAGMENT_SIZE-REFERENCE_REPEATS_SIZE)) / (double)SIZE_OF_UNIT, 2);
			}
        }
	...
	public static String[] translateIntoAlleles(int[] ladder, int[][][] ladderCoordinates, String[] values) {
		String[] alleles;
		int[] ys;
		...
		d = ((double)values.length-1.0)/2.0;
		if (d - Math.floor(d) > 0.0001) {
			System.err.println("Error - odd number of coordinates after identifier '"+values[0]+"'; translation requires pairs of coordinates");
		}
		...
		for (int i = 0; i<numPoints; i++) {
			try {
	            ...
	            System.err.println("Error parsing point "+(i+1)+" for sample "+values[0]+": "+values[i*2+1+0]+","+values[i*2+1+1]);
            }
        }
		meanX /= count;
		...
		for (int i = 0; i<ladder.length; i++) {
			slopeAndIntercept = Maths.slopeAndIntercept(ladderCoordinates[0][i][0], ladderCoordinates[0][i][1], ladderCoordinates[1][i][0], ladderCoordinates[1][i][1]);
...
//        }
	...
	public static int linearTranslation(int[] ladder, int[] customLadderCoordinates, double x) {
		...
		slopeAndIntercept = Maths.slopeAndIntercept(customLadderCoordinates[rung-1], ladder[rung-1], customLadderCoordinates[rung], ladder[rung]);
