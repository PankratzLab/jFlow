...
public class LineChartSample extends Application {
	...
	public void start(Stage stage) {
		...
		double[] pcs = Array.toDoubleArray(HashVec.loadFileToStringArray(file, true, new int[] { 0 }, false));
		double[] pvals = Array.toDoubleArray(HashVec.loadFileToStringArray(file, true, new int[] { 22 }, false));

		double[] correl = Array.toDoubleArray(HashVec.loadFileToStringArray(file, true, new int[] { 15 }, false));
		...
		for (int i = 0; i < pcs.length; i++) {
			if (pvals[i] == 0.05) {
				if (types[i].contains("NATURAL")) {
					all[ext.indexOfStr(types[i], uniqTypes)].getData().add(new XYChart.Data(pcs[i], correl[i]));
				}
			}
		}
...
//		stage.show();
		...
		try {
			ImageIO.write(SwingFXUtils.fromFXImage(snapShot, null), "png", new File("C:/data/misc/betaPlots/test.png"));
		} catch (IOException e) {
