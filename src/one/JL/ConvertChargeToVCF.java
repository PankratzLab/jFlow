...
public class ConvertChargeToVCF {
	...
	private static final String[] CHARGE_HEADER = { "SNP", "REF", "ALT", "SKATgene", "func_region", "MAF_whites", "n_whites", "MAF_blacks", "n_blacks" };
	...
	public static void processChargeMafFile(String fullPathToFile, Logger log) {
		if (!Files.headerOfFileContainsAll(fullPathToFile, CHARGE_HEADER, log)) {
			log.reportTimeError("This is designed for a specific file format with header " + Array.toStr(CHARGE_HEADER));
		} else {
			...
			try {
				int[] indices = ext.indexFactors(CHARGE_HEADER, Files.getHeaderOfFile(fullPathToFile, log), true, false);
				...
				while (reader.ready()) {
					...
					if ((!ref.equals(".") || !alt.equals(".")) && !alt.contains(",")) {
						...
						for (int i = 0; i < CHARGE_HEADER.length; i++) {
							vBuilder.attribute(CHARGE_HEADER[i], i <= 4 ? line[indices[i]] : Float.parseFloat(line[indices[i]]) + "");
						}
			...
			for (int i = 0; i < segsA.length; i++) {
				...
				for (int j = 0; j < indices.length; j++) {
					if (segsA[i].equals(segsASorted[indices[j]])) {
						order[i] = indices[j];
						if (found) {
			...
			for (int i = 0; i < order.length; i++) {
				writer.add(vcs.get(order[i]));
			}
			writer.close();
			if (countSkipped > 0) {
				log.reportTimeWarning("Skipped " + countSkipped + " variants due to ambigous alt alleles, or to lack of ref/alt alleles");
			}
