...
public class VCFToPlinkExtractor {
    ...
    private static String[] get1000GFiles(final String srcDir, int[] chrs) {
        ...
        for (int i = 0; i < chrs.length; i++) {
            ...
            for (int f = 0; f < files.length; f++) {
                if (files[f].contains(search)) {
                    results[i] = files[f];
                    break;
                }
            }
            if (results[i] == null) {
                System.err.println("Error - unable to find 1000G data file for chromosome " + chrs[i] + ".  Please ensure that data files names include \".chr#.\".");
            }
        }
    ...
    public static void run(String dir, String rgns, String ids) {
        ...
        for (int i = 0; i < ucsc.length; i++) {
            ...
            sb = new StringBuilder("plink2 --noweb")
                .append(" --vcf ").append(dir).append(dataFiles[i])
        ...
        String plinkMerge = "plink2 --noweb --bfile " + current + mergeRoot + " --merge-list " + current + "temp/mergeList.txt --make-bed --out " + current + "merged";
        commands.add("");
    ...
    public static void main(String[] args) {
        ...
        String usage = "\n" + 
                       "one.VCFToPlinkExtractor requires 0-1 arguments\n" + 
                       "   (1) Directory of 1000G vcf files (i.e. dir=" + dir + " (default))\n" +
                       "   (2) UCSC regions filename (i.e. rgns=" + rgnFile + " (default))\n" +
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
                ...
                dir = ext.verifyDirFormat(args[i].split("=")[1]);
                numArgs--;
            } else if (args[i].startsWith("rgns=")) {
                rgnFile = args[i].split("=")[1];
                numArgs--;
