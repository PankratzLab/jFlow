...
//        short[][] alleleCounts;
        ...
        if (!new File(DIR+ext.rootOf(RS_LOOKUP_FILE)+".ser").exists()) {
            ...
            try {
    	        ...
    	        while (reader.ready()) {
    	        	...
    	        	if (!line[0].startsWith("#")) {
    	        		if (!line[2].startsWith("chr")) {
    	        			log.reportError("Error - snp '"+line[5]+"' doesn't have a valid chromosome: "+line[2]);
    	        		}
            	...
            	log.reportError("Error: file \""+DIR+RS_LOOKUP_FILE+"\" not found in current directory");
    	        System.exit(1);
            } catch (IOException ioe) {
	        ...
	        new SnpMarkerSet(markerNames, chrs, positions, null, Array.toMatrix(annotation), true, false).serialize(DIR+ext.rootOf(RS_LOOKUP_FILE)+".ser");
	        
	        genes = HashVec.getKeys(geneSNPs);
	        new File(DIR+"genes/").mkdir();
	        for (int i = 0; i<genes.length; i++) {
	        	v = geneSNPs.get(genes[i]);
	        	try {
                    writer = new PrintWriter(new FileWriter(DIR+"genes/"+genes[i]+".txt"));
                    for (int j = 0; j<v.size(); j++) {
                    	writer.println(v.elementAt(j));
                    }
                    writer.close();
                } catch (Exception e) {
                	log.reportError("Error writing to "+DIR+"genes/"+genes[i]+".txt");
                    e.printStackTrace();
                }
            }
        }
...
//		        						alleleCounts[snpIndex][j]++;
		        				...
		        				if (data[indIndex][snpIndex][i] == -1) {
		    	        			data[indIndex][snpIndex][i] = 0;
		        					log.reportError(Array.toStr(annotation[snpIndex].split("-"))+"\t"+line[2+i]+"\tinstead of either\t"+alleles[snpIndex][0]+"\t"+alleles[snpIndex][1]);
		        				}
	        				}
                        }
	        		} else {
	        			System.err.println("Error - duplicate attempts to fill "+annotation[snpIndex]+" for sample "+samples[indIndex]);
	        		}
	        	}
	        }
	        reader.close();
        ...
        try {
	        ...
	        for (int i = 0; i<samples.length; i++) {
	        	writer.print(samples[i]+"\t"+sampleLookup.get(samples[i]).substring(0,7)+"\t0\t0\t1\t1");
