...
import bioinformatics.MapGenesToSNPs;
import common.*;
import filesys.*;
        ...
        try {
	        ...
	        while (reader.ready()) {
	        	if (temp.startsWith("PATHWAY")) {
					...
					if (v.size() > 0) {
						System.err.println("Error - did not reset after ENTRY "+entry+" starting at: "+temp);
					}
					v = new Vector<String>();
					while (temp.startsWith(" ")) {
						...
						if (!pathwayLookup.containsKey(pathway)) {
							...
							System.err.println("Error - mismatched ID ("+pathway+") with pathway name (was '"+pathwayLookup.get(pathway)+"', now '"+pathwayName+"')");
						}
						v.add(pathway);
						temp = reader.readLine();
					}
						...
						if (temp.startsWith("HSA:")) {
							...
							for (int i = 1; i<line.length; i++) {
								if (line[i].indexOf("(") == -1 || line[i].indexOf(")") == -1) {
									gene = line[i];
//									System.err.println("Error - no gene name for '"+line[i]+"' in entry "+entry+" for pathways: "+ext.listWithCommas(Array.toStringArray(v)));
								} else {
								...
								for (int j = 0; j<v.size(); j++) {
									HashVec.addToHashVec(genes, gene, v.elementAt(j), true); // count both ways and compare
									HashVec.addToHashVec(pathways, v.elementAt(j), gene, true); // count both ways and compare
	                            }
                            }
                        }
						temp = reader.readLine();
        ...
        try {
	        ...
	        while (reader.ready()) {
	        	...
	        	for (int i = 1; i < line.length; i++) {
					...
					HashVec.addToHashVec(pathways, "ko"+line[i], line[0], true); // count both ways and compare
				}
	        }
	        reader.close();
        } catch (FileNotFoundException fnfe) {
	...
	public void matchMarkersToGenes(String mapfile, String geneLocFile, int offset) {
		...
		genes = new Hashtable<String, Vector<String>>();
		maxChr = -1;
		for (int i = 0; i < genesAndSegs.length; i++) {
			seg = new Segment(genesAndSegs[i][1]);
			if (seg.getChr() > maxChr) {
				maxChr = seg.getChr();
			}
			HashVec.addToHashVec(genes, seg.getChr()+"", genesAndSegs[i][0]+"\t"+genesAndSegs[i][1], false);
		}
			...
			if (segs.length <= chrs[i] || segs[chrs[i]] == null) {
				...
				for (int j = 0; j < segs[chrs[i]].length; j++) {
					try {
						...
						System.err.println("Error - problem trying to overlap "+seg.getUCSClocation()+" and "+geneIDs[chrs[i]][j]);
					}
				}
			}
		}
		...
		for (int i = 0; i < noChrValues.length; i++) {
			System.err.println("Error - no genes on chromosome "+noChrValues[i]+" (affects "+noChrCounts[i]+" snps)");
		}
	}
}
...
//	public static final String KEGG_GENE_POSITIONS = ROOT_DIRECTORY+"NCBI/KEGG/geneLocs.xln";
	...
	public static String[] permutePheno(String pheno, String covars, int rep) {
        ...
        if (covars != null) {
        	if (!new File("pheno_covar_checks_out").exists()) {
        		if (!Array.equals(HashVec.loadFileToStringArray(pheno, true, new int[] {0,1}, false), HashVec.loadFileToStringArray(covars, true, new int[] {0,1}, false), false)) {
        			System.err.println("Error - the covars files needs to match the ids in the pheno file line for line");
            		Files.writeList(new String[] {"Error - the covars files needs to match the ids in the pheno file line for line"}, "NEED_TO_SYNC_PHENO_AND_COVAR_FILES.txt");
            		new File("wait").delete();
            		System.exit(1);
        		}
	...
	public static void permAndRun(String pheno, String covars, boolean sex, boolean linkCovToPheno, boolean compress) {
        ...
        while (new File("plug").exists()) {
        	if (new File("wait").exists()) {
        		...
        		if (covars == null && !sex) {
        			CmdLine.run(ROOT_DIRECTORY+"bin/plink --bfile plink --noweb --assoc --pheno "+filenames[0]+" --out "+root+"."+rep, "./");
            		results = root+"."+rep+"."+(quant?"q":"")+"assoc";
        		} else {
        			CmdLine.run(ROOT_DIRECTORY+"bin/plink --bfile plink --noweb --"+(quant?"linear":"logistic")+" --pheno "+filenames[0]+" --covar "+(linkCovToPheno?filenames[1]:covars)+(sex?" --sex":"")+" --out "+root+"."+rep, "./");
...
//        			return;
	...
	public static void compressAll(String pheno, String covars) {
        ...
        quant = Array.determineType(pheno, 2, true)==1;
    	markerNames = SerialStringArray.load(ext.rootOf(pheno, false)+".markerNames", false).getArray();
    	...
    	while (!done) {
    		...
    		if (covars == null) {
            	...
            	filename = ext.rootOf(pheno, false)+"."+count+".assoc."+(quant?"linear":"logistic");
    		}
	...
	public static String evaluateGeneset(String pheno, String[] markerList, String results, String repDir, double p_thresh, double nrss_thresh) {
        int count, hits;
        long time;
        float[] pvals;
		...
		for (int i = 0; i < markerList.length; i++) {
			markerSetHash.put(markerList[i], "");
		}
...
//        markerNames = HashVec.loadFileToStringArray(results, true, new int[] {1}, false);
        ...
        if (!new File(repDir+ext.rootOf(pheno, false)+".markerNames").exists()) {
        	System.err.println("Error - need file '"+repDir+ext.rootOf(pheno, false)+".markerNames' to verify marker order");
        	return "error";
        }
        if (Array.equals(markerNames, SerialStringArray.load(repDir+ext.rootOf(pheno, false)+".markerNames", false).getArray(), false)) {
...
//		System.out.println("score = "+score);
		...
		System.out.println("score = "+score+"; final permutation: "+hits+"/"+count+"="+ext.prettyP(((double)hits+1)/((double)count+1)));
        
        return ext.prettyP(((double)hits+1)/((double)count+1));
	}
	
	public static void evaluateAllSets(String pheno, String mapfile, int offset, String results, String repDir, String groupings_file, double p_thresh, double nrss_thresh) {
		BufferedReader reader;
        PrintWriter writer;
        String temp;
        String pathway;
        ...
        try {
	        ...
	        while (reader.ready()) {
	        	if (temp.startsWith("<h4>")) {
    				...
    				pvalue = evaluateGeneset(pheno, Array.toStringArray(snpsInPathway), results, repDir, p_thresh, nrss_thresh);
    				
    				writer.print("\t"+"\t"+pathway+"\t"+kegg.pathwayLookup.get(pathway)+"\t"+subheader+"\t"+(kegg.pathways.containsKey(pathway)?kegg.pathways.get(pathway).size():0));
	...
	public static double calculateScore(float[] pvals, boolean[] included, double p_threshold, double nrss_index_threshold) {
		double score = 0;
	...
	public static void parseKEGG(String ko_file, String groupings_file) {
	    ...
	    for (int i = 0; i<keys.length; i++) {
			Files.writeList(Array.toStringArray(kegg.pathways.get(keys[i])),	dir+"pathways/"+keys[i]+".list");
	   		try {
		        writer = new PrintWriter(new FileWriter(dir+"pathways/"+keys[i]+".crf"));
	        	writer.println("genes");
	        	writer.println(dir+"pathways/"+keys[i]+".list 0 out="+dir+"pathways/"+keys[i]+".snps");
        ...
        try {
	        ...
	        for (int i = 0; i<keys.length; i++) {
				writer.println(keys[i]+"\t"+kegg.genes.get(keys[i]).size()+"\t"+Array.toStr(Array.toStringArray(kegg.genes.get(keys[i]))));
            }
	        writer.close();
        } catch (Exception e) {
        ...
        try {
	        ...
	        while (reader.ready()) {
	        	if (temp.startsWith("<h4>")) {
	        		...
	        		pathway = "ko"+temp.substring(temp.indexOf(".html")-5, temp.indexOf(".html"));
	        		subheader = temp.substring(temp.indexOf(".html")+7, temp.indexOf("</a>"));
	        		writer.println("\t"+"\t"+pathway+"\t"+kegg.pathwayLookup.get(pathway)+"\t"+subheader+"\t"+(kegg.pathways.containsKey(pathway)?kegg.pathways.get(pathway).size():0));
	        	}
	        	temp = reader.readLine();
	        }
	        reader.close();
...
//					evaluateGeneset("icam_whites/pheno.dat", HashVec.loadFileToStringArray("pathways/"+sets[i]+".snps", false, new int[] {0}, false), "icam_whites/actual.qassoc", "icam_whites/") +"\t"+
//					evaluateGeneset("psel_whites/pheno.dat", HashVec.loadFileToStringArray("pathways/"+sets[i]+".snps", false, new int[] {0}, false), "psel_whites/actual.qassoc", "psel_whites/")
//				);
//				writer.flush();
//            }
//	        writer.close();
...
//        kegg = new KEGGpathway(keggFile, true);
	...
	public static void parseSNPsInPathway(String pathway, String mapfile, int offset) {
		...
		if (kegg.pathways.containsKey(pathway)) {
			...
			Files.writeList(Array.toStringArray(snpsInPathway), ext.parseDirectoryOfFile(mapfile)+pathway+"_"+((int)((double)offset/1000.0))+"K.dat");
		} else {
...
//	    String list = "";
	    ...
	    "   (2) covariates filename (i.e. covars="+covars+" (optional) (default))\n"+
	    "   (2b) include sex as a covariate (i.e. sex="+sex+" (optional) (default))\n"+
	    "   (3) link covariates to pheno, not geno (i.e. link="+linkCovToPheno+" (default))\n"+
	    ...
	    "   (6) result set to compare against (i.e. results="+results+" (default))\n"+
	    "   (7) name of directory containing replicates (i.e. repDir="+repDir+" (default))\n"+
...
//	    "   (9) evaluate set of snps (i.e. set=list.txt (not the default))\n"+
	    ...
	    "   (12) max offset from SNP position to gene start/stop (i.e. offset="+offset+" (default))\n"+
	    "   (13) p-value threshold for inclusion in score (i.e. p_thresh="+p_threshold+" (default))\n"+
	    "   (14) index threhsold for NRS score (i.e. nrss_thresh="+nrss_index_threshold+" (default; 1=nrss_not_used))\n"+
	    "";
...
//			    numArgs--;
		    	...
		    	covars = ext.parseStringArg(args[i], null);
			    numArgs--;
		    } else if (args[i].startsWith("sex=")) {
		    	sex = ext.parseBooleanArg(args[i]);
			    numArgs--;
		    } else if (args[i].startsWith("link=")) {
		    	linkCovToPheno = ext.parseBooleanArg(args[i]);
			    numArgs--;
		    } else if (args[i].startsWith("results=")) {
		    	results = args[i].split("=")[1];
			    numArgs--;
		    } else if (args[i].startsWith("evaluate=")) {
		    	geneset = args[i].split("=")[1];
			    numArgs--;
		    } else if (args[i].startsWith("repDir=")) {
		    	repDir = args[i].split("=")[1];
			    numArgs--;
		    } else if (args[i].startsWith("-permAndRun")) {
		    	permAndRun = true;
			    numArgs--;
		    } else if (args[i].startsWith("-compress")) {
		    	compress = true;
			    numArgs--;
		    } else if (args[i].startsWith("-parseGeneInfo")) {
		    	parseGeneInfo = true;
			    numArgs--;
		    } else if (args[i].startsWith("-allSets")) {
		    	allSets = true;
			    numArgs--;
		    	...
		    	offset = ext.parseIntArg(args[i]);
			    numArgs--;
		    } else if (args[i].startsWith("p_thresh=")) {
		    	p_threshold = ext.parseDoubleArg(args[i]);
			    numArgs--;
		    } else if (args[i].startsWith("nrss_thresh=")) {
		    	nrss_index_threshold = ext.parseDoubleArg(args[i]);
			    numArgs--;
		    } else {
		    	System.err.println("Error - what to do with "+args[i]);
		    }
	    }
	    if (numArgs!=0) {
		    System.err.println(usage);
...
//	    		runList(list);
	    		...
	    		evaluateAllSets(pheno, mapfile, offset, results, repDir, KEGG_GROUPINGS, p_threshold, nrss_index_threshold);
	    	} else if (!geneset.equals("")) {
	    		evaluateGeneset(pheno, HashVec.loadFileToStringArray(geneset, false, new int[] {0}, false), results, repDir, p_threshold, nrss_index_threshold);
	    	}
	    } catch (Exception e) {
		    e.printStackTrace();
	    }
