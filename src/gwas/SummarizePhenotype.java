...
public class SummarizePhenotype {
	...
	public static final String[] ID_NAMES = {"IndividualID", "ID", "IID", "UID", "UniqueID", "IndID", "Sample"}; 
	...
	public static final String[][] DEFAULT_PARAMS = {
		...
		{"Age - standard deviation", "stdev", "Age", "Age=ValidDouble", "-blank", "-sf=2", "-stdev"},
	};

	public static String runDescriptiveStat(double[] data, String statOperation, boolean includeStdev, boolean useBlankForNull, boolean showPercent, int sf, Logger log) {
        double mean, stdev = Double.NaN;
        int percentile;
        String result;

        if (statOperation.equalsIgnoreCase("mean")) {
			mean = (Array.sum(data) / (double)data.length);
			if (includeStdev) {
				stdev = Array.stdev(data);
			}
			result = (mean!=0? (showPercent? ext.formDeci(mean * 100, sf) + "%" : ext.formDeci(mean, sf)) + (includeStdev? " (" + (showPercent? ext.formDeci(stdev * 100, sf) + "%" :  ext.formDeci(stdev, sf)) + ")" : "") : (useBlankForNull? "" : "."));

		} else if (statOperation.equalsIgnoreCase("stdev")) {
			stdev = Array.stdev(data);
			result =  (!Double.isNaN(stdev))? (showPercent? ext.formDeci(stdev * 100, sf) + "%" : ext.formDeci(stdev, sf))  : (useBlankForNull? "" : ".");
			...
			Arrays.sort(data);
			if (percentile == 100) {
				result = (data.length==0? (useBlankForNull? "" : "0" ) : ext.formDeci(data[data.length- 1], sf));
			} else {
				result = (data.length==0? (useBlankForNull? "" : "0" ) : ext.formDeci(data[(int) (((double)percentile / 100) * data.length + .5)], sf));
			}
	...
	public static String[] parseStatisticsFromParameters(Vector<String[]> modelData, String[] columnLabels, String[][] parameters, Logger log) {
			...
			for (int j = 3; j < parameters[i].length; j++) {
				if (parameters[i][j].equals("-stdev")) {
					...
					if (line.length < 2 || line[1].equals("")) {
						log.reportError("Parameter contains invalid filter: " + parameters[i][j] + ". Should include an '=' sign.");
						return null;
					}
...
//							if (columnIndex == -1) {
								log.reportError("Cannot find ANY match for the column specified by the parameter (" + parameters[i][j] + ") in the data. Ignored this parameter.");
								return null;
//							} else {
//								log.reportError("Cannot find EXACT match for the column specified by the parameter (" + parameters[i][j] + ") in the data. Instead, used the following column from the data: " + columnLabels[columnIndex]);
//							}
						}
//					}
					line[1] = line[1].trim();
					if (! line[1].equalsIgnoreCase("validDouble") && ! line[1].equalsIgnoreCase("validInteger") && ! ext.isValidInteger(line[1]) && ! ext.isValidDouble(line[1])) {
						log.reportError("Unrecognized parameter: " + parameters[i][j] + ". Expecting 'variableName = validDouble' or 'variableName = validInteger'.");
					}
...
//			} else {
				...
				if (columnIndex == -1) {
//					columnIndex = ext.indexOfStr(parameters[i][2].trim(), columnLabels, false, false, log, true);
//					if (columnIndex == -1) {
						log.reportError("Cannot find ANY match for the column specified by the parameter (" + parameters[i][2] + ") in the data. Ignored this parameter.");
						return null;
//					} else {
//						log.reportError("Cannot find EXACT match for the column specified by the parameter (" + parameters[i][2] + ") in the data. Instead, used the following column from the data: " + columnLabels[columnIndex]);
//					}
				}
//			}
	...
	public static Vector<String[]> selectColumnsAndRows(Hashtable<String, String[]> traitDataTmp, int[] columnsToKeep, String idListFileName, Logger log) {
	...
	public static Hashtable<String, String[]> loadData(String dataFileName, Logger log) {
		...
		delimiter = null;
		for (int i = 0; i < FILETYPE_DELIMINATOR.length; i++) {
			if (dataFileName.toLowerCase().endsWith(FILETYPE_DELIMINATOR[i][0])) {
				delimiter = FILETYPE_DELIMINATOR[i][1];
			}
		}
		if (delimiter != null) {
			try {
				...
				if (ext.indexOfStr(line[0].trim(), ID_NAMES, false, true, log, true) == -1) {
					log.reportError("Cannot identify the 1st column of the following file as an ID '" + dataFileName + "'");
				}
		...
		try {
			...
			while (reader.ready()) {
				...
				if (! line[0].startsWith("#")) {
					if (line.length != 4) {
						...
						models.put(modelName, new String[][] {splitAndTrim(line[1], PARAMETERS_FILE_DELIMITER_FOR_DATAFILES), splitAndTrim(line[2], PARAMETERS_FILE_DELIMITER_FOR_VARIABLES_IN_MODEL), new String[] {line[3].toLowerCase()}});
			...
			for (int i = 0; i < result.length; i ++) {
				...
				for (int j = 0; j < dataFileNames.length; j++) {
					...
					currentData.addAll(selectColumnsAndRows(currentDataTmp, columnsToKeep, wkDir + idListFileName, log));
				}
				...
				exportData(wkDir + modelName + FILENAME_EXTIONSION_TO_OUTPUT_PHENOTYPE_DATA_WITH, line, currentData);
//				result[i] = parseStatisticsFromParameters(currentData, variablesInModel, parameters, log);
				result[i] = parseStatisticsFromParameters(currentData, line, adjustParameters(parameters, line, log), log);
			}
	...
	public static String[][] adjustParameters(String[][] parameters, String[] columnLabels, Logger log) {
		String[][] result;
		String[] line;
		int index;
				...
				if (! result[i][j].startsWith("-")) {
					...
					if (ext.indexOfStr(line[0], columnLabels, false, true, log, true) == -1) {
						if (line[0].toLowerCase().contains("trait")) {
							...
							if (index > -1) {
								log.report("No exact match for parameter " + result[i][j] + " in column labels. Instead, used " + columnLabels[index]);
							} else {
								log.reportError("No exact match for parameter " + result[i][j] + " in the column labels. Program exit on error.");
								System.exit(0);
							}
...
//					logFileName = line.split("=")[1];
		...
		Files.generateTables(resultFileName, new String[] {phenoDataFileName}, new String[] {ext.rootOf(phenoDataFileName)}, parameters, log);
	}
...
//								if (index > -1) {
//									log.report("The parameter " + line[j] + " does not match any column label in the dataset. Instead, used " + columnLabels[index]);
//								} else {
//									log.reportError("The parameter " + line[j] + " does not match any column label in the dataset. Program exit on error.");
//									System.exit(0);
//								}
...
//			Files.generateTables(wkDir + "result_" + ext.rootOf(filename) + ".xln", new String[] {filename}, new String[] {ext.rootOf(filename)}, parameters, log);
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
//	}
...
//			summarizeFromParameters("N:/statgen/ExomeChip/Hemostasis/CARDIA/cardia_blacks_f7_y5.xln", log);
