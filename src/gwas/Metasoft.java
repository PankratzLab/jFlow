...
//				for (int i = 5; i < line.length; i += 4) {
				for (int i = 0; i < numStudies; i++) {
					if (line[1+i*4+0].equals(".") || line[1+i*4+1].equals(".")) {
		        		...
		        		switch (Metal.determineStrandConfig(new String[] {line[1+i*4+0], line[1+i*4+1]}, stdAlleles)) {
		        		...
		        		case Metal.STRAND_CONFIG_DIFFERENT_ALLELES:
		        			System.err.println("Error - for marker "+line[0]+" Study"+(i+1)+" has different alleles ("+line[1+i*4+0]+"/"+line[1+i*4+1]+") than the rest ("+stdAlleles[0]+"/"+stdAlleles[1]+")");
		        			break;
		        		case Metal.STRAND_CONFIG_SPECIAL_CASE:
		        			System.err.println("Warning - marker "+line[0]+" has a special case starting with Study"+(i+1)+": alleles ("+line[1+i*4+0]+"/"+line[1+i*4+1]+") where previous had only ("+stdAlleles[0]+"/"+stdAlleles[1]+")");
		                    break;
	                    default:
...
//	        		writer.print("\t"+line[1+i*4+0]+"\t"+line[1+i*4+1]+"\t"+line[1+i*4+2]+"\t"+line[1+i*4+3]);
	...
	public static void applyLambdas(String inputFile, String outputFile, double[] lambdas, Logger logger) {
		String[] line;
		...
		try {
			...
			while (reader.ready()) {
				...
				if (line.length != lambdas.length*2+1) {
					System.err.println("Error - mismatched number of lambdas and/or columns in '"+inputFile+"' (with "+lambdas.length+" lambdas, expecting "+(lambdas.length*2+1)+" (1+"+lambdas.length+"*2) columns");
				}
				for (int i = 0; i < lambdas.length; i++) {
					if (!line[1+i*2+1].equals("NA")) {
						line[1+i*2+1] = ext.formDeci(Double.parseDouble(line[1+i*2+1])*lambdas[i], 9);
					}
				}
