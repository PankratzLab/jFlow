...
public class BatchEffects {
	...
	public static void setupBatchEffects(String pedfile, String dnas) {
		...
		try {
			...
			for (int i = 1; i<=numBatches; i++) {
				...
				writer.println("plink --bfile pd_gwas --keep noWGAs.txt --pheno batchListing.txt --mpheno "+i+" --assoc --out assoc."+i);
	...
	public static void parseBatchEffects(String dir) {
		...
		try {
			...
			System.err.println("Error: file \""+"batchListing.txt"+"\" not found in current directory");
			System.exit(1);
		} catch (IOException ioe) {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				numArgs--;
			} else if (args[i].startsWith("dnas=")) {
				dnas = args[i].split("=")[1];
				numArgs--;
			}
		}
