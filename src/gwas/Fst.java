...
public class Fst {
	...
	private double[] f;	// inbreeding coefficient (higher indicates inbreeding, negative indicates isolate breaking (i.e. Wahlund effect))
	...
	private double hT; // expected heterozygosities for overall total population
	private double fis;
	private double fst;
	private double fit;
	...
	public static String[][] prepFiles(String filename, String plinkRoot) {
		...
		if (new File(dir+root+".bim.ser").exists()) {
			...
			System.err.println("Error - algorithm requires a '"+root+".bim' file to ensure marker order and allelic strand");
			return null;
		} else {
			markerInfo = HashVec.loadFileToStringMatrix(dir+root+".bim", false, new int[] {1, 0, 3, 4, 5}, false);
			...
			for (int i = 0; i<files.length; i++) {
				...
				new File(dir+files[i]).renameTo(new File(dir+trav));
            }			
		}
        ...
        for (int i = 0; i<keys.length; i++) {
        	...
        	if (v.size() == 1) {
        		if (!v.elementAt(0).equals("FID\tIID")) {
        			System.err.println("Error - found population subgroup ("+keys[i]+") with only one member: "+v.elementAt(0)+"; this will not be run");
        			return null;
        		}
        	} else {
        		if (v.size() < 20) {
        			System.err.println("Warning - subgroup "+keys[i]+" only has "+v.size()+" members");
        		}
        		list = keys[i]+".list";
        		v.insertElementAt("FID\tIID", 0);
        		array = Array.toStringArray(v);
        		if (!new File(dir+list).exists() || !Array.equals(array, HashVec.loadFileToStringArray(dir+list, false, new int[] {0,1}, false), false) || !new File(dir+keys[i]+".hwe.ser").exists()) {
	        		Files.writeList(array, dir+list);        		
	        		CmdLine.run("plink --bfile "+root+" --keep "+keys[i]+".list --hardy --out "+keys[i], dir);
	        		...
	        		while (parser.ready()) {
	        			...
	        			if (line != null) {
		        			if (!line[0].equals(markerInfo[count][0])) {
		        				...
		        				reverse = true;
		        			} else if (!line[1].equals(markerInfo[count][3]) || !line[2].equals(markerInfo[count][4])) {
		        				System.err.println("Error - mismatched alleles in "+keys[i]+".hwe"+" for marker "+line[0]+"; expecting "+markerInfo[count][3]+"/"+markerInfo[count][4]+", found "+line[1]+"/"+line[2]);
		        				return null;
		        			} else {
		        				reverse = false;
		        			}
		        			alleleCounts[count] = Array.toIntArray(Sort.putInOrder(line[3].split("/"), reverse?new int[] {2, 1, 0}:new int[] {0, 1, 2}));
		        			count++;
	        			}
	        		}
	...
	public static void runAnalyses(String filename, String plinkRoot, String[] incl) {
		...
		if (incl == null) {
			...
			if (count > 0) {
				System.err.println("Error - could not find "+count+" of the "+incl.length+" necessary files to perform calculations");
				return;
			}
		}
		...
		try {
			...
			for (int i = 0; i<markerInfo.length; i++) {
				...
				writer.println(markerInfo[i][0]+"\t"+calcFst(counts, true)+"\t"+calcFst(counts, false)+"\t"+Array.toStr(calcPs(counts)));
	...
	public static void splitIndividualsIntoNgroups(String filename, int n) {
        ...
        for (int i = 0; i<keys.length; i++) {
        	...
        	for (int j = 0; j<inds.length; j++) {
        		fileContents[count%n].add(inds[j]+"\t"+keys[i]);
        		count++;
            }
        }
	...
	public static void main(String[] args) {
	    ...
	    for (int i = 0; i<args.length; i++) {
		    if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
			    ...
			    plinkRoot = args[i].split("=")[1];
			    numArgs--;
		    } else if (args[i].startsWith("file=")) {
			    filename = args[i].split("=")[1];
			    numArgs--;
		    } else if (args[i].startsWith("incl=")) {
			    incl = args[i].split("=")[1].split(",");
			    numArgs--;
		    } else if (args[i].startsWith("split=")) {
			    n = Integer.parseInt(args[i].split("=")[1]);
			    numArgs--;
		    }
	    }
	    if (numArgs!=0) {
...
//	    filename = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Consortium\\Fst\\testAshk\\ashkbrit.txt";
//	    plinkRoot = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Consortium\\Fst\\testAshk\\plink";
	    
//	    filename = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Consortium\\Fst\\EuropeanClasses.txt";
//	    plinkRoot = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Consortium\\Fst\\plink";
//	    n = 2;
