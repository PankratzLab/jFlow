...
//		int step;
		...
		commands = "cd chr#\n";
//		commands += EXECUTABLE+" -d "+prefixSubset+".chr#.dat -p "+prefixSubset+".chr#.ped --autoFlip --hapmapFormat --snps  "+(trimming?"truncated":"genotypes")+"_chr#_CEU_r22_nr.b36_fwd_legend.txt --haps "+(trimming?"truncated":"genotypes")+"_chr#_CEU_r22_nr.b36_fwd.phase --rounds 100 --greedy --prefix MACH_step1_chr#\n";
//		commands += EXECUTABLE+" -d "+prefixAll+".chr#.dat -p "+prefixAll+".chr#.ped --autoFlip --hapmapFormat --snps "+(trimming?"truncated":"genotypes")+"_chr#_CEU_r22_nr.b36_fwd_legend.txt --haps "+(trimming?"truncated":"genotypes")+"_chr#_CEU_r22_nr.b36_fwd.phase --crossover MACH_step1_chr#.rec --errormap MACH_step1_chr#.erate --greedy --mle --mldetails --prefix MACH_step2_chr#\n";
		commands += EXECUTABLE+" -d "+prefixSubset+".chr#.dat -p "+prefixSubset+".chr#.ped --rounds 30 --states 400 --phase --interim 5 --sample 5 --prefix MACH_step1_chr#\n";
		commands += EXECUTABLE+" -d "+prefixAll+".chr#.dat -p "+prefixAll+".chr#.ped --crossover MACH_step1_chr#.rec --errormap MACH_step1_chr#.erate --phase --prefix MACH_step2_chr#\n";
		commands += "cd .."+"\n";
		commands += "\n";
...
//				list = Files.qsub("", null, i*step+0, i==nodesToUse.length-1?22:((i+1)*step-1), commands, "chr", null, -1, nodesToUse[i]);
	...
	public static void trimReference(String prefixAll, int chr) {
        ...
        try {
	        ...
	        System.err.println("Error: file \""+prefixAll+".chr"+chr+".map"+"\" not found in current directory");
	        System.exit(1);
        } catch (IOException ioe) {
        ...
        count = 0;
        try {
	        reader = new BufferedReader(new FileReader("genotypes_chr"+chr+"_CEU_r22_nr.b36_fwd_legend.txt"));
	        writer = new PrintWriter(new FileWriter("truncated_chr"+chr+"_CEU_r22_nr.b36_fwd_legend.txt"));
	        ...
	        System.err.println("Error: file \""+"genotypes_chr"+chr+"_CEU_r22_nr.b36_fwd_legend.txt"+"\" not found in current directory");
	        System.exit(1);
        } catch (IOException ioe) {
	        System.err.println("Error reading file \""+"genotypes_chr"+chr+"_CEU_r22_nr.b36_fwd_legend.txt"+"\"");
	        System.exit(2);
        }
        
        System.out.println("Truncating HapMap files to chr"+chr+":"+start+"-"+stop+" (indices: "+startAtIndex+" to "+stopBeforeIndex+")");
        ...
        try {
	        ...
	        System.err.println("Error: file \""+"genotypes_chr"+chr+"_CEU_r22_nr.b36_fwd.phase"+"\" not found in current directory");
	        System.exit(1);
        } catch (IOException ioe) {
	        System.err.println("Error reading file \""+"genotypes_chr"+chr+"_CEU_r22_nr.b36_fwd.phase"+"\"");
	        System.exit(2);
        }
	}
	...
	public static void decodePhasedHapMap(String dir, int chr) {
        ...
        snpData = HashVec.loadFileToStringArray(dir+"truncated_chr"+chr+"_CEU_r22_nr.b36_fwd_legend.txt", false, true, new int[] {0,1,2,3}, false);
        ...
        try {
	        reader = new BufferedReader(new FileReader(dir+"truncated_chr"+chr+"_CEU_r22_nr.b36_fwd.phase"));
	        ...
	        System.err.println("Error: file \""+dir+"truncated_chr"+chr+"_CEU_r22_nr.b36_fwd.phase"+"\" not found in current directory");
	        System.exit(1);
        } catch (IOException ioe) {
	        System.err.println("Error reading file \""+dir+"truncated_chr"+chr+"_CEU_r22_nr.b36_fwd.phase"+"\"");
	        System.exit(2);
        }
	}
	...
	public static void listIndividualsInMldoseSlowly(String filename, String outfile) {
        ...
        try {
			...
			while (c>=0) {
				...
				if (c>=0) {
					writer.println(trav.substring(0, trav.indexOf("->"))+"\t"+trav.substring(trav.indexOf("->")+2));
					writer.flush();
				}
...
//			reader = new BufferedReader(new FileReader(filename));
	...
	public static void extractDosageForPlink(String dir, String pedfile, String machDosage, String machMarkers, String markersToTest) {
		...
		markerNames = Array.toStringArray(HashVec.loadFileToVec(dir+machMarkers, true, new int[] {0,1,2}, false, false));
			...
			while (reader.ready()) {
				...
				if (Integer.parseInt(line[5])>0) {
					if (hash.containsKey(line[0]+"->"+line[1])) {
						System.err.println("Warning - multiple entries for individual "+line[0]+"-"+line[1]+" in the .fam file (only the last one will be used)");
					}
					hash.put(line[0]+"->"+line[1], line[2]+"\t"+line[3]+"\t"+line[4]+"\t"+line[5]);
				}
				count++;
			}
			reader.close();
...
//	            System.out.println("There are "+reader.readLine().trim().split("[\\s]+").length+" columns in the first row");
//	            
//	            reader.close();
//            } catch (FileNotFoundException fnfe) {
...
//            }
			...
			if (step==0) {
				...
				step = (int)(((double)heapMaxSize*0.75)/bytesPerMarker);
				System.out.println("Heap size (memory) was set to "+ext.formDeci((double)heapMaxSize/1024/1024/1024, 2)+" Gb; which allows us to do "+step+" markers at a time comfortably");
			...
			if (step < markerNames.length) {
				...
				System.out.println("Running and doing all "+markerNames.length+" markers in a single pass");
			...
			for (int i = 0; i<rowCount; i++) {
            	writer.print("\t"+indIDs[i].substring(0,indIDs[i].indexOf("->"))+" "+indIDs[i].substring(indIDs[i].indexOf("->")+2));
            }
			writer.println();
			...
			while (inc<markerNames.length) {
				...
				for (int i = 0; i<rowCount; i++) {
					...
					for (int j = 0; j<(inc+step>markerNames.length?markerNames.length-inc:step); j++) {
						trav = "";
					...
					if ((char)c!='\n') {
						System.err.println("Error transposing file: did not find "+(markerNames.length+2)+" columns for row "+(i+1));
						System.err.println("   Make sure that the delimiter is set correctly");
					}
				}

				for (int i = 0; i<(inc+step>markerNames.length?markerNames.length-inc:step); i++) {
			...
			try {
	            ...
	            for (int i = 0; i<indIDs.length; i++) {
	            	writer.print(indIDs[i].substring(0,indIDs[i].indexOf("->"))+"\t"+indIDs[i].substring(indIDs[i].indexOf("->")+2));
	            	if (hash.containsKey(indIDs[i])) {
	            		writer.println("\t"+hash.get(indIDs[i]));
	            	} else {
	            		writer.println("\t0\t0\t0\t-9");
	            		System.err.println("Error - missing gender and phenotype for "+indIDs[i]);
	            	}
                }
	            writer.close();
            } catch (Exception e) {
			...
			System.out.println("Looks like we used "+(Runtime.getRuntime().maxMemory() - freeMemory)+" in this run");
		} catch (FileNotFoundException fnfe) {
			System.err.println("Error: file \""+dir+machDosage+"\" not found in current directory");
			System.exit(1);
		} catch (IOException ioe) {
		...
		for (int i = 0; i<chrKeys.length; i++) {
			...
			try {
				...
				for (int ind = 0; ind<indIDs.length; ind++) {
					...
					while (ext.indexOfChar((char)(c = in.read()), DELIMITERS) == -1&&c>=0); // MLDOSE column
...
//	        regionKeys = HashVec.getKeys(regionMarkerHashes, true, false);
	...
	public static void extractGenotypesForPlink(String dir, String pedfile, String machProb, String machMarkers, String markersToTest, double rsqThreshold, double probRequirement) {
		...
		try {
			...
			for (int i = 0; i<use.length; i++) {
				use[i] = (allMarkers || subset.containsKey(markerNames[i])) && Double.parseDouble(rsqs[i]) > rsqThreshold;
				if (use[i]) {
			...
			while (c >= 0) {
				if (count == -2) {
					...
					if (c>=0) { // final run does pass through here
			        	writer.print(trav.substring(0,trav.indexOf("->"))+"\t"+trav.substring(trav.indexOf("->")+2));
				...
				if (c>=0) {
					...
					if ((char)c=='\n' || c < 0) {
						...
						if (count != markerNames.length) {
							System.err.println("Error - mismatched number of columns/markers in row "+(rowCount+1));
							System.exit(1);
						}
						count = -2;
						writer.println();
	...
	public static void pickBest(int size) {
        ...
        try {
	        ...
	        ext.checkHeader(reader.readLine().trim().split("[\\s]+"), new String[] {"FID", "IID", "MISS_PHENO", "N_MISS", "N_GENO", "F_MISS"}, true, true);
	        while (reader.ready()) {
	        	line = reader.readLine().trim().split("[\\s]+");
        ...
        for (int i = 0; i<ids.length; i++) {
        	HashVec.addToHashVec(hashV, genders[i]+"_"+affections[i], ids[i][0]+"\t"+ids[i][1]+"\t"+hash.get(ids[i][0]+"\t"+ids[i][1]), false);
        }
	...
	public static void createDatabase(String machRoot) {
		...
		try {
	        ...
	        System.err.println("Error: file \""+machRoot+".mldose"+"\" not found in current directory");
	        System.exit(1);
        } catch (IOException ioe) {
...
//		extract = "list11.txt";
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				keeps = ext.parseStringArg(args[i], null);
				numArgs--;
			} else if (args[i].startsWith("exclude=")) {
				excludes = ext.parseStringArg(args[i], null);
				numArgs--;
			} else if (args[i].startsWith("convert=")) {
				convert = args[i].split("=")[1];
				numArgs--;
				...
				numArgs--;
			} else if (args[i].startsWith("trim=")) {
				trim = Integer.parseInt(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("decode=")) {
				decode = Integer.parseInt(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("list=")) {
				list = ext.parseStringArg(args[i], null);
				numArgs--;
			} else if (args[i].startsWith("ped=")) {
				pedfile = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("mach=")) {
				machRoot = args[i].split("=")[1];
				numArgs--;
				...
				numArgs--;
			} else if (args[i].startsWith("-extractGenotypes")) {
				extractGenotypes = true;
				numArgs--;
			} else if (args[i].startsWith("-extractAllGenotypes")) {
				extractAllGenotypes = true;
				numArgs--;
			} else if (args[i].startsWith("rsq=")) {
				rsqThreshold = Double.parseDouble(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("prob=")) {
				probRequirement = Double.parseDouble(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("-pickBest")) {
				pickBest = true;
				numArgs--;
			} else if (args[i].startsWith("pickSize=")) {
				pickSize = Integer.parseInt(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("-check")) {
				checkResults = true;
				numArgs--;
			} else {
				System.err.println("Error - don't know what to do with: "+args[i]);
			}
		}
		if (numArgs!=0) {
			System.err.println(usage);
