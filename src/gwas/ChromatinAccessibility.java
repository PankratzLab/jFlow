...
//	public static final String SRC_DIR = "D:/grants/2012.06 Hepatoblastoma/Chromatin accessibility/rawData/heme/";
//	public static final String SRC_DIR = "D:/grants/2012.06 Hepatoblastoma/Chromatin accessibility/rawData/cancers/";
//	public static final String SRC_DIR = "D:/grants/2012.06 Hepatoblastoma/Chromatin accessibility/rawData/other/";
	public static final String SRC_DIR = "D:/grants/2012.06 Hepatoblastoma/Chromatin accessibility/rawData/all/";
	public static final String[][] PVAL_HEADER = {{"Marker", "SNP"}, {"Chr"}, {"Pos"}, {"pval", "p-val"}};
	public static final String[] CLASSES = {"All", "Valid", "DNaseOnly_OpenChrom", "FAIREOnly_OpenChrom", "ChIPOnly"};
	...
	public static void stats(String dir) {
		...
		try {
			...
			for (int i = 0; i < files.length; i++) {
				...
				System.out.println("Parsed: "+ext.rootOf(files[i]));
				writer.println(ext.rootOf(files[i]));
...
//					System.out.println("Re-loaded serialized version of '"+ext.rootOf(files[i])+"' in " + ext.getTimeElapsed(time));
	...
	//					Segment.loadRegions(dir+files[i], 0, 1, 2);
						...
						while (reader.ready()) {
							...
							allSeg.elementAt(0).elementAt(chr).add(new Segment(chr, Integer.parseInt(line[1]), Integer.parseInt(line[2])));
							allSeg.elementAt(Integer.parseInt(line[20])).elementAt(chr).add(new Segment(chr, Integer.parseInt(line[1]), Integer.parseInt(line[2])));
							for (int j = 2; line[20].equals("1") && j < CLASSES.length; j++) {
								allSeg.elementAt(j).elementAt(chr).add(new Segment(chr, Integer.parseInt(line[1]), Integer.parseInt(line[2])));
							}
						}
						reader.close();
					} catch (FileNotFoundException fnfe) {
	...
	public static void filter(String bed_dir, String filename, String levels) {
		...
		try {
			...
			try {
				for (int i = 0; i < files.length+1; i++) {
					for (int j = 0; j < CLASSES.length; j++) {
						...
						if (i == files.length) {
							writers[i][j][0] = new PrintWriter(new FileWriter(file_dir+ext.rootOf(filename)+"/"+ext.addToRoot(filename, "_allMerged_"+CLASSES[j]+"_in")));
							writers[i][j][1] = new PrintWriter(new FileWriter(file_dir+ext.rootOf(filename)+"/"+ext.addToRoot(filename, "_allMerged_"+CLASSES[j]+"_out")));
						} else {
							writers[i][j][0] = new PrintWriter(new FileWriter(file_dir+ext.rootOf(filename)+"/"+ext.addToRoot(filename, "_"+ext.rootOf(files[i])+"_"+CLASSES[j]+"_in")));
							writers[i][j][1] = new PrintWriter(new FileWriter(file_dir+ext.rootOf(filename)+"/"+ext.addToRoot(filename, "_"+ext.rootOf(files[i])+"_"+CLASSES[j]+"_out")));
						}
					}
				}
			} catch (Exception e) {
			...
			try {
				...
				indices = ext.indexFactors(PVAL_HEADER, reader.readLine().trim().split("[\\s]+"), false, false, true, true);
				count = 0;
				while (reader.ready()) {
					count++;
					if (count % 10000 == 0) {
...
//						System.out.println(count);
					...
					seg = new Segment(chr, Integer.parseInt(line[indices[2]]), Integer.parseInt(line[indices[2]]));
					for (int i = 0; i < files.length+1; i++) {
						for (int j = 0; j < CLASSES.length; j++) {
							if (i < files.length) {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				...
				numArgs--;
			} else if (args[i].startsWith("levels=")) {
				filename = args[i].split("=")[1];
				numArgs--;
			} else {
