			...
			try {
				...
				while (reader.ready()) {
					...
					if (!probeGeneLookup.containsKey(probeName)) {
						probeGeneLookup.put(probeName, ext.rootOf(files[i].getName()));
					}
					...
					if (regions.containsKey(region)) {
						...
						if (!values[0].equals(line[8])||!values[1].equals(line[7])||!values[2].equals(line[12])||!values[3].equals(line[13])||!values[4].equals(line[14])) {
							System.err.println("Error - mismatch with "+ind);
						}
					} else {
						// AvgCt, CtStderr, RQ, RQ Min, RQ Max
						regions.put(region, values = new String[] {line[8], line[7], line[12], line[13], line[14]});
					}
				}
				reader.close();
		...
		try {
			...
			for (int i = 0; i<probeNames.length; i++) {
				for (int j = 0; j<regionNames.length; j++) {
					writer.print("\t"+regionNames[j]+"_"+probeGeneLookup.get(probeNames[i])+"-"+probeNames[i]+"_AvgCt\t"+regionNames[j]+"_"+probeGeneLookup.get(probeNames[i])+"-"+probeNames[i]+"_CtStderr\t"+regionNames[j]+"_"+probeGeneLookup.get(probeNames[i])+"-"+probeNames[i]+"_RQ\t"+regionNames[j]+"_"+probeGeneLookup.get(probeNames[i])+"-"+probeNames[i]+"_RQ Min\t"+regionNames[j]+"_"+probeGeneLookup.get(probeNames[i])+"-"+probeNames[i]+"_RQ Max");
				}
			}
			...
			for (int i = 0; i<probeNames.length; i++) {
				for (int j = 0; j<regionNames.length; j++) {
					writer.print("\t"+regionNames[j]+"_"+probeGeneLookup.get(probeNames[i])+"-"+probeNames[i]+"_RQ_log");
				}
			}
			writer.println("\tMeanSampleStderr");
			means = new double[inds.length][probeNames.length][regionNames.length];
			...
			for (int j = 0; j<probeNames.length; j++) {
				writer = new PrintWriter(new FileWriter(dir+probeGeneLookup.get(probeNames[j])+"_"+probeNames[j]+".xln"));
				writer.print("Sample");
				for (int k = 0; k<regionNames.length; k++) {
					writer.print("\t"+regionNames[k]+"_"+"_AvgCt\t"+regionNames[k]+"_"+"_CtStderr\t"+regionNames[k]+"_"+"_RQ\t"+regionNames[k]+"_"+"_RQ Min\t"+regionNames[k]+"_"+"_RQ Max");
				}
				writer.println();
				for (int i = 0; i<inds.length; i++) {
					regions = individuals.get(inds[i]).get(probeNames[j]);
					writer.print(inds[i]);
					for (int k = 0; k<regionNames.length; k++) {
						values = regions.get(regionNames[k]);
						if (values==null) {
