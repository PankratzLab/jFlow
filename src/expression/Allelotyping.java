...
public class Allelotyping {
	...
	public static void parse(String dir, String filename) {
		...
		Hashtable<String,String> lookup = HashVec.loadFileToHashString(dir+"snpGeneLookup.txt", false);
...
//			summary.println("gene\tsnp\tN\tproportion\tstd dev\tt-value	df\tregion A\tregion B\tregion C");
//			summary.println("Gene\tSNP\tAllele\tN\tProportion\tStdev\tt-value\tdf\tregion A ratio\tp-value\tcountAbove\tcountBelow\tregion B ratio\tp-value\tcountAbove\tcountBelow\tregion C ratio\tp-value\tcountAbove\tcountBelow");
			...
			for (int i = 0; i<snpKeys.length; i++) {
				regions = snps.get(snpKeys[i]);
				...
				for (int j = 0; j<regionKeys.length; j++) {
					...
					if (j==0) {
						summary.print(lookup.get(snpKeys[i])+"\t"+snpKeys[i]+"\t"+(swaps.get(snpKeys[i]).split("[\\s]+")[0].equals("yes")?swaps.get(snpKeys[i]).split("[\\s]+")[2]:swaps.get(snpKeys[i]).split("[\\s]+")[1])+"\t"+indKeys.length+"\t"+mean+"\t"+stdev+"\t"+t+"\t"+(indKeys.length-1));
					}
...
//					ciHigh = mean+1.96*stdev/Math.sqrt(regionLog2ratios.size());
					...
					writer.println("\t\t\t\t\t\t\t\t\t\t\t\t\t"+mean+"\t"+stdev+"\t"+z+"\t"+ProbDist.NormDist(z)+"\t"+t+"\t"+ext.formDeci(Math.pow(2, mean), 3, true)+" ("+ext.formDeci(Math.pow(2, ciLow), 3, true)+", "+ext.formDeci(Math.pow(2, ciHigh), 3, true)+")"+"\t"+ProbDist.TDist(t, indKeys.length-1)+"\tOne observation (ratio) per sample");
					...
					z = Stats.ztest(mean, stdev, 0.5);
					t = Stats.ttestOneSample(mean, stdev, regionMeans.size(), 0.5);
					writer.println("\t\t\t\t\t\t\t\t\t\t\t\t\t"+mean+"\t"+stdev+"\t"+z+"\t"+ProbDist.NormDist(z)+"\t"+t+"\t\t"+ProbDist.TDist(t, indKeys.length-1)+"\tWith pooled variance estimator");
				}
				summary.println();
			}
			writer.close();
...
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD1\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD1corr\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD2\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD2corr\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\merge\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\mergeCorr\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD1fixedAgain\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD2fixedAgain\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD1RawAgain\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD2RawAgain\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\Econs1\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD3\\Plate1\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD3\\Plate2\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Expression\\Alleleotypes\\PD3\\";
