...
//	public static final String[] SOURCES = {"reference", "Celera", "c22_H2", "c5_H2", "c6_COX", "c6_QBL", "DR53", "CRA_TCAGchr7v2"};
	...
	public static class GenePosition implements Cloneable {
		...
		public GenePosition() {
			this.name = this.chr = this.start = this.stop = this.sense = ".";
			this.placed = this.imputed = this.originally_unplaced = false;
		}
		...
		public Object clone() {
			...
			gp.stop = this.stop;
			gp.sense = this.sense;
			gp.placed = this.placed;
			gp.imputed = this.imputed;
			gp.originally_unplaced = this.originally_unplaced;
		...
		try {
			...
			for (int i = 0; i<geneIDs.length; i++) {
				for (int j = 0; j<SOURCES.length; j++) {
					try {
						...
						poslarlar[j][i] = Double.parseDouble(temp+"."+ext.formNum(data[i][j].start.equals(".")?"0":data[i][j].start, maxDigits));
					} catch (Exception e) {
						poslarlar[j][i] = -1;
					}
				}
				...
				for (int j = 0; j<SOURCES.length; j++) {
					if (IMPORTANT[j]&&data[i][j].chr.equals(".")) {
						for (int k = 0; k<SOURCES.length; k++) {
							if (j!=k&&data[i][j].chr.equals(".")&&!data[i][j].originally_unplaced&&data[i][k].originally_unplaced) {
							...
							} else if (j!=k&&data[i][j].chr.equals(".")&&!data[i][k].chr.equals(".")&&!data[i][k].originally_unplaced) {
								low = high = ext.indexOfInt(i, orderler[k]);
								do {
									low--;
								} while (low>=0&&data[orderler[k][low]][k].chr.equals(data[i][k].chr)&&data[orderler[k][low]][j].chr.equals("."));
								if (low<0||!data[orderler[k][low]][k].chr.equals(data[i][k].chr)) {
									low = -1;
								}
								do {
									high++;
								} while (high<geneIDs.length&&data[orderler[k][high]][k].chr.equals(data[i][k].chr)&&data[orderler[k][high]][j].chr.equals("."));
								if (high==geneIDs.length||!data[orderler[k][high]][k].chr.equals(data[i][k].chr)) {
									high = -1;
								}
								if (low!=-1||high!=-1) {
									data[i][j].name = data[i][k].name;
									data[i][j].chr = data[i][k].chr;
									if ((low!=-1&&high==-1)||(low!=-1&&high!=-1&&poslarlar[k][i]-poslarlar[k][orderler[k][low]]<poslarlar[k][orderler[k][high]]-poslarlar[k][i])) {
										data[i][j].start = (Integer.parseInt(data[orderler[k][low]][k].start)+Integer.parseInt(data[i][k].start)-Integer.parseInt(data[orderler[k][low]][k].start))+"";
										data[i][j].stop = (Integer.parseInt(data[i][j].start)+Integer.parseInt(data[i][k].stop)-Integer.parseInt(data[i][k].start))+"";
									} else {
										data[i][j].start = (Integer.parseInt(data[orderler[k][high]][k].start)+Integer.parseInt(data[i][k].start)-Integer.parseInt(data[orderler[k][high]][k].start))+"";
										data[i][j].stop = (Integer.parseInt(data[i][j].start)+Integer.parseInt(data[i][k].stop)-Integer.parseInt(data[i][k].start))+"";
									}
									data[i][j].sense = data[i][k].sense;
									data[i][j].placed = false;
			...
			for (int i = 0; i<geneIDs.length; i++) {
				try {
					...
					poslar[i] = Double.parseDouble(temp+"."+ext.formNum(data[i][DOMINANT].start.equals(".")?"0":data[i][DOMINANT].start, maxDigits));
				} catch (Exception e) {
					poslar[i] = -1;
				}
			}
			...
			try {
				...
				for (int i = 0; i<SOURCES.length; i++) {
					if (IMPORTANT[i]) {
						writer.print("\t"+SOURCES[i]+"_name\t"+SOURCES[i]+"_chr\t"+SOURCES[i]+"_start\t"+SOURCES[i]+"_stop\t"+SOURCES[i]+"_sense\t"+SOURCES[i]+"_placed\t"+SOURCES[i]+"_imputed");
					}
				}
				writer.println();
				for (int i = 0; i<geneIDs.length; i++) {
					writer.print(geneIDs[order[i]].indexOf("_asPlacedBy_")>0?geneIDs[order[i]].substring(0, geneIDs[order[i]].indexOf("_asPlacedBy_")):geneIDs[order[i]]);
					for (int j = 0; j<SOURCES.length; j++) {
						if (IMPORTANT[j]) {
							writer.print("\t"+data[order[i]][j].name+"\t"+(data[order[i]][j].chr.equals(".")?"Un":data[order[i]][j].chr)+"\t"+data[order[i]][j].start+"\t"+data[order[i]][j].stop+"\t"+data[order[i]][j].sense+"\t"+data[order[i]][j].placed+"\t"+data[order[i]][j].imputed);
						}
					}
					writer.println();
				}
				...
				System.err.println("Error: file \""+dir+"genes.xls"+"\" is still in use or otherwise unavailable");
				System.exit(1);
