...
public class OMIM {
	...
	private Hashtable<String, ArrayList<OMIMGene>> gHashtable;
	...
	public OMIM(String omimDir, Logger log) {
		...
		this.log = log;
		verify();
		this.gHashtable = loadGeneOmimMap(omimDir + GENE_MAP_FILE, log);
	...
	public ArrayList<OMIMGene> getOmimGene(String gene) {
		if (gHashtable.containsKey(gene)) {
			...
			blank.add(new OMIMGene(new String[] { "NA" }, new String[] { "NA" }, "NA", "NA", "NA", "NA", "NA", "NA", "NA", "NA"));
			return blank;
		}
	}

	private static Hashtable<String, ArrayList<OMIMGene>> loadGeneOmimMap(String filename, Logger log) {
		...
		status.put("L", "limbo - evidence not as strong as that provisional, but included for heuristic reasons");
		...
		try {
			...
			while (reader.ready()) {
				...
				for (int i = 0; i < genes.length; i++) {
					...
					OMIMGene mGene = new OMIMGene(line, genes, tmp[4], tmp[6], tmp[7], tmp[8], tmp[9], tmp[11], tmp[13], status.containsKey(tmp[6]) ? status.get(tmp[6]) : tmp[6]);
	...
	public static class OMIMGene {
		...
		private String CytogeneticLocation;
		...
		public OMIMGene(String[] all, String geneSymbol[], String cytogeneticLocation, String geneStatus, String title, String mIMNumber, String method, String disorders, String references, String diseaseEvidence) {
			super();
			this.all = all;
