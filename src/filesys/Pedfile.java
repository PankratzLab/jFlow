		...
		try {
			...
			while (reader.ready()) {
				...
				if (numMarkers == -1) {
					...
					System.err.println("Error - different number of markers listed on line "+(count+1)+" ("+line[0]+"/"+line[1]+")");
				}
				count++;
			}
			reader.close();
					...
					for (int k = 0; k<2; k++) {
						...
						if (ext.indexOfStr(allele+"", MISSING_VALUES) >= 0) {
							...
							if (alleles[j][0] == 0 || alleles[j][0] == allele) {
								...
								System.err.println("Error - more than 2 alleles for marker "+(i+1)+"; first "+alleles[j][0]+" and "+alleles[j][1]+" and now "+allele);
							}
						}
                    }
                }
...
//	            reader.close();
	            ...
	            in = new FileReader(Files.getBakFilename("re_chrom"+ext.chrome(chr)+".pre", ext.rootOf(phenotypeFile)+"-"+phenoCol, true)); 
	            writer = new PrintWriter(new FileWriter("re_chrom"+ext.chrome(chr)+".pre"));
	...
	public static void main(String[] args) {
	    ...
	    "   (3) value to use for those missing phenotype (i.e. missing="+missVal+" (default))\n"+
	    "";
