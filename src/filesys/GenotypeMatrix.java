...
//	public static final String[][] HEADS = {{}, {"id"}};
	...
	public GenotypeMatrix(String genotypeFile, String idFile, String markerFile, int type, Logger log) {
		this(genotypeFile, idFile, markerFile, PARAMETERS[type], log);
	}
	
	public GenotypeMatrix(String genotypeFile, String idFile, String markerFile, int[] parameters, Logger log) {
		...
		if (markerFileType == -1) {
			markerSet = new SnpMarkerSet(markerFile, Files.determineDelimiter(markerFile, log), null, true, new String[][] {{"\"", ""}}, false, log);
...
//			reader = new BufferedReader(new FileReader(genotypeFile));
			...
			if (idFile != null) {
				ids = HashVec.loadFileToStringArray(idFile, false, true, new int[] {0}, false, false, Files.determineDelimiter(idFile, log));
			}
				...
				if (dominance == INDIVIDUAL_DOMINANT_FORMAT) {
					for (int i = 0; i < markerNames.length; i++) {
						if (!markerNames[i].equals(line[headerHead.length+i]))	{
							log.reportError("Error - mismatched name at marker "+(i+1)+" of "+genotypeFile+"; expecting "+markerNames[i]+" given map file "+markerFile+", found "+line[headerHead.length+i]);
							System.exit(1);
						}
					}
					...
					if (ids == null) {
						...
						for (int i = 0; i < ids.length; i++) {
							if (ids != null && !ids[i].equals(line[headerHead.length+i]))	{
								log.reportError("Error - mismatched IDs at individual "+(i+1)+" of "+genotypeFile+"; expecting "+ids[i]+" given id file "+idFile+", found "+line[headerHead.length+i]);
								System.exit(1);
							}
						}
					}
			...
			if (dominance == MARKER_DOMINANT_FORMAT) {
				for (int i = 0; i < markerNames.length; i++) {
					...
					if (line.length - firstGenotypeCol != ids.length) {
						log.reportError("Error - mismatched number of elements in line "+(i+1+(headerRow?1:0))+" of "+genotypeFile+"; expecting "+ids.length+"+"+firstGenotypeCol+", found "+line.length);
						log.reportError("First few ids: '"+ids[0]+"', '"+ids[1]+"', '"+ids[2]+"'... '"+ids[ids.length-2]+"', '"+ids[ids.length-1]+"'");
						System.exit(1);
					}
				...
				for (int i = 0; i < ids.length; i++) {
					...
					if (line.length - firstGenotypeCol != markerNames.length) {
						log.reportError("Error - mismatched number of elements in line "+(i+1+(headerRow?1:0))+" of "+genotypeFile+"; expecting "+markerNames.length+"+"+firstGenotypeCol+", found "+line.length);
						System.exit(1);
					}
					...
					for (int j = 0; j < ids.length; j++) {
						if (use[j]) {
							if (values[i][j] < 0) {
								...
								indeps[count][0] = (double)values[i][j];
								countUsed++;
							}
							count++;
						}
...
//					System.err.println(betas.length+"\t"+traits.length);
					...
					if (model.getFinalDependentVariables().length != countUsed) {
						log.reportError("Mismatched number of missing values for marker '"+markerNames[i]+"' ("+model.getFinalDependentVariables().length+", expected "+countUsed+"); might want to check missing value codes: "+model.getFinalIndependentVariables()[2][0]);
					}
...
//					System.exit(1);
			...
			System.err.println("Error writing to " + ext.rootOf(phenoFile, false)+".results."+(logistic?"logistic":"linear"));
			e.printStackTrace();
		}
	}
...
//		private byte[][] values;
		...
		CmdLine.run("plink --file "+root+" --make-bed --out "+root, ext.parseDirectoryOfFile(root));
