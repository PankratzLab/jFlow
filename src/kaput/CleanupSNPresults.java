...
public class CleanupSNPresults {
	...
	public static void clean(String dir, String filename) {
		...
		try {
			...
			indices = ext.indexFactors(REQS, reader.readLine().trim().split("\t", -1), false, false, true, true);
			while (reader.ready()) {
				...
				HashVec.addToHashVec(hash, line[indices[3]], line[indices[2]]+"\t"+line[indices[4]]+"\t"+line[indices[5]]+"\t"+line[indices[0]]+"\t"+line[indices[1]], false);
			...
			keys = Sort.quicksort(Array.toIntArray(hashKeys));
				...
				for (int j = 0; j<v.size(); j++) {
					...
					if (ext.indexOfStr(line[1].toLowerCase(), MISSING_CALL_CODES)>=0) {
						...
						if (call.equals("nada")) {
							...
							System.err.println("Error - discrepant calls for "+hashKeys[keys[i]]+" ("+call+" and "+line[1]+"/"+line[2]+")");
						}
