		...
		for (int chromosome = 1; chromosome<=23; chromosome++) {
			try {
				...
				for (int i = 0; i<dupeTravs.size(); i++) {
					...
					copy.println("Genotypes agree "+numAgree+" times in "+dupeGenos[0].length+" markers, and "+(numDisagree==0?"never disagree.":"disagree "+numDisagree+" time(s)!"));
					if (numDisagree!=0) {
						System.out.println(dupeTravs.elementAt(i)+" genotypes agree "+numAgree+" times in "+dupeGenos[0].length+" markers, and disagree "+numDisagree+" time(s)!");
					}
					for (int j = 0; j<dupeGenos.length; j++) {
						copy.print(((j<dupeGenos.length-1)?DNAs.elementAt(j):dupeTravs.elementAt(i)+"/CO")+"\t"+(dupeTravs.elementAt(i)).replace('-', '\t'));
				...
				if (fix) {
					if (!new File("chromosome"+chromosome+".dat").exists()) {
						System.err.println("Error - could not find "+"chromosome"+chromosome+".dat"+" in current directory");
						System.exit(2);
					}
					bakFilename = Files.getBakFilename("chromosome"+chromosome+".dat", super.getClass().getName());
