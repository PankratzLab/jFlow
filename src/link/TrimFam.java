	...
	public class Person extends Object {
		...
		public void createExtendedOutput(boolean listGeneration) {
			...
			if (necessary&&!isRoot()&&!written) {
				...
				temp += "\t"+(father.isRoot()?"0":(father.genotyped||!rename?father.id:father.recoded_id))+"\t"+(mother.isRoot()?"0":(mother.genotyped||!rename?mother.id:mother.recoded_id))+"\t"+sex+"\t"+(phenotyped?"2":"0")+"\t"+(genotyped?"1\t1":"0\t0");
		...
		public void createNuclearOutput() {
			...
			for (int i = 0; i<children.size(); i++) {
				...
				if (kid.genotyped) {
					...
					v.add(famid+"\t"+kid.id+"\t"+(kid.father.genotyped||!rename?kid.father.id:kid.father.recoded_id)+"\t"+(kid.mother.genotyped||!rename?kid.mother.id:kid.mother.recoded_id)+"\t"+kid.sex+"\t"+(kid.phenotyped?"2":"0")+"\t"+(kid.genotyped?"1\t1":"0\t0"));
				}
			}
			...
			nukular.add(famid+"\t"+(purse.genotyped||!rename?purse.id:purse.recoded_id)+"\t0\t0\t"+purse.sex);
	...
	public TrimFam(Vector<String> preinfo, boolean deleteSinglets, boolean unrelatedsOnly, boolean canRename, int scheme, int favorGeneration, boolean listGeneration, boolean allowMissingIndividuals, Logger logger) {
		...
		for (int i = 0; i<preinfo.size(); i++) {
			...
			if (line[6].equals("1")) {
				...
				log.reportError("Error - individual "+line[0]+"-"+line[1]+" was not properly declared as genotyped (1) or not genotyped (0): '"+line[6]+"'");
			}
			if (line.length > 7) {
		...
		if (unrelatedsOnly) {
			...
			// I thought this was necessary to eliminate ungenotyped selections, but it appears to be redundant
//			link();
//			hash.get("root").trimUselessDescendents();
//			link();
//			trimUselessFounders(); 
		...
		if (hash.size()>1) {
			...
			if (!unrelatedsOnly) {
				...
				if (numSubFams>1) {
					log.report("Family "+famid+" needs to be split into "+numSubFams+" sub families.");
				}
			...
			for (int i = 0; i < keys.length; i++) {
				if (genoed.containsKey(keys[i])) {
					genoed.put(keys[i], "used");
				}
		...
		for (int i = 0; i < keys.length; i++) {
			if (genoed.get(keys[i]).equals("unused")) {
				unused.add(keys[i]);
			}
	...
	public boolean link(boolean allowMissingIndividuals) {
		...
		for (int i = 0; i<keys.length; i++) {
			hash.get(keys[i]).children.removeAllElements();
		}
			...
			if (!keys[i].equals("root")) {
				...
				if (purse.father==null) {
					if (allowMissingIndividuals) {
						...
						log.reportError("Link error - Pedigree '"+purse.famid+"' does not contain all the indivudals referenced as parents (father: "+purse.dad+")");
						error = true;
					}
				}
				if (purse.mother==null) {
					if (allowMissingIndividuals) {
						...
						log.reportError("Link error - Pedigree '"+purse.famid+"' does not contain all the indivudals referenced as parents (mother: "+purse.mom+")");
						error = true;
					}
				}
				if (!error) {
	...
	public void trimUselessFounders() {
		...
		for (int i = 0; i<source.children.size(); i++) {
			...
			if (purse.children.size()==1&&purse.children.firstElement().father.children.size()==1&&purse.children.firstElement().mother.children.size()==1&&!purse.children.firstElement().father.genotyped&&!purse.children.firstElement().mother.genotyped&&purse.children.firstElement().mother.dad.equals("root")&&purse.children.firstElement().father.dad.equals("root")) {
	...
	// keeps the first highest ranked valid individual in each founder branch, deletes all individuals related to that individual, and repeats until there is only one 
		...
		while (!done) {
			...
			for (int i = 0; i < inds.length; i++) {
				...
				if (!purse.necessary && purse.genotyped) {
					HashVec.addToHashVec(famIndHash, purse.subFamily+"", inds[i], false);
				}
			}
			subfams = HashVec.getKeys(famIndHash);
			for (int i = 0; i < subfams.length; i++) {
				inds = Array.toStringArray(famIndHash.get(subfams[i]));
...
//					}
	...
	public void recodeFromLargestSibship(String fatherOfLargestSibship) {
		...
		for (int i = 0; i < keys.length; i++) {
			...
			if (!keys[i].equals("root")&&purse.recoded_id.equals("-1")) {
				count = (purse.genotyped?900:(purse.generation>1?purse.generation*100:100))+(purse.isMale()?1:2);
				while (usedRecodedIDs.contains(count+"")) {
					count += 2;
				}
	...
	// priority scores are not required, default scores will favor those genotyped over those ungenotyped, need a separate file to favor affected over unaffected
	public static void trimAllFams(String pedigreeFile, String outfile, boolean renameUngenotypedFounders, boolean deleteSinglets, boolean unrelatedsOnly, String priorityScoresFile, int namingScheme, int favorGeneration, boolean listGeneration, boolean allowMissingIndividuals, Logger log)  {
		...
		if (priorityScoresFile == null) {
			...
			priorityScores = HashVec.loadFileToHashString(priorityScoresFile, new int[] {0,1}, new int[] {2}, false, "\t", false, false, false);
		}
		...
		try {
			...
			while (reader.ready()) {
				...
				if (priorityScores == null) {
					...
					if (countMissing <= 20) {
						log.reportError("Error - no valid value in hash for individual "+line[0]+"-"+line[1]);
					} else {
						log.reportError("Error - no valid value in hash for individual "+line[0]+"-"+line[1], true, false);
					}
					score = "\t-1";
					countMissing++;
				} else {
				...
				HashVec.addToHashVec(preHash, line[0], line[0]+"\t"+line[1]+"\t"+line[2]+"\t"+line[3]+"\t"+line[4]+"\t"+line[5]+"\t"+(count==line.length-6?"0":"1")+score, false);
				HashVec.addIfAbsent(line[0], fams);
			}
			if (countMissing > 0) {
				log.reportError("Just so you know... "+countMissing+" indiviudal(s) are missing priority scores");
			}
			reader.close();
	...
	public static void fromParameters(String filename, Logger log) {
		...
		paramV = Files.parseControlFile(filename, "trimFam", new String[] {"fams.pre", "out=trimmed.pre", "renameUngenotypedFounders=false", "deleteSinglets=false", "unrelatedsOnly=false", "scores=null", "scheme=num", "favorGeneration=0", "allowMissing=false", "# add -h as an argument if you want a full description"}, log);
		if (paramV != null) {
...
//		String filename = "C:\\Documents and Settings\\npankrat\\My Documents\\LOAD\\finalPedigree_geno.pre";
//		String filename = "C:\\Documents and Settings\\npankrat\\My Documents\\LOAD\\finalPedigree_testSplit.pre";
//		String filename = "C:\\Documents and Settings\\npankrat\\My Documents\\hearing\\struct.dat";
//		String filename = "C:\\Documents and Settings\\npankrat\\My Documents\\hearing\\00src\\PCA_data\\struct_PCA1_pruned.pre";
//		String filename = "C:\\Documents and Settings\\npankrat\\My Documents\\BOSS\\master_pedigree.txt";
...
//		boolean deleteSinglets = true;
		...
		"   (4) only keep unrelated indiviudals (i.e. unrelatedsOnly="+unrelatedsOnly+" (default))\n"+
		"   (5) priority scores filename (i.e. score="+priorityScoresFile+" (default))\n"+
		"   (6) naming scheme for split fams (i.e. scheme="+NAMING_SCHEME_KEY[namingScheme]+" (default))\n"+
		"   (7) favor older or earlier generations if unrelatedsOnly (i.e. favorGeneration="+favorGeneration+" (default; +1 for older, -1 for younger, 0 for neither))\n"+
		"   (8) rename ungenotyped founders 101, 102, etc. (i.e. renameUngenotypedFounders="+renameUngenotypedFounders+" (default))\n"+
		"   (9) list generation in final file (i.e. listGeneration="+listGeneration+" (default))\n"+
		"   (10) if an individual is missing, create them and code as a founder of appropriateSex (i.e. allowMissing="+allowMissingIndividuals+" (default))\n"+
		"";
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				numArgs--;
			} else if (args[i].startsWith("renameUngenotypedFounders=")) {
				renameUngenotypedFounders = ext.parseBooleanArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("deleteSinglets=")) {
				deleteSinglets = ext.parseBooleanArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("unrelatedsOnly=")) {
				unrelatedsOnly = ext.parseBooleanArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("scores=")) {
				priorityScoresFile = ext.parseStringArg(args[i], null);
				numArgs--;
			} else if (args[i].startsWith("scheme=")) {
				scheme = ext.parseStringArg(args[i], null);
				numArgs--;
			} else if (args[i].startsWith("logfile=")) {
				logfile = ext.parseStringArg(args[i], null);
				numArgs--;
			} else if (args[i].startsWith("favorGeneration=")) {
				favorGeneration = ext.parseIntArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("listGeneration=")) {
				listGeneration = ext.parseBooleanArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("allowMissing=")) {
				allowMissingIndividuals = ext.parseBooleanArg(args[i]);
				numArgs--;
			} else if (args[i].contains("=")) {
				System.err.println("Error - unknown paramter: "+args[i]);
			} else {
				filename = args[i];
				numArgs--;
			}
		...
		try {
			// demo();
			trimAllFams(filename, outfile, renameUngenotypedFounders, deleteSinglets, unrelatedsOnly, priorityScoresFile, namingScheme, favorGeneration, listGeneration, allowMissingIndividuals, log);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
