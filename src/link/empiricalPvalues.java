	...
	public empiricalPvalues(int chromosome, int numReps) throws IOException {
		...
		while (!done) {
			if (st.countTokens()>0) {
				...
				writer.println(trav+"\t"+st.nextToken()+"\t"+st.nextToken()+"\t"+st.nextToken()+"\t"+st.nextToken()+"\t1\t1");
		...
		batch.println();
		batch.println("java -classpath /home/npankrat/park.jar park.bat.dat2loc map"+chrome+".dat");
		...
		for (int repNum = 1; repNum<=numReps; repNum++) {
			...
			file = "chrom"+chrome+"-template."+ext.formNum(repNum+"", String.valueOf(numReps).length());
			...
			if (chromosome!=23) {
				batch.println("echo -e \"pairs\\n3\\nload map"+chrome+".loc\\nprep chrom"+chrome+"-"+repNum+".pre\\nn\\nscan\\nestimate\\ny\\nchrom"+chrome+"-"+repNum+"-mls.out\\ntrash1\\ny\\ntrash2\\ny\\nestimate\\nn\\nchrom"+chrome+"-"+repNum+"-Dv-mls.out\\ntrash1\\ny\\ntrash2\\ny\\nquit\\n\" | /software/bin/sibs > chrom"+chrome+"-"+repNum+".log"); // >
				// /dev/null"
			} else {
		...
		if (args.length==0||args.length>2) {
			...
			try {
				if (args.length==1) {
					...
					new empiricalPvalues(Integer.valueOf(args[0]).intValue(), Integer.valueOf(args[1]).intValue());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
