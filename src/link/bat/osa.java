		...
		for (int chromosome = start; chromosome<=stop; chromosome++) {
			...
			for (int i = 0; i<=IDs.size(); i++) {
				...
				if (trav!=prev&&prev!=-99999.777) {
					increment++;
					writer = new PrintWriter(new FileWriter("chrom"+chrome+"/re_chrom"+chrome+"-a"+ext.formNum(increment, 4)+".pre"));
					for (int j = 0; j<i; j++) {
			...
			for (int i = IDs.size()-1; i>=-1; i--) {
				...
				if (trav!=prev&&prev!=-99999.777) {
					increment++;
					writer = new PrintWriter(new FileWriter("chrom"+chrome+"/re_chrom"+chrome+"-d"+ext.formNum(increment, 4)+".pre"));
			...
			for (int ascending = 0; ascending<2; ascending++) {
				...
				for (int i = 1; i<=increment; i++) {
					writer.println("echo -e \""+((chromosome==23)?"sex on\\n":"")+"pairs\\n3\\nload map"+chrome+".loc\\nprep re_chrom"+chrome+"-"+temp+ext.formNum(i, 4)+".pre\\nn\\nscan\\nestimate\\n"+((chromosome==23)?"":(dv?"n":"y")+"\\n")+"chrom"+chrome+"-"+temp+ext.formNum(i, 4)+".out\\nchrom"+chrome+"-"+temp+ext.formNum(i, 4)+"-share.ps\\nchrom"+chrome+"-"+temp+ext.formNum(i, 4)+"-mls.ps\\nquit\\n\" | /software/bin/sibs > /dev/null");
					writer.println();
				}
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				if (args[i].split("=")[1].equals("true")) {
					...
					System.err.println("Error in syntax - '"+args[i].split("=")[1]+"' is not a valid flag for dv (use true/false)");
				}
				numArgs--;
			}
		}
