...
public class PlinkToLinkage {
	public static String slim(String dir, String root, int slim) {
		...
		for (int i = numToDel-1; i>=0; i--) {
			xs.removeElementAt((int)((double)i*(double)numPres/(double)numToDel));
		}
		...
		numToDel = numPres-numWanted;
		for (int i = numToDel-1; i>=0; i--) {
			autos.removeElementAt((int)((double)i*(double)numPres/(double)numToDel));
		}
		...
		CmdLine.run("plink --bfile "+root+" --extract slimmedList.txt --make-bed --out slimmedDown", dir);
	...
	public static void convert(String dir, String root) {
		for (int chr = 1; chr<=23; chr++) {
			...
			CmdLine.run("plink --bfile "+root+" --chr "+chr+" --recode12 --out chrom"+ext.chrome(chr), dir);
			if (new File(dir+"chrom"+ext.chrome(chr)+".map").exists()) {
				new File(dir+"chrom"+ext.chrome(chr)+".ped").renameTo(new File(dir+"chrom"+ext.chrome(chr)+".pre"));
				new SnpMarkerSet(dir+"chrom"+ext.chrome(chr)+".map").createLinkageMap().createFile(dir+"map"+ext.chrome(chr)+".dat");
	...
	public static void updatePedigreeStructure(String dir, String pedigree) {
		...
		for (int chr = 1; chr<=23; chr++) {
			...
			try {
				...
				System.err.println("Error: file \""+dir+"chrom"+ext.chrome(chr)+".pre"+"\" not found in current directory");
				System.exit(1);
...
//		String pedigree = "FullPedigreeWithoutDuplicates.dat";
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				slim = Integer.parseInt(args[i].split("=")[1]);
				numArgs--;
