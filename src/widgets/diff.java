...
//	    System.out.println(fingerprint);
	    ...
	    for (int i = 0; i < 15; i++) {
	    	...
	    	if (files.length > 1) {
    			...
    			if (fingerprint == prints[order[0]]) {
    				runDiff(filename, HashVec.loadFileToStringArray(prints[order[1]]+".difftemp", false, null, false)[0]);
    		    	deleteAll();
    		    	return;
    			} else {
    		    	try {
    ...
    public static void runDiff(String file1, String file2) {
        ...
        try {
            ...
            while (trav1 != null || trav2 != null) {
                ...
                if (trav1 != null && trav2 != null && !trav1.equals(trav2) || trav1 == null && trav2 != null || trav1 != null && trav2 == null || buffer1.size() + buffer2.size() > 0) {
                    if (nothingChanged) {
                        writer = new PrintWriter(new FileWriter("diff bw '" + file1 + "' & '" + file2 + "'.out"));
                    ...
                    while(pos1 < buffer1.size()) {
                        for (int pos2 = 0; pos2 < buffer2.size(); pos2++) {
                            if (!buffer1.elementAt(pos1).equals(buffer2.elementAt(pos2)) && (trav1 != null && trav2 != null || pos1 != buffer1.size() - 1 || pos2 != buffer2.size() - 1)) {
                                continue;
                            }
                            if (pos1 == pos2) {
                                for (int i = 0; i < (trav1 != null || trav2 != null ? pos1 : pos1 + 1); i++) {
                                    ...
                                    if (offsets[0] == 1) {
                                        writer.println("  Swapped '" + buffer1.elementAt(i).substring(offsets[1], offsets[2]) + "' for '" + buffer2.elementAt(i).substring(offsets[1], offsets[3]) + "'");
                                    }
                                }
    ...
    public static int[] whatsTheDiff(String str1, String str2) {
        int offs[];
        for (offs = new int[4]; offs[1] < str1.length() && offs[1] < str2.length() && str1.charAt(offs[1]) == str2.charAt(offs[1]); offs[1]++);
        offs[2] = str1.length();
        for (offs[3] = str2.length(); offs[2] > offs[1] && offs[3] > offs[1] && str1.charAt(offs[2] - 1) == str2.charAt(offs[3] - 1); offs[3]--) {
            offs[2]--;
        }

        if (offs[2] == offs[1] && offs[3] == offs[1]) {
            offs[0] = 2;
        } else if ((double)(offs[2] - offs[1]) / (double)str1.length() < 0.5D && (double)(offs[3] - offs[1]) / (double)str2.length() < 0.5D) {
            offs[0] = 1;
        } else {
            offs[0] = 0;
        }
