...
public class AlleleFreq {
	...
	public static double calcFrequency(double[] genotypes) {
		if (genotypes.length!=3) {
			System.err.println("Error - can't compute an allele frequency from "+genotypes.length+" classes of genotypes");
		}
	...
	public static double HWEsig(double[] genotypes) {
		...
		if (genotypes.length==6) {
			return HWEsig(genotypes[0], genotypes[1], genotypes[2], genotypes[3], genotypes[4], genotypes[5]);
		}
		System.err.println("Error - can't compute Hardy Weinberg from "+genotypes.length+" classes of genotypes");
		return Double.NaN;
	}
	...
	public static double HWEsig(int pp, int pq, int pr, int qq, int qr, int rr) {
		return ProbDist.ChiDist(HWE((double)pp, (double)pq, (double)pr, (double)qq, (double)qr, (double)rr), 1);
	}
