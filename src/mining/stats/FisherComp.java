	...
	public static void comp(String filename, int permutations) {
        ...
        try {
	        ...
	        while (reader.ready()) {
	        	...
	        	writer.println(Array.toStr(line)+"\t"+ProbDist.ChiDist(ContingencyTable.ChiSquare(prunedMatrix, false), (prunedMatrix.length-1)*(prunedMatrix[0].length-1))+"\t"+FishersExact.calc(prunedMatrix, 0, false)+"\t"+(new Date().getTime()-time)+"\t"+(matrix.length == prunedMatrix.length && matrix[0].length == prunedMatrix[0].length?0:1));
	        	writer.flush();
	        }
	        reader.close();
            writer.close();
...
//	    String filename = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Consortium\\00src\\Miami\\fishFood1.dat";
	    ...
	    for (int i = 0; i<args.length; i++) {
		    if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
			    ...
			    numArgs--;
		    } else if (args[i].startsWith("perms=")) {
			    perms = Integer.parseInt(args[i].split("=")[1]);
			    numArgs--;
		    }
	    }
	    if (numArgs!=0) {
