...
public class CytoPanel extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source.equals(selectFilesButton)) {
			...
			if (fileChooser.isSelected()) {
				...
				log.report(startDir);
				// this.importFiles = Files.list(startDir, SAMPLES_EXT, false); 
				// checkFiles(importFiles, SAMPLES_EXT, true);
	...
	// TODO, check add new files, or remove it
	/**
	...
	/**
	 * We do not convert if the cnp file is already present
	 */
	private static void convertCNPfiles(Project proj, String cnpFile, String output, Logger log) {
		if (!projectHasCNVFile(proj, output, log)) {
			if (convertCNPFile(cnpFile, proj.getProjectDir() + output, log)) {
	...
	// if (keepGoing()) {
	// try {
	// CytoCompare.compare(null, startDir, workBenchFiles, proj.getFilename(Project.COMMON_CNP_FILENAME), proj.getFilename(Project.REPORTED_CNP_FILENAME), proj.getFilename(Project.UNREPORTED_CNP_FILENAME), CytoCompare.DEFAULT_SCORE_THRESHOLD, startDir, true, new Logger(startDir + System.currentTimeMillis() + "_" + CytoCompare.DEFAULT_LOG));
	// } catch (Exception exc) {
	// JOptionPane.showMessageDialog(this, "An error occured while parsing the following " + (workBenchFiles.length == 1 ? " file: " : " files: ") + Array.toStr(workBenchFiles, "\n"));
	// log.reportError("An error occured while parsing the following " + (workBenchFiles.length == 1 ? " file: " : " files: ") + Array.toStr(workBenchFiles, "\n"));
	// log.reportException(exc);
	// }
	// }
	// if (checkFiles(importFiles, startDir, SAMPLES_EXT)) {
	// log.report(ext.getTime() + " Info - found " + importFiles.length + " files");
	// Thread thread = new Thread(new Runnable() {
	// public void run() {
	// // this will be run in a separate thread
	// CytoAgilentParse.parseCytoToGenvisis(proj, startDir, importFiles, log);
	// cnv.manage.ParseIllumina.createFiles(proj, proj.getInt(Project.NUM_THREADS), log);
	// TransposeData.transposeData(proj, 2000000000, false, proj.getLog());
	// }
	// });
	...
	//
	//
	// public class ScrollFileBox extends JScrollPane {
	//
	// private static final long serialVersionUID = 1L;
	// private FileBox[] fileBoxes;
	//
	// public ScrollFileBox(FileBox[] fileBoxes) {
	// super();
	...
	// }
	//
	// public void addAll(JPanel jpanel) {
	// for (int i = 0; i < fileBoxes.length; i++) {
	// jpanel.add(fileBoxes[i]);
	// }
	// validate();
	// repaint();
	// }
	...
	// FileChooser fileChooser = new FileChooser(this, startDir, true, false, "Please select the Agilent file(s) to import", log);
	// this.startDir = fileChooser.getNavDir();
	// this.importFiles = fileChooser.getFiles();
	// if (importFiles != null && importFiles.length > 0) {
	// CytoAgilentParse.parseCytoToGenvisis(proj, startDir, importFiles, log);
	// }
	// }
	...
	// /**
	// * Prompts for the three overlap files (Common, reported, and unreported) to compare the selected segments to
	// */
	// public void getAuxFiles() {
	// String[] options = new String[] { "OK" };
	// String dialoge = "This parser can compute the overlap of the workbench output with three optional files:\n";
	// dialoge += "(1) A file of common CNPs (i.e. HG19 CNV edit for AGW.txt)\n";
	// dialoge += "(2) A file of reported aberrations (i.e. HG19 Reported 2012.05.22.txt)\n";
	// dialoge += "(3) A file of un-reported aberrations (i.e. HG19 Unreported 2012.05.22-2.txt)\n";
	// dialoge += " Please select your options from the next dialog (Ctrl click to select multiple)";
	//
	// JOptionPane.showOptionDialog(null, dialoge, "Auxilary Info", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
	// String[] fileTypes = new String[] { "common CNPs", "reported CNPs", "un-reported CNPs", "none (skip overlap) " };
	// JList<String> list = new JList<String>(fileTypes);
	// JOptionPane.showMessageDialog(null, list, "Select auxilary file(s)", JOptionPane.PLAIN_MESSAGE);
	// int[] selectedIndices = list.getSelectedIndices();
	// for (int i = 0; i < selectedIndices.length; i++) {
	// if (selectedIndices[i] >= auxFiles.length) {
	// log.report("Proceeding without auxilary files");
	// return;
	// }
	// }
	// for (int i = 0; i < selectedIndices.length; i++) {
	// auxFiles[selectedIndices[i]] = getFile(startDir, "Select file for " + fileTypes[selectedIndices[i]], log);
	// log.report(auxFiles[selectedIndices[i]]);
	// }
	// }
	//
	// public String getFile(String startDir, String title, Logger log) {
	// String[] file = new FileChooser(this, startDir, true, false, title, log).getFiles();
...
// private static final String REMOVE = "REMOVE File";
//
// private static final String SAMPLES = " sample file";
// private static final String NO_FILE_SELECTED = " No file selected";
//
// private static final String WORKBENCH_EXT = "workbench.xls";
// private static final String SAMPLES_EXT = ".txt";
...
// private String startDir;
// private Thread thread;
// private String[] workBenchFiles, importFiles;
// private Logger log;
// private JLabel jLabelMain, jLabelBench, jLabelSamp;
// private JButton selectFilesButton, parseButton, cancelButton;
// private FileBox[] benchfileBoxes, sampfileBoxes;
...
// }
//
// private void findFilesFromWorkBench(){
// for (int i = 0; i < benchfileBoxes.length; i++) {
//
//
// }
...
// private void parseAndImport() {
// importFiles = getFinalFullPath(sampfileBoxes);
// if (checkFiles(importFiles, SAMPLES_EXT,false)) {
// log.report(ext.getTime() + " Info - found " + importFiles.length + " files");
// CytoAgilentParse.parseCytoToGenvisis(proj, importFiles, log);
// cnv.manage.ParseIllumina.createFiles(proj, proj.getInt(Project.NUM_THREADS), log);
// deleteSampleList();
...
//
// /**
// * We delete the sample list so that it is created only from available files in the transpose step (after sample import)
// */
// private void deleteSampleList() {
//
// if (Files.exists(proj.getFilename(Project.SAMPLELIST_FILENAME)) && proj.getSampleList() != null && proj.getSampleList().getSamples().length > 0) {
...
//
// /**
// * Here we create FileBox objects corresponding to the files discovered when listing the directory
// */
// private void updateFiles() {
// int numWorkBoxes = Math.min(workBenchFiles.length, DEFAULT_MAX_NUM_WORKBENCH);
// int numSampBoxes = Math.min(importFiles.length, DEFAULT_MAX_NUM_SAMPLES);
...
// log.report("" + i);
// if (i < workBenchFiles.length) {
// benchfileBoxes[i] = new FileBox(cytoGUI, startDir, workBenchFiles[i], SELECT_NEW + WORKBENCH, REMOVE);
// } else {
// benchfileBoxes[i] = new FileBox(cytoGUI, NO_FILE_SELECTED, null, SELECT_NEW + WORKBENCH, REMOVE);
// }
// if (i < DEFAULT_MAX_NUM_WORKBENCH) {
// benchfileBoxes[i].setVisible(true);
// add(benchfileBoxes[i]);
...
// for (int i = 0; i < sampfileBoxes.length; i++) {
// if (i < importFiles.length) {
// sampfileBoxes[i] = new FileBox(cytoGUI, startDir, importFiles[i], SELECT_NEW + SAMPLES, REMOVE);
// } else {
// sampfileBoxes[i] = new FileBox(cytoGUI, NO_FILE_SELECTED, null, SELECT_NEW + SAMPLES, REMOVE);
// }
// // we only add up to a max amount to the Gui
// if (i < DEFAULT_MAX_NUM_SAMPLES) {
// sampfileBoxes[i].setVisible(true);
...
// int num = 0;
// for (int i = 0; i < fileBoxes.length; i++) {
// System.out.println(fileBoxes[i].getText());
// if (fileBoxes[i].isUsed()) {
// num++;
// }
// }
//
// finalFiles = new String[num];
// num = 0;
// for (int i = 0; i < fileBoxes.length; i++) {
// if (fileBoxes[i].isUsed()) {
// finalFiles[num] = fileBoxes[i].getText();
// num++;
// }
// }
// return finalFiles;
...
// */
//
// public FileBox(JFrame jframe, String startDir, String file, String titleSelect, String titleRemove) {
// this.startDir = startDir;
// this.file = file;
// this.jFileNameTextField = new JTextField();
// this.selectButton = new JButton(titleSelect);
