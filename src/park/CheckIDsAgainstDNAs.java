...
//	public static final String DNA_YEAR_BUG_SOURCE = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Global PD files\\year_bug_dnas.txt";
	...
	public CheckIDsAgainstDNAs() {
			...
			while (reader.ready()) {
				...
				if (line[0].equals("SWAPPED")) {
					...
					if (!hashMatches.containsKey(trav)) {
						System.err.println("Error - '"+trav+"' is listed as a swap in ninfo3 but was not found in ninfo1");
					} else {
		...
		try {
			...
			ext.checkHeader(reader.readLine().trim().split("[\\s]+"), new String[] {"UniqueID", "FamID", "IndID", "WrongYear", "CorrectYear"}, true);
			while (reader.ready()) {
			...
			System.err.println("Error: file \""+DNA_YEAR_BUG_SOURCE+"\" not found in current directory");
			System.exit(1);
		} catch (IOException ioe) {
	...
	public String checkPair(String famID, String indID, String dna, boolean verbose) {
		...
		if (!hashMatches.containsKey(trav)) {
			...
			if (hashDNAs.containsKey(dna)) {
				...
				return dna+"\t"+famID+"\t"+indID+":"+dna+"\t"+hashDNAs.get(dna).replace('-', '\t');
			} else {
			...
			if (idMatches.elementAt(0).size()==1) {
				...
				if (verbose) {
					System.err.println("Having trouble filling in the blank for "+trav+" -- possibilities include:");
				}
...
//					temp += ":"+idMatches.elementAt(0).elementAt(i)+"\t"+famID+"\t"+indID;
			...
			if (idMatches.elementAt(0).contains(dna)) {
				if (idMatches.elementAt(3).contains(dna)) {
					if (verbose) {
						System.out.println("Make sure "+dna+" (for "+trav+") is data from the resent vial; there were problems with data before Jan 1st, 2007");
					}
				...
				try {
					if (yearBugs.containsKey(famID+":"+indID+":"+dna)) {
						...
						return "yearbug\t"+yearBugs.get(famID+":"+indID+":"+dna)+"\t"+famID+"\t"+indID;
					}
				...
				if (idMatches.elementAt(1).contains(dna)) {
					...
					if (idMatches.elementAt(0).size()==1) {
						System.err.println("; did you mean "+idMatches.elementAt(0).elementAt(0)+(hashDNAs.containsKey(dna)?" or "+hashDNAs.get(dna):""));
						return dna+"\t"+famID+"\t"+indID+":"+idMatches.elementAt(0).elementAt(0)+"\t"+famID+"\t"+indID+(hashDNAs.containsKey(dna)?":"+dna+"\t"+hashDNAs.get(dna).replace('-', '\t'):"");
					} else {
	...
	public static void fixit(String fixit) throws IOException {
		...
		try {
			...
			while (reader.ready()) {
				...
				line = temp.split(":", -1);
				if (line.length==1||line[0].split("\t", -1).length!=3||line[1].split("\t", -1).length!=3) {
