	...
	public swapDNA(String DNA1, String DNA2, int start, int stop) throws IOException {
		...
		for (int i = start; i<=stop; i++) {
			try {
				...
				if (famInd1.equals("")) {
					System.err.println("Error: could not find first DNA, "+DNA1+", in file \'chromosome"+i+".dat\'");
				} else if (famInd2.equals("")&&!DNA2.equalsIgnoreCase("trash")) {
					System.err.println("Error: could not find second DNA, "+DNA2+", in file \'chromosome"+i+".dat\'");
				} else {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				numArgs--;
			} else if (args[i].startsWith("DNA1=")) {
				DNA1 = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("DNA2=")) {
				DNA2 = args[i].split("=")[1];
				numArgs--;
