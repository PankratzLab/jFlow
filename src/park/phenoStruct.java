...
public class phenoStruct {
	public phenoStruct(boolean include_mutations) throws IOException {
		...
		if (count>23) {
			count = 0;
			while (count<=23&&!new File("/home/npankrat/park/00masters/chromosome"+(++count)+".dat").exists());
			if (count>23) {
				...
				genos = new BufferedReader(new FileReader("/home/npankrat/park/00masters/chromosome"+count+".dat"));
			}
		} else {
		...
		while (extra!=null&&extra.ready()) {
			...
			if (line[0].equals("CONF")) {
				...
				// confirmed
				deleteThese.add(line[1]+"\t"+line[2]);
			} else if (line[0].equals("SWAPPED")) {} else if (line[0].equals("MUT")) {
			...
			while (info.ready()) {
				...
				if (!aoo.containsKey(id)) {
					...
					if (confirmedDx.containsKey(id)) {
						...
						if (temp.equals("CONF")) {
							System.err.println("Error - Diagnosis for "+id+" was CONF, but autopsy results were not found in ninfo3 file");
							System.err.println("        if came back VPD, add line to ninfo3 file saying 'CONF\t"+id+"\tVPD (DLB)' or as appropriate");
					...
					if (!(diagnosis.get(id)).equals(line[3])) {
						if (confirmedDx.containsKey(id)) {
							...
							System.err.println("Warning - "+id+" has two different diagnoses ("+(diagnosis.get(id))+" and "+line[3]+") and is not listed as an autopsy in ninfo3 file");
							System.err.println("        - add line to ninfo3 file saying 'CONF\t"+id+"\tVPD (DLB)' or as appropriate");
							System.exit(1);
						}
					}
		...
		while (!done) {
			...
			if (!done) {
				if (!diagnosis.containsKey(trav+"\t"+id)) {
					...
					System.err.println("      - could be that chromosome"+count+".dat is empty or corrupt?");
					System.exit(21);
				}
				// if
		...
		while (!done) {
				...
				if (VIPlists.containsKey(prev)) {
					for (int allvpd = 0; allvpd<=1; allvpd++) {
						...
						if (allvpd==0&&vips.size()<2) {
							System.err.println("Warning - dropping family "+prev+" because it is uninformative for linkage");
						}
						...
						for (int extnuc = 0; extnuc<=1; extnuc++) {
							v = (extnuc==0?tf.getExtendedFamilyInformation():tf.getNuclearFamilyInformation());
								...
								if (vips.contains(id+"")) {
									...
									if (diagnosis.get(id)!=null) {
										...
										if (affected.contains(temp)) {
											...
											System.err.println("Warning - Unknown phenotype for individual '"+id+"': "+temp);
											writer[2*allvpd+extnuc].print("\t0");
										}
						...
						if (tf.hasUnused()) {
							if (unused==null) {
								unused = new PrintWriter(new FileWriter("unused_individuals-"+(allvpd==0?"111":"100")+".dat"));
							}
							v = tf.getUnused();
							for (int i = 0; i<v.size(); i++) {
			...
			id = trav+"\t"+line[1];
			temp = id+"\t"+line[4]+"\t"+line[5]+"\t"+(line[2].equals("M")?"1":(line[2].equals("F")?"2":"0"));
			members.add(temp);
	...
	public boolean checkForParentOffspringSinglets(Vector<String> v, Vector<String> vips) {
		...
		if (presentVIPs.size()==2) {
			if ((v.elementAt(0)).split("[\\s]+")[1].equals((v.elementAt(1)).split("[\\s]+")[2])||(v.elementAt(0)).split("[\\s]+")[1].equals((v.elementAt(1)).split("[\\s]+")[3])||(v.elementAt(1)).split("[\\s]+")[1].equals((v.elementAt(1)).split("[\\s]+")[2])||(v.elementAt(1)).split("[\\s]+")[1].equals((v.elementAt(1)).split("[\\s]+")[3])) {
				return true;
			}
		}
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				System.exit(1);
			} else if (args[i].indexOf("nomuts")!=-1) {
				include_mutations = false;
				numArgs--;
			} else if (args[i].startsWith("muts=")) {
				if (args[i].split("=")[1].toLowerCase().equals("true")) {
					include_mutations = true;
					numArgs--;
				} else if (args[i].split("=")[1].toLowerCase().equals("false")) {
					include_mutations = false;
					numArgs--;
				} else {
					System.err.println("Error in syntax - '"+args[i].split("=")[1]+"' is not a valid flag for nomuts (use true/false)");
				}
			}
		}
		if (numArgs!=0) {
		...
		if (args.length==0) {
			System.err.println("Using defaults (mutations "+(include_mutations?"included":"excluded")+")");
		}
