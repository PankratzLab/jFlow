...
//	public static final String controlInfo = "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Global PD files\\control.db.xln";
	...
	public static final String[] CTRL_HEADER = {"DNA#", "FamID", "IndID", "Index", "UniqueID", "Source", "Affected", "Male", "RaceCode", "Caucasian", "DateOfBirth", "DateOfExam", "AgeAtExam", "Use", "Comment"};
//	public static final String[] KNOWN_POLYMORPHISMS = {"S167N", "Ser167Asn", "V380L", "D394N"};
	public static final String[] KNOWN_POLYMORPHISMS = {"S167N", "Ser167Asn", "V380L", "D394N", "A91A", "C238C", "H279H", "L174L", "L228L", "L261L", "L307L", "P37P"};
	public static final String[][] CATEGORY_CRITERIA = { {"KnownPolymorphisms", "equals", "S167N", "Ser167Asn", "V380L", "D394N"}, {"SynonymousMissense", "equals", "A91A", "C238C", "H279H", "L174L", "L228L", "L261L", "L307L", "P37P"}, {"BenignMissense", "equals", "R33Q", "A82E", "T83A", "M192V", "E310D", "T240M", "R256C", "P437L"}, {"DeleteriousMissense", "equals", "T173M", "K211N", "R275W", "G430D"}, {"Nonsense", "endswith", "x"}, {"Frameshift", "contains", "fs"}, {"Deletion", "contains", "deletion"}, {"Duplication", "contains", "duplication", "triplication"}, {"Indel", "contains", "dup"}, {"SpliceSite", "contains", "IVS"}};
	public static final String[] MUT_CLASSES = {"nada", "het", "homo", "comp het", "odd"};
	public static final String[] DIANE_LIST_HEADER = {"DNA # ", "Fam. and Ind. #", "Sequencing Plate"};
	public static final String[] DIANE_RESULTS_HEADER = {"Exon", "DNA #", "Family and Ind. #", "Nucleotide Change", "Amino Acid change"};
	public static final String[] SEAN_RESULTS_HEADER = {"Family", "ID", "UniqueID", "nummuts", "AR lod", "Age of Onset", "dx", "unique mutation", "mutation", "Exon", "type", "Nucleotide Change", "Amino Acid Change", "Homo/Hetero"};
	public static final String[] MLPA_HEADER = {"DNA #", "Fam.& Ind. #", "Exon 2", "Exon 3", "Exon 4", "Exon 5", "Exon 6", "Exon 7", "Exon 8", "Exon 9", "Exon 10", "Exon 11", "Exon 12"};
	public static final String[] PLATE_HEADER = {"FamID", "IndID", "UniqueID", "time 1", "time 2", "time 3"};
	public static final String[] LODS_HEADER = {"Family", "linearLOD", "NPL", "AD LOD", "AR LOD", "", "positive"};
	public static final String[] INLAB_HEADER = {"FamNo", "IndNo", "DNA.", "CountOfIndNo"};

	// Recorded: Sean, Diane, MLPA
	// Change to: Sean, Diane, MLPA
	public static final String[][][][] INTERPRETATIONS = { { { {"Gln57fs (+328 amino acids)", "Gln57fs (+328 amino acids)"}, {}, {"deletion 3", "deletion 2-4"}}, { {}, {}, {"deletion 2-3", "deletion 3-4"}}}, { { {"Q57fs (+328 amino acids)", "Q57fs (+328 amino acids)"}, {}, {"deletion 3", "deletion 2-4"}}, { {}, {}, {"deletion 2-3", "deletion 3-4"}}}, { { {"Gln57fs (+35 amino acids)", "Gln57fs (+35 amino acids)"}, {}, {"deletion 3-4", "deletion 3-4"}}, { {}, {}, {"deletion 3-4", "deletion 3-4"}}}, { { {"Q57fs (+35 amino acids)", "Q57fs (+35 amino acids)"}, {}, {"deletion 3-4", "deletion 3-4"}}, { {}, {}, {"deletion 3-4", "deletion 3-4"}}}, { { {}, {"G430D"}, {"duplication 2-4", "duplication 2-4"}}, { {}, {"G430D"}, {"triplication 2-4"}}}, { { {}, {}, {"duplication 2-4", "duplication 2-4"}}, { {}, {}, {"triplication 2-4"}}}, { { {}, {"T240M", "T240M"}, {"deletion 5-6"}}, { {}, {"T240M"}, {"deletion 5-6"}}}, { { {"P113fs (+51 amino acids)", "P113fs (+51 amino acids)"}, {}, {"deletion 3-4"}}, { {"P113fs (+51 amino acids)"}, {}, {"deletion 3-4"}}}, { { {}, {"P113fs (+51 amino acids)", "P113fs (+51 amino acids)"}, {"deletion 2-3"}}, { {}, {"P113fs (+51 amino acids)"}, {"deletion 2-3"}}}};
	...
	public static void audit() throws IOException {
		...
		Hashtable<String,String> diagnoses = new Hashtable<String,String>(), lods = new Hashtable<String,String>();;
		...
		try {
			...
			while (reader.ready()) {
				...
				if (indData.containsKey(uniqueID)) {
					...
					System.err.println("Error - a nonexistent person was sent to lab: "+line[0]+"-"+line[1]);
				}
			}
			reader.close();
		} catch (FileNotFoundException fnfe) {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (indData.containsKey(uniqueID)) {
					...
					System.err.println("Error - Sean sequenced a nonexistent person: "+line[0]+"-t"+line[1]);
				}
			}
			reader.close();
		} catch (FileNotFoundException fnfe) {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (line[0].equals("Seans_choice")&&!idCheck.checkPair(line[1], line[0], false).equals("")) {
					...
					if (line[8].equals("point")) {
						for (int i = 0; i<(line.length>13&&line[13].startsWith("Homo")?2:1); i++) {
							if (line.length>12&&ext.indexOfStr(line[12], KNOWN_POLYMORPHISMS)!=-1) {
								...
								mc.seanMutations = Array.addStrToArray(line.length>12&&!line[12].equals("")?line[12]:line[11], mc.seanMutations);
							}
						}
					}
				} else {
					System.err.println("Error - Sean results for a nonexistent person: "+line[0]+"-"+line[1]);
				}
			}
			reader.close();
		} catch (FileNotFoundException fnfe) {
				...
				if (indData.containsKey(uniqueID)) {
					...
					for (int i = 0; i<(temp.indexOf("Homozygous")>0?2:1); i++) {
						if (line.length>4&&ext.indexOfStr(line[4], KNOWN_POLYMORPHISMS)!=-1) {
							...
							mc.dianeMutations = Array.addStrToArray(line.length>4&&!line[4].equals("")?line[4]:line[3], mc.dianeMutations);
						}
					}
					if (mc.dna.equals("")) {
						mc.dna = line[1];
					} else if (!mc.dna.equals(line[1])) {
						System.err.println("Different DNAs used between methods for "+mc.FamID+"-"+mc.IndID+" ("+mc.dna+" and "+line[1]+")");
						mc.dna += "/"+line[1];
					}
				} else {
					System.err.println("Error - Diane results for a nonexistent person: "+line[1]+"/"+line[2]);
				}
			}
			reader.close();
		} catch (FileNotFoundException fnfe) {
				...
				if (line.length>1) {
					...
					if (indData.containsKey(uniqueID)) {
						...
						if (mc.dna.equals("")) {
							...
							System.err.println("Different DNAs used between methods for "+mc.FamID+"-"+mc.IndID+" ("+mc.dna+" and "+line[0]+")");
							mc.dna += "/"+line[0];
						}
						if (temp.substring(line[0].length()+1+line[1].length()).trim().length()>0) {
							line = temp.split("\t", -1);
							if (temp.indexOf("Deletion")>0&&temp.indexOf("Duplication")>0) {
								System.err.println("Warning - Both a deletion and a duplication found in individual "+mc.FamID+"-"+mc.IndID);
							}
							if (temp.indexOf("Deletion")==-1&&temp.indexOf("Duplication")==-1) {
								System.err.println("Warning - MLPA result that is neither a deletion nor a duplication for individual "+mc.FamID+"-"+mc.IndID);
							}
							...
							try {
								for (int i = 1; i<(line.length>12?12:line.length-1); i++) {
									if (line[1+i].equals("")) {
										...
										parkinExons[i] = 1;
									} else if (line[1+i].equals("Homo. Duplication")||line[1+i].equals("Hom. Duplication")) {
										parkinExons[i] = 2;
									} else if (line[1+i].equals("Deletion")) {
										parkinExons[i] = -1;
									} else if (line[1+i].equals("Homo. Deletion")||line[1+i].equals("Hom. Deletion")) {
										parkinExons[i] = -2;
									} else {
										System.err.println("Error - invalid mutation for "+mc.FamID+"-"+mc.IndID+" in MLPA file: "+line[1+i]);
									}
								}
							} catch (Exception e) {
								System.err.println("Error - invalid size for "+mc.FamID+"-"+mc.IndID+" in MLPA file");
							}
							trav = parseMLPAMutations(parkinExons);
							if (trav.length==0) {
								System.err.println("Error - parse zero mutations for individual "+mc.FamID+"-"+mc.IndID+" (the file format expected some)");
							}
							for (int i = 0; i<trav.length; i++) {
								mc.MLPAresults = Array.addStrToArray(trav[i], mc.MLPAresults);
							}
						...
						System.err.println("Error - MLPA results for a nonexistent person: "+line[0]+"/"+line[1]);
					}
				}
			}
			reader.close();
		...
		for (int i = 0; i<inds.size(); i++) {
			...
			for (int j = 0; j<2; j++) {
				if (j==0) {
					trav = mc.seanMutations.length>mc.dianeMutations.length?mc.seanMutations:mc.dianeMutations;
				} else {
					trav = mc.MLPAresults;
				}
					...
					for (int l = 0; l<CATEGORY_CRITERIA.length; l++) {
						if (CATEGORY_CRITERIA[l][1].equals("equals")) {
							for (int m = 2; m<CATEGORY_CRITERIA[l].length; m++) {
								if (trav[k].toLowerCase().equals(CATEGORY_CRITERIA[l][m].toLowerCase())) {
									mc.categories[l] = Array.addStrToArray(trav[k], mc.categories[l]);
							...
							for (int m = 2; m<CATEGORY_CRITERIA[l].length; m++) {
								if (trav[k].toLowerCase().contains(CATEGORY_CRITERIA[l][m].toLowerCase())) {
									mc.categories[l] = Array.addStrToArray(trav[k], mc.categories[l]);
									m = CATEGORY_CRITERIA[l].length-1;
									l = CATEGORY_CRITERIA.length-1;
								}
							...
							for (int m = 2; m<CATEGORY_CRITERIA[l].length; m++) {
								if (trav[k].toLowerCase().endsWith(CATEGORY_CRITERIA[l][m].toLowerCase())) {
									mc.categories[l] = Array.addStrToArray(trav[k], mc.categories[l]);
									m = CATEGORY_CRITERIA[l].length-1;
									l = CATEGORY_CRITERIA.length-1;
								}
							...
							System.err.println("Error - Don't know how to process category with operator: "+CATEGORY_CRITERIA[l][1]);
							System.exit(1);
						}
					}
					if (Matrix.getSize(mc.categories)==size) {
						System.err.println("Error - Could not find a category for: "+trav[k]);
					}
				}
			}
			...
			mc.numMutations = (mc.seanMutations.length>mc.dianeMutations.length?mc.seanMutations.length:mc.dianeMutations.length)+mc.MLPAresults.length;
			mc.numSeq = mc.seanMutations.length>mc.dianeMutations.length?mc.seanMutations.length:mc.dianeMutations.length;
			...
			if (!mc.shouldBe&&!mc.partiallyChecked&&mc.lod<0&&diagnoses.containsKey(mc.FamID+"\t"+mc.IndID)&&(diagnoses.get(mc.FamID+"\t"+mc.IndID)).equals("VPD")) {
		...
		for (int i = -1; i<inds.size(); i++) {
			...
			writer.println((i==-1?"UniqueID":mc.UniqueID)+"\t"+(i==-1?"DNA#":(mc.dna.equals("")?"":mc.dna))+"\t"+(i==-1?"FamID":mc.FamID)+"\t"+(i==-1?"IndID":mc.IndID)+"\t"+(i==-1?"Sean seq":(mc.seanSeqd?"1":"0"))+"\t"+(i==-1?"Sean Found":mc.seanMutations.length)+"\t"+(i==-1?"Diane Seq":(mc.dianeSeqd?"1":"0"))+"\t"+(i==-1?"Diane Found":mc.dianeMutations.length)+"\t"+(i==-1?"MLPA'd":(mc.MLPAd?"1":"0"))+"\t"+(i==-1?"MLPA found":mc.MLPAresults.length)+"\t"+(i==-1?"Num Mutations":mc.numMutations)+"\t"+(i==-1?"DNA sent":(mc.inLab?"1":"0"))+"\t"+(i==-1?"# additional fam members":mc.numberFamMems)+"\t"+(i==-1?"Partially Checked":(mc.partiallyChecked?"1":"0"))+"\t"+(i==-1?"Fully Checked":(mc.fullyChecked?"1":"0"))+"\t"+(i==-1?"Family member fully checked":(mc.familyMemberFullyChecked?"1":"0"))+"\t"+(i==-1?"First Plate":(mc.firstPlate==-1?".":mc.firstPlate))+"\t"+(i==-1?"NPL score":(mc.lod==-999?".":mc.lod))+"\t"+(i==-1?"Dx":(diagnoses.containsKey(mc.FamID+"\t"+mc.IndID)?diagnoses.get(mc.FamID+"\t"+mc.IndID):"."))+"\t"+(i==-1?"AOO":(mc.AOO==-1?".":mc.AOO))+"\t"+(i==-1?"Random Candidate":(mc.randomCandidate?"1":"0"))+"\t"+(i==-1?"Should Be":(mc.shouldBe?"1":"0"))+"\t"+(i==-1?"But Wasn't":(mc.butWasnt?"1\t*":"0"))+"\t"+"");
		}
		writer.close();
		...
		for (int i = 0; i<inds.size(); i++) {
			mc = indData.get(inds.elementAt(i==-1?0:i));
			if (mc.numMutations>0) {
				writer.print((mc.dna.equals("")?"unknown":mc.dna)+"\t"+mc.UniqueID+"\t"+mc.FamID+"\t"+mc.IndID);
				writer.print("\t"+mc.numMutations+"\t"+MUT_CLASSES[mc.mutClass]+"\t"+(diagnoses.containsKey(mc.FamID+"\t"+mc.IndID)?diagnoses.get(mc.FamID+"\t"+mc.IndID):".")+"\t"+(mc.AOO==-1?".":mc.AOO));
				for (int j = 0; j<mc.seanMutations.length; j++) {
					writer.print("\tSean: "+mc.seanMutations[j]);
				}
				for (int j = 0; j<mc.dianeMutations.length; j++) {
					writer.print("\tDiane: "+mc.dianeMutations[j]);
				}
				for (int j = 0; j<mc.MLPAresults.length; j++) {
					writer.print("\tMLPA: "+mc.MLPAresults[j]);
				}
				writer.println();
			}
		}
		...
		for (int i = 0; i<CATEGORY_CRITERIA.length; i++) {
			writer.print(",#"+CATEGORY_CRITERIA[i][0]);
		}
		for (int i = 0; i<CATEGORY_CRITERIA.length; i++) {
			writer.print(",Just1"+CATEGORY_CRITERIA[i][0]);
		}
		for (int i = 0; i<CATEGORY_CRITERIA.length; i++) {
			writer.print(","+CATEGORY_CRITERIA[i][0]);
		}
		...
		for (int i = 0; i<inds.size(); i++) {
			...
			if (mc.fullyChecked||mc.numMutations>0||mc.knownPolymorphisms.length>0) {
				writer.print((mc.dna.equals("")?"unknown":mc.dna)+","+mc.UniqueID+","+mc.FamID+","+mc.IndID+","+(mc.fullyChecked?1:0)+","+mc.numMutations);
		...
		for (int i = 0; i<inds.size(); i++) {
			...
			if (mc.knownPolymorphisms.length>0) {
				writer.print((mc.dna.equals("")?"unknown":mc.dna)+"\t"+mc.UniqueID+"\t"+mc.FamID+"\t"+mc.IndID);
				writer.print("\t"+mc.numMutations+"\t"+MUT_CLASSES[mc.mutClass]+"\t"+(diagnoses.containsKey(mc.FamID+"\t"+mc.IndID)?diagnoses.get(mc.FamID+"\t"+mc.IndID):".")+"\t"+(mc.AOO==-1?".":mc.AOO)+"\t"+mc.knownPolymorphisms.length);
				for (int j = 0; j<mc.knownPolymorphisms.length; j++) {
					writer.print("\t"+mc.knownPolymorphisms[j]);
				}
				writer.println();
			}
		}
		...
		for (int i = 0; i<inds.size(); i++) {
			...
			if (mc.partiallyChecked||mc.numMutations>0) {
				writer.println((mc.dna.equals("")?".":mc.dna)+","+mc.FamID+","+mc.IndID+","+mc.UniqueID+","+mc.numMutations+","+(mc.numMutations>0?1:0)+","+(mc.numMutations>1?1:0)+","+(mc.numMutations==1?1:0)+","+mc.knownPolymorphisms.length+","+mc.numSeq+","+mc.numDosage+","+(mc.numSeq>0?1:0)+","+(mc.numDosage>0?1:0)+","+(mc.numMutations>=2||mc.numDosage>=1?".":(mc.numSeq==1?1:0))+","+(mc.numMutations>=2||mc.numSeq>=1?".":(mc.numDosage==1?1:0))+","+(mc.numMutations==1?".":(mc.numMutations>=2?1:0))+","+(mc.fullyChecked?1:0));
			}
		}
		writer.close();
		...
		for (int i = 0; i<inds.size(); i++) {
			...
			if (mc.numMutations>=2) {
				...
				for (int j = 0; j<mc.seanMutations.length; j++) {
					writer.print(" ("+mc.seanMutations[j]+")");
				}
				for (int j = 0; j<mc.dianeMutations.length; j++) {
					writer.print(" ("+mc.dianeMutations[j]+")");
				}
				for (int j = 0; j<mc.MLPAresults.length; j++) {
					writer.print(" ("+mc.MLPAresults[j]+")");
				}
				writer.println();
			}
		}
	...
	public static String unique(String famID, String indID) throws IOException {
		if (famID.startsWith("ND")) {
			...
			try {
				...
				System.err.println("Error - could not form a unique identifier with Family ID = '"+famID+"' and Individual ID = '"+indID+"'");
				return "error";
			}
		}
	}
	...
	public static void transformMe(MutationCarrier mc) throws IOException {
		for (int i = 0; i<INTERPRETATIONS.length; i++) {
			if (eqArrays(mc.seanMutations, INTERPRETATIONS[i][0][0])&&eqArrays(mc.dianeMutations, INTERPRETATIONS[i][0][1])&&eqArrays(mc.MLPAresults, INTERPRETATIONS[i][0][2])) {

				mc.seanMutations = INTERPRETATIONS[i][1][0].clone();
				mc.dianeMutations = INTERPRETATIONS[i][1][1].clone();
				mc.MLPAresults = INTERPRETATIONS[i][1][2].clone();
			}
		}
	}
	...
	public MutationCarrier(String newFamID, String newIndID) {
		...
		try {
			...
			if (!newFamID.equals(newIndID)) {
				System.err.println("Warning - new MutationCarrier has either a FamID ("+newFamID+") or an IndID ("+newIndID+") that is non-numeric, and yet not identical. Declaring UniqueID = IndID.");
			}
		}
	}
}
