...
public class batch {
	static public int[] maxes = {281, 253, 216, 202, 193, 180, 174, 153, 151, 168, 144, 164, 104, 127, 106, 115, 125, 126, 89, 95, 36, 47, 176};
	...
	public batch(String[] arguments) throws IOException {
		...
		for (int i = start; i<=stop; i++) {
			...
			case 4:
				writer.println("java "+classpath+" park.bat.dat2loc map"+chrome+".dat");
				if (i!=23) {
					writer.println("echo -e \"pairs\\n3\\nload map"+chrome+".loc\\nprep re_chrom"+chrome+".pre\\nn\\nsingle point on\\nscan\\nestimate\\ny\\nchrom"+chrome+"-2pt.out\\nestimate\\nn\\nchrom"+chrome+"-Dv-2pt.out\\nquit\\n\" | /software/bin/sibs > chrom"+chrome+".log");
				} else {
					writer.println("echo -e \"sex on\\npairs\\n3\\nload map"+chrome+".loc\\nprep re_chrom"+chrome+".pre\\nn\\nsingle point on\\nscan\\nestimate\\nchrom"+chrome+"-2pt.out\\nquit\\n\" | /software/bin/sibs > chrom"+chrome+".log");
				}
				writer.println();
			...
			case 5:
				...
				// }
				writer.println("/opt/local/software/aspex-2.3-solaris/sib_phase -v -f chrom"+chrome+"k-mpt.param chrom"+chrome+".pre > chrom"+chrome+"-K-mpt.out");
				writer.println("/opt/local/software/aspex-2.3-solaris/sib_phase -f chrom"+chrome+"k-2pt.param chrom"+chrome+".pre > chrom"+chrome+"-K-2pt.out");
				writer.println();
				break;
			case 6:
				writer.println("echo -e \"analysis npl\\npost on\\nload map"+chrome+".dat\\nscan re_chrom"+chrome+".pre\\ntotal\\nchrom"+chrome+"-npl.ps\\nchrom"+chrome+"-lod.ps\\nchrom"+chrome+"-info.ps\\nquit\\n\" | /opt/local/software/gh2/gh.sol > chrom"+chrome+".out");
				writer.println();
				break;
			case 7:
			...
			case 13:
				if (!chrome.equals("23")) {
					...
					writer.println("java "+classpath+" park.bat.createSolar chr="+i+" trait="+trait);
			...
			case 16:
				...
				writer.println("echo -e \"load gh_map"+chrome+".dat\\nincrement distance 1\\nscan gh_chrom"+chrome+".pre\\nvariance\\nn\\nn\\nvc"+chrome+".out\\ncorr"+chrome+".out\\nn\\nml\\nml"+chrome+".out\\nhase\\ntrad-he"+chrome+".out\\nem-he"+chrome+".out\\nnon\\nnp.out\\nno dom\\nml_ndv"+chrome+".out\\nquit\\n\" | /home/npankrat/bin/gh2 > /dev/null");
			...
			case 17:
				...
				if (i!=23) {
					writer.println("merlin -d merlin_data."+chrome+" -p re_chrom"+chrome+".pre -m merlin_map."+chrome+" --npl --grid 1 > chrom"+chrome+".out");
				} else {
					writer.println("minx -d merlin_data."+chrome+" -p re_chrom"+chrome+".pre -m merlin_map."+chrome+" --npl --grid 1 > chrom"+chrome+".out");
				}
				writer.println();
				break;
			case 18:
				...
				break;
			case 19:
				writer.println("java "+classpath+" park.bat.osa file=trait.dat chr="+i+" dv=true");
				writer.println("cd chrom"+chrome);
