...
public class demographics {
	...
	public demographics(String filename) throws IOException {
		...
		int males = 0, females = 0, unaffected = 0, brothers, sisters, malesInFam, femalesInFam, unaffInFam;
		...
		while (reader.ready()) {
			...
			if (line[7].equals("-99")) {
				...
				try {
					...
					System.err.println("Error - Could not parse age of onset for "+line[0]+"-"+line[1]+": "+line[7]);
			...
			if (line[5].equals("1")) {
				...
				if (h.containsKey(line[2]+"/"+line[3])) {
					...
					info[Integer.valueOf(line[4]).intValue()-1] = (Integer.valueOf(info[Integer.valueOf(line[4]).intValue()-1]).intValue()+1)+"";
		...
		writer.println(females+" females ("+ext.formDeci((double)females/(males+females), 2, true)+")");
		writer.println();
		writer.println(bb_bs_ss[0]+" brother-brother pairs ("+ext.formDeci((double)bb_bs_ss[0]/(double)total, 2, true)+")");
		writer.println(bb_bs_ss[1]+" brother-sister pairs ("+ext.formDeci((double)bb_bs_ss[1]/(double)total, 2, true)+")");
		writer.println(bb_bs_ss[2]+" sister-sister pairs ("+ext.formDeci((double)bb_bs_ss[2]/(double)total, 2, true)+")");
		writer.println(total+" total pairs");

		writer.println();
		for (int i = 0; i<20; i++) {
			if (affPairs[i]>0) {
				writer.println("families with "+(i==1?" singlets/halfsibs = ":i+" affected sibpairs = ")+affPairs[i]);
				if (i!=2) {
