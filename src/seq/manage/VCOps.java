...
public class VCOps {
	...
	public static Segment getSegment(VariantContext vc) {
		return new Segment(Positions.chromosomeNumber(vc.getContig()), vc.getStart(), vc.getEnd() <= vc.getStart() ? vc.getStart() + 1 : vc.getEnd());
	}
	...
	public static VariantContext getIndividualPassingContext(final VariantContext vc, final VariantContextFilter variantContextFilter, Logger log) {
	...
	public static VariantContext getAltAlleleContext(final VariantContext vc, FilterNGS filterNGS, final VariantContextFilter variantContextFilter, final ALT_ALLELE_CONTEXT_TYPE type, boolean verbose, final Logger log) {
		...
		if (filterNGS != null && filterNGS.getAltAlleleDepthRatioFilter() != null && filterNGS.getAltAlleleDepthRatioFilter()[0] > 0) {
			// log.reportTimeError("Alt Allele depth filter is currently not in here, JOHN");
	...
	public static int[] getAppropriateAlleleDepths(VariantContext vc, Genotype g, boolean verbose, Logger log) {
		...
		if (gAlleles.size() != 2 && g.hasAD()) {
			...
			if (gAlleles.size() == 1) {
				...
				} else if (g.isHomVar() && AD[1] == 0 && Array.sum(gAD) > 0 && Array.sum(gAD) != Array.sum(AD)) {
					if (verbose) {
