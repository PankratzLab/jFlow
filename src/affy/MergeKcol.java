...
public class MergeKcol implements Runnable{
			...
			for (int j =0; j< files.length; j++){
				...
				for (int i = 0; i < dirList.length; i++) {
					try{
						...
						System.err.println("Error: file \""+files[j]+"\" not found in " +kColDir+dirList[i]);
						return;
					}
	...
	public static void combineKcolFiles( int numThreads ,String kColDir, String prefix, String suffix )  {
		...
		for (int i = 0; i<numThreads; i++) {
			threads[i] = new Thread(new MergeKcol(fileCabinet.elementAt(i).toArray(new String[fileCabinet.elementAt(i).size()]),  timeBegan, i,kColDir ));
			threads[i].start();
			try {
				Thread.sleep(100L);
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i<args.length; i++) {
			...
			else if (args[i].startsWith("inputDir=")) {
				kColDir = args[i].split("=")[1];
				numArgs--;
			}
