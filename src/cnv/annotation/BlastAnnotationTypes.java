...
public class BlastAnnotationTypes {
	...
	public enum BLAST_ANNOTATION_TYPES {
		 ...
		 * 
		 */
		HAS_PERFECT_MATCH(VCFHeaderLineType.String, "HAS_PERFECT_MATCH", "There is a perfect match Alignment", ANNO_DELIM, DEFAULT_VALUE_PM), /**
		 * 
		 */
		...
		private BLAST_ANNOTATION_TYPES(VCFHeaderLineType vType, String name, String Description, String sep, String defaultValue) {
			this.vType = vType;
			this.name = name;
	...
	public static boolean shouldBeAnnotatedAs(Project proj, BlastResults blastResults, BLAST_ANNOTATION_TYPES type, Segment seg, Logger log) {
		...
		switch (type) {
		case OFF_T_ALIGNMENT_GAPS_AND_MISMATCHES:
			annotatedAs = blastResults.getMismatches() > 0 && blastResults.getGapOpens() > 0 && !blastResults.getSegment().overlaps(seg);
			break;
		case OFF_T_ALIGNMENT_NO_MISMATCHES_NO_GAPS:
			annotatedAs = blastResults.getMismatches() == 0 && blastResults.getGapOpens() == 0 && !blastResults.getSegment().overlaps(seg);
			break;
		case OFF_T_ALIGNMENT_NO_GAPS:
			annotatedAs = blastResults.getMismatches() > 0 && blastResults.getGapOpens() == 0 && !blastResults.getSegment().overlaps(seg);

			break;
		case OFF_T_ALIGNMENT_NO_MISMATCHES:
			annotatedAs = blastResults.getMismatches() == 0 && blastResults.getGapOpens() > 0 && !blastResults.getSegment().overlaps(seg);
			break;
		case HAS_PERFECT_MATCH:
