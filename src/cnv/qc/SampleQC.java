...
public class SampleQC {
	...
	private boolean gcCorrectedLrrSd;
	...
	private int removeEmptyMetrics(){
		...
		if (gcCorrectedLrrSd && getLrr_sdIndex() == -1){
			proj.getLog().reportTimeError("GC corrected LRR SD is missing, run again without selecting GC corrected LRR SD or after regenerating Sample QC file with GC correction");
			return -1;
		}
	...
	private int getLrr_sdIndex(){
			return gcCorrectedLrrSd ? ext.indexOfStr("LRR_SD_Post_Correction", getQctitles()) : ext.indexOfStr("LRR_SD", getQctitles());
	}
	...
	private int addExcludes(){
		...
		for (int i = 0; i < samples.length; i++){
			...
			if (lrr_sd && qcMatrix[lrr_sdIndex][i] > EXCLUDE_LRR_SD_THRESHOLD){
				if (addToExcludes(i,"LRR_SD > " + EXCLUDE_LRR_SD_THRESHOLD)) numExcluded++;
			}
	...
	private void addUse_cnvs(){
		...
		proj.getLog().reportTime("Calculated use_cnvs column, dropping " + numLowIntensity + " low quality intensity samples");
	}
	...
	public static SampleQC loadSampleQC(Project proj, boolean generate, boolean gcCorrectedLrrSd, String duplicatesSetFile) {
		return loadSampleQC(proj, LrrSd.SAMPLE_COLUMN, LrrSd.NUMERIC_COLUMNS, generate, gcCorrectedLrrSd, duplicatesSetFile);
	}
	...
	public static void parseAndAddToSampleData(Project proj, int numQ, int numPCs, boolean justQuantiles, boolean gcCorrectedLrrSd, String duplicatesSetFile, boolean correctFidIids) {
		...
		if (correctFidIids) sampleQC.correctFidIidCols();
	}
	...
	public static void main(String[] args) {
		...
		usage += "   (5) use GC corrected LRR SD for filtering (i.e. gcCorrectedLrrSd=" + gcCorrectedLrrSd + " (default))\n" + "";
		...
		usage += "   (7) replace fid and iid with pedigree values (i.e. correctFidIids=" + correctFidIids + " (default))\n" + "";
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				...
				numArgs--;
			} else if (args[i].startsWith("gcCorrectedLrrSd=")) {
				gcCorrectedLrrSd = ext.parseBooleanArg(args[i]);
				numArgs--;
		...
		try {
			...
			parseAndAddToSampleData(proj, numQ, numPCs, justQuantiles, gcCorrectedLrrSd, duplicatesSetFile, correctFidIids);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
