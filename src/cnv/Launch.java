...
import cnv.manage.ExtractPlots;
import cnv.plots.CompPlot;
import cnv.plots.MosaicPlot;
import cnv.plots.QQPlot;
import cnv.plots.ScanForProblemSnp;
import cnv.plots.ScatterPlot;
import cnv.plots.SexPlot;
import cnv.plots.StratPlot;
import cnv.plots.Test_CnpDetection;
...
public class Launch extends JFrame implements ActionListener, WindowListener {
	...
	public static final String[] BUTTONS = { MAP_FILES, GENERATE_MARKER_POSITIONS, PARSE_FILES, CHECK_SEX, LRR_SD, EXTRACT_PLOTS, SLIM_PLOTS, GENERATE_PLINK_FILES, GENERATE_PENNCNV_FILES, CNP_SCAN, SCATTER, QQ, STRAT, MOSAICISM, MOSAIC_PLOT, SEX_PLOT, TRAILER, POPULATIONBAF, TEST, GCMODEL, TWOD, COMP };
	...
	public void actionPerformed(ActionEvent ae) {
		...
		try {
			if (command.equals(MAP_FILES)) {
				...
				/*
				 * cnv.manage.PlinkFormat.createPlink(proj); CmdLine.run("plink --file gwas --make-bed --out plink", proj.getProjectDir()); new File(proj.getProjectDir()+"genome/").mkdirs(); CmdLine.run("plink --bfile ../plink --freq", proj.getProjectDir()+"genome/"); CmdLine.run("plink --bfile ../plink --missing", proj.getProjectDir()+"genome/"); // vis cnv.manage.PlinkFormat root=../plink genome=6
				 */
