...
public class ReclusterOutliers {
	 ...
	 * 
	 */
	public static void loadData(Project proj, String markerListfilename) {
		PrintWriter writer;
		...
		output = proj.getProjectDir()+"resultOfReclusterByTheta.txt";
		...
		if (markerListfilename == null) {
			//TODO !!! somebody might move the .scat files, and thus markerList will not match the genotypes derived from markerData 
			...
			try {
				...
				for (int i=0; i<files.length; i++) {
					...
					for (int j=0; j<markerData.length; j++) {
						//TODO how to get
//						result = reclusterNullGenotypeByTheta(markerData[j], null, 12);
...
//					System.out.println(rsByGenotype[0].elementAt(j)+","+thetasByGenotype[0].elementAt(j)+" where mean R is "+meanR[n]+" and theta is "+meanTheta[n]+" +/- "+sdTheta[n]+" (n="+rsByGenotype[n].size()+")");
	...
	public static void main(String[] args) {
		Project proj = new Project("C:/workspace/Genvisis/projects/boss.properties", false);
		loadData(proj, null);
