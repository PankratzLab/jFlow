...
public class PrincipalComponentsIntensity extends PrincipalComponentsResiduals {
	...
	public void correctXYAt(int atComponent) {
		if (!fail) {
			...
			if (hasSomeGenotypes()) {
				this.genoXClusters = parseToClusters(Array.toDoubleArray(centroid.getMarkerData().getXs()), genoSampleClusters);
				this.genoYClusters = parseToClusters(Array.toDoubleArray(centroid.getMarkerData().getYs()), genoSampleClusters);
	...
	private void residualCorrection(int index, int clusterComponent) {
		correctedXCluster[index] = Array.toFloatArray(new LeastSquares(genoXClusters[index], genoPCsClusters[index].returnUpToNumComponents(clusterComponent), null, false, true, svdRegression).getResiduals());
		correctedYCluster[index] = Array.toFloatArray(new LeastSquares(genoYClusters[index], genoPCsClusters[index].returnUpToNumComponents(clusterComponent), null, false, true, svdRegression).getResiduals());
	}
	...
	private void estimateNewGenotypes(int atComponent) {
		...
		for (int i = 0; i < genoSampleClusters.length; i++) {
			...
			if (validClusterComponent(genoClusterCounts[i], clusterComponent)) {
				...
				CrossValidation cvalY = getPredictedValues(genoYClusters[i], train_indeps, yMatchedPCs, getTrimmedPreppedPCs(clusterComponent), svdRegression, proj.getLog());
	...
	/**
	 * @param toParse
	 *            data that will be assigned to each cluster
	 * @param clusters
	 *            boolean cluster definition, currently by genotype
	 * @return a double[][] representation of the clusters
	 */
	private static double[][] parseToClusters(double[] toParse, boolean[][] clusters) {
		double[][] clustersData = new double[clusters.length][];
		for (int i = 0; i < clusters.length; i++) {
			clustersData[i] = Array.subArray(toParse, clusters[i]);
		}
		return clustersData;
	}
