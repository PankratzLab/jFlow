    ...
    public static class ParseConstants {
        ...
        public static String determineFilename(String dir, String sampleName, long timeBegan, Logger log) {
            ...
            if (versionToOverwrite != -1) {
                while (new File(dir + ParseConstants.HOLD_OPTION_FILE).exists()) {
                    try {
                        Thread.sleep(500);
                    } catch (InterruptedException ie) {}
                }
                if (new File(dir + ParseConstants.CANCEL_OPTION_FILE).exists()) {
                    return null;
                }
                
                Files.write("", dir + ParseConstants.HOLD_OPTION_FILE);
                if (new File(dir + ParseConstants.OVERWRITE_OPTION_FILE).exists()) {
                    response = 1;
                } else {
                    ...
                    if (!System.getProperty("os.name").startsWith("Windows")) {
                        log.reportError("Error - the same sample name '"+sampleName+"' is being parsed again and the previous file existed before the current command began. "
                                ...
                                + dir + ParseConstants.OVERWRITE_OPTION_FILE
                        );
                        return null;
                    }
                ...
                new File(dir + ParseConstants.HOLD_OPTION_FILE).delete();
                switch (response) {
                case 0:
                    return dir + trav + Sample.SAMPLE_DATA_FILE_EXTENSION;
                case 2:
                    Files.write("", dir + ParseConstants.OVERWRITE_OPTION_FILE);
                case 1:
                    return dir + filename;
                case 3:
                    Files.write("", dir + ParseConstants.CANCEL_OPTION_FILE);
                    return null;
                default:
        ...
        public static void parseAlleleLookupFromFinalReports(Project proj) {
            ...
            for (int i = 0; i<files.length; i++) {
                if (new File("report").exists()) {
                    ParseConstants.writeToLookupFile(proj, hash, i+1);
                ...
                try {
                    ...
                    } while (reader.ready()&&(ext.indexFactors(SNP_HEADER_OPTIONS, line, false, true, false, false)[0]==-1 || (!idHeader.equals(ParseConstants.FILENAME_AS_ID_OPTION) && ext.indexOfStr(idHeader, line)==-1)));
            ...
            ParseConstants.writeToLookupFile(proj, hash, 0);
        }
...
//                  ParseIllumina ::>
	...
	public NewParseIllumina(Project proj, String[] files, String[] markerNames, int[] keysKeys, char[][] abLookup, String delimiter, long fingerprint, Hashtable<String,String> fixes, long timeBegan) {
		this(proj, files, markerNames, keysKeys, abLookup, delimiter, fingerprint, fixes, timeBegan, -1);
	}

	public NewParseIllumina(Project proj, String[] files, String[] markerNames, int[] keysKeys, char[][] abLookup, String delimiter, long fingerprint, Hashtable<String,String> fixes, long timeBegan, int threadId) {
		this.proj = proj;
		this.files = files;
        ...
        try {
			for (int i = 0; i<files.length; i++) {
				...
				if (!headers.containsKey(files[i])) {
				    // TODO error - found extra source file
				    return; // or should continue?
				}
				try {
					...
					for (int l = 0; l < headerData.columnHeaderLineIndex; l++) {
					    reader.readLine();
					}
//					do {
//						line = reader.readLine().trim().split(delimiter, -1);
...
//					if (headerData.colSampleIdent == -99) {
//						sampIndex = -7;
//					} else {
//						sampIndex = ext.indexFactors(new String[] { idHeader }, line, false, true)[0];
//					}
//					snpIndex = ext.indexFactors(SNP_HEADER_OPTIONS, line, false, true, false, true)[0];
					...
					while (reader.ready()) {
						...
						if (headerData.colSampleIdent == -99) {
							...
							if (parseAtAt && line[headerData.colSampleIdent].indexOf("@") == -1) {
								log.reportError("Error - " + headerData.cols[headerData.colSampleIdent] + " '" + line[headerData.colSampleIdent] + "' did not contain an @ sample");
								parseAtAt = false;
							}
							trav = parseAtAt ? line[headerData.colSampleIdent].substring(0, line[headerData.colSampleIdent].indexOf("@")) : line[headerData.colSampleIdent];
						}
						if (count==0) {
							sampleName = trav;
							...
							return;
						} else if (!markerNames[count].equals(line[headerData.colSnpIdent]) && !markerNames[count].startsWith("Blank")) {
							log.reportError("Found "+line[headerData.colSnpIdent]+" at marker #"+(count+1)+" in file "+files[i]+"; expecting "+markerNames[count]);
							return;
						}
						key = keysKeys[count];
//						for (int j = 0; j<Sample.DATA_FIELDS.length; j++) {
							try {
					            ...
					            if (proj.XY_SCALE_FACTOR.getValue() != 1) {
					                data[3][key] = (float) (data[3][key] / proj.XY_SCALE_FACTOR.getValue());
			                        data[4][key] = (float) (data[4][key] / proj.XY_SCALE_FACTOR.getValue());
					            }
							    ...
							    log.reportException(nfe); // TODO lost some error info
						...
						if (ignoreAB) {
							...
							if (genotypes[0][key] == 0) {
                                ...
                                if (line[headerData.colGeno1].charAt(0) == abLookup[count][1]) {
                                    ...
                                    log.reportError("Error - alleles for individual '" + (headerData.colSampleIdent < 0 ? trav : line[headerData.colSampleIdent]) + "' (" + line[headerData.colGeno1] + "/" + line[headerData.colGeno2] + ") do not match up with the defined AB lookup alleles (" + abLookup[count][0] + "/" + abLookup[count][1] + ") for marker " + markerNames[count]);
                                }
...
//		keys = Markers.orderMarkers(markerNames, proj.getFilename(proj.MARKER_POSITION_FILENAME), proj.getFilename(proj.MARKERSET_FILENAME, true, true), proj.getLog());
		...
		for (int i = 0; i<numThreads; i++) {
			threads[i] = new Thread(new NewParseIllumina(proj, fileCabinet.elementAt(i).toArray(new String[fileCabinet.elementAt(i).size()]), markerNames, keysKeys, lookup, delimiter, fingerprint, fixes, timeBegan, i));
			threads[i].start();
			try {
				Thread.sleep(100L);
