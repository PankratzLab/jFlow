...
public class MitoPipeline {
	...
	public static int catAndCaboodle(Project proj, int numThreads, String medianMarkers, int numComponents, String outputBase, boolean homosygousOnly, boolean markerQC, double markerCallRateFilter, String useFile, String betaOptFile, String pedFile, String sampleMapCsv, boolean recomputeLRR_PCs, boolean recomputeLRR_Median, boolean sampLrr, boolean doAbLookup, boolean imputeMeanForNaN, boolean gcCorrect, String refGenomeFasta, int bpGcModel, int regressionDistance, GENOME_BUILD build, double[] pvalOpt, String betaFile, boolean plot) {
		...
		if (Files.exists(sampleDirectory) && Files.list(sampleDirectory, Sample.SAMPLE_FILE_EXTENSION, false).length > 0 && proj.getSampleList() != null && proj.getSampleList().getSamples().length > 0) {
					...
					if (verifyAuxMarkers(proj, proj.INTENSITY_PC_MARKERS_FILENAME.getValue(), PC_MARKER_COMMAND)) {
						...
						if (verifyAuxMarkers(proj, medianMarkers, MITO_MARKER_COMMAND)) {
							...
							if (mitoResourceAvailable) {
								...
								BetaOptimizer.optimize(proj, proj.PROJECT_DIRECTORY.getValue()+pcApply.getExtrapolatedPCsFile(), proj.PROJECT_DIRECTORY.getValue() + outputBase + "_beta_opt/", requireBeta ? Resources.MITO_SUB_DIR : ext.parseDirectoryOfFile(betaFile), betaOptFile, proj.PROJECT_DIRECTORY.getValue() + outputBase + PCA_SAMPLES, pvalOpt, numComponents, markerCallRateFilter, numThreads);
								//merge
							} else {
	...
	public static void main(String[] args) {
		...
		usage += "	 (8) A file listing a subset of samples (DNA ID) to use for determining optimal PC selection, typically a list of unrelated and single race samples. If a list is not provided, only samples passing sample qc thresholds will be used. (i.e. " + PC_OPT_FILE + betaOptFile + " (no default))\n";

		usage += "   (9) The full path for a tab-delimited .PED format file with header \"" + Array.toStr(PED_INPUT) + "\" (i.e. pedFile=" + pedFile + "(no default))\n";
		usage += "   OR:\n";
		usage += "   (10) The full path for a Sample_Map.csv file, with at least two columns having headers \"" + SAMPLEMAP_INPUT[1] + "\" and \"" + SAMPLEMAP_INPUT[2] + "\"(i.e. mapFile=" + sampleMapCsv + " (default))\n\n";
		usage += "   NOTE:\n";
		...
		usage += "   (21) If marker qc is performed, the call rate cutoff for markers to be passed on to the sample QC step (i.e. markerCallRate=" + markerCallRateFilter + " (default))\n";
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				...
				pvalOpt = Array.toDoubleArray(ext.parseStringArg(args[i], null).split(","));
				numArgs--;
				...
				numArgs--;
			} else if (args[i].startsWith(PC_OPT_FILE)) {
				betaOptFile = ext.parseStringArg(args[i], null);
				numArgs--;
			}
