...
//	public static final int[] TRANSFORMATION_TYPES = {Transformations.IDENTITY, Transformations.QUANTILE, Transformations.INVERSE_NORMALIZE, Transformations.INVERSE_TDIST_5DF};
	public static final String[] TRANFORMATIONS = {"Raw Values", "Inverse normalized", "Inverse T-distribution with 5 df"};
	public static final int[] TRANSFORMATION_TYPES = {Transformations.IDENTITY, Transformations.INVERSE_NORMALIZE, Transformations.INVERSE_TDIST_5DF};
	...
	public static float[] transform(float[] input, int transformation_type, int[][] indices, boolean[] transform) {
		float[] output, trav;
		int count;
//		long time;
//		String timeString;
...
//		time = new Date().getTime();
		...
		for (int i = 0; i < indices.length; i++) {
			if (transform[i] && indices[i].length > 0) {
				...
				trav = Transformations.transform(Array.subArray(trav, 0, count), TRANSFORMATION_TYPES[transformation_type]);
