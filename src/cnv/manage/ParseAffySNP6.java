...
public class ParseAffySNP6 implements Runnable {
	...
	public ParseAffySNP6(Project proj, String[] files, String[] markerNames, int[] keysKeys, char[][] abLookup, long fingerprint, Hashtable<String, String> fixes, long timeBegan) {
		this(proj, files, markerNames, keysKeys, abLookup, fingerprint, fixes, timeBegan, -1);
	}

	public ParseAffySNP6(Project proj, String[] files, String[] markerNames, int[] keysKeys, char[][] abLookup, long fingerprint, Hashtable<String, String> fixes, long timeBegan, int threadId) {
		this.proj = proj;
		this.files = files;
	...
	public void run() {
		...
		PrintWriter writer;
		String[] line;
		int count, snpIndex, sampIndex, key, linecount;
		String trav;
		...
		try {
			writer = new PrintWriter(new FileWriter(proj.SOURCE_DIRECTORY.getValue(false, true) + "blank_list.txt", true));
...
//					parseAtAt = Boolean.parseBoolean(proj.getProperty(proj.PARSE_AT_AT_SYMBOL));
					...
					while (reader.ready()) {
						...
						linecount++;
						if (testline == null) {
							writer.println("Warning - " + files[i] + " has a blank line on line " + linecount);
						...
						if (!ignoreAB) {
							...
							// assign genotypes, since only AA BB etc are provided
							String A = Character.toString(abLookup[count][0]);
							String B = Character.toString(abLookup[count][1]);
							if (line[genotypeIndices[0]].equals("AA")) {
								line[genotypeIndices[0]] = A;
								line[genotypeIndices[1]] = A;
								// System.out.println("This is a problem: " + sta + "    " + line[genotypeIndices[0]] +line[genotypeIndices[1]] + "    " + abLookup[count][0] + abLookup[count][1]);

							} else if (line[genotypeIndices[0]].equals("AB")) {
								line[genotypeIndices[0]] = A;
								line[genotypeIndices[1]] = B;
								// System.out.println("This is a problem: " + sta + "    " + line[genotypeIndices[0]] +line[genotypeIndices[1]] + "    " + abLookup[count][0] + abLookup[count][1]);
							} else if (line[genotypeIndices[0]].equals("BB")) {
								line[genotypeIndices[0]] = B;
								line[genotypeIndices[1]] = B;
								// System.out.println("This is a problem: " + sta + "    " + line[genotypeIndices[0]] +line[genotypeIndices[1]] + "    " + abLookup[count][0] + abLookup[count][1]);

							} else if (!(ext.indexOfStr(line[genotypeIndices[0]], Sample.ALT_NULLS) == -1)) {
								line[genotypeIndices[0]] = "-";
								...
								// System.err.println("This is a problem: " + sta +"    "+ line[genotypeIndices[0]]);
							}
				...
				for (int j = 0; j < ParseConstants.DELIMITERS.length; j++) {
					if (Array.quantWithExtremeForTie(delimiterCounts[j], 0.5) > 4 && Array.quantWithExtremeForTie(delimiterCounts[j], 0.9) - Array.quantWithExtremeForTie(delimiterCounts[j], 0.1) == 0) {
						if (delimiter == null) {
							...
							JOptionPane.showMessageDialog(null, "Could not auto-detect the delimiter used in the Final Reports file: could be '" + delimiter + "' or '" + ParseConstants.DELIMITERS[j] + "'", "Error", JOptionPane.ERROR_MESSAGE);
							return;
						}
					}
				}
...
//			if (Boolean.parseBoolean(proj.getProperty(proj.LONG_FORMAT))) {
			...
			while (reader.ready()) {
				...
				lineCount++;
				if (testline == null) {
					System.err.println("there is a blank line on   " + lineCount);
					break;
				}
...
//		keys = Markers.orderMarkers(markerNames, proj.getFilename(proj.MARKER_POSITION_FILENAME), proj.getFilename(proj.MARKERSET_FILENAME, true, true), proj.getLog());
		...
		for (int i = 0; i < numThreads; i++) {
			threads[i] = new Thread(new ParseAffySNP6(proj, fileCabinet.elementAt(i).toArray(new String[fileCabinet.elementAt(i).size()]), markerNames, keysKeys, lookup, fingerprint, fixes, timeBegan, i));
			threads[i].start();
			try {
				Thread.sleep(100L);
...
//					parseAtAt = Boolean.parseBoolean(proj.getProperty(proj.PARSE_AT_AT_SYMBOL));
	...
	public static void main(String[] args) {
		...
		filename = "C:/workspace/Genvisis/projects/dbGap_compare_CEL_256.properties";
		// proj = null;
