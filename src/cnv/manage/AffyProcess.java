...
public class AffyProcess {
	...
	public void combineAll(int numthreads) {
		if (valid) {
			for (int i = 0; i < chpFiles.length; i++) {
				combineChp(proj, chpFiles[i], cn5chpFiles[i], combinedOutputFiles[i], delimiter, log);
			}
		}
	}
	...
	private static boolean combineChp(Project proj,String chp, String cnC5Chp, String output, String delimiter, Logger log) {
		...
		if (headerChp != null && headerCNChp != null) {
			try {
				...
				} while (reader.ready() && ext.indexFactors(ParseIllumina.SNP_HEADER_OPTIONS, line, false, true, false, false)[0] == -1);
