...
public class ParseDbgap implements Runnable {
	...
	public ParseDbgap(Project proj, String[] files, String[] markerNames, int[] keysKeys, char[][] abLookup, long fingerprint, Hashtable<String, String> fixes, long timeBegan) {
		this(proj, files, markerNames, keysKeys, abLookup, fingerprint, fixes, timeBegan, -1);
	}

	public ParseDbgap(Project proj, String[] files, String[] markerNames, int[] keysKeys, char[][] abLookup, long fingerprint, Hashtable<String, String> fixes, long timeBegan, int threadId) {
		this.proj = proj;
		this.files = files;
...
//					parseAtAt = Boolean.parseBoolean(proj.getProperty(proj.PARSE_AT_AT_SYMBOL));
					...
					while (reader.ready()) {
						...
						// System.out.println(genos[1]);
						// System.out.println(genos[0]);
						// }
...
//		keys = Markers.orderMarkers(markerNames, proj.getFilename(proj.MARKER_POSITION_FILENAME), proj.getFilename(proj.MARKERSET_FILENAME, true, true), proj.getLog());
		...
		for (int i = 0; i < numThreads; i++) {
			threads[i] = new Thread(new ParseDbgap(proj, fileCabinet.elementAt(i).toArray(new String[fileCabinet.elementAt(i).size()]), markerNames, keysKeys, lookup, fingerprint, fixes, timeBegan, i));
			threads[i].start();
			try {
				Thread.sleep(100L);
...
//			abLookup = new ABLookup(markerNames, proj.getFilename(proj.AB_LOOKUP_FILENAME), true, false, proj.getLog());
		if (abLookupRequired && Files.exists(proj.AB_LOOKUP_FILENAME.getValue())) {
			...
			if (lookup == null) {
				System.err.println("Warning - filed to provide columns \"" + Sample.GENOTYPE_FIELDS[2][0] + "\" / \"" + Sample.GENOTYPE_FIELDS[3][0] + "\" and the specificed AB_lookup file '" + proj.getProperty(proj.AB_LOOKUP_FILENAME) + "' does not exist; you'll need reconstruct the B allele for analysis");
			} else {
...
//					parseAtAt = Boolean.parseBoolean(proj.getProperty(proj.PARSE_AT_AT_SYMBOL));
	...
	public static void main(String[] args) {
		...
		String usage = "\n" + "cnv.manage.ParseDbgap requires 0-1 arguments\n" + 
