...
public class ExportCNVsToPedFormat {
	 ...
	 */
	public static void cnvBySample(String cnvFilename, String pedFilename, String outputRoot, String endOfLine, boolean includeDele, boolean includeDupl, boolean ordered, boolean collapsed, boolean homozygous, boolean excludeMonomorphicLoci, int markersPerFile, int win) {
		PrintWriter writer;
		CNVariant[] cnvs;
...
//		cnvBySample(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) + "../results/deletionsAndDuplications", endOfLine, true, true, false, true, false, true, markersPerFile, window);
		cnvBySample(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) + "../results/deletionsOnly", endOfLine, true, false, false, true, false, true, markersPerFile, window);
		cnvBySample(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) + "../results/homozygousDeletionsOnly", endOfLine, true, false, false, true, true, true, markersPerFile, window);
		System.exit(1);
		...
		try {
			...
			if (fileAction == JFileChooser.APPROVE_OPTION) {
				...
				cnvBySample(input.toString(), null, output, endOfLine, true, true, true, false, false, false, markersPerFile, window);
			} else {
				cnvBySample(cnvFilename, pedFilename, outputFilename, endOfLine,includeDele, includeDupl, ordered, collapsed, homozygous, excludeMonomorphicLoci, markersPerFile, window);
