...
//              UIManager.put("ToolTip.background", defaultBG);
	...
	public Trailer(Project proj, String selectedSample, String[] filenames, String location, final int startX, final int startY, final int width, final int height) {
		...
		for (int i = 0; cnvs != null && i<cnvs.length; i++) {
			...
			for (int j = 0; j<cnvs[i].length; j++) {
				if (cnvs[i][j].overlaps(currentView)) {
					...
					Color cnvColor = cnvs[i][j].getCN() == PennHmm.LOH_FLAG ? Color.GRAY : colors[cnvs[i][j].getCN() < 2 ? 0 : 1];
					g.setColor(cnvColor);
    		...
    		ItemListener cnvListener = new ItemListener() {
    			public void itemStateChanged(ItemEvent ie) {
    				...
    				if (pfb == null || pennHmm == null) {
						...
						CNVCallResult callResult = CNVCaller.callCNVsFor(proj, pennHmm, sample, Array.toDoubleArray(lrrs), Array.toDoubleArray(bafs), gcModel, pfb, markerSet, new int[] { chr }, null, true, proj.NUM_THREADS.getValue(), true);
