...
public class GenderPlot extends JFrame implements ActionListener {
	...
	public GenderPlot(Project proj, String[][] samples, double[][] data) {
		super("Gender Plot");
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);

		GenderPanel panel = new GenderPanel(proj, samples, data);
		...
		JLabel label = new JLabel("Gender Plot", JLabel.CENTER);
		label.setFont(new Font("Arial", 0, 20));
		descrPanel.add(label);

		label = new JLabel("Maybe or May not be correct: Only those B Allele Frequency (BAF) values between 0.15 and 0.85 are used in these calculations", JLabel.CENTER);
	...
	public static void loadGenderResults(Project proj) {
		...
		new GenderPlot(proj, Matrix.toStringArrays(samples), Matrix.toDoubleArrays(datapoints));
	}
