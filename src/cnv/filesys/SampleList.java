...
public class SampleList implements Serializable {
	...
	public static SampleList generateSampleList(Project proj) {
		...
		if (Files.list(proj.getDir(Project.PLOT_DIRECTORY, true), ".scat", proj.getJarStatus()).length>0) {
		...
		if (Files.exists(proj.getDir(Project.SAMPLE_DIRECTORY), false)) {
			...
			System.err.println("Error - failed to find the SAMPLE_DIRECTORY ("+proj.getDir(Project.SAMPLE_DIRECTORY)+"); no SmapleList could be generated");
			return null;
		}
