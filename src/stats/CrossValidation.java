...
public class CrossValidation {
	...
	/**
	 * @param train_deps
	 *            dependent variables for training data
	 * @param train_indeps
	 *            independent variables for training data
	 * @param validation_deps
	 *            dependent variables for validation data
	 * @param validation_indeps
	 *            independent variables for validation data
	 * @param verbose
	 *            verbose reporting of the training regression
	 * @param svdRegression
	 *            use an SVD based regression
	 * @param log
	...
	public CrossValidation(double[] train_deps, double[][] train_indeps, double[] validation_deps, double[][] validation_indeps, boolean verbose, boolean svdRegression, Logger log) {
		...
		this.val_deps = validation_deps;
		this.val_indeps = validation_indeps;
		this.predicteds = new double[validation_deps.length];
		this.residuals = new double[validation_deps.length];
		this.SSerr = 0;
	...
	public void train() {
		if (verifyData()) {
			if (logistic) {
				// TODO, logistic validation can be done in a similar manner? , please check before removing this?
				this.model = (RegressionModel) new LogisticRegression(train_deps, train_indeps, false, verbose);
	...
	/**
	 * Computes the predicted values for the validation set from the betas obtained in the training set
	 */
	public void computePredictedValues() {
		if (!analysisFailed) {
			...
			for (int i = 0; i < N; i++) {
				...
				for (int j = 1; j <= M; j++) {
					predicteds[i] += betas[j] * X[j][i];// if X[j][i] is NaN, predicteds[i] and residuals[i] will be NaN as well
				}
				if (logistic) {
	...
	/**
	 * @return true if the training data set and validation data set contain the same (gt 0) number of independent variables
	 *         <p>
	...
	public void clearInputData(boolean hard) {
		this.train_deps = new double[0];
		this.train_indeps = new double[0][0];
		this.val_deps = new double[0];
		this.val_indeps = new double[0][0];
		this.model = null;
		if (hard) {
			this.predicteds = new double[0];
	 ...
	 * <p>
	 * and the remaining k- 1 subsamples are used as training data. - http://en.wikipedia.org/wiki/Cross-validation_(statistics)#k-fold_cross-validation
	 * <p>
	 * Note: we do not randomly partition the data, we split it up as it comes. If random is desired, please shuffle before this method
	 * <p>
	 ...
	 *            dependent variables
	 * @param fold
	 *            a boolean array defining the comparisons
	 * @param train
	 *            if train, true entries an array of deps corresponding to true entries in fold will be returned. Otherwise the false entries will be returned
	 * @param log
	 * @return
	 */
	 ...
	 */
	private static double[][] extractIndeps(double[][] indeps, boolean[] fold, boolean train, Logger log) {
		int num = Array.booleanArraySum(fold);
		num = train ? num : fold.length - num;
		double[][] extract = new double[num][];
		int depIndex = 0;
	 ...
	 * @param deps
	 * @param chunks
	 *            we leave the number specified at chunks[i] out of each analysis
	 * @param log
	 *
	 */
