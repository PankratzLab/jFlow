...
public class CTable {
	public static String[] DEFAULT_NULL_VALUES = {"", ".", "NA", "na", "N/A", "n/a", "NaN", "Missing", "missing"};
	...
	public CTable(String[] array1, String[] array2) {
		...
		// Quick error check of the input data
...
//		for(int i=0; i<array1.length; i++) {
	...
	public void setCustomLabelsAndOrder(String[][] customRowLabelsAndOrder, String[][] customColumnLabelsAndOrder) {
	...
	public int[][] getContingencyTable() {
		...
		for (int i=0; i<rowLabels.length; i++) {
			for (int j=0; j<columnLabels.length; j++) {
				...
				for (int i2=0; i2<possibleRowValues.length; i2++) {
					for (int j2=0; j2<possibleColumnValues.length; j2++) {
						if (cells.containsKey(possibleRowValues[i2] + " x " + possibleColumnValues[j2])) {
							result[i][j] += cells.get(possibleRowValues[i2] + " x " + possibleColumnValues[j2]);
						}
					}
				}
			}
