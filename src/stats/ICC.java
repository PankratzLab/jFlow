...
/**
 * Class to compute the intraclass correlation coefficient. Currently only a "One-way Random" model is used;
 * 
...
public class ICC implements Serializable {
	...
	/**
	 * @param initData
	 *            double array that will form the values used to compute the ICC
	 * @param response
	 *            must be same length as input data. Represents the groupings to compute the ICC. For example, to compute ICC between sexes, this array would contain 1's and 2's
	 * @param maskedResponses
	 *            if not null, these responses will be excluded
	 * @param onlyTheseResponses
	 *            if not null, only these responses will be used.
	 * @param verbose
	 * @param log
	 *            <p>
	 *            Note : must call {@link ICC#computeICC()} to compute the ICC
	 */
	public ICC(double[] initData, String[] response, String[] maskedResponses, String[] onlyTheseResponses, boolean verbose, Logger log) {
		super();
		this.parsedData = initData;
		this.log = log;
		this.response = response;
		this.rowEffects = null;
		this.maskedResponses = maskedResponses;
		this.onlyTheseResponses = onlyTheseResponses;
		this.valid = verifyData(initData, response, log);
		this.ICC = Double.NaN;
		this.nTotal = 0;
	...
	private void populateFullStats() {
		if (valid) {
			this.nTotal = parsedData.length;
			this.sumTotal = Array.sum(parsedData);
			this.meanTotal = Array.mean(parsedData, sumTotal);
			this.SSTotal = Array.sumSq(parsedData, meanTotal);
			this.MSTotal = Array.variance(parsedData, SSTotal);
	...
	private void computeMSBetween() {
		...
		for (int i = 0; i < rowEffects.length; i++) {
			if (rowEffects[i].isValid()) {
				// System.out.println("Mean Total " + meanTotal + "\tmean\t" + i + "\t" + rowEffects[i].getRowMean() + "\t" + rowEffects[i].getN());
				MSBetween += rowEffects[i].getN() * Math.pow(rowEffects[i].getRowMean() - meanTotal, 2);
			}
		}
		if (verbose) {
			System.out.println("Between Rows\tSS: " + MSBetween + "\tMS: " + (MSBetween / (rowEffects.length - 1)));
		}
	...
	private void computeMSWithin() {
		...
		for (int i = 0; i < rowEffects.length; i++) {
			if (rowEffects[i].isValid()) {
				MSWithin += rowEffects[i].getSS();
			}
		}
		if (verbose) {
			System.out.println("Within Rows\tSS: " + MSWithin + "\tMS: " + (MSWithin / (rowEffects.length)));
		}
	...
	private void init(double[] data) {
		if (valid) {
			...
			this.parsedData = new double[dataPointsToUse];
	...
	private static class ResponseEffect implements Serializable {
		...
		public ResponseEffect(String label, double[] data) {
			...
			this.data = data;
			this.n = data.length;
			this.valid = verifyData(data);
			this.sum = Double.NaN;
			this.rowMean = Double.NaN;
			this.SS = Double.NaN;
			this.MS = Double.NaN;
			popluateRowMetrics();
			get();// TODO
		...
		private void popluateRowMetrics() {
			if (valid) {
				this.sum = Array.sum(data);
				this.rowMean = Array.mean(data, sum);
				this.SS = Array.sumSq(data, rowMean);
				this.MS = Array.variance(data, SS);
