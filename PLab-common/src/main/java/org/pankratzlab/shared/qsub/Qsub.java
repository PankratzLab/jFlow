package org.pankratzlab.shared.qsub;
...
public class Qsub {
  ...
  /**
   * Create a qsub file. <br />
   * <br />
   * Uses values for mem/proc/walltime based on the default JobQueue as defined in the default queue
   * properties file ({@link QueueProperties.PROPERTIES_FILE}). If this file is missing and default
   * values cannot be parsed from available queue information, Genvisis defaults will be used.
   * 
   ...
   * Create a qsub file. <br />
   * <br />
   * Uses values for mem/proc/walltime based on the default JobQueue as defined in the specified
   * queue properties file. If this file is missing and default values cannot be parsed from
   * available queue information, Genvisis defaults will be used.
   * 
  ...
  /**
   * Create a qsub file using the default mem/wall/proc values in the given JobQueue. If these
   * values are missing or not set, use Genvisis defaults.
   * 
  ...
  public static void makeQsub(String filename, boolean multiple, int start, int stop,
    ...
    if (multiple) { // could be more elegant and incorporated with those below if desired
      ...
      if (start == stop) {
        ...
        qsubs = Qsub.qsub("", ext.rootOf(filename) + "#", start, stop,
                          ArrayUtils.toStr(lines, "\n"), patterns, Files.PBS_MEM, Files.PBS_PROC,
                          null);
        if (qsubs.length > 1) {
  ...
  public static void qsub(String filename, String command, int totalMemoryRequestedInMb,
    ...
    Qsub.writeQsubHeader(writer, filename, totalMemoryRequestedInMb, walltimeRequestedInHours,
                         numProcs, null);
  ...
  public static void qsub(String root_batch_name, String dirToSwitchToBeforeRunning, int numBatches,
    ...
    try {
      if (numBatches > 1) {
        writer = Files.openAppropriateWriter(ext.parseDirectoryOfFile(root_batch_name) + "master."
