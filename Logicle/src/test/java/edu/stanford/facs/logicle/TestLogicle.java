...
public class TestLogicle {
  ...
  /**
   * Test whether the specified Logicle scale is invertible to the stated precision using
   * pseudorandom data from the supplied distribution.
   * 
   * @param logicle
   * @param distribution
   * @param precision
   */
  public void testLogicleScale(Logicle logicle, Distribution distribution, double precision) {
    for (int i = 0; i < NUMBER_OF_VALUES; ++i) {
      ...
      if (Math.abs(trueScale - testScale) > precision) fail("scale invertability test failed");
    }
  }

  /**
   * Test whether the inverse of the specified Logicle scale is accurate to the stated tolerance
   * using pseudorandom data from the supplied distribution. Precision is calculated as the
   * tolerance times the slope of the biexponential at a given point on the scale.
   * 
   * @param logicle
   * @param distribution
   * @param tolerance
   */
  public void testLogicleInverse(Logicle logicle, Distribution distribution, double tolerance) {
    for (int i = 0; i < NUMBER_OF_VALUES; ++i) {
      ...
      if (Math.abs(trueData - testData) > tolerance
                                          * logicle.slope(scale)) fail("inverse data test failed");
    }
  }

  /**
   * Test whether two arrays are equal to single precision accuracy. Used to test equivalence of
   * sets of scale coordinates returned by {@link edu.stanford.facs.logicle.Logicle#axisLabels()}.
   * 
  ...
  public void testLabels(double[] actual, double expected[]) {
    if (actual.length != expected.length) fail("number of labels differs");
    for (int i = 0, n = expected.length; i < n; ++i) {
      double delta = actual[i] - expected[i];
      if (expected[i] != 0) delta /= expected[i];
      if (Math.abs(delta) >= Math.ulp(1F)) fail("label values differ");
    }
  }

  /**
   * Test method for {@link edu.stanford.facs.logicle.Logicle#solve(double,double)}. Test whether
   * the solution found actually solves the problem.
   */
  @Test
  public void testSolve() throws Exception {
    ...
    for (int i = 0; i < NUMBER_OF_VALUES; ++i) {
      ...
      if (Math.abs(true_w - test_w) > tolerance) fail("solve for d failed");
    }
  }

  /**
   * Test method for {@link edu.stanford.facs.logicle.Logicle#scale(double)}. Test that the scale
   * function really is the inverse of the biexponential
   */
  @Test
  ...
  /**
   * Test method for {@link edu.stanford.facs.logicle.Logicle#inverse(double)}. Check that the
   * inverse function really is the inverse of the scale function
   */
  @Test
  ...
  /**
   * Test method for {@link edu.stanford.facs.logicle.Logicle#axisLabels()}. Test whether the
   * function returns the expected results in various circumstances.
   */
  @Test
  ...
  /**
   * Test method for {@link edu.edu.stanford.facs.logicle.FastLogicle#intScale(double)}. Test that
  ...
  public void testIntScale() throws Exception {
    ...
    for (int i = 0; i < NUMBER_OF_VALUES; ++i) {
      ...
      if (trueInt != testInt) fail("fast int scale test failed");
    }
  }

  /**
   * Test method for {@link edu.edu.stanford.facs.logicle.FastLogicle#scale(double)}. Test that the
   * scale function really is the inverse of the biexponential
   */
  @Test
