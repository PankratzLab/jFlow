...
public class Logicle {
  ...
  protected Logicle(double T, double W, double M, double A, int bins) {
    if (T <= 0) throw new LogicleParameterException("T is not positive");
    if (W < 0) throw new LogicleParameterException("W is negative");
    if (M <= 0) throw new LogicleParameterException("M is not positive");
    if (2 * W > M) throw new LogicleParameterException("W is too large");
    // if (-A > W || A + W > M - W)
