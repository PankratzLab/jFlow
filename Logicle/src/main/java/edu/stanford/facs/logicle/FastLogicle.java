...
/**
 * A fast implementation of the Logicle data scale and its inverse. This version is substantially
 * faster than the more general implementation but will throw a LogicleArgumentException if the
 * arguments are outside of the normal scale range 0 <= scale < 1.0. It should provide suitable
 * speed and resolution for real time graphical applications.
 * 
 * @author Wayne A. Moore
 * @version 1.0
 */
public class FastLogicle extends Logicle {
   ...
   * @param bins number of data values in the lookup table
   */
  ...
  /**
   * Constructs a <code>FastLogicle</code> object with the default number of decades and no
  ...
  /**
   * Constructs a <code>FastLogicle</code> object with the default number of bins
   * 
   * @param T the double maximum data value or "top of scale"
   * @param W the double number of decades to linearize
   * @param M the double number of decades that a pure log scale would cover
  ...
  /**
   * Constructs a <code>FastLogicle</code> object with the default number of bins and no additional
   * negative decades
   * 
   * @param T the double maximum data value or "top of scale"
   * @param W the double number of decades to linearize
  ...
  /**
   * Constructs a <code>FastLogicle</code> object with the default number of bins, the default
   * number of decades and no additional negative decades
   * 
   * @param T the double maximum data value or "top of scale"
   * @param W the double number of decades to linearize
   */
  ...
  /**
   * Looks up a data value in the internal table. Provides a fast method of binning data on the
   * Logicle scale
   * 
   * @param value a double data value
   * @return the bin for that data value
   * @throws LogicleArgumentException if the data is out of range
   */
  public int intScale(double value) {
    ...
    if (hi < 0 || lo > bins) throw new LogicleArgumentException(value);
   ...
   * 
   * @param index a bin number
   * @return the double data value
   * @throws LogicalArgumentException if the index is out of range
   */
  public double inverse(int index) {
    if (index < 0 || index >= bins) throw new LogicleArgumentException(index);

    return lookup[index];
  }

  /**
   * Computes an approximation of the Logicle scale of the given data value. This method looks up
   * the data in the table then performs an inverse linear interpolation.
   * 
   * @throws LogicleArgumentException if the data value would be off scale
  ...
  public double scale(double value) {
    ...
    double delta = (value - lookup[index]) / (lookup[index + 1] - lookup[index]);

    return (index + delta) / (double) bins;
  }

  /**
   * Computes the approximate data value corresponding to a scale value. This method uses linear
   * interpolation between tabulated data values.
   * 
   * @throws LogicleArgumentException if the scale is out of range
  ...
  public double inverse(double scale) {
    ...
    if (index < 0 || index >= bins) throw new LogicleArgumentException(scale);
