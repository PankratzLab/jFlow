...
import org.pankratzlab.shared.bioinformatics.MapSNPsAndGenes;
import org.pankratzlab.shared.gwas.Conditional;
...
public class CALiCo {
  ...
  public static void parseAllFilesInDirectory(String genoPlinkDirPlusRoot, String phenoCovarDir,
    ...
    if (Files.exists(phenoCovarDir + CONDITIONALS_TXT_FILE)) {
      ...
      for (int i = 0; i < models.length; i++) {
        ...
        if (!new File(resultDir + models[i] + ".out").exists()) {
          ...
          idVariable[0] = ext.indexOfStr(Files.getHeaderOfFile(phenoCovarDir + root + ".xln",
                                                               null)[0],
                                         SAMPLE_ID_SYNONYMS);
    ...
    for (int i = 0; i < models.length; i++) {
      ...
      // if (conditionals == null || conditionals.size() == 0) {
      fileParameters[i + 2] = resultDir + root
                              + ".out 'MarkerName' 'N' 'Effect_allele_frequency' 'BETA'=beta_"
                              + root + " 'P-value'=pval_" + root;
      // } else {
  ...
  // public static void parseAllFilesInDirectory_2(String genoFiles, String phenoCovarDir, String
  // resultDir, String scratchDir, Logger log) {
  // String[] files;
  // String[] args;
  // String[] uniqueMarkers;
  // String[] fileParameters;
  ...
  // idVariable[i] = ext.indexOfStr(Files.getHeaderOfFile(phenoCovarDir + files[i], null)[0],
  // SAMPLE_ID_SYNONYMS);
  // if (idVariable[i] < 0) {
  ...
  // sexAsCovariate = parsePhenotypes(genoFiles, phenoCovarDir+files[i],
  // SAMPLE_ID_SYNONYMS[idVariable[i]], phenoCovarDir, log);
  // runAndParseResults(resultDir, ext.rootOf(files[i]), sexAsCovariate, genoFiles, scratchDir,
  // log);
  // }
  ...
  //
  //
  //// uniqueMarkers = HashVec.loadFileToStringArray(dir+"cat_hits.txt", false, new int[] {0,1,2},
  // true);
  ...
  // if (i==0) {
  // fileParameters[0] = phenoCovarDir+root+".out 'MarkerName' 'Chr' 'Position' 'Effect_allele'
  // 'Reference_allele'";
  // fileParameters[1] = phenoCovarDir+"minimumPvalues.txt 0 2=Gene(s) tab";
  // }
  // fileParameters[i+2] = phenoCovarDir+root+".out 'MarkerName' 'N' 'Effect_allele_frequency'
  // 'BETA'=beta_"+root+" 'P-value'=pval_"+root;
  // }
  // fileParameters[files.length+2] = phenoCovarDir+"minimumPvalues.txt 0 1=minPval tab";
  //
  // Files.combine(uniqueMarkers, fileParameters, null, "MarkerName", phenoCovarDir+"topHits.xln",
  // log, true, true, false, false);
  //
  // }

  // public static void parseAllFilesInDirectory_1(String dir, String locationOfGenotypeFiles,
  // String scratchDir) {
  // String[] files;
  // String[] args;
  // String[] uniqueMarkers;
  ...
  // String root = ext.rootOf(files[i]);
  // if (i==0) {
  // fileParameters[i] = dir+root+".out 'MarkerName' 'Chr' 'Position' 'Effect_allele'
  // 'Reference_allele' 'N' 'Effect_allele_frequency' 'BETA'=beta_"+root+" 'P-value'=pval_"+root;
  // } else {
  // fileParameters[i] = dir+root+".out 'MarkerName' 'N' 'Effect_allele_frequency'
  // 'BETA'=beta_"+root+" 'P-value'=pval_"+root;
  // }
  // }
  //
  // Files.combine(uniqueMarkers, fileParameters, null, "MarkerName", dir+"topHits.xln", log, true,
  // true, true, false);
  //
  // }

  // public static void parsePhenotypes(String genoFiles, String phenoCovarFilename, String
  // idVarName, String scratchDir, Logger log) {
  // BufferedReader reader;
  // PrintWriter writer;
  //// Logger log;
  // String[] header;
  ...
  // phenos = HashVec.loadFileToStringArray(scratchDir+root+"_pheno.dat", true, new int[] {2},
  // false);
  ...
  // // 1 -> 2
  // // 0 -> 1
  // // anything else becomes missing; if value is not among ext.MISSING_VALUES, then report error
  // to log
  ...
  // System.err.println("Error: file \"" + scratchDir+root+"_pheno.dat.temp" + "\" not found in
  // current directory");
  // System.exit(1);
  ...
  // }
  //
  // public static boolean parseCovars(String genoFiles, String phenoCovarFilename, String
  // idVarName, String scratchDir, Logger log) {
  //// Logger log;
  // String[] header;
  // String temp;
  ...
  // hashPheno = HashVec.loadFileToHashString(scratchDir+root+"_pheno.dat", new int[] {0,1}, new
  // int[] {2}, false, "\t", true, false, false);
  // hashCovariates = HashVec.loadFileToHashString(scratchDir+root+"_covars.dat", new int[] {0,1},
  // Array.subArray(Array.intArray(args.length-6), 2), false, "\t", true, false, false);
  ...
  public static boolean parsePhenotypes(String genoFiles, String phenoCovarFilename,
    ...
    // load phenotype to memory
    phenos = HashVec.loadFileToStringArray(outputDir + root + "_pheno.dat", true, new int[] {2},
                                           false);
    isBinary = RegressionModel.isBinaryTrait(phenos, log);
    if (isBinary) {
      if (ArrayUtils.max(ArrayUtils.toIntArray(phenos)) == 1
          && ArrayUtils.min(ArrayUtils.toIntArray(phenos)) == 0) {
        ...
        try {
          ...
          while (reader.ready()) {
            // 1 -> 2
            // 0 -> 1
            // anything else becomes missing; if value is not among ext.MISSING_VALUES, then report
            // error to log
            line = reader.readLine().split("\t");
            if (line[2].equals("1")) {
              line[2] = "2";
              ...
              for (String element : ext.MISSING_VALUES) {
                if (element.contains(line[2])) {
                  log.reportError("Unrecognized pheno type: " + line[2]);
                }
              }
              line[2] = "";
  ...
  // public static void runAndParseResults(String genoFiles, String resultDir, String root, boolean
  // sexAsCovariate, String scratchDir, Logger log) {
  // String[] commands;
  //
  // if (scratchDir == null) {
  ...
  // "C:/PLINK/plink --bfile N:/statgen/CALiCo/filteredGenotypes/plink --pheno " + scratchDir + root
  // + "_pheno.dat --covar " + scratchDir + root + "_covars.dat" + (sexAsCovariate?" --sex":"") + "
  // --logistic --ci 0.95 --out " + scratchDir + root,
  // "C:/PLINK/plink --bfile N:/statgen/CALiCo/filteredGenotypes/plink --keep " + scratchDir + root
  // + "_used.dat --freq --out " + scratchDir + root + "_freq"
  // };
  ...
  // ResultsPackager.parseStdFormatFromPlink("", scratchDir + root + ".assoc.logistic", "ADD",
  // genoFiles+".bim", scratchDir + root + "_freq.frq", null, resultDir + root + ".out", log);
  // } else {
  // ResultsPackager.parseStdFormatFromPlink("", scratchDir + root + ".assoc.linear", "ADD",
  // genoFiles+".bim", scratchDir + root + "_freq.frq", null, resultDir + root + ".out", log);
  // }
  // }
  ...
  public static void runAndParseResults(String genoFileDirPlusRoot, String phenoFileDirPlusRoot,
    ...
    commands = new String[] {// "Path = %Path%; C:\plink",
                             ...
                             plinkCommand + " --bfile " + genoFileDirPlusRoot + " --keep "
                                                         + phenoFileDirPlusRoot
                                                         + "_used.dat --freq --out " + scratchDir
                                                         + outFileRoot + "_freq"};
    ...
    // ResultsPackager.parseStdFormatFromPlink("", scratchDir + outFileRoot + ".assoc.logistic",
    // "ADD", genoFileDirPlusRoot+".bim", scratchDir + outFileRoot + "_freq.frq", null, 1, resultDir
    // + outFileRoot + ".out", log);
    // ResultsPackager.parseStdFormatFromPlink("", scratchDir + outFileRoot + ".assoc.logistic",
    // outFileRoot.replace("_", ":"), genoFileDirPlusRoot+".bim", scratchDir + outFileRoot +
    // "_freq.frq", null, 1, resultDir + outFileRoot + "_cov.out", log);
    // } else {
    // ResultsPackager.parseStdFormatFromPlink("", scratchDir + outFileRoot + ".assoc.linear",
    // "ADD", genoFileDirPlusRoot+".bim", scratchDir + outFileRoot + "_freq.frq", null, 1, resultDir
    // + outFileRoot + ".out", log);
    // ResultsPackager.parseStdFormatFromPlink("", scratchDir + outFileRoot + ".assoc.linear",
    // outFileRoot.replace("_", ":"), genoFileDirPlusRoot+".bim", scratchDir + outFileRoot +
    ...
    while (models.hasMoreElements()) {
      ...
      resultsSnp = SkatMeta2.loadFile(resultFilesDir + filename + ".out", null,
                                      ...
                                      new String[] {"Effect_allele", "Reference_allele",
                                                    "Effect_allele_frequency", "N", "BETA", "SE",
                                                    "P-value"},
                                      new String[] {"Chr ==" + modelLine[1],
                                                    "Position >= " + modelLine[2],
      ...
      while (snps.hasMoreElements()) {
        ...
        outLine = model1[0] // model
                  ...
                  + delimiterForOutputFile
                  + (indexLine == null || indexLine.length < 5 ? "NA" : indexLine[2] // index.CAF
                                                                        + delimiterForOutputFile
                                                                        + (indexLine == null
                                                                           || indexLine.length < 4
  ...
  public static void metaAnalyzeSOL(String dir, String filePattern, String root, String mapFile) {
    ...
    for (String site : sites) {
      if (Files.exists(dir + root + site + ".out")) {
        System.out.println(root + site + ".out already exists");
      } else {
        ...
        Files.cat(ArrayUtils.toStringArray(v), dir + root + site + ".out", new int[0],
                  new Logger());
      }
    }
  ...
  public static void metaAnalyzeSOL(String dir, String root, String mapFile) {
    ...
    if (Files.exists(dir + root + "_NWeighted1.out")) {
      ...
      Metal.metaAnalyze(dir, inputFiles, Aliases.MARKER_NAMES, root + "_NWeighted",
                        Metal.PVAL_ANALYSIS, null, ArrayUtils.doubleArray(inputFiles.length, -9),
                        log);
    }
    ...
    log.report("QQ_FILENAMES=" + dir
               + ArrayUtils.toStr(ArrayUtils.toStringArray(qqFiles), ";" + dir));
  }
  ...
  public static void main(String[] args) {
    ...
    // String scratchDir = phenoCovarDir.substring(0, phenoCovarDir.substring(0,
    // phenoCovarDir.length()-2).lastIndexOf("/")) + "/scratches/";
    ...
    String resultDir = phenoCovarDir
                       + (Files.exists(phenoCovarDir + CONDITIONALS_TXT_FILE) ? "conditionals/"
                                                                              : "results/");
    String plinkCommand;
    ...
    // metaAnalyzeSOL("D:/data/SOL/", "MODEL3wPCs_",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");
    // metaAnalyzeSOL("D:/data/SOL/", "MODEL4wPCs_",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");
    // System.exit(1);

    // metaAnalyzeSOL("D:/data/SOL/models12/", "MODEL1",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");
    // metaAnalyzeSOL("D:/data/SOL/models12/", "MODEL2",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");

    // metaAnalyzeSOL("D:/data/SOL/GlucoseInsulin/", "MODEL1",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");
    // metaAnalyzeSOL("D:/data/SOL/GlucoseInsulin/", "MODEL2",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");
    // metaAnalyzeSOL("D:/data/SOL/GlucoseInsulin/", "MODEL3",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");
    // metaAnalyzeSOL("D:/data/SOL/GlucoseInsulin/", "MODEL4",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");

    // metaAnalyzeSOL("N:/statgen/CALICo_SOL/T2DM/results_ver6/",
    // "T2DM_MODEL1_SolGeno#_%%_ver6.out", "T2DM",
    // "N:/statgen/CALICo_SOL/SOL-2013-04-05_Metabochip-mappingfile.txt");
    // System.exit(1);
     ...
     */
    // parseResultsForPageGlucoseInsulinPaperFormat("D:/CALiCo_conditional/instructions/RequestedConditionalAnalyses.txt",
    // "D:/CALiCo_conditional/results/", "D:/CALiCo_conditional/parsedResults.csv");
    // System.exit(0);
    // modelListFileNameWhenParsingForPageGlucoseInsulinConditionalPaper =
    // "D:/CALiCo_conditional/instructions/RequestedConditionalAnalyses.txt";
    // resultDir = "D:/CALiCo_conditional/results/";
    // outputFileNameWhenParsingForPageGlucoseInsulinConditionalPaper =
    // "D:/CALiCo_conditional/parsedResults.csv";

    /**
     * The following 3 parameters are all what needed for conditional analysis, in addition to the
     * "conditionals.txt" file mentioned in the help.
     */
    ...
    for (String arg : args) {
      if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
        ...
        System.exit(1);
      } else if (arg.startsWith("plinkcommand=")) {
        plinkCommand = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("phenocovarfile=")) {
        phenoCovarFilename = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("geno=")) {
        geno = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("scratchdir=")) {
        scratchDir = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("phenocovardir=")) {
        phenoCovarDir = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("resultdir=")) {
        resultDir = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("pageglucoseinsulinmodellist=")) {
        modelListFileNameWhenParsingForPageGlucoseInsulinConditionalPaper = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("pageglucoseinsulinoutput=")) {
        outputFileNameWhenParsingForPageGlucoseInsulinConditionalPaper = arg.split("=")[1];
        numArgs--;
      } else {
