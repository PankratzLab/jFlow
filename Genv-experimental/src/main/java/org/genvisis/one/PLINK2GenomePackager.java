...
public class PLINK2GenomePackager {
  ...
  void process(String phenoDir, String pmAllFile, String N) {
    ...
    Hashtable<String, String> pmAll =
        HashVec.loadFileToHashString(pmAllFile, 1, new int[] {0, 3}, "\t", false, false);
    ...
    String path =
        PLINK2GenomePackager.class.getProtectionDomain().getCodeSource().getLocation().getPath();
    ...
    for (String pheno : phenos) {
      ...
      if (parts.length == 1) {
        ...
        if (setupForScript(dir, fileList, famFile, covarPrefices.get(parts[0]), pheno)) {
          ...
          String script =
              createScript(fileList, famFile, covarPrefices.get(parts[0]), pheno, phenoDir);
