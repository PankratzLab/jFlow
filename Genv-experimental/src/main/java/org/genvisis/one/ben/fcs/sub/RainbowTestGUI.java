...
public class RainbowTestGUI extends JFrame {
  ...
  private void reCalcTableData() {
    ...
    for (String f : baseFCSFiles) {
      if (!this.baseFiles.containsKey(f) && !cache.containsKey(f)) {
        this.baseFiles.put(f, loadFCSFile(baseDir + f));
      } else if (cache.containsKey(f)) {
        this.baseFiles.put(f, null);
      }
      List<String> p = (cache.containsKey(f) ? Arrays.asList(cache.get(f).params) : baseFiles.get(f).getAllDisplayableNames(currData));
      paramNames.addAll(p);
      paramSet.addAll(p);
    }
    ...
    for (String f : allFilesFullPaths) {
      if (!this.compFiles.containsKey(f) && !cache.containsKey(f)) {
        this.compFiles.put(f, loadFCSFile(f));
      } else if (cache.containsKey(f)) {
        this.compFiles.put(f, null);
      }
      List<String> p = (cache.containsKey(f) ? Arrays.asList(cache.get(f).params) : compFiles.get(f).getAllDisplayableNames(currData));
      paramNames.addAll(p);
      paramSet.addAll(p);
    }
    ...
    TreeMap<Date, ArrayList<String>> fileMap = new TreeMap<Date, ArrayList<String>>();
    for (Entry<String, FCSDataLoader> l : compFiles.entrySet()) {
      Date key = cache.containsKey(l.getKey()) ? cache.get(l.getKey()).runDate : l.getValue().getRunDate();
      ArrayList<String> files = fileMap.get(key);
      if (files == null) {
        files = new ArrayList<String>();
        fileMap.put(key, files);
  ...
  private ArrayList<String> check(String[] params) {
    ...
    for (String param : params) {
      ...
      trendMap.put(param, new ArrayList[] {all1TrendsForParam, all2TrendsForParam,
          all15TrendsForParam});
    }
    ...
    for (Entry<String, ArrayList<ArrayList<String>>[]> entry : trendMap.entrySet()) {
      ...
      if (!entry.getValue()[0].isEmpty()) {
        for (ArrayList<String> trend : entry.getValue()[0]) {
          ...
          String warn =
              trend.size() + "-count trend in parameter " + p[p.length - 1]
        ...
        for (ArrayList<String> trend : entry.getValue()[2]) {
          ...
          for (String s : trend) {
            String warn =
                "Source " + s + " has a deviant (>15% of mean) value for parameter "
                    + p[p.length - 1];
            trendWarnings.add(warn);
          }
        }
  ...
  private void addBaseToModel(String[] paramNames) {
    ...
    for (ArrayList<String> fls : map.values()) {
      for (String f : fls) {
        ...
        if (co != null && ((gateStrat == null && "".equals(co.gateFileApplied)) || gateStrat.getFile().equals(co.gateFileApplied)) && Array.equals(paramNames, co.params, false)) {
