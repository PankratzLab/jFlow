...
//        AXIS_SCALE.LOG.setTransform(new LogTransform(this));
    ...
    public synchronized void drawAll(Graphics g, boolean base) {
        ...
        if (DEBUGGING) {
            System.out.println("Finished drawing "  + points.length + " pts in "
                                                    + ext.getTimeElapsed(pointTime));
        }
        ...
        for (int i = 0; rectangles != null && i < rectangles.length && flow; i++) {
            if ((base
                ...
                widthPixel = Math.abs(getXPixel(rectangles[i].getStartXValue())
                                                            - getXPixel(rectangles[i].getStopXValue()));
                heightPixel = (Math.abs(getYPixel(rectangles[i].getStartYValue())
                                                                - getYPixel(rectangles[i].getStopYValue())));
    ...
    private void drawYAxis(Graphics g, double[] plotMinMaxStep) {
        ...
        Grafik.drawThickLine(   g, canvasSectionMaximumX, getHeight() - canvasSectionMaximumY,
                                                    canvasSectionMaximumX, canvasSectionMaximumY - axisXHeight + HEAD_BUFFER,
...
//        return l.inverse((linTrans.inverseY(val) - 256) / 512);
    ...
    class LogTransform extends AxisTransform {
      ...
      private double scale(double val, boolean xAxis) {
        ...
        float log_low = plotMinMax[0] == 0 ? 0 : (plotMinMax[0] < 0 ? -1 * flog10(plotMinMax[0] * -1) : flog10(plotMinMax[0]));
        float log_high = plotMinMax[1] == 0 ? 0 : (plotMinMax[1] < 0 ? -1 * flog10(plotMinMax[1] * -1) : flog10(plotMinMax[1]));
        float log_val = val == 0 ? 0 : (val < 0 ? -1 * flog10(val * -1) : flog10(val));
    ...
    class LinearTransform extends AxisTransform {
      ...
      public double scaleX(double val) {
        ...
        if (invertX) {
          return (int) ((plotMinMax[1] - val) / (plotMinMax[1] - plotMinMax[0]) * (screenMinMax[1] - screenMinMax[0])) + screenMinMax[0];
        } else {
      ...
      public double scaleY(double val) {
        ...
        if (invertY) {
          return panel.getHeight() - (int) ((plotMinMax[1] - val) / (plotMinMax[1] - plotMinMax[0]) * (screenMinMax[1] - screenMinMax[0]) + screenMinMax[0]);
        } else {
          return panel.getHeight() - (int) ((val - plotMinMax[0]) / (plotMinMax[1] - plotMinMax[0]) * (screenMinMax[1] - screenMinMax[0]) + screenMinMax[0]);
        }
      }
      ...
      public double inverseX(double val) {
        ...
        double[] plotMinMax = getPlotX();
        if (invertX) {
            return plotMinMax[1] - ((double) (val - screenMinMax[0]) / (double) (screenMinMax[1] - screenMinMax[0]) * (plotMinMax[1] - plotMinMax[0]));
        } else {
            return plotMinMax[0] + ((double) (val - screenMinMax[0]) / (double) (screenMinMax[1] - screenMinMax[0]) * (plotMinMax[1] - plotMinMax[0]));
        }
      }
      ...
      public double inverseY(double val) {
        ...
        double[] plotMinMax = getPlotY();
        if (invertY) {
          return plotMinMax[1] + ((double) (val + screenMinMax[0] - panel.getHeight()) / (double) (screenMinMax[1] - screenMinMax[0]) * (plotMinMax[1] - plotMinMax[0]));
        } else {
          return plotMinMax[0] - ((double) (val + screenMinMax[0] - panel.getHeight()) / (double) (screenMinMax[1] - screenMinMax[0]) * (plotMinMax[1] - plotMinMax[0]));
        }
      }
