...
public class FCSPanel extends AbstractPanel2 implements MouseListener, MouseMotionListener {
	...
	public void generatePointsRectanglesAndLines() {
		...
		if (showMedSD[0] || showMedSD[1]) {
			...
			xMin = columnsChangedX
							|| dataChanged || Double.isNaN(xMin)
																											? Math.min(Math.min(0, plotXmin)	- xMed,
																																Array.min(xData) - xMed)
																										: xMin;
		...
		if (showMedSD[2] || showMedSD[3]) {
			...
			yMin = columnsChangedY	|| dataChanged
							|| Double.isNaN(yMax)	? Math.min(Math.min(0, plotYmin) - yMed, Array.min(yData))
																		: yMin;
			...
			if (showMedSD[3]) {
				...
				lineList.add(new GenericLine(	(float) yMin, (float) (yMed - ySD), (float) yMax,
																			(float) (yMed - ySD), (byte) 1, (byte) 9, (byte) 1, 0,
																			false));
				lineList.add(new GenericLine(	(float) yMin, (float) (yMed + ySD), (float) yMax,
																			(float) (yMed + ySD), (byte) 1, (byte) 9, (byte) 1, 0,
																			false));
	...
	private void updateGateColor() {
		...
		for (int i = 0, count =
													parentGating == null	? points.length
					...
					case 'R':
						...
						if (newName == null) {
							...
							if (fcp.duplicateGateName(newName)) {
								do {
									newName =
													JOptionPane.showInputDialog(fcp,
