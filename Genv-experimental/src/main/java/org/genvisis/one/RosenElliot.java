...
public class RosenElliot {
  ...
  public static void makePedigree(String dir) {
    ...
    try {
      ...
      System.out.println("Assuming we're taking the F1s of a " + dnaList.get(0) + " x "
                         + dnaList.get(1) + " cross and backcrossing into " + dnaList.get(1));
  ...
  public static void updateTrait(String dir, String traitfile) {
    ...
    try {
      ...
      while (reader.ready()) {
        line = reader.readLine().trim().split(PSF.Regex.GREEDY_WHITESPACE);
        if (line[1].startsWith(TRAIT_STRAIN_PREFIX)
            || line[1].startsWith(BC_STRAIN_PREFIX)) {} else if (hash.containsKey(line[1])) {
