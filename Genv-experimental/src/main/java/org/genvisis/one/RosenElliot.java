...
public class RosenElliot {
	...
	public static final String[] PED_HEADER = {"Animal", "Sire", "DOB", "DOD", "DaysAtDeath", "DaysLivedSoFar", "Affection"};
		...
		try {
			...
			while (reader.ready()) {
				...
				if (!temp.trim().equals("")) {
					...
					for (int i = 0; i<line.length/2; i++) {
						if (!line[i*2].trim().equals("")) {
							...
							if (dna.startsWith(TRAIT_STRAIN_PREFIX)||dna.startsWith(BC_STRAIN_PREFIX)) {
								System.err.println("Error - sample '"+trav+"' has a dna '"+dna+"' that starts with a reserved prefix");
							}
					...
					writer.println(trav+"\t"+sire+"\t"+dob+"\t"+dod+"\t"+(daysAtDeath==-1?".":daysAtDeath)+"\t"+(daysLivedSoFar==-1?".":daysLivedSoFar)+"\t"+aff);
					daysAtDeath = daysLivedSoFar = aff = -1;
				}
			}
			reader.close();
	...
	public static void reformatGenotypes(String dir, String filename, String refGenotypes) {
		...
		try {
			...
			if (index==-1) {
				System.err.println("Error - the reference dna '"+refGenotypes+"' was not found in the header");
				System.exit(1);
			}

			writer.print(line[0]+"\t"+line[1]+"\t"+line[2]+"\t"+TRAIT_STRAIN+"\t"+BC_STRAIN);
			...
			while (reader.ready()) {
				...
				writer.print(line[0]+"\t"+line[1]+"\t"+line[2]+"\t"+(reversi?line[index]+"\t"+flip(line[index]):flip(line[index])+"\t"+line[index]));
				for (int i = 3; i<line.length; i++) {
					if (line[i].equals("H")) {
						line[i] = "AB";
					}
	...
	public static String cleanUpID(String str) {
		...
		for (int i = 0; i<SUFFIXES.length; i++) {
			if (str.endsWith(SUFFIXES[i])) {
				str = str.substring(0, str.length()-SUFFIXES[i].length());
			}
		}
		if (str.length()!=EXP_DNA_LEN) {
			System.err.println("Error - parsed DNA# '"+str+"' was not of expected length: "+EXP_DNA_LEN);
		}
	...
	public static String flip(String str) {
		for (int i = 0; i<DIP_TRIP.length; i++) {
			if (str.equals(DIP_TRIP[i][0])) {
				return DIP_TRIP[i][1];
			}
		}
	...
	public static void makePedigree(String dir) {
		...
		try {
			...
			System.out.println("Assuming we're taking the F1s of a "+dnaList.elementAt(0)+" x "+dnaList.elementAt(1)+" cross and backcrossing into "+dnaList.elementAt(1));
			...
			while (reader.ready()) {
				...
				if (line[1].equals("Inbred")) {
					writer.println("1\t"+dna+"\t"+TRAIT_STRAIN_PREFIX+"99\t"+BC_STRAIN_PREFIX+"01\t1\t"+(dnaList.contains(dna)?"2":"0"));
				} else {
					...
					writer.println("1\t"+dna+"\t"+sire+"\t"+dam+"\t1\t"+(dnaList.contains(dna)?"2":"0"));
				}
			}
			reader.close();
			writer.close();
	...
	public static void updateTrait(String dir, String traitfile) {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (!line[1].equals("2")&&!line[1].equals("1")&&!line[1].equals("0")) {
					System.err.println("Error - the value for '"+trav+"' ('"+line[1]+"') is not valid; should be either 0, 1, or 2");
				}
		...
		try {
			...
			while (reader.ready()) {
				line = reader.readLine().trim().split("[\\s]+");
				if (line[1].startsWith(TRAIT_STRAIN_PREFIX)||line[1].startsWith(BC_STRAIN_PREFIX)) {} else if (hash.containsKey(line[1])) {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				pedigreeData = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("geno=")) {
				genotypicData = args[i].split("=")[1];
				numArgs--;
		...
		try {
			if (option==0) {
				...
				System.out.println("Determination of how to proceed next is based on files present in "+(dir.equals("")?"current directory":dir));
				...
				if (!GinsburgDavid.finishedExploration(dir, GinsburgDavid.DEFAULT_EXPLORE_DIR, GinsburgDavid.DEFAULT_EXPLORE_FILE)) {
					GinsburgDavid.exploreModels(dir, GinsburgDavid.DEFAULT_EXPLORE_DIR, GinsburgDavid.DEFAULT_EXPLORE_FILE);
					System.out.println();
				} else {
