...
public class CARe_Analyses {
  ...
  public static final String[][] RACES =
      new String[][] {{"asians", "ASN", "Asian"}, {"blacks", "AAM", "Black"},
                      {"hispanics", "HIS", "Hispanic"}, {"whites", "CEU", "White"}};

  public static final String[][] DEMO_NEEDS =
      {{"FID"}, {"IID"}, {"CAReID"}, {"icam"}, {"ln_icam"}, {"Age"}, {"Male"}, {"BMI"}, {"Race"},
       {"rs5491"}, {"rs5491carrier"}};
  ...
  public static final String[][] COUNT_PARAMETERS =
      {{"# whites", "count", "icam", "Race=White", "-blank"},
       ...
       {"# whites / rs5491 carrier", "count", "icam", "icam=ValidDouble", "Race=White", "-blank",
        "rs5491=1"},
       {"# whites / rs5491 non-carrier", "count", "icam", "icam=ValidDouble", "Race=White",
        "-blank", "rs5491=0"},
       ...
       {"# blacks / rs5491 carrier", "count", "icam", "icam=ValidDouble", "Race=Black", "-blank",
        "rs5491=1"},
       {"# blacks / rs5491 non-carrier", "count", "icam", "icam=ValidDouble", "Race=Black",
        "-blank", "rs5491=0"},
       ...
       {"# Asians / rs5491 carrier", "count", "icam", "icam=ValidDouble", "Race=Asian", "-blank",
        "rs5491=1"},
       {"# Asians / rs5491 non-carrier", "count", "icam", "icam=ValidDouble", "Race=Asian",
        "-blank", "rs5491=0"},
       ...
       {"# Hispanics / rs5491 non-carrier", "count", "icam", "icam=ValidDouble", "Race=Hispanic",
        "-blank", "rs5491=0"},
       {"# Hispanics / carrier status unknown", "count", "icam", "icam=ValidDouble",
  ...
  public static final String[][] LEVEL_PARAMETERS =
      {{"mean ICAM levels in whites / rs5491 carrier", "mean", "icam", "icam=ValidDouble",
       ...
       {"mean ICAM levels in Hispanics / carrier status unknown", "mean", "icam",
        "icam=ValidDouble", "Race=Hispanic", "-blank", "rs5491=."},};
  ...
  public static final String[][] FINAL_PARAMETERS =
      {{"# whites with ICAM levels", "count", "icam", "icam=ValidDouble", "Race=White", "-blank",
       ...
       {"mean age in Hispanics", "mean", "Age", "icam=ValidDouble", "Race=Hispanic", "-blank",
        "sf=2", "rs5491=0", "-stdev"},
       {"% male in Hispanics", "mean", "Male", "icam=ValidDouble", "Race=Hispanic", "-blank",
        ...
        try {
          ...
          while (reader.ready()) {
            ...
            if (hash.contains(line[0])) {
              try {
                writer =
                    new PrintWriter(new FileWriter(DRIVE_ROOT + "Analyses/ICAM/" + line[0] + ".txt",
                                                   true));
            ...
            if (hash.contains(line[0])) {
              try {
                ...
                if (new File(DRIVE_ROOT + "Analyses/ICAM/" + line[0] + ".txt").length() == 0) {
                  ...
                  try {
                    PrintWriter writer2 =
                        new PrintWriter(new FileWriter(DRIVE_ROOT + "Analyses/ICAM/" + line[0]
                                                       + ".R"));
      ...
      for (String[] element : RACES) {
        ...
        if (new File(dir + "abo_icam.dosage").exists()) {
          ...
          if (STUDIES[i].equals("MESA")
            ...
            hitlist =
                Array.addStrToArray(DRIVE_ROOT + "Analyses/ICAM/IBC/whites/" + STUDIES[i]
                                    + "/pheno/" + element[3] + "QC/rs1799969.xln 1 4=rs1799969c",
                                    hitlist, 5);
          }
          ...
          CmdLine.run("java -cp C:/home/npankrat/" + org.genvisis.common.PSF.Java.GENVISIS
                      ...
                      dir);
          if (FAMILY_BASED[i]) {
            Files.writeList(new String[] {"lookup",
    ...
    for (String[] element : RACES) {
      ...
      for (String studie : studies) {
        ...
        if (new File(dir + root + ".chr1.mlinfo").exists()) {
          ...
          travPos =
              HashVec.loadFileToHashString(dir + root + ".pmap", new int[] {1}, new int[] {0, 3},
                                           false, "\t", false, false, false);
          keys = HashVec.getKeys(travPos, false, false);
          for (int k = 0; k < keys.length; k++) {
            if (!positions.containsKey(keys[k])) {
        ...
        for (String[] element2 : RACES) {
          ...
          if (new File(dir + phenofile).exists()) {
            commands = "/home/npankrat/bin/plink --noweb --fam leslie_lange." + element + ".IBC."
                       ...
                       + "_chr#";
            Files.writeList(Files.qsub(dir, "chr#_" + element, 1, 22, commands, null, 10000, 12,
                                       null),
                            dir + "master." + pheno);
