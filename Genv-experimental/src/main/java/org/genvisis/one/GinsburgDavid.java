...
public class GinsburgDavid {
  ...
  public static final double[] EXPLORE_HET =
  ...
  public static void exploreModels(String dir, String exploreDir, String exploreFile) {
    ...
    markerSet =
        new SnpMarkerSet(dir + "markers.dat", SnpMarkerSet.PLINK_MAP_FORMAT, false, new Logger());
    ...
    try {
      ...
      for (int model = 0; model < 2; model++) {
        ...
        for (int k = 0; k < markerNames.length; k++) {
          ...
          for (int i = 0; i < EXPLORE_PEN.length; i++) {
            for (int j = 0; j < EXPLORE_HET.length; j++) {
              array[Matrix.indexInArray(i, j, EXPLORE_HET.length)] =
                  Double.parseDouble(allResults[model][i][j][k][1]);
            }
          }
  ...
  public static void yossiPedigreeMaker(String phenoFile) {
    ...
    try {
      ...
      indices =
          ext.indexFactors(headerTargets, reader.readLine().trim().split("[\\s]+"), false, true);
