      ...
      for (GeneData geneData : geneDatas) {
        if (genes.contains(geneData.getGeneName())) {
          for (int j = 0; j < geneData.getExonBoundaries().length; j++) {
            ...
            out.append("\n" + exon.getChromosomeUCSC() + "\t" + exon.getStart() + "\t"
                       + exon.getStop() + "\tEXON_INDEX" + (j + 1) + "|" + geneData.getGeneName()
                       + "|" + exon.getUCSClocation());

          }
        }
      }
    }
    Files.write(out.toString(),
                "/Volumes/Beta/data/Cushings/QC/" + ArrayUtils.toStr(genes, "_") + ".exons.bed");
  }
}
