...
public class processAricExomeBetas {
  ...
  public static void main(String[] args) {
    ...
    String flipFile =
        "/home/pankrat2/shared/MitoPipeLineResources/betas/ExomeBetas/ExomeChipFlip.txt";

    Hashtable<String, String> flipLook =
        HashVec.loadFileToHashString(flipFile, 0, new int[] {1, 2}, "\t", false);
    ...
    for (int i = 0; i < betas.size(); i++) {
      ...
      try {
        ...
        // writer.println(Array.toStr(betaHeader) +
        // "\tMarkerName\tPos\trsID\trsRef\trsAlt\tmarkerA\tmarkerB\tconfig\tsiteType");
        ...
        while (reader.ready()) {
          ...
          if (!effAllele.contains("0")) {
            CONFIG config =
                StrandOps.determineStrandConfig(m.getMarkerAlleles(), effAllele.split("\t"));
            try {
          ...
          // writer.println(Array.toStr(line) + "\t" + m.getMarkerName() + "\t" + m.getPosMarker() +
          // "\t" + m.getRs() + "\t" + Array.toStr(m.getDbSnpAlleles()) + "\t" +
          // Array.toStr(m.getMarkerAlleles()) + "\t" + m.getConfig() + "\t" + m.getType());
