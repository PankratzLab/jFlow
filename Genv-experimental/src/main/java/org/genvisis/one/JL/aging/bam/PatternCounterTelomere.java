...
/**
 * primarily methods to mimic Telseq:
 * 
 * 
 * A issues noticed in TelSeq (that are repeated here):
 * 
 * 1. GC content calculation includes Ns -
 * https://github.com/zd1/telseq/blob/35019d2291c89162a34f322680a23d1a46a87990/
 * src/Telseq/telseq.cpp#L516-L527
 * 
 * 2. Telomeric reads are included as normalization reads
 * 
 * 3. Total reads is off by one -
 * https://github.com/zd1/telseq/blob/35019d2291c89162a34f322680a23d1a46a87990/
 * src/Telseq/telseq.cpp#L214-L215
 * 
 * 4. No "wiggle" for motif counting
 * 
 * 5.
 * 
 */
public class PatternCounterTelomere implements BamAnalysis {
	...
	/**
	 * @param pattern
	 *            a {@link PatternFilter} to count
	 * @param gcPatternRange
	 *            range of GC content to use for pattern matches
	 * @param gcBinNonTelomericRanges
	 *            ranges of GC content to use for non - pattern matches
	 * @param exclusionSet
	 *            reads overlapping any ({@link Segment#overlaps(Segment)}) will
	 *            not be used
	 * 
	 * @param baseFilters
	 *            All of these {@link SamRecordFilter}s will be applied to every
	 *            read
	 * @param nonCountFilters
	 *            will exclusively be applied to reads that pass the baseFilter,
	 *            but are not flagged by the {@link PatternFilter}
	 */
	public PatternCounterTelomere(PatternFilter pattern, GCRangeFilter gcPatternRange,
			List<SamRecordFilter> baseFilters, List<SamRecordFilter> nonCountFilters) {
		super();
	...
	/**
	 * @param minCount
	 *            minimum number of times for a pattern to be seen
	 * @return
	 */
	...
	/**
	 * @param telomericFlagStats
	 *            {@link FlagStats} generated for telomeric reads
	 * @param normalizationFlagStats
	 *            {@link FlagStats} generated for normalization reads
	 * @param genomeLengthAtTelGC
	 *            length of the genome at telomeric GC
	 * @param lengthUnit
	 *            bp length unit
	 * @param telomereEnds
	 *            number of telomere ends, 46 in humans
	 * @return
	 */
	private static double computeTelomereLength(FlagStats telomericFlagStats, FlagStats normalizationFlagStats,
			double genomeLengthAtTelGC, double lengthUnit, double telomereEnds) {
	...
	private static double computeTelomereLength(FlagStats telomericFlagStats, FlagStats normalizationFlagStats) {
		return computeTelomereLength(telomericFlagStats, normalizationFlagStats, GENOME_LENGTH_AT_TEL_GC, LENGTH_UNIT,
				TELOMERE_ENDS);
	}

	public static GCRangeFilter getDefaultGCTelRange() {
		return new GCRangeFilter(GC_TELOMERIC_UPPERBOUND, GC_TELOMERIC_LOWERBOUND, GC_COMP_METHOD.N_COUNTS_FOR_TOTAL);
	}
