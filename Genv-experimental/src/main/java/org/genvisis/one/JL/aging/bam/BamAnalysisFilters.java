...
public class BamAnalysisFilters {
	...
	/**
	 * @param minMapQ
	 *            minimum mapping quality to use
	 * @param log
	 * @return a list of filters that will capture appropriate mapping and
	 *         higher quality reads for normalization
	 */
	public static List<SamRecordFilter> getDefualtNormReadFilters(double minMapQ, Logger log) {
		List<SamRecordFilter> filters = new FilterNGS().getStandardSAMRecordFilters(SAM_FILTER_TYPE.GENOTYPE, log);
	...
	public static class PatternFilter implements SamRecordFilter {
		 ...
		 *            patterns to count
		 * @param minCount
		 *            minimum times pattern is seen (i.e. TelSeq requires 7
		 *            telomeric repeats).
		 */
	...
	public static class GCRangeFilter implements SamRecordFilter {
		...
		/**
		 * @param minGC
		 *            minimum GC content threshold (proportion)
		 * @param maxGC
		 *            maximum GC content threshold (proportion)
		 * @param gMethod
		 *            {@link GC_COMP_METHOD}
		 */
	...
	public static class RegionFilter<T extends Segment> implements SamRecordFilter {
		...
		@Override
		public boolean filterOut(SAMRecord record) {
			int[] olaps = exclusionSet.getOverlappingIndices(SamRecordOps.getReferenceSegmentForRecord(record, log));
