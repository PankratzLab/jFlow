  ...
  public void compare() {
    ...
    for (int i = 0; i < CNVariant.CONSENSUS_TYPE.values().length; i++) {
      ...
      for (int j = 0; j < CNVariant.OVERLAP_TYPE.values().length; j++) {
        ...
        HistogramComparison[] comparisons =
            new HistogramComparison[] {new HistogramComparison(), new HistogramComparison()};// with
                                                                                             // sig
                                                                                             // overlap,
                                                                                             // without
                                                                                             // sig
        ...
        if (cType == CONSENSUS_TYPE.CN_AWARE && otype == OVERLAP_TYPE.OVERLAP_LOC_AND_INDIVIDUAL) {
          ...
          LocusSet<CNVariant> hitSet =
              new LocusSet<CNVariant>(comparisons[0].getCnvsToStore(Sort.DESCENDING), false, log) {
          ...
          LocusSet<CNVariant> missSet =
              new LocusSet<CNVariant>(comparisons[1].getCnvsToStore(Sort.DESCENDING), false, log) {
