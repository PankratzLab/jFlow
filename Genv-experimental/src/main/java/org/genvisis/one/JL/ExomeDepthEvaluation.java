...
import org.pankratzlab.shared.filesys.CNVariant.CONSENSUS_TYPE;
import org.pankratzlab.shared.filesys.CNVariant.MatchResults;
import org.pankratzlab.shared.filesys.CNVariant.OVERLAP_TYPE;
...
public class ExomeDepthEvaluation {
    ...
    for (int i = 0; i < CNVariant.CONSENSUS_TYPE.values().length; i++) {
      ...
      for (int j = 0; j < CNVariant.OVERLAP_TYPE.values().length; j++) {
        ...
        if (cType == CONSENSUS_TYPE.CN_AWARE && otype == OVERLAP_TYPE.OVERLAP_LOC_AND_INDIVIDUAL) {
          ...
          LocusSet<CNVariant> missSet = new LocusSet<CNVariant>(comparisons[1].getCnvsToStore(true),
                                                                false, log) {
  ...
  private String plot(DynamicHistogram[] exomeDHits, DynamicHistogram[] exomeDMisses,
                      List<RScatter> rScatters, int i, boolean prop, CNVariant.CONSENSUS_TYPE cType,
