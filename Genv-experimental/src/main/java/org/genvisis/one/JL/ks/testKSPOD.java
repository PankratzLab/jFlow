...
public class testKSPOD {
	...
	static List<Segment> searchSpace = Arrays
			.asList(new Segment[] { new Segment("chrX:142000000-155000000"), new Segment("chrX:51000000-97000000"),
					new Segment("chrX:18000000-38000000"), new Segment("chrX:41000000-62000000"),
					new Segment("chrX:115000000-146000000"), new Segment("chrX:31000000-135000000") });
	...
	public static void main(String[] args) {
		...
		if (runMos) {
			...
			List<Segment> restricted = Arrays
					.asList(set.removeThese(new LocusSet<>(searchSpace, true, proj.getLog()), 0).getLoci());
	...
	private static void runKS(Project proj, MarkerDetailSet markerDetailSet, String ped, List<Segment> segments,
			TYPE type, SEARCH_SPACE_TYPE searchType) {
		...
		for (int[] pair : completePairs) {
			if (pair != null) {
				...
				for (PODResults poResults : results) {
					writer.println(poResults.toString() + "\t" + stat + "\t" + proj.PROJECT_NAME.getValue() + "\tTRUE");
				}
			}
		}
			...
			if (pair != null) {
				...
				if (map.containsKey(pair[0])) {
					...
					if (!completed.contains(offDNA)) {
						...
						String faDNA = null;
						String moDNA = null;
						if (pedigree.getFaDNAIndex(offIndex) > 0 && pedigree.getiDNA(map.get(pair[0])) == pedigree
								.getiDNA(pedigree.getIndexOfFaInIDs(offIndex))) {
							faDNA = pedigree.getiDNA(pedigree.getIndexOfFaInIDs(offIndex));
						}
						if (pedigree.getMoDNAIndex(offIndex) > 0 && pedigree.getiDNA(map.get(pair[0])) == pedigree
								.getiDNA(pedigree.getIndexOfMoInIDs(offIndex))) {
