...
public class MarkerBlastIterator {
  ...
  public static void blastIter(Project proj, String fileSeq, FILE_SEQUENCE_TYPE type,
    ...
    MarkerBlastResult[] results =
        new MarkerBlastResult[blastWordSizes.length * reportWordSizes.length];
    int index = 0;
  ...
  public static void blastIter(Project proj, String fileSeq, FILE_SEQUENCE_TYPE type,
    ...
    for (MarkerBlastResult result : results) {
      ...
      String notOneHittersOut =
          ext.addToRoot(result.getOutput(), ".NOToneHitWonders_" + oneHitWonderDef);
      String notAppropriateMatchOut =
          ext.addToRoot(result.getOutput(), ".NOTAppropriateMatch_" + oneHitWonderDef);
      ...
      try {
        ...
        for (int j = 0; j < dHistograms.length; j++) {// for each blast metric
          ...
          for (int k = 0; k < dHistograms[j].length; k++) {
            String plotRoot =
                ext.rootOf(allPlotFiles[j], false) + "_" + parser.getNumericDataTitles()[k];
