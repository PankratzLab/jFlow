...
public class ControlSelection {
    ...
    for (int i = 500; i < SAMPLE_SIZE; i += 50) {
      ...
      for (String phenoFile : phenoFiles) {
        ...
        QQPlot qqPlot = QQPlot.loadPvals(ArrayUtils.toStringArray(qqFiles), "Hi", false, true,
                                         false, -1, -1, false, Float.MAX_VALUE, new Logger());
  ...
  private static void runit(ArrayList<String> lambdaReport, int i, String rootOut, String mds,
                            String plinkQc, Hashtable<String, String> hash,
    ...
    Files.writeIterable(centers.get(cent1).subList(0, min - 1),
                        antiOptimalDir + centComp + ".anchors.txt");
    ...
    run(plinkQc, mds, hash, qqFiles, centComp, matchDirRandom, "random", cases,
        ArrayUtils.toStringArray(sub), lambdaReport, i, pheno);
  }
  ...
  private static void run(String plinkQc, String mds, Hashtable<String, String> hash,
    ...
    for (String sample : ins) {
      // System.out.println(sample.split(PSF.Regex.GREEDY_WHITESPACE)[0]);
      ...
      if (cont.contains(sample.split(PSF.Regex.GREEDY_WHITESPACE)[0])) {
        String[] c = hash.get(sample.split(PSF.Regex.GREEDY_WHITESPACE)[0]).split("\t");
        c[5] = "1";
        newFam.add(ArrayUtils.toStr(ArrayUtils.subArray(c, 0, 6)));
      }
