...
public class plotContam {
  ...
  public static void main(String[] args) {
    String[] files =
        new String[] {"D:/data/Project_Tsai_21_25_26_spector/QC/bamContam/fullBamContam.txt",
    ...
    for (String file : files) {
      ...
      String[] sampsToPlot = HashVec.loadFileToStringArray(
          "D:/data/Project_Tsai_21_25_26_spector/QC/bamContam/CushingSamps.txt", false,
          new int[] {0}, true);
      ...
      try {
        ...
        writer.println(
            all[0][0] + "\t" + Array.toStr(Array.subArray(all[0], indices)) + "\tPopulationMedian");
        for (int j = 1; j < all.length; j++) {
          double[] counts = Array.toDoubleArray(Array.subArray(all[j], indices));
          double median = Array.median(counts);
          writer.println(
              all[j][0] + "\t" + Array.toStr(Array.subArray(all[j], indices)) + "\t" + median);
        }
        writer.close();
      } catch (Exception e) {
      ...
      for (int j = 1; j < samps.length; j++) {
        ...
        if (index >= 0) {
          ...
          double[] data = Array.toDoubleArray(HashVec.loadFileToStringArray(withMedian, true,
              new int[] {ext.indexOfStr(samps[j], newheader)}, false));
          double kurt = Array.kurtosis(data);
          double skew = Array.skewness(data);
          RScatter rscScatter = new RScatter(withMedian, root + ".rscript",
              ext.removeDirectoryInfo(root), root + ".jpeg", "PROP_REF_BIN",
              new String[] {samps[j], "PopulationMedian"}, SCATTER_TYPE.POINT, new Logger());
          rscScatter.setTitle(
