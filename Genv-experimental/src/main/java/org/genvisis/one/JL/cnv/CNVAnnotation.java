...
/**
 * Methods to annotate CNVariants with features of interest (Genes overlapped,
 * mappability, etc)
 */
public class CNVAnnotation {
	 ...
	 *            number of threads
	 */
	public static void annotate(Project proj, String cnvFile, String[] freqCnvFiles, String problemRegionsFile,
			String outDir, double maxProblematicOverlap, int threads) {
		...
		for (String freqCnvFile : freqCnvFiles) {
			...
			proj.getLog().reportTimeInfo(
					"Loaded " + freqs.getLoci().length + " cnvs for frequency annotation from " + freqCnvFile);
		}

		Map<String, LocusSet<CNVariant>> set = CNVariant.breakIntoInds(all, proj.getLog());
		proj.getLog().reportTimeInfo(
				"Loaded " + all.getLoci().length + " cnvs across " + set.size() + " individuals to annotate");
		...
		WESMappabilityAnnotator wesMappabilityAnnotator = WESMappabilityAnnotator.getDefaultAnnotator(geneAnnotator,
				proj.GENOME_BUILD_VERSION.getValue(), proj.getLog());
		LocusSet<Segment> probs = new LocusSet<>(Segment.loadUCSCregions(problemRegionsFile, 0, false, proj.getLog()),
				true, proj.getLog());
		...
		for (String sample : proj.getSamples()) {
			...
			if (set.containsKey(key)) {
				...
				if (samp != null) {
					beastScore = new BeastScore(samp.getLRRs(), mds.getIndicesByChr(), cnvIndices, proj.getLog());
				...
				for (int i = 0; i < cnvIndices.length; i++) {
					...
					if (beastScore != null) {
						segmentAnotation.getAttributes().put(SegmentAnnotationKeys.BEAST.toString(),
		...
		new LocusSet<>(annotateds, true, proj.getLog()).writeRegions(
				outDir + ext.rootOf(cnvFile) + "BEAST.annotated.cnvs", TO_STRING_TYPE.REGULAR, true, proj.getLog());
	...
	public static void main(String[] args) {
		...
		c.addArgWithDefault(MAX_PROBLEMATIC_OVERLAP,
				"if a cnv is covered by a problematic regions more than this, flag it", "0.5");

		c.parseWithExit(args);
		annotate(new Project(c.get(CLI.ARG_PROJ)), c.get(CNV_FILE), c.get(FREQ_CNV).split(","), c.get(PROBLEM_REGIONS),
				c.get(CLI.ARG_OUTDIR), c.getD(MAX_PROBLEMATIC_OVERLAP), c.getI(CLI.ARG_THREADS));
	}
