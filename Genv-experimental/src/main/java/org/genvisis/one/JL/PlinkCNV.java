...
public class PlinkCNV {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      for (String sampFile : sampFiles) {
        run(cnvFile, sampFile, build);
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  private static void run(String cnvaFile, String sampFile, final String build) {
    ...
    GeneTrack geneTrack = GeneTrack.load("N:/statgen/NCBI/RefSeq_" + build + ".gtrack", false);
    if (Files.exists(dir + "mitoCarta.txt")) {
      String[] mitos =
          HashVec.loadFileToStringArray(dir + "mitoCarta.txt", false, new int[] {0}, true);
      ArrayList<String> mitoList = new ArrayList<String>();
      int count = 0;
      for (String mito : mitos) {
        if (geneTrack.lookupAllGeneData(mito) != null
            && geneTrack.lookupAllGeneData(mito).length > 0) {
          count++;
    ...
    for (String filter : filters) {// for each filter crf found
      ...
      if (!Files.exists(out)) {
        FilterCalls.fromParameters(dir + filter, new Logger());
      }
      ...
      if (!Files.exists(sampCNVs) || !Files.exists(geneSampCnvs)) {
        ...
        try {
          ...
          // CNVariant cheater = new
          // CNVariant("ALOUD_p_ARIC_batch15_005_affy_GenomeWideSNP_6_E09_236930.CEL.IND.txt",
          // "ALOUD_p_ARIC_batch15_005_affy_GenomeWideSNP_6_E09_236930.CEL.IND.txt", (byte) 6,
          // 155662711, 155675684, 1, 24.5930, 9, -99);
          //
      ...
      for (int i = 3; i < phenos[0].length; i++) { // test each phenotype
        ...
        final String key = phenos[0][i] + "_" + ext.rootOf(sampFile) + "_" + ext.rootOf(filter);
        ...
        if (quant) {
          Callable<PlinkResult> c1 = new Callable<PlinkResult>() {
            ...
            public PlinkResult call() throws Exception {
              ...
              boolean complete = CmdLine.runCommandWithFileChecks(Array.toStringArray(cmd), "",
                  null, new String[] {out}, true, oWrite, true, log);
          ...
          Callable<PlinkResult> c4 = new Callable<PlinkResult>() {
            ...
            public PlinkResult call() throws Exception {
              ...
              String[][] dataS =
                  HashVec.loadFileToStringMatrix(out, true, cols, "[\\s]+", false, 1000, true);
          ...
          Callable<PlinkResult> c5 = new Callable<PlinkResult>() {
            ...
            public PlinkResult call() throws Exception {
              ...
              String[][] dataS =
                  HashVec.loadFileToStringMatrix(load, true, cols, "[\\s]+", false, 1000, true);
              double[][] data = Array.toDoubleArrays(dataS, true);
              double[] phe = Matrix.extractColumn(data, 0);
              for (int j = 1; j < cols.length; j++) {
                double[] test = Matrix.extractColumn(data, j);
              ...
              Files.writeArrayList(summary,
                  opDir + pheno + "_mito_tradBurden_count.cnv.indiv.sigs");
          ...
          Callable<PlinkResult> c1 = new Callable<PlinkResult>() {
            ...
            public PlinkResult call() throws Exception {
              ...
              boolean complete = CmdLine.runCommandWithFileChecks(Array.toStringArray(cmd), "",
                  null, new String[] {out}, true, false, true, log);
    ...
    combo.add(
        "Type\tCHR\tSNP\tEMP1\tEMP2\tUCSC\tGENE\tGENE_5k\tLink_buffer\tAFF\tUNAFF\tNCNV\tM0\tM1\tCMD");
    ...
    for (PlinkResult result : results) {
      if (result.complete) {
        ...
        String[][] data =
            HashVec.loadFileToStringMatrix(result.file, false, null, "[\\s]+", false, 1000, true);
