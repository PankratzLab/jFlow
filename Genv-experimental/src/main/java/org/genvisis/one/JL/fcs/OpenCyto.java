...
public class OpenCyto {
  ...
  private static String prepRscript(CLI c, String batchFile, Logger log) {
    ...
    for (int i = 0; i < rscript.length; i++) {
      if (rscript[i].startsWith("inputDir = ")) {
        ...
        rscript[i] = "fcsFilesAll ="
                     + Rscript.generateRVector(ArrayUtils.toStringArray(toUse), true);
  ...
  private static void run(CLI c) {
    ...
    CmdLine.runCommandWithFileChecks(cmd, "", necessaryInputFiles, expectedOutputFiles, true, true,
                                     false, false, log);
  ...
  private static String[] getInput(String input, Logger log) {
    if (Files.isDirectory(input)) {
      log.reportTimeInfo("Assuming " + input
                         + " is a directory, finding all files with .fcs extension");
  ...
  public static void main(String[] args) {
    ...
    c.addArgWithDefault(R_SOURCE, "full path to R source file for running openCyto",
                        "~/src/openCytoSrc.R");
    c.addArgWithDefault(INPUT_FCS,
                        "directory or complete path to a file containing .fcs input of interest",
                        "~/fcs/");
    ...
    c.addArgWithDefault(TEMPLATE_LYMPH, "full path to lymphocyte gating .csv template",
                        "~/templates/lymph.csv");
    c.addArgWithDefault(TEMPLATE_MONOCYTE, "full path to monocyte gating .csv template",
                        "~/templates/mono.csv");
