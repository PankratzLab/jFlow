...
public class SkatMtDNA {
	...
	private static String filterMaf(String inputVCF, double maf, String outDir, Logger log) {
		...
		filter.append("vcftools --vcf " + inputVCF + " --max-maf  " + maf
									+ " --recode --recode-INFO-all --out " + root + "\n");
	...
	private static String filterCR(String inputVCF, double cr, String outDir, String pseqDir,
																 Logger log) {
	...
	private static String[] convertToPlink(String inputVCF, String outDir, Logger log) {
		...
		if (!Files.exists("", outFiles)) {
			String[] plinkCommand = new String[] {"plink2", "--double-id", "--vcf", inputVCF, "--no-fid",
																						"--no-parents", "--no-sex", "--no-pheno", "--out",
																						rootOut};
			if (CmdLine.runCommandWithFileChecks(plinkCommand, "", new String[] {inputVCF}, outFiles,
																					 true, true, false, log)) {
	...
	private static void runSkat(String[] plinks, SNPInfo snInfo, String hapFile, Logger log) {
		...
		skatBuilder.append("fam =Read_Plink_FAM_Cov('" + plinks[2] + "','" + hapFile
											 + "', Is.binary=TRUE)\n");
		skatBuilder.append("SSD.info <- Open_SSD('" + ssd + "', '" + info + "')\n");
		skatBuilder.append("Null_Model <- SKAT_Null_Model(formula = fam$Phenotype ~  fam$HAPH+fam$HAPL, out_type=\"D\")\n");
		skatBuilder.append("All_SKAT_Data  <- SKATBinary.SSD.All(SSD.INFO = SSD.info, obj = Null_Model) \n");
		skatBuilder.append("All_SKAT_Data$results$P.value.bonf =p.adjust(All_SKAT_Data$results$P.value, method = \"bonferroni\", n = length(All_SKAT_Data$results$P.value))\n");
	...
	private static void runType(String pseqDir, VARIANT_TYPES vtTypes, VARIANT_SETS vSets,
															String outDir, String cases, String controls, Logger log,
															VcfPopulation vpop, String hqVcf, String haps) {
