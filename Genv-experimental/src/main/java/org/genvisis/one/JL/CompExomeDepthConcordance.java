...
public class CompExomeDepthConcordance {
  ...
  public static void comp(String[] files, Hashtable<String, String> iidMatch, String mapFile,
    ...
    for (int i = 0; i < sets.size(); i++) {
      ...
      // System.exit(1);
      LocusSet<CNVariant> matchSet =
          new LocusSet<CNVariant>(match.toArray(new CNVariant[match.size()]), true, log) {
      ...
      LocusSet<CNVariant> noMatchSet =
          new LocusSet<CNVariant>(noMatch.toArray(new CNVariant[noMatch.size()]), true, log) {
  ...
  private static void dumpAndSummarize(LocusSet<CNVariant> out, String outFile,
    ...
    LocusSet<Segment> pSet =
        LocusSet.loadSegmentSetFromFile(problematicRegionFIle, 0, 1, 2, 0, true, true, 0, log);
