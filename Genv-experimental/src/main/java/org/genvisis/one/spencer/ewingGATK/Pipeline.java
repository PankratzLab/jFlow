...
public class Pipeline {
	...
	private void runHaplotypeCallerItasca() {
		...
		Set<String> itascaIDs = HashVec.loadFileToHashSet(BASE_DIR + "bams/itascaIDs",
																											false);
			...
			if (write) {
				...
				if (cmdCount++ > 400) {
					Files.writeIterable(commands,
															BASE_DIR + "pbsScripts/splitGVCFJobs_"
																				+ commandScriptCount);
					commands.clear();
					commandScriptCount++;
					cmdCount = 0;
	...
	private void runJointGenotyping() {
		...
		for (int i = 1; i < 26; i++) {
			...
			String chrGVCFs = BASE_DIR + "chr" + chr
												+ "_gVCFs.list";

			Files.writeIterable(ids.stream()
														 .map(s -> BASE_DIR + "gVCFs_byChr/"
	...
	private void runCombinedGVCFJointGenotyping() {
		...
		for (int i = 1; i < 26; i++) {
			...
			if (!Files.checkAllFiles("", combinedGVCFjointVCF(chr), combinedGVCFjointVCF(chr))) {
				...
				cmd.add("\t-o " + combinedGVCFjointVCF(chr) + " \\")
					 .add("\t-L " + chr);
				cmd.add("");
				if (ready)
					Qsub.qsubGb(combinedGVCFjointGenoPBS(chr), cmd.toString(), 20, 288, threads);
			}
		}
	...
	private void runVQSR() {
		GATK gatk = new GATK("/home/pankrat2/public/bin/GATK_3.7/", REF_GENO, null,
												 SEQ_TARGET.GENOME, null, 250000, null, null, true, false,
												 new Logger("VQSR.log"));
	...
	private String jointVCF(String chr) {
		return BASE_DIR + "vcfs/ES_JointGenotypes_chr"
	...
	private String combinedGVCFjointVCF(String chr) {
		return BASE_DIR + "vcfs/ES_JointGenotypes_chr"
					 + chr + "_combinedGVCFs.vcf.gz";
	}

	private String combinedGVCFjointVCFExtraAnno(String chr) {
		return BASE_DIR + "vcfs/ES_JointGenotypes_chr"
					 + chr + "_combinedGVCFs_withPED.vcf.gz";
	}
