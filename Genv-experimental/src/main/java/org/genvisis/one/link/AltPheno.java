...
public class AltPheno {
  ...
  public static void rerunAnalyses(String dir, String filename) {
        ...
        try {
          ...
          while (reader.ready()) {
            ...
            if (i == fams.length || line[0].equals(fams[i])) {
              phenos = hash.get(line[0]).containsKey(
                                                     line[1]) ? hash.get(line[0]).get(line[1]).split(PSF.Regex.GREEDY_WHITESPACE) : ArrayUtils.stringArray(phenoNames.length, "0");
              for (int j = 0; j < phenoNames.length; j++) {
    ...
    classpath = "-classpath /home/npankrat/" + org.pankratzlab.common.PSF.Java.GENVISIS + "";
    if (new File(".").getAbsolutePath().contains("bc2/pankratz")) {
      classpath = "-classpath /home/bc2/pankratz/" + org.pankratzlab.common.PSF.Java.GENVISIS + "";
    }
    try {
      writers = new PrintWriter[phenoNames.length];
      for (int j = 0; j < phenoNames.length; j++) {
