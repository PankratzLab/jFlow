...
import org.genvisis.gwas.Metasoft;
...
public class Mega_Analyses {
  ...
  public static void findDuplicateMarkers() {
    ...
    for (int i = 0; i < STUDIES.length; i++) {
      ...
      try {
        ...
        allFilteredFiles[i] =
            DIR + "duplicatedMarkers/filtered/" + STUDIES[i][0] + "_duplicatedSNPs.txt";
      } catch (Exception e) {
  ...
  private static void purifyDuplicateMarkers(String filename, int indexOfDuplicatedFactor,
    ...
    mostExtremeTie =
        minimizeInsteadofMaximize ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY;
    hash = new Hashtable<String, String>();
