...
public final class CFCSListModeData extends CFCSAbstractData implements CFCSErrorCodes {
  ...
  public final byte[][][] cinchBytes(final byte[] bytes) {
    ...
    for (int event = 0, position = 0; event < nEvents; event++) {
      for (int parameter = 0, used = 0; parameter < nParameters; parameter++, position += used) {
        if (variable) size = sizes[parameter];
        ...
        if (size == CFCSParameter.FIELDSIZE_FREE_FORMAT) {
          ...
          System.arraycopy(bytes, position, cinched[event][parameter] = new byte[size], 0,
                           used = size);
        }
      }
    }
  ...
  public boolean isLoaded() {
    return true;// loadCount == 2;
  }

  public final void setBytes(final byte[] bytes) {
    super.setBytes(bytes);

    ((ArrayList<Object>) (((CFCSAbstractDatatype) datatype).data)).ensureCapacity((int) (getCount()
                                                                                         * 1.76));
  ...
  protected final byte[] packBytes(final byte[] bytes) {
    ...
    try {
      ...
      if (valid > 0) stream.write((int) (accumulator << (BITSPERBYTE - valid)));
  ...
  public final byte[] unpackBytes(final byte[] bytes) {
    ...
    for (int i = 0; i < swap.length; i++) {
      if ((swapped = (swap[i] != JAVA_BYTESWAP_ARRAY[i])) == true) break;
    }

    if (swapped) return unpackSwappedBytes(bytes);
  ...
  public final void addEvent(final byte[] array) {
    ...
    // JS: Moved into the try section, otherwise the count gets added multiple times if exception
    // occurs
    // addCount(1);
  }
