...
public final class CFCSParameter extends CFCSAbstractParameter {
  ...
  public enum PreferredDisplayScale {
    Linear, Logarithmic, Undefined
  };
  ...
  public final String getFieldSizeString() {
    if (fieldSize == FIELDSIZE_FREE_FORMAT) return FIELDSIZE_FREE_FORMAT_STRING;
  ...
  public final int scaleToChannel(double scale) {
    ...
    if (decades > 0.0) { // logarithmic
      double offset = getOffset();

      if (offset > 0.0) scale /= offset;
  ...
  public final double channelToScale(final int channel) {
    ...
    if (decades > 0.0) { // logarithmic
      ...
      if (offset > 0.0) return (scale * offset);
      /* else */ return scale;
    } else { // linear
