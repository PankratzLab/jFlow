...
public final class CFCSDataSet implements CFCSErrorCodes {
  ...
  CFCSDataSet(final CFCSSystem.SentientInputStream stream) {
    ...
    try {
      ...
      if (getVersionIntegerValue() < 2) { // Need spec for FCS1.0
      ...
      for (int i = TEXT; i < OTHER_START; i++) {
        ...
        if (begin > 0L && begin < first) first = begin;
      }
      ...
      for (int i = OTHER_START;; i++) { // Private segments
        ...
        // a valid one.
        /*
         * if ((begin = readOffset(stream)) == 0L) break; if ((end = readOffset(stream)) == 0L)
         * break;
         */
      ...
      if (getVersionIntegerValue() >= 3) { // Process the oversize (> 100MB) segments
        headers.clear();

        segments.set(TEXT, null); // Clear space for STEXT

        for (int i = TEXT; i < OTHER_START; i++) { // Well-known segments
          CFCSKeyword keyword = keywords.getKeyword(CFCSKeywords.SEGMENT_BEGIN_PREFIX
                                                    + CFCSKeywords.SEGMENT_ROOTS[i]);
          long begin = keyword.getKeywordLongValue();

          keyword = keywords.getKeyword(CFCSKeywords.SEGMENT_END_PREFIX
                                        + CFCSKeywords.SEGMENT_ROOTS[i]);
        ...
        if ((checksum != CHECKSUM_DISABLED)
            && (checksum != crc16)) throw new CFCSError(CFCSBadFCS, "Malformed checksum, read in file: " + checksum + ", computed: " + crc16);
      }

      if (segments.get(ANALYSIS) != null) addKeywords(ANALYSIS);
  ...
  final void writeDataSet(final CFCSSystem.SentientOutputStream stream, final long another) {
    ...
    for (int i = HEADER, size = segments.size(); i < size; i++) { // all segments
      ...
      if ((i > HEADER && i < OTHER_START) || i == stext) {
        ...
        CFCSKeyword keyword = keywords.getKeyword(CFCSKeywords.SEGMENT_BEGIN_PREFIX
                                                  + CFCSKeywords.SEGMENT_ROOTS[index]);
    ...
    segments.set(TEXT,
                 (stext == UNDEFINED) ? keywords.getBytes(TEXT) : keywords.getTextBytes(true));
    ...
    try {
      ...
      if (another > -1L) stream.skip(next - stream.getFilePointer()); // optional but nice
    } catch (IOException exception) {
      throw new CFCSError(CFCSNetworkIOError, exception);
    }
  }
  ...
  private byte[] generateHeader(final List<long[]> headers, final int stext) {
    ...
    for (int i = TEXT; i < size; i++) {
      if (i == stext) continue; // Only appears in TEXT, not HEADER

      long[] header = {i, 0L, 0L};

      if (i < headers.size()) header = (long[]) headers.get(i);

      if (header[BEGIN] > HEADER_OFFSET_LIMIT || header[END] > HEADER_OFFSET_LIMIT) {
        if (i >= OTHER_START) continue;
        // Can't have OTHER segments beyond 100MB limit
  ...
  private void readSegments(final List<long[]> headers,
                            final CFCSSystem.SentientInputStream stream) throws IOException {
    ...
    if (bReadSegmentsFirstTime && (headers.size() > 0)) {
      ...
      for (int i = 0; i < headers.size(); i++) {
        ...
        if (header[END] == 0) continue; // Doesn't exist
    ...
    for (int i = 0; i < headers.size(); i++) {
      ...
      long offset = stream.getFilePointer();
      if (header[BEGIN] < offset) continue; // Already read this one

      if (offset < header[BEGIN]) stream.skip(header[BEGIN] - offset);

      long size = (header[END] - header[BEGIN]) + 1;

      if (size > Integer.MAX_VALUE) throw new CFCSError(CFCSNotImplemented,
                                                        "Segment larger than 2G");
  ...
  private void writeSegments(final List<long[]> headers,
                             final CFCSSystem.SentientOutputStream stream) throws IOException {
    Collections.sort(headers, comparator);

    for (int i = 0; i < headers.size(); i++) {
      long[] header = (long[]) headers.get(i);

      if (header[END] == 0L) continue; // Doesn't exist?

      long offset = stream.getFilePointer();
      if (header[BEGIN] < offset) continue; // Already wrote this one?
      if (offset < header[BEGIN]) stream.skip(header[BEGIN] - offset);
  ...
  private static void writeSegment(final OutputStream stream,
                                   final byte[] segment) throws IOException {
  ...
  private static int readFileChecksum(final InputStream stream) throws IOException {
    ...
    if (string.equals(DISABLED_CHECKSUM)) return -1;
  ...
  private static void writeFileChecksum(final OutputStream stream,
                                        final String checksum) throws IOException {
