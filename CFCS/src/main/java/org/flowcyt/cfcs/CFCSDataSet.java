...
public final class CFCSDataSet implements CFCSErrorCodes {
    ...
    try {
      ...
      if (getVersionIntegerValue() >= 3) { // Process the oversize (> 100MB) segments
        ...
        for (int i = TEXT; i < OTHER_START; i++) { // Well-known segments
          ...
          keyword =
              keywords.getKeyword(CFCSKeywords.SEGMENT_END_PREFIX + CFCSKeywords.SEGMENT_ROOTS[i]);
        ...
        if ((checksum != CHECKSUM_DISABLED) && (checksum != crc16))
          throw new CFCSError(CFCSBadFCS, "Malformed checksum, read in file: " + checksum
                                          + ", computed: " + crc16);
      }
    ...
    for (int i = 0; i < headers.size(); i++) {
      ...
      if (header[BEGIN] == 0L && header[END] == 0L) {
        ...
        for (int j = 0; j < prefixes.length; j++) {
          ...
          if (keyword != null)
            header[j + 1] = keyword.getKeywordLongValue();
        }
   ...
   * sun.misc.CRC16(); for (int i = 0; i < bytes.length; i++) { crc.update(bytes[i]); }
   * 
   * return crc.value; }
   */
