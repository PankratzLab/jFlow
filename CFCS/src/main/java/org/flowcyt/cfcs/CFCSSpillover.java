...
public class CFCSSpillover {
  ...
  CFCSSpillover(String spilloverKeywordValue) throws CFCSError {
    ...
    if (sTmp.length < 2)
      throwCFCSIllegalValueError(spilloverKeywordValue);
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        try {
          spilloverCoefficients[i][j] = Double.parseDouble(sTmp[1 + n + n * i + j]);
        } catch (Exception e) {
          throwCFCSIllegalValueError(spilloverKeywordValue);
        }
      }
  ...
  public final double[][] getSpilloverCoefficients() {
    ...
    for (int i = 0; i < n; i++) {
      System.arraycopy(spilloverCoefficients[i], 0, values[i], 0, n);
    }
    return values;
  }
  ...
  public final int getParameterIndex(String parameterName) {
    int n = parameterNames.length;
    for (int i = 0; i < n; i++) {
      if (parameterNames[i].compareToIgnoreCase(parameterName) == 0)
        return i;
    }
    return -1;
  }
  ...
  public final double getSpilloverCoefficient(String fromParameterName, String toParameterName) {
    ...
    for (int x = 0; x < n; x++) {
      if (parameterNames[x].compareToIgnoreCase(fromParameterName) == 0)
        i = x;
      if (parameterNames[x].compareToIgnoreCase(toParameterName) == 0)
        j = x;
    }
  ...
  /**
   * Set a spillover coefficient, return true if succeeded, false otherwise (e.g., index out of
   * range)
   * 
  ...
  public final boolean addSpilloverParameter(String parameterName, double[] spilloverFromValues,
                                             double[] spilloverToValues) {
    int n = parameterNames.length;
    if (spilloverFromValues == null || spilloverFromValues.length != n + 1
        || spilloverToValues == null || spilloverToValues.length != n + 1)
      return false;
    for (int i = 0; i < n; i++)
      if (parameterNames[i].compareToIgnoreCase(parameterName) == 0)
        return false;
  ...
  public static CFCSSpillover createSpilloverMatrix(String[] parameterNames,
                                                    double[][] spilloverCoefficients) throws CFCSError {
  ...
  public static CFCSSpillover createSpilloverMatrix(CFCSSpillover sourceMatrix) throws CFCSError {
    return createSpilloverMatrix(sourceMatrix.getParameterNames(),
                                 sourceMatrix.getSpilloverCoefficients());
  }

  private CFCSSpillover(String[] parameterNames,
                        double[][] spilloverCoefficients) throws CFCSError {
    int n = parameterNames.length;
    if (spilloverCoefficients.length != n)
      throwCFCSIllegalValueError("");
    for (int i = 0; i < n; i++)
      if (spilloverCoefficients[i].length != n)
        throwCFCSIllegalValueError("");
    ...
    for (int i = 0; i < n; i++) {
      System.arraycopy(spilloverCoefficients[i], 0, this.spilloverCoefficients[i], 0, n);
    }
  }
