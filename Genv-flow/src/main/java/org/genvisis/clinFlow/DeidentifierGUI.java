...
public class DeidentifierGUI extends JDialog {
  ...
  private void setupContentPanel() {
    ...
    final Border b1 =
        BorderFactory.createCompoundBorder(
            BorderFactory.createLineBorder(Color.gray), new EmptyBorder(2, 10, 1, 10));
    final Border b2 =
        BorderFactory.createCompoundBorder(
            BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(
                    new Color(
                        Color.gray.getRed(), Color.gray.getGreen(), Color.gray.getBlue(), 100),
                    1),
                BorderFactory.createCompoundBorder(
    ...
    selectInBtn.addActionListener(
        (e) -> {
          selectIn();
        });
    selectInBtn.addMouseListener(
        new MouseAdapter() {
    ...
    selectOutBtn.addActionListener(
        (e) -> {
          selectOut();
        });
    selectOutBtn.setBorder(b1);
    ...
    selectLinkBtn.addMouseListener(
        new MouseAdapter() {
    ...
    selectLinkBtn.addActionListener(
        (e) -> {
          selectLink();
        });
    selectLinkBtn.setBorder(b1);
  ...
  private void selectOut() {
    FileChooserPkl fc =
        new FileChooserPkl(this, "", false, true, "Select Output Directory", new Logger());
  ...
  private void selectIn() {
    FileChooserPkl fc =
        new FileChooserPkl(this, "", false, true, "Select Source Directory", new Logger());
    if (!fc.isSelected()) return;
    final String dir = fc.getSelectedFile().getAbsolutePath();
  ...
  private void selectLink() {
    FileChooserPkl fc =
        new FileChooserPkl(this, "", false, true, "Select Link File Directory", new Logger());
    if (!fc.isSelected()) return;
    final String dir = fc.getSelectedFile().getAbsolutePath();
  ...
  private boolean checkAndWarn() {
    ...
    if (in.isEmpty()) {
      JOptionPane.showMessageDialog(
          null, "Error - missing source directory", "Error", JOptionPane.ERROR_MESSAGE);
      return false;
    }
    if (!Files.exists(ext.verifyDirFormat(in))) {
      JOptionPane.showMessageDialog(
          null, "Error - source directory invalid / not found", "Error", JOptionPane.ERROR_MESSAGE);
      return false;
    }

    if (out.isEmpty()) {
      JOptionPane.showMessageDialog(
          null, "Error - missing destination directory", "Error", JOptionPane.ERROR_MESSAGE);
      return false;
    }
    if (!Files.exists(ext.verifyDirFormat(out))) {
      JOptionPane.showMessageDialog(
          null,
          "Error - destination directory invalid / not found",
          "Error",
    ...
    if (!link.isEmpty() && !Files.exists(ext.verifyDirFormat(link))) {
      JOptionPane.showMessageDialog(
          null,
          "Error - link file directory invalid / not found",
          "Error",
          JOptionPane.ERROR_MESSAGE);
      return false;
    }
  ...
  private void run() {
    ...
    String out = rootOutFld.getText();
    Deidentifier deid =
        new Deidentifier(in, out, linkFld.getText().isEmpty() ? out : linkFld.getText());
