...
public class FCSProcessingPipeline {
  ...
  private void run(PIPELINE pipeToRun, List<Panel> panels) throws IOException {
    ...
    switch (pipeToRun) {
      case BOOL:
        pf =
            new ProcessorFactory<SampleProcessor>() {
              ...
              public SampleProcessor createProcessor(Object owner, int index) {
                return new InclusionProcessor(
                    outDir, ovvrDir, ovvrSuff, ovvrMatch, dimensionOverrideFile);
              }
            };
        break;
      case VIZ:
      ...
      case PCTS_CNTS:
        pf =
            new PercentageAndCountWriterFactory(
                outDir, ovvrDir, ovvrSuff, ovvrMatch, dimensionOverrideFile);
        break;
      default:
        System.err.println(
            "Error - pipeline option "
                + pipeToRun
    ...
    SamplingPipeline sp =
        new SamplingPipeline(
            ...
            outDir,
            panels,
            new String[] {highPrioFile, lowPrioFile},
            pf);
  ...
  public static void main(String[] args) throws IOException {
    ...
    cli.addArg(
        "fcs", "Directory with FCS files (finds all FCS files, including in subdirectories)", true);
    cli.addArg(
        "wsp", "Directory with WSP files (finds all WSP files, including in subdirectories)", true);
    cli.addArg("out", "Output directory", true);
    cli.addArg(
        "pipe",
        "Pipeline to run, one of "
            + Arrays.stream(PIPELINE.values())
                .map(PIPELINE::name)
    ...
    String dimensionOverrideFile =
        cli.has("dimensionOverrides") ? cli.get("dimensionOverrides") : null;
    ...
    String gateOverrideMatch =
        cli.has("gateOverrideMatchup") ? cli.get("gateOverrideMatchup") : null;
