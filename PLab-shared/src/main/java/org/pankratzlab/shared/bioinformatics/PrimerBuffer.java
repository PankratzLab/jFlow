...
public class PrimerBuffer {
  ...
  private static void extractBuffer(String queryFile, String referenceGenomeFast, int bpBuffer,
    ...
    if (!Files.exists(referenceGenomeFast)) {
      log.reportError("Error - could not find the reference genome provided as an argument: "
                      + referenceGenomeFast);
      return;
    } else if (!Files.exists(referenceGenomeFast + ".fai")) {
      log.reportError("Error - could not find the index file for the reference genome: "
                      + referenceGenomeFast + ".fai");
      return;
    }
