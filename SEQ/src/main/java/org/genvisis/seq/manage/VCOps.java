...
public class VCOps {
  ...
  /**
   * @author lane0212 types of alternate contexts available
   */
    ...
    for (Genotype geno : gc) {
      ...
      switch (type) {
        ...
        case HOM_ONLY:
          ...
          break;
        default:
          log.reportError("Invalid alt context type " + type);
          break;
      }
      if (use) {
        int[] AD = new int[] {0, 0};
        if (altAlleleDepth >= 0 || altAlleleDepthRatio >= 0) {
          try {
            ...
            if (verbose) {
              // TODO, report?
              log.reportError("Could not compute appropriate allele Depths");
  ...
  public static int[] getAppropriateAlleleDepths(VariantContext vc, Genotype g, boolean verbose,
    ...
    if (gAlleles.size() != 2 && g.hasAD()) {
      log.reportError("Number of alleles must equal 2, return AD[0,0]");
      return AD;
