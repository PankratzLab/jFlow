...
public class VCMut {
  ...
  private List<String> getMutMapFormat() {
    ...
    data.add(Positions.getChromosomeUCSC(Positions.chromosomeNumber(chromosome), false));
  ...
  public static VCMut fromVariantContext(VariantContext vc, PARSE_METHOD parseMethod,
    ...
    String hugoSymbol = VCOps.getAnnotationsFor(new String[] {parseMethod.geneFlag}, vc, ".")[0];
    String proteinChange = VCOps.getAnnotationsFor(new String[] {parseMethod.aaFlag}, vc, ".")[0];
    String mutationAnn = VCOps.getAnnotationsFor(new String[] {parseMethod.effFlag}, vc, ".")[0];
  ...
  public static void run(String vcf, String outputDir, String vpopFile, double maf,
                         boolean removeFilter, Segment seg) {
    ...
    for (String pop : vpop.getSuperPop().keySet()) {
      ...
      CloseableIterator<VariantContext> iter = reader.query(seg.getChromosomeUCSC(), seg.getStart(),
                                                            seg.getStop());
      while (iter.hasNext()) {
