...
import org.pankratzlab.shared.bioinformatics.OMIM;
import org.pankratzlab.shared.bioinformatics.OMIM.OMIMGene;
...
public class VCFSimpleTally {
  ...
  private static DistanceResult distance(List<Integer> from, List<Integer> to, boolean identical) {
    double sum = 0;
    int num = 0;
        ...
        for (VariantContext vc : result) {
          ...
          if (genelimit == null || genelimit.isEmpty() || genelimit.contains(geneName)) {
            ...
            for (Genotype g : gc) {
              ...
              for (int j = 0; j < controlsOrdered.size(); j++) {
                annoWriterSample.print("\t" + ArrayUtils.toStr(controlGroupSummaries.get(j)
                                                                                    .getSummary()));
              }
              annoWriterSample.print("\t"
  ...
  private static void addEntries(String caseDef, String hqCaseDef, List<String> controlsOrdered,
  ...
  public static class GeneVariantPositionSummary implements Serializable {
    ...
    public static GeneVariantPositionSummary[] readSerial(String filename, Logger log) {
      return (GeneVariantPositionSummary[]) SerializedFiles.readSerial(filename, log, false, true);
    }
