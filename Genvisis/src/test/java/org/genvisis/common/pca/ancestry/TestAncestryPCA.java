...
public class TestAncestryPCA {
  ...
  public void testAncestryPCsAndLoadings() {
    ...
    for (int component = 0; component < svd.getPCs().getRowDimension(); component++) {//
      boolean flip = false;

      for (int sample = 0; sample < svd.getPCs().getColumnDimension(); sample++) {

        double standard = OUTPUT_PCS[sample][component];
        double test = svd.getPCs().getEntry(component, sample);
    ...
    for (int component = 0; component < svd.getLoadings().getM().numCols; component++) {//
      boolean flip = false;

      for (int marker = 0; marker < svd.getLoadings().getM().numRows; marker++) {

        double standard = OUTPUT_LOADINGS[marker][component];
        double test = svd.getLoadings().getM().get(marker, component);
