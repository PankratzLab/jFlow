...
public class CreateDatabaseFromPlink {
  ...
  public static void createDatabase(String dir, String filename, Logger log) {
    ...
    boolean labelAlleles, listPositions, displayNotes, maskModel, maskFIDs, maskSex, maskAffStat,
        allMarkers, commaDelimited;
    byte[] chrs;
    int[] positions;
    String delimiter;
    ...
    if (!ArrayUtils.equals(markerNames, HashVec.loadFileToStringArray(dir + freqfile, true,
                                                                      new int[] {1}, false),
                           true)) {
  ...
  public static void addHeaderToPed(String root, String output, boolean split) {
    ...
    try {
      ...
      for (String markerName : markerNames) {
        writer.print("\t" + markerName + "A" + "\t" + markerName + "B");
      }
      writer.println();
      count = 0;
      while (reader.ready()) {
  ...
  public static void main(String[] args) {
    ...
    for (String arg : args) {
      if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
        ...
        numArgs--;
      } else if (arg.startsWith("rlinker=")) {
        rlinker = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("gwaf=")) {
        outfile = arg.split("=")[1];
        numArgs--;
