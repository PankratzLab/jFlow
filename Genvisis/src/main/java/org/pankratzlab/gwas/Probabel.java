...
public class Probabel {
  ...
  public static final String[] LOGIST_OUTPUT_HEADER = {"name", "A1", "A2", "Freq1", "MAF",
                                                       "Quality", "Rsq", "n",
                                                       "Mean_predictor_allele", "chrom",
                                                       "beta_SNP_add", "sebeta_SNP_add",
                                                       "chi2_SNP"};
  ...
  public static void createQsubs(String pheno, int type, boolean minimac, int maxGb) {
    ...
    for (int chr = 1; chr <= 22; chr++) {
      ...
      if (size > maxGb * Math.pow(2, 30)) {
        ...
        if (minimac) {
          markerNames = new SnpMarkerSet("chr" + chr + "/chr" + chr
                                         + ".info", SnpMarkerSet.MINIMAC_INFO_FORMAT, false,
  ...
  public static void parseMetalFiles(String pheno) { // incomplete
    ...
    try {
      ...
      writer.close();
      System.out.println("Data for chromosomes "
                         + ext.listWithCommas(ArrayUtils.toStringArray(var)));
      if (yok.size() > 0) {
  ...
  public static void stratifyByClass(String pheno, String optionalLookupFile, String classFile,
    ...
    try {
      ...
      writer = Files.openAppropriateWriter(ext.rootOf(pheno, false) + "_" + selectedClass + ".dat");
  ...
  public static void compareClasses(String pheno, String optionalLookupFile, String classFile,
    ...
    try {
      reader = new BufferedReader(new FileReader(pheno));
      writer = Files.openAppropriateWriter(ext.rootOf(pheno, false) + "_" + affectedClass + "_vs_"
                                           + unaffectedClass + ".dat");
      line = reader.readLine().trim().split(PSF.Regex.GREEDY_WHITESPACE);
