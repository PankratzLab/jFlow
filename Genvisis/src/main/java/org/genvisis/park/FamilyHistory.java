...
public class FamilyHistory {
	...
	public static void proc(String db_file, String trait) {
		...
		try {
			...
			index = ext.indexFactors(new String[] {trait}, reader.readLine().split("\t", -1), true, true)[0];
			while (reader.ready()) {
		...
		try {
			...
			while (reader.ready()) {
					...
					if (!data[1].equals(".")) {
						...
						if (!data[0].equals(line[2])) {
							if (quant) {
								...
								avg_max[1] = Double.parseDouble(data[1])>avg_max[1]?Double.parseDouble(data[1]):avg_max[1];
								// first degree relatives calculations
								if ((data[2].equals(trav[2])&&data[3].equals(trav[3]))||data[0].equals(trav[2])||data[0].equals(trav[3])||data[2].equals(trav[0])||data[3].equals(trav[0])) {
									counts[1]++;
									avg_max[2] += Double.parseDouble(data[1]);
									avg_max[3] = Double.parseDouble(data[1])>avg_max[3]?Double.parseDouble(data[1]):avg_max[3];
								}
				...
				if (quant) {
					writer.println(line[1]+","+line[2]+","+(counts[0]>0?(avg_max[0]/(double)counts[0])+","+avg_max[1]:".,.")+","+(counts[1]>0?(avg_max[2]/(double)counts[1])+","+avg_max[3]:".,.")+","+(counts[2]>0?(avg_max[4]/(double)counts[2]):".")+","+counts[2]);
				} else {
					writer.println(line[1]+","+line[2]+","+counts[0]+","+counts[1]+","+(counts[0]>0?1:0)+","+(counts[1]>0?1:0));
				}
			}
			reader.close();
			writer.close();
...
//		String trait = "Depression";
		...
		"   (1) trait (i.e. trait="+trait+" (default)\n"+
		"";
