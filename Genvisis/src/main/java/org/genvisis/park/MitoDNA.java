...
//	public static final String DEFAULT_SNPS = "errs.xln";
	...
	public static final String[] ANALYSIS_VARS = {"IJK", "JTUK", "JTIWX", "mito1719_dom", "mito4580_dom", "mito7028_dom", "mito8251_dom", "mito9055_dom", "mito10398_dom", "mito10400_dom", "mito12308_dom", "mito13368_dom", "mito13708_dom", "mito16391_dom", "H", "I", "J", "K", "M", "T", "U", "V", "W", "X", "Other", "I_v_H", "J_v_H", "K_v_H", "M_v_H", "T_v_H", "U_v_H", "V_v_H", "W_v_H", "X_v_H"};
	...
	public static void hapit(String dir, String referenceHaps, String snps) {
		...
		try {
			...
			System.err.println("Error: file \""+dir+referenceHaps+"\" not found in current directory");
			System.exit(1);
		...
		System.out.println("Found "+refs.length+" reference haplotypes using "+refSNPs.length+" SNPs each");
				...
				for (int j = 0; j<refSNPs.length; j++) {
					if (line[i].contains(refSNPs[j])) {
						if (indices[j]==-1) {
							...
							System.err.println("Error - more than one column contains '"+refSNPs[j]+"'; please advise");
							prob = true;
						}
					}
				}
			...
			while (reader.ready()) {
				...
				for (int i = 0; i<refSNPs.length; i++) {
					...
					if (!trav.equals(".")&&!trav.equals("1")) {
						if (trav.length()!=2) {
							writer.println("! INVALID GENOTYPE ("+trav+")\t"+missing+"\t"+(missing>0?1:0)+genos+"\t"+Array.toStr(Array.stringArray(refs.length*2, "."))+"\t"+temp);
							prob = true;
							genos += "\t?";
							...
							genos += "\t"+((int)trav.charAt(0)<69?"0":"1");
						}
					} else {
						genos += "\t.";
						if (indices[i]>=0) {
				...
				if (!prob) {
					if (count>0) {
						writer.println("! "+count+" HET"+(count>1?"S":"")+"\t"+missing+"\t"+(missing>0?1:0)+genos+"\t"+Array.toStr(Array.stringArray(refs.length*2, "."))+"\t"+temp);
					} else {
						poss = Array.booleanArray(refs.length, true);
						for (int i = 0; i<refSNPs.length; i++) {
							trav = indices[i]==-1?".":line[indices[i]];
							for (int j = 0; j<refs.length; j++) {
								if (!trav.equals(".")&&!trav.equals("1")&&!refs[j][i].equals(".")&&!trav.substring(1).equals(refs[j][i])) {
									poss[j] = false;
								}
							}
						}
						...
						if (count==0) {
							writer.println("Other\t"+missing+"\t"+(missing>0?1:0)+genos+"\t"+Array.toStr(Array.stringArray(refs.length, "0"))+"\t"+Array.toStr(Array.stringArray(refs.length, "."))+"\t"+temp);
						} else {
							trav = "";
							...
							trans = count==1?Array.booleanArrayToStringArray(poss):Array.stringArray(refs.length, ".");
							writer.print((count>1?count+" ":"")+trav+"\t"+missing+"\t"+(missing>0?1:0)+genos+"\t"+Array.toStr(trans));
							for (int i = 0; i<poss.length; i++) {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				dir = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("refs=")) {
				refs = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("snps=")) {
				snps = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("-hapit")) {
				hapit = true;
				numArgs--;
			} else if (args[i].startsWith("-testit")) {
				testit = true;
				numArgs--;
			}
		}
