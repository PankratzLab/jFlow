	...
	public prioritize(String filename, String databases, String variables) throws IOException {
		...
		for (int db = 0; db<DBs.length; db++) {
			try {
				...
				if (lookupIndices[db]==-1) {
					System.err.println("Error - lookup variable '"+lookup+"' was not found in database '"+DBs[db]+"'");
					System.exit(1);
				}
				...
				for (int i = 0; i<l2.length; i++) {
					...
					if (variableIndices[db][i]==-1) {
						System.err.println("Error - variable '"+l2[i].substring(1)+"' was not found in database '"+DBs[db]+"' (do you need an operator)");
						System.exit(1);
					}
				}
				while (reader.ready()) {
					...
					for (int i = 0; i<data[db].length; i++) {
						...
						case '^':
							...
							try {
								if (data[db][i].equals("")||Integer.parseInt(temp)>Integer.parseInt(data[db][i])) {
									data[db][i] = temp;
								}
		...
		try {
			...
			if (lookupIndices[DBs.length]==-1) {
				System.err.println("Error - lookup value for the databases ('"+lookup+"') does not match a column in the file "+filename);
				System.exit(1);
			}
			...
			while (reader.ready()) {
				...
				for (int i = 0; i<DBs.length; i++) {
					for (int j = 0; j<variableIndices[i].length; j++) {
						writer.print("\t"+(data==null?"XXX":(data[i][j].equals("-1")||data[i][j].equals("")?".":data[i][j])));
					}
				}
				writer.println();
			}
