...
public class PlateLists {
	...
	public static final String[][] LABELS = { {"probands.xln", "Probands", "Sporadics"}, {"other_affecteds.xln", "OtherAffecteds", "ShouldNotBeOtherAffecteds"}, {"unaffecteds.xln", "Unaffecteds", "Controls"}, {"temps.xln", "TEMP", "CARES_TEMPS"}};
	public static final String[] IGNORE_CODES = new String[] {"CONF", "HS", "SWAPPED", "MZLINKAGE", "DEL4LINKAGE", "POORYIELD", "MUT", "PHENO"};
	public static final String[] CREATE_HEADER = {"DNA", "ID", "OldPlate", "OldWell", "NewPlate", "NewWell", "Batch", "Dx", "Note"};
	public static final String[] UPDATE_FROM_CREATE_HEADER = {"DNA", "ID", "NewPlate", "NewWell", "Batch", "Dx", "Note"};
	public static final String[] UPDATE_HEADER = {"DNA", "ID", "Plate", "Well", "Batch", "Dx", "Note"};
	...
	public static void create(String dir, String filename) {
		...
		try {
			...
			while (reader.ready()) {
				line = reader.readLine().trim().split("\t");
				dna = new DnaSample(line[indices[ext.indexOfStr("DNA#", COLS)]], line[indices[ext.indexOfStr("FamInd", COLS)]], line[indices[ext.indexOfStr("Plate", COLS)]], line[indices[ext.indexOfStr("Well", COLS)]],
				// ext.rootOf(filename),
				".", false);
				dna.setNote(".");
		...
		try {
			...
			for (int i = 0; i<fams.length; i++) {
				...
				for (int j = 0; j<inds.length; j++) {
					...
					if (dx==null) {
						System.err.println("Error - no diagnosis for "+inds[j]+" (will be assigned to the unaffecteds plate)");
						dx = "Unknown";
					} else {
				...
				for (int j = 0; j<inds.length; j++) {
					...
					if (dx==null) {
						...
						categories.elementAt(2).add(family.remove(inds[j]).elementAt(0));
					} else if (ext.indexOfStr(dx, tools.PD_DX)>=0) {
						categories.elementAt(1).add(family.remove(inds[j]).elementAt(0));
		...
		try {
			...
			for (int i = 0; i<LABELS.length; i++) {
				printDNAsToNewPlates(dir, LABELS[i][0], writer, categories.elementAt(i), LABELS[i][PROGENI_NOT_CARES?1:2], 0);
			}
			writer.close();
		} catch (Exception e) {
	...
	public static void update(String dir, String filename) {
		...
		for (int i = 0; i<LABELS.length; i++) {
			try {
				reader = new BufferedReader(new FileReader(dir+(PROGENI_NOT_CARES?PROGENI:CARES)+"/"+LABELS[i][0]));
				line = reader.readLine().trim().split("\t");
				...
				while (reader.ready()) {
					line = reader.readLine().trim().split("\t");
					dna = new DnaSample(line[indices[0]], line[indices[1]], line[indices[2]], line[indices[3]], line[indices[4]], true);
		...
		try {
			...
			for (int i = 0; i<fams.length; i++) {
				...
				for (int j = 0; j<inds.length; j++) {
					...
					if (dx==null) {
						...
						if (v.elementAt(0).getProband()) {
							prior = j;
							priorLevel = dx.equals("CONF_PD")?3:(dx.equals("VPD")?2:(dx.equals("NVPD")?1:0));
						}
						if (dx.equals("CONF_PD")) {
							if (probandLevel<3) {
								proband = j;
				...
				if (probandLevel>priorLevel&&priorLevel!=-1) {
					System.err.println("FYI: "+family.get(inds[prior]).elementAt(0).getID()+" ("+diagnoses.get(family.get(inds[prior]).elementAt(0).getID())+") could be replaced by "+family.get(inds[proband]).elementAt(0).getID()+" ("+diagnoses.get(family.get(inds[proband]).elementAt(0).getID())+")");
				}
				if (prior>=0) {
				...
				for (int j = 0; j<inds.length; j++) {
					...
					if (dx==null) {
						...
						System.err.println("Error - failed to place "+inds[j]+" ("+family.get(inds[j]).elementAt(0).getDNA()+")");
						family.get(inds[j]).elementAt(0).setNote("No valid phenotype for this indiviudal");
		...
		try {
			...
			for (int i = 0; i<LABELS.length; i++) {
				printDNAsToNewPlates(dir, LABELS[i][0], writer, categories.elementAt(i), LABELS[i][PROGENI_NOT_CARES?1:2], maxes[i]+1);
			}
			writer.close();
		} catch (Exception e) {
			System.err.println("Error writing to master file");
	...
	public static void printDNAsToNewPlates(String dir, String filename, PrintWriter master, Vector<DnaSample> v, String plateName, int startIndex) {
		PrintWriter writer;
		DnaSample dna;
		int count;
		...
		try {
			...
			for (int i = 0; i<v.size(); i++) {
				dna = v.elementAt(i);
				if (dna.getPrior()) {
					writer.println(dna.getDNA()+"\t"+dna.getID()+(startIndex==0?"\tprior\tprior":"")+"\t"+dna.getPlate()+"\t"+dna.getWell()+"\t"+dna.getBatch()+"\t"+dna.getDx()+"\t"+dna.getNote());
					master.println(dna.getDNA()+"\t"+dna.getID()+(startIndex==0?"\tprior\tprior":"")+"\t"+dna.getPlate()+"\t"+dna.getWell()+"\t"+dna.getBatch()+"\t"+dna.getDx()+"\t"+dna.getNote());
				}
			}
			count = 0;
			for (int i = 0; i<v.size(); i++) {
				dna = v.elementAt(i);
				if (!dna.getPrior()) {
					writer.println(dna.getDNA()+"\t"+dna.getID()+(startIndex==0?"\t"+dna.getPlate()+"\t"+dna.getWell():"")+"\t"+translateToPlateWell(plateName, startIndex+count, v.size()<NUM_ON_PLATE)+"\t"+dna.getBatch()+"\t"+dna.getDx()+"\t"+dna.getNote());
					master.println(dna.getDNA()+"\t"+dna.getID()+(startIndex==0?"\t"+dna.getPlate()+"\t"+dna.getWell():"")+"\t"+translateToPlateWell(plateName, startIndex+count, v.size()<NUM_ON_PLATE)+"\t"+dna.getBatch()+"\t"+dna.getDx()+"\t"+dna.getNote());
					count++;
				}
			}
			writer.close();
	...
	public static String translateToPlateWell(String plateName, int index, boolean singlePlate) {
		...
		return plateName+(singlePlate?"":ext.formNum(plateNumber+1, 2))+"\t"+translateToWell(wellIndex);
	}
	...
	public static int translateFromPlateWell(int plate, String well) {
		...
		index += ext.getIndexFromColumn(well.substring(0, 1))*NUM_IN_ROW+Integer.parseInt(well.substring(1))-1;
