	...
	public static void affRent() {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (!trav.equals(prev)) {
					writer.print(trav+"\t"+(Integer.valueOf(fam).intValue()*1000+Integer.valueOf(id).intValue()));
					affy = false;
	...
	public static void anyExtras() {
		...
		try {
			...
			for (int i = 0; i<line.length; i++) {
				if (!line[i].equals(tools.NINFO1_HEADER[i])) {
					System.err.println("Error - header has been changed; expecting: "+tools.NINFO1_HEADER[i]+", got: "+line[i]);
					System.exit(1);
				}
			}
		...
		try {
			...
			for (int i = 0; i<fams.size(); i++) {
				...
				writer.println(fam+"\t"+(data.elementAt(0).size()+data.elementAt(2).size()+data.elementAt(3).size())+"\t"+(data.elementAt(0).size())+"\t"+(data.elementAt(6).size())+"\t"+(data.elementAt(1).size())+"\t"+(data.elementAt(2).size())+"\t"+(data.elementAt(3).size())+"\t"+(data.elementAt(0).size()+data.elementAt(1).size())+"\t"+(numDads)+"\t"+(numMoms)+"\t"+(Array.toStr(Array.toStringArray(data.elementAt(2)), " "))+"\t"+(Array.toStr(Array.toStringArray(data.elementAt(6)), " ")));
		...
		try {
			...
			while (reader.ready()) {
				...
				if (hash.containsKey(trav)) {
					...
					writer.println(line[0]+"\t"+line[1]+"\t"+tools.getUniqueID(line[0], line[1])+"\t"+(counts[0].size()+counts[2].size()+counts[4].size())+"\t"+(counts[1].size()+counts[3].size()+counts[5].size())+"\t"+counts[0].size()+"\t"+counts[1].size()+"\t"+counts[2].size()+"\t"+counts[3].size()+"\t"+counts[4].size()+"\t"+counts[5].size());
				} else {
