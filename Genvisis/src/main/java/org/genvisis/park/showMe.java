		...
		while (reader.ready()) {
			...
			if (v.contains(line[0])) {
				...
				if ((last[0]+"\t"+last[1]).equals(line[0]+"\t"+line[1])) {
					...
					if (pelican) {
						peeps.add(new String[] {line[0], line[1], (line[4].equals(".")?"0":line[4]), (line[5].equals(".")?"0":line[5]), (line[2].toUpperCase().equals("M")?"1":line[2].toUpperCase().equals("F")?"2":"0"), aff, (veeps.contains(line[0]+"\t"+line[1])?"1":"0"), (veeps.contains(line[0]+"\t"+line[1])?"1":"0"), (ages.containsKey(line[0]+"\t"+line[1])&&aff.equals("2")?ages.get(line[0]+"\t"+line[1]):"-"), (affection.containsKey(line[0]+"\t"+line[1])?affection.get(line[0]+"\t"+line[1]):"-"), "<< <PelicanData>"+(line[3].equals("TRUE")?"1":"0")+(line[1].equals("1")?"1":"0")+aff+"</PelicanData>"});
					} else {
						peeps.add(new String[] {line[0], line[1], (line[4].equals(".")?"0":line[4]), (line[5].equals(".")?"0":line[5]), (line[2].toUpperCase().equals("M")?"1":line[2].toUpperCase().equals("F")?"2":"0"), aff, (veeps.contains(line[0]+"\t"+line[1])?"1":"0"), (veeps.contains(line[0]+"\t"+line[1])?"1":"0")});
...
//		String filename = "SFHfams.dat";
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				separate = Boolean.parseBoolean(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].equals("trim=")) {
				trimmed = Boolean.parseBoolean(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].equals("pelican=")) {
				pelican = Boolean.parseBoolean(args[i].split("=")[1]);
				numArgs--;
			}
		}
		if (numArgs!=0) {
