...
public class hweAPOE {
	...
	public hweAPOE(String filename, int numReps) throws IOException {
		...
		StringTokenizer st;
		String temp, trav, gender, alleles, affStat, mmse, aoo, demented, duration, smoked, data, ejukashun;
		...
		while (reader.ready()) {
			...
			if (duration.equals(".")&&!aoo.equals(".")&&!alleles.equals("0")) {
				System.err.println("Error - "+trav+"-"+temp+" has an age of onset but not a duration; why?");
			...
			if (!mmse.equals(".")&&(group!=-1)&&affStat.equals("2")&&!aoo.equals(".")) {
				if (smoked.equals(".")) {
					System.err.println("Warning: "+trav+"-"+temp+" does not know if he/she ever smoked; assuming not");
					smoked = "0";
				}

				demented = "1";
				if ((ejukashun.equals(".")||Integer.valueOf(ejukashun).intValue()<=8)&&Integer.valueOf(mmse).intValue()<21) {
					demented = "0";
				} else if (Integer.valueOf(ejukashun).intValue()<=12&&Integer.valueOf(mmse).intValue()<23) {
					demented = "0";
				} else if (Integer.valueOf(mmse).intValue()<24) {
					demented = "0";
				}
				data = group+"\t"+demented+"\t"+aoo+"\t"+gender+"\t"+duration+"\t"+smoked+"\t"+ejukashun;
		...
		for (int rep = 0; rep<numReps; rep++) {
			...
			for (int j = 0; j<fams.size(); j++) {
				...
				means[2*group+Integer.valueOf(demented).intValue()] += Double.valueOf(aoo).doubleValue();
			}
		...
		for (int j = 0; j<GENOTYPES.length; j++) {
			...
			writer.print(GENOTYPES[j]+"\t"+ext.formDeci(means[2*j], 1, true)+" ("+ext.formDeci(100*means[2*j]/affTotal, 1, true)+"%)\t");
			means[2*j+1] = (double)totals[2*j+1]/(double)numReps;
			writer.println(ext.formDeci(means[2*j+1], 1, true)+" ("+ext.formDeci(100*means[2*j+1]/unaffTotal, 1, true)+"%)\t");
		}
	...
	public boolean addPerson(Hashtable<String,Vector<String>> hash, String trav, String data) throws IOException {
	...
	public String oddsRatio(double N21, double N22, double N11, double N12) throws IOException {
		...
		line = ext.formDeci(OR, 2, true)+" (95% CI: ("+ext.formDeci(Math.exp(logOR-1.96*se_logOR), 2, true)+", "+ext.formDeci(Math.exp(logOR+1.96*se_logOR), 2, true)+"); p="+ext.formDeci(ProbDist.NormDist(Math.abs(logOR/se_logOR)), 3, true)+")";
		...
		line += " corrected: "+ext.formDeci(OR, 2, true)+" (95% CI: ("+ext.formDeci(Math.exp(logOR-1.96*se_logOR), 2, true)+", "+ext.formDeci(Math.exp(logOR+1.96*se_logOR), 2, true)+"); p="+ext.formDeci(ProbDist.NormDist(Math.abs(logOR/se_logOR)), 3, true)+")";
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				numReps = Integer.valueOf(args[i].split("=")[1]).intValue();
				numArgs--;
			}
		}
		if (numArgs!=0) {
