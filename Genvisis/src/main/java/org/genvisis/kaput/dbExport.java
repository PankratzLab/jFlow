...
public class dbExport {
	...
	public static String[] ALT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\1_CRFdb\\", "/home/npankrat/park/00masters/crf_db/", "/home/npankrat/reed/CRFdb/"};

	public static String DEFAULT_DB = "crf_db.dat";

	public dbExport(String db_file, String filename, Vector<String> traits, boolean uniqueIDs) throws IOException {
		...
		while (reader.ready()) {
			...
			if (line.length!=allTraits.length) {
				System.err.println("Error - problem with crf_db.dat file; not enough columns for line starting with '"+line[0]+"'");
				System.exit(5);
			}
		...
		if (filename.equals("")) {
			...
			for (int i = 0; i<inds.size(); i++) {
				...
				writer.println(formatData(uniqueIDs?trav:trav.substring(0, 5), uniqueIDs?trav:Integer.valueOf(trav.substring(5)).intValue()+"", hash.get(trav)));
			...
			while (reader.ready()) {
				...
				writer.println(formatData(line[0], line[1], hash.containsKey(trav)?hash.get(trav):new String[traits.size()]));
				if (hash.containsKey(trav)) {
					dv.add(Double.parseDouble((hash.get(trav))[0]));
				}
			}
		...
		if (Math.abs(Array.kurtosis(dist))>1) {
			...
			if (filename.equals("")) {
				...
				for (int i = 0; i<inds.size(); i++) {
					...
					writer.println(formatData(uniqueIDs?trav:trav.substring(0, 5), uniqueIDs?trav:Integer.valueOf(trav.substring(5)).intValue()+"", data));
				}
			} else {
				reader = new BufferedReader(new FileReader(filename));
				reader.mark(500);
	...
	public String formatData(String famid, String indid, String[] data) {
		...
		for (int i = 0; i<data.length; i++) {
			output += delimit+(data[i]==null||data[i].equals(".")||data[i].equals("")?SOLAR?"":".":data[i]);
		}
	...
	public static void main(String[] args) throws IOException {
		...
		if (!traits.contains(firstBlood)) {
			System.err.println("Error - main variable '"+firstBlood+"' was not found in the database; aborting");
			System.exit(1);
		}
		...
		if (filename.equals("default")) {
			if (!new File(DEFAULT_FILE).exists()) {
				System.out.println("Warning - default file ("+DEFAULT_FILE+") does not exist, so exporting info for all individuals");
				filename = "";
			} else {
