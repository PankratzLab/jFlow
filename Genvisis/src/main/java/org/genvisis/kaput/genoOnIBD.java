...
public class genoOnIBD {
	public genoOnIBD(String mutation, String dumpfile, String pos, String pre) throws IOException {
		...
		while (reader.ready()) {
			...
			hash.put(line[1]+"\t"+line[2], (Integer.valueOf(line[3]).intValue()+Integer.valueOf(line[4]).intValue()-2)+"");
		}
		reader.close();
		...
		while (!done) {
			...
			if (!trav.equals(prev)) {
				if (founders.size()!=2&&prev!="") {
					System.err.println("Error - there should only be sibpairs in file "+pre+"; problem encountered with family "+prev);
					System.exit(3);
				}
				preIDs.put(prev, ids);
				ids = new Vector<String>();
		...
		while (reader.ready()) {
			line = reader.readLine().split("[\\s]+");
			if (Math.abs(Double.valueOf(line[0]).doubleValue()-Double.valueOf(pos).doubleValue())<0.5) {
				...
				for (int i = 0; i<=1; i++) {
					if (Integer.valueOf(pair[i]).intValue()<=ids.size()) {
						...
						System.err.println("Error - Individual "+line[1]+"-"+pair[i]+" was not found in "+pre);
					}
				}
				...
				writer.print(line[1]+"\t"+pair[0]+"\t"+geno1+"\t"+line[1]+"\t"+pair[1]+"\t"+geno2+"\t"+line[3]+"\t"+line[4]+"\t"+line[5]);
				writer.println("\t"+temp+"\t"+ext.formDeci(Double.valueOf(line[4]).doubleValue()/2+Double.valueOf(line[5]).doubleValue(), 6, true));
			}
		}
	...
	public static void main(String[] args) throws IOException {
		...
		String usage = "\n"+"park.genoOnIBD requires 3 arguments:\n"+"   (1) a chromosome#.dat-like file with genotypes of the putative mutation (i.e. mut="+mutation+" (default))\n"+"   (2) a mapmaker dumpIBD file (i.e. dump="+dump+" (default))\n"+"   (3) position to regress (i.e. pos="+pos+" (default))\n"+"   (4) the (re)chrom##.pre file used to create the dump (i.e. pre="+pre+" (default))\n"+"";
		...
		System.out.println("Regressing the counts of "+mutation+" on the IBD estimates of "+dump+" at position "+pos+" using the IDs from "+pre);
		try {
