...
public class bootstrapSNP {
	...
	public bootstrapSNP(String filename, boolean single) throws IOException {
		...
		System.out.println("bootstrapped mean (95% CI):\t"+ext.formDeci(bootstrapped[0], 4, true)+" ("+ext.formDeci(bootstrapped[1], 4, true)+","+ext.formDeci(bootstrapped[2], 4, true)+")");
		System.out.println("corresponding p-values:\t"+ext.formDeci(ProbDist.ChiDist(bootstrapped[0], 1), 4, true)+" ("+ext.formDeci(ProbDist.ChiDist(bootstrapped[1], 1), 4, true)+","+ext.formDeci(ProbDist.ChiDist(bootstrapped[2], 1), 4, true)+")");
		System.out.println();
	...
	public bootstrapSNP(String filename) throws IOException {
		...
		writer = new PrintWriter(new FileWriter(filename.substring(0, filename.length()-4)+"-summary.out"));
		...
		writer.println("average chi square for the 22 genotype:\t"+ext.formDeci(sums[6]/count, 4, true));
		writer.println("\t\tbootstrapped mean (95% CI):\t"+ext.formDeci(bootstrapped[0], 4, true)+" ("+ext.formDeci(bootstrapped[1], 4, true)+","+ext.formDeci(bootstrapped[2], 4, true)+")");
		writer.println("\t\tcorresponding p-values:\t"+ext.formDeci(ProbDist.ChiDist(bootstrapped[0], 1), 4, true)+" ("+ext.formDeci(ProbDist.ChiDist(bootstrapped[1], 1), 4, true)+","+ext.formDeci(ProbDist.ChiDist(bootstrapped[2], 1), 4, true)+")");
		writer.println();

		bootstrapped = Bootstrap(hash12);
		writer.println("average chi square for the 12 genotype:\t"+ext.formDeci(sums[7]/count, 4, true));
		writer.println("\t\tbootstrapped mean (95% CI):\t"+ext.formDeci(bootstrapped[0], 4, true)+" ("+ext.formDeci(bootstrapped[1], 4, true)+","+ext.formDeci(bootstrapped[2], 4, true)+")");
		writer.println("\t\tcorresponding p-values:\t"+ext.formDeci(ProbDist.ChiDist(bootstrapped[0], 1), 4, true)+" ("+ext.formDeci(ProbDist.ChiDist(bootstrapped[1], 1), 4, true)+","+ext.formDeci(ProbDist.ChiDist(bootstrapped[2], 1), 4, true)+")");
		writer.println();

		bootstrapped = Bootstrap(hash11);
		writer.println("average chi square for the 11 genotype:\t"+ext.formDeci(sums[8]/count, 4, true));
		writer.println("\t\tbootstrapped mean (95% CI):\t"+ext.formDeci(bootstrapped[0], 4, true)+" ("+ext.formDeci(bootstrapped[1], 4, true)+","+ext.formDeci(bootstrapped[2], 4, true)+")");
		writer.println("\t\tcorresponding p-values:\t"+ext.formDeci(ProbDist.ChiDist(bootstrapped[0], 1), 4, true)+" ("+ext.formDeci(ProbDist.ChiDist(bootstrapped[1], 1), 4, true)+","+ext.formDeci(ProbDist.ChiDist(bootstrapped[2], 1), 4, true)+")");
		writer.println();
	...
	public double[] Bootstrap(Hashtable<String,String> hash) { // determines the median of the ordered distribution, 2.5% percentile, 97.5%
		...
		for (int i = 0; i<NUM_BOOTSTRAP_REPS; i++) {
			...
			for (int j = 0; j<hash.size(); j++) {
				sumOfMeans += Double.valueOf(hash.get(""+(int)(Math.random()*hash.size()))).doubleValue();
			}
			replicates[i] = sumOfMeans/hash.size();
		}
		keys = Sort.quicksort(replicates);

		results[0] = replicates[keys[(int)((double)NUM_BOOTSTRAP_REPS*0.5)]];
		results[1] = replicates[keys[(int)((double)NUM_BOOTSTRAP_REPS*0.025)]];
		results[2] = replicates[keys[(int)((double)NUM_BOOTSTRAP_REPS*0.975)]];
