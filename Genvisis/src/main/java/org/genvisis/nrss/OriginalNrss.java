...
public class OriginalNrss {
	...
	public static void procFile(String dir, String filename, int p_column, int variate_column, int window, String outfile) {
			...
			while (!done) {
				...
				if (line[1].equals(chr)) {
					...
					for (int i = 0; i<indexSNPs.length; i++) {
						...
						highIndex--;
						stats[i] = computeStatistic(markerNames, pvals, lowIndex, highIndex, hash, INCLUSION_THRESHOLD, vars);
	...
	public static double[] computeStatistic(String[] markerNames, double[] pvalues, int lowIndex, int highIndex, Hashtable<String,String> hash, double threshold, double[] vars) {
		...
		for (int i = 0; i<pval_window.length; i++) {
			if (pval_window[keys[i]]<threshold) {
				maxR2 = 0;
				for (int j = 0; j<i; j++) {
					if (hash.containsKey(markerNames[lowIndex+keys[i]]+":"+markerNames[lowIndex+keys[j]])) {
						r2 = Double.parseDouble(hash.get(markerNames[lowIndex+keys[i]]+":"+markerNames[lowIndex+keys[j]]));
					} else if (hash.containsKey(markerNames[lowIndex+keys[j]]+":"+markerNames[lowIndex+keys[i]])) {
						r2 = Double.parseDouble(hash.get(markerNames[lowIndex+keys[j]]+":"+markerNames[lowIndex+keys[i]]));
					} else {
					...
					for (int t = 0; t<MODELS.length; t++) {
						...
						for (int i = 0; i<indexSNPs.length; i++) {
							...
							if (pvals[indexSNPs[i]]<indexThresholds[t]) {
								...
								highIndex--;
								stats[i] = computeStatistic(markerNames, pvals, lowIndex, highIndex, hash, inclusionThresholds[t], null)[0];
								markerCounts[i] = highIndex-lowIndex+1;
								//
								// System.out.println((i+1)+")
								// "+markerNames[indexSNPs[i]]+"\t"+positions[indexSNPs[i]]+"\t"+ext.formDeci(stats[i],
