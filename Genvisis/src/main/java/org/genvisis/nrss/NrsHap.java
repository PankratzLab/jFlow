...
public class NrsHap {
	...
	public static final String[][] MODELS = { {"0.001", "0.01", "150000"}, {"0.001", "0.01", "50000"}, {"0.001", "0.01", "25000"}, {"0.0001", "0.01", "250000"}, {"0.0001", "0.01", "150000"}, {"0.0001", "0.01", "25000"},};
			...
			while (!done) {
				...
				if (line[1].equals(chr)) {
					...
					for (int i = 0; i<indexSNPs.length; i++) {
						...
						highIndex--;
						stats[i] = computeStatistic(chr, markerNames, pvals, lowIndex, highIndex, INCLUSION_THRESHOLD, "");
						...
						if (i==0) {
							...
							writer.println(markerNames[indexSNPs[maxSNP]]+"\t"+chr+"\t"+positions[indexSNPs[maxSNP]]+"\t"+ext.formDeci(stats[maxSNP], 2)+"\t"+markerCounts[maxSNP]+"\tchr"+chr+":"+(positions[indexSNPs[maxSNP]]-WINDOW)+"-"+(positions[indexSNPs[maxSNP]]+WINDOW)+"\t"+minPvalue);
							writer.flush();
							cluster.clear();
							cluster.add(i);
						}
		...
		try {
			...
			while (reader.ready()) {
				...
				if (chr==null) {
					...
					System.err.println("Error - different chromosomes present in file to be processed: "+chr+" and "+line[1]);
				}
	...
	public static double computeStatistic(String chr, String[] markerNames, double[] pvalues, int lowIndex, int highIndex, double threshold, String extraStuff) {
		int[] keys;
		double[] pval_window;
		...
		String temp = "plink --bfile chr"+chr+extraStuff+" --hap-assoc --snps "+Array.toStr(Array.toStringArray(snps), ",")+" --hap-window "+snps.size();
		...
		try {
			...
			if (!reader.readLine().trim().split("[\\s]+")[1].equals("HAPLOTYPE")) {
				System.err.println("Error - could not find the result of the haplotype test for "+Array.toStr(Array.toStringArray(snps), ","));
			}
			line = reader.readLine().trim().split("[\\s]+");
			reader.close();
	...
	public static void procSimulations(final String suffix, int p_column, int chr_column, int pos_column, int markerName_column) {
		BufferedReader reader;
		PrintWriter writer;
		String[] line;
		Vector<String> markerVector = new Vector<String>(EST_NUM_MARKERS_IN_LARGEST_CHR);
		...
		for (int chr_target = 1; chr_target<=22; chr_target++) {
			...
			if (!new File("chr"+chr_target+".bed").exists()) {
				CmdLine.run("plink --bfile plink --chr "+chr_target+" --make-bed --out chr"+chr_target, ".");
			}
			for (int f = 0; f<files.length; f++) {
				filename = files[f].getName();
					...
					for (int t = 0; t<MODELS.length; t++) {
						...
						for (int i = 0; i<indexSNPs.length; i++) {
							...
							if (pvals[indexSNPs[i]]<indexThresholds[t]) {
								...
								highIndex--;
								stats[i] = computeStatistic(chr_target+"", markerNames, pvals, lowIndex, highIndex, inclusionThresholds[t], " --pheno sim."+filename.split("\\.")[1]+".dat");
								markerCounts[i] = highIndex-lowIndex+1;
								//
								// System.out.println((i+1)+")
								// "+markerNames[indexSNPs[i]]+"\t"+positions[indexSNPs[i]]+"\t"+ext.formDeci(stats[i],
								...
								if (firstOne) {
									...
									writer.println(markerNames[indexSNPs[maxSNP]]+"\t"+chr_target+"\t"+positions[indexSNPs[maxSNP]]+"\t"+ext.formDeci(stats[maxSNP], 2)+"\t"+markerCounts[maxSNP]+"\tchr"+chr_target+":"+(positions[indexSNPs[maxSNP]]-windowSizes[t])+"-"+(positions[indexSNPs[maxSNP]]+windowSizes[t]));
									writer.flush();
									cluster.clear();
									cluster.add(i);
								}
