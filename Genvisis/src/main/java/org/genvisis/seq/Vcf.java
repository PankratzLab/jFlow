...
//		variants = HashVec.loadFileToStringMatrix(variantList, false, new int[] {0}, false);
		...
		if (iv.size() > 0) {
			...
			try {
				...
				indices = ext.indexFactors(new String[] {"#CHROM", "POS", "REF", "ALT", "ID", "INFO"}, temp.trim().split("[\\s]+"), false, log, true, true);
				currentChrom = "";
//				while (reader.ready()) { // returning false at the same places in the file every time for no reason
				do {
					temp = reader.readLine();
					if (temp != null) {
						line = temp.trim().split("[\\s]+");
						if (line[0].startsWith("#")) {
							log.reportError("Error - ran into some unexpected trailing comments: "+Array.toStr(line));
						} else {
							infoToKeep = "";
							splitInfo = line[indices[5]].split(";");
							for (int i = 0; i < splitInfo.length; i++) {
								for (int j = 0; j < BITS_TO_KEEP.length; j++) {
									if ((BITS_TO_KEEP[j].endsWith("=") && splitInfo[i].startsWith(BITS_TO_KEEP[j])) || (splitInfo[i].equals(BITS_TO_KEEP[j]))) {
										infoToKeep += (infoToKeep.equals("")?"":";")+splitInfo[i];
									}
								}
							}
							hash.put(line[indices[0]]+"\t"+line[indices[1]], new String[] {line[indices[2]], line[indices[3]], line[indices[4]], infoToKeep});
...
//							}
	...
	//							hash.put(aliases[i], new String[] {line[indices[2]], line[indices[3]], line[indices[4]], line[indices[5]]});
	//						}
						}
					}

					if (temp == null || !line[indices[0]].equals(currentChrom)) {
						log.report(ext.getTime()+"\tSaving hash (n="+hash.size()+" entries) to "+vcfFile+"."+currentChrom+".serHash");
						SerialHash.createSerializedStringArrayHash(vcfFile+"."+currentChrom+".serHash", hash);
						hash.clear();
						if (temp != null) {
		...
		try {
			...
			for (int i = 0; i < variants.length; i++) {
				if (!variants[i][0].equals(currentChrom)) {
					...
					hash = SerialHash.loadSerializedStringArrayHash(vcfFile+"."+currentChrom+".serHash");
				}
...
//					line = hash.get(variants[i][0]);
					if (!variants[i][2].equalsIgnoreCase(line[0])) {
						log.reportError("Error - mismatched reference allele at position chr"+variants[i][0]+":"+variants[0][1]+" (found "+variants[i][2]+", expecting "+line[0]+")");
					}
					...
					for (int j = 0; j < alleles.length; j++) {
						...
						for (int k = 0; k < info.length; k++) {
							if (info[k].startsWith("AC=")) {
								splitInfo = ext.parseStringArg(info[k], "").split(",");
...
//					writer.println(variants[i][0]+"\t"+Maths.min(freqs[0], 1-freqs[0])+"\t"+dbsnp);
					if (index == -1) {
						log.reportError("Warning - the alternate allele that was seen at position chr"+variants[i][0]+":"+variants[i][1]+" ("+variants[i][3]+") was not seen in the reference vcf");
						writer.println(variants[i][0]+"\t"+variants[i][1]+"\t"+variants[i][2]+"\t"+variants[i][3]+"\t.\t.\t.\t"+alleles.length+"\t"+(dbsnp?"1":"0")+"\t"+line[2]);
					} else {
						writer.println(variants[i][0]+"\t"+variants[i][1]+"\t"+variants[i][2]+"\t"+variants[i][3]+"\t"+alleleCounts[index]+"\t"+numAlleleCalled[index]+"\t"+freqs[index]+"\t"+alleles.length+"\t"+(dbsnp?"1":"0")+"\t"+line[2]);
					}					
				} else {
					writer.println(variants[i][0]+"\t"+variants[i][1]+"\t"+variants[i][2]+"\t"+variants[i][3]+"\t.\t.\t.\t.\t.\t.");
				}
			}
			if (missingSplitAlleleCounts > 0) {
				log.reportError("       - there were "+missingSplitAlleleCounts+" sites with multiple alleles that only listed a single allele count");
			}			
			writer.close();
		...
		Files.makeVLookupReadyFile(outfile, lookupReadyFile, new int[] {0,1,2,3}, new int[] {4,5,6,7,8,9});
	}
	
	public static void createFromParameters(String filename, Logger log) {
		Vector<String> params;

		params = Files.parseControlFile(filename, "vcf", new String[] { "list=snps.txt", "vcf=D:/home/npankrat/NCBI/1000G/ALL.wgs.project_consensus_vqsr2b.20101123.snps.low_coverage.sites.vcf.gz", "out1=finalProduct.out", "out2=vlookup_table.xln" }, log);
...
//		String vcfFile = "D:/home/npankrat/NCBI/1000G/CEU/CEU.low_coverage.2010_09.sites.vcf.gz";
		String vcfFile = "D:/home/npankrat/NCBI/1000G/ALL.wgs.project_consensus_vqsr2b.20101123.snps.low_coverage.sites.vcf.gz";
		String outfile = null;
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				...
				variantList = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("vcf=")) {
				vcfFile = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("out1=")) {
				outfile = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("out2=")) {
				lookupReadyFile = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("log=")) {
				logfile = args[i].split("=")[1];
				numArgs--;
			} else {
				errors += "Error - invalid argument: " + args[i]+"\n";
			}
		}
