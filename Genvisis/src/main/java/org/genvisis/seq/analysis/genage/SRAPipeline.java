...
public class SRAPipeline implements Callable<List<PipelinePart>> {
	 ...
	 * @param cleanup
	 *            clean up by deleting the .sra and .bam file after completion
	 * @param log
	 */
	public SRAPipeline(SRASample sraSample, String inputSRA, String rootOutDir, String referenceGenome,
			String captureBed, String binBed, String vcf, String computelLocation, int numThreads, boolean cleanup,
		...
		if (!Files.exists(getCompleteFile(rootOutDir, inputSRA))) {
			...
			if (!Files.exists(bam)) {
				...
				if (!Files.exists(vdbcache) && sraSample.getPlatform() != PLATFORM.ABI_SOLID) {
					log.reportError(vdbcache + " was was not seen, this sample might fail conversion to bam, skipping");
			...
			List<PipelinePart> parts = Pipeline.pipeline(bam, rootOutDir, referenceGenome, captureBed, binBed, vcfFile,
					sraSample, partsToRun, computelLocation, numThreads, log);
	...
	private static List<SRASample> loadSraSamples(String sraInput, String sraRunTableFile, PLATFORM platform,
			Logger log) {
	...
	private static String[] getAssociatedBams(List<SRASample> samples, String rootOutDir, Logger log) {
	...
	private static void runCompile(List<SRASample> samples, String rootOutDir, String referenceGenome,
			String captureBed, String binBed, String vcf, int numThreads, Logger log) {
		...
		BamImport.importTheWholeBamProject(proj, binBed, captureBed, vcf, BamImport.CAPTURE_BUFFER, 4, true, atType,
				aName, bams, numThreads);
	}

	private static void compilePrep(String sraInput, String sraRunTableFile, String rootOutDir, String referenceGenome,
			String captureBed, String binBed, String vcf, PLATFORM platform, int numThreads) {
		...
		log.reportTimeInfo("Found " + wgsSamples.size() + " " + ASSAY_TYPE.WGS + " samples and " + wxsSamples.size()
				+ " " + ASSAY_TYPE.WXS + " samples");
	...
	private static void runAll(String sraInput, String sraRunTableFile, String rootOutDir, String referenceGenome,
			String captureBed, String binBed, String vcf, String computelLocation, int numThreads,
		...
		List<SRASample> samples = loadSraSamples(sraInput, sraRunTableFile, PLATFORM.valueOf(c.get(PLATFORM_TYPE)),
				log);
		...
		for (SRASample sample : samples) {
			...
			SRAPipeline pipeline = new SRAPipeline(sample, sample.getSraFile(), rootOutDir, referenceGenome, captureBed,
					binBed, vcf, computelLocation, numThreadsPipeline, cleanup, partsToRun, log);
			...
			case WXS:
				if (!prelimGenvisisWXS) {
					generatePrelim(rootOutDir, referenceGenome, captureBed, binBed, vcf, log, sample.getaType());
	...
	private static void generatePrelim(String rootOutDir, String referenceGenome, String captureBed, String binBed,
			String vcf, Logger log, ASSAY_TYPE aType) {
		Project proj = Pipeline.getProjectFor(aType, rootOutDir, referenceGenome);
		if (!Files.exists(proj.MARKERSET_FILENAME.getValue())) {

			BamImport.generateAnalysisSet(proj, binBed, captureBed, vcf, BamImport.CAPTURE_BUFFER, aType, log,
	...
	private static void fullPipeline(CLI c) {
		...
		log.reportTimeInfo("Detected " + sraFilesToAnalyze.size() + " samples to analyze for platform " + platform + ","
				+ (sraFiles.length - sraFilesToAnalyze.size()) + " samples are already complete");
		...
		for (int i = 0; i < batches.length; i++) {
			for (int j = 0; j < batches[i].length; j++) {
				...
				process.add("echo \"start " + ext.rootOf(batches[i][j]) + "\" `date` >>" + processDir + "sraDL.times");
		...
		if (c.has(GENVISIS_PART) || c.has(ALL_PART)) {
			generatePrelim(c.get(OUT_DIR), c.get(REFERENCE_GENOME), c.get(CAPTURE_BED), c.get(BIN_BED), c.get(VCF), log,
					ASSAY_TYPE.WXS);
			generatePrelim(c.get(OUT_DIR), c.get(REFERENCE_GENOME), null, null, c.get(VCF), log, ASSAY_TYPE.WGS);
		}
	...
	private static String[][] batch(String[] sraFiles, String rootOutDir, CLI c, Logger log) {
		...
		String runningJar = SRAPipeline.class.getProtectionDomain().getCodeSource().getLocation().getFile();
		PLATFORM platform = PLATFORM.valueOf(c.get(PLATFORM_TYPE));
		...
		if (c.has(ALL_PART)) {
			...
			if (platform != PLATFORM.ILLUMINA) {
				throw new IllegalArgumentException(
						"Pipeline " + ALL_PART + " not completely implemented for " + platform);
			}
		}
		if (c.has(COMPUTEL_PART)) {
			...
			if (platform != PLATFORM.ILLUMINA) {
				throw new IllegalArgumentException(
						"Pipeline " + COMPUTEL_PART + " not completely implemented for " + platform);
			}
		}
		if (c.has(TELSEQ_PART)) {
			...
			if (platform != PLATFORM.ILLUMINA) {
				throw new IllegalArgumentException(
						"Pipeline " + TELSEQ_PART + " not completely implemented for " + platform);
			}
		}
		if (c.has(MTDNACN_PART)) {
		...
		if (c.has(GENVISIS_PART)) {
			...
			if (platform != PLATFORM.ILLUMINA) {
				throw new IllegalArgumentException(
						"Pipeline " + GENVISIS_PART + " not completely implemented for " + platform);
			}
		}
	...
	public static void main(String[] args) {
		...
		c.addArgWithDefault(SRA_RUN_TABLE, "a sra run table providing sample information", sraRunTableDefault);

		int numThreads = 24;

		c.addArgWithDefault(NUM_THREADS, "number of threads across samples", Integer.toString(numThreads));

		int numThreadsPipe = 1;
		c.addArgWithDefault(NUM_THREADS_PIPELINE, "number of threads within samples", Integer.toString(numThreadsPipe));
		...
		c.addFlag(FULL_PIPELINE, "prepare for a run of the full pipeline, which will include downloading .sra files");
		...
		if (c.has(COMPILE)) {
			compilePrep(c.get(SRA_INPUT), c.get(SRA_RUN_TABLE), c.get(OUT_DIR), c.get(REFERENCE_GENOME),
					c.get(CAPTURE_BED), c.get(BIN_BED), c.get(VCF), PLATFORM.valueOf(c.get(PLATFORM_TYPE)),
					c.getI(NUM_THREADS));
		} else if (c.has(FULL_PIPELINE)) {
			fullPipeline(c);
		} else {
			runAll(c.get(SRA_INPUT), c.get(SRA_RUN_TABLE), c.get(OUT_DIR), c.get(REFERENCE_GENOME), c.get(CAPTURE_BED),
					c.get(BIN_BED), c.get(VCF), c.get(COMPUTEL_LOCATION), c.getI(NUM_THREADS),
