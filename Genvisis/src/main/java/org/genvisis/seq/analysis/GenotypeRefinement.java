...
public class GenotypeRefinement {
	 ...
	 * @param log
	 */
	private static void combineResults(List<GenotypeRefiner> refinedResults,
	 ...
	 * @param outputDir
	 * 
	 *        Extracts potential denovos from refined results, and converts contigs to hg19 ...for now
	 */
	private static List<String> filterAndCombineDenovos(List<GenotypeRefiner> refinedResults,
		...
		if (!Files.exists(hiConfVCF) && !Files.exists(hiLowConfVCF)) {
			...
			hiConfwriter.close();
			hiLowConfwriter.close();
			log.reportTimeInfo("found "
												 + potentialDenovos + " potential denovos in total from " + total
												 + " variants scanned");
		}
		return filtDenovos;
	}
	...
	private static boolean isHighConfPotentialDenovo(VariantContext vc) {
		String[] confDenovos = VCOps.getAnnotationsFor(new String[] {HI_CONF_DE_NOVO},
																									 vc, ".");
	...
	private static class RefinementProducer implements Producer<GenotypeRefiner> {
		...
		public Callable<GenotypeRefiner> next() {
			...
			index++;
			return () -> gatk.refineGenotypes(current.getOutputVCF(), ped,
																				outputDir, log);
		}
