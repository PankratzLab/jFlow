...
/**
 * Class for processing ClinVar data
 *
 * wget ftp://ftp.ncbi.nlm.nih.gov/pub/clinvar//vcf_GRCh37/clinvar_20170530.vcf.gz wget
 ...
 * 
 * ClinVar's VCF files are currently limited to records that have been assigned rs# in dbSNP. Thus
 * there may be two types of gaps:
 * 
 * The variant is not in scope for dbSNP (i.e. the length is greater than 50 bp or the exact
 * location is not known). The variant has not yet been processed by dbSNP. The ClinVar VCF and
 * related VCF set can be retrieved from ClinVar's ftp site:
 */
public class ClinVar {
	...
	private static void run(String vcf, Segment seg, String outputDir,double[] mafs) {
		...
		String subsetHG19Vcf = outputDir
													 + VCFOps.getAppropriateRoot(vcf, true)
													 + ".clinvar."
		...
		VcfPopulation fakeVpop = new VcfPopulation(fake, fake,
																							 POPULATION_TYPE.ANY,
																							 new Logger());
