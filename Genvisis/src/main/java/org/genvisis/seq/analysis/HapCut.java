 ...
 * VCFs
 * 
 * Paper: http://genome.cshlp.org/content/early/2016/12/09/gr.213462.116.abstract
 *
 * Basic process: <br>
...
public class HapCut {
	 ...
	 */
	public static void run(String vcf, String inputDir, String outDir,
												 String variantSet, String hapCutLoc, String extractHairsLoc,
												 String fgbioLoc,
												 int threads) {
	...
	private static class HapCutProducer implements Producer<HapCutResult> {
		...
		private static HapCutResult processBam(String vcf, String outDir, String variantSet,
																					 String hapCutLoc,
																					 String extractHairsLoc, String fgBioLoc, Logger log,
																					 String bam) {
			HapCutResult result = extractSampleForBam(bam, vcf, outDir, variantSet, log);
			if (result.success) {
				result.success = runExtractHairs(result, bam, extractHairsLoc,
																				 log);
				if (result.success) {
					result.success = runHapCut2(result, hapCutLoc,
																			log);
					if (!result.success) {
		...
		private static boolean runFGBIOConversion(HapCutResult result,
																							String fgBioLoc,
																							Logger log) {
		 ...
		 */
		private static boolean runHapCut2(HapCutResult result,
																			String hapCutLoc,
																			Logger log) {
			Set<String> necessaryInputFiles = new HashSet<>();
			necessaryInputFiles.add(result.extractedVCF);
		 ...
		 * @return true if successful
		 */
		private static boolean runExtractHairs(HapCutResult result, String bam,
																					 String extractHairsLoc, Logger log) {
			Set<String> necessaryInputFiles = new HashSet<>();
			necessaryInputFiles.add(result.extractedVCF);
	...
	public static void main(String[] args) {
		...
		c.addArgWithDefault("variantSet",
												"Variant set to use, if your vcf utilizes variant sets",
												"");
