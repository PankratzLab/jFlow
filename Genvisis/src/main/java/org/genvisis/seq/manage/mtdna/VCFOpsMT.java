...
public class VCFOpsMT {
  ...
  private static class AlleleSwapper {
    private HashSet<Integer> toSwaps;
    private RCRS rcrs;

    private AlleleSwapper(Logger log) {
      this.toSwaps = new HashSet<Integer>();
      for (int i = 0; i < BP_DIFFS.length; i++) {
        toSwaps.add(BP_DIFFS[i]);
      }
      this.rcrs = RCRS.getRCRS(log);
    }
    ...
    private Allele getRcrsAllles(VariantContext vc) {
      String rcrsS =
                   Array.toStr(Array.subArray(rcrs.getBases(), vc.getStart() - 1, vc.getEnd()), "");
