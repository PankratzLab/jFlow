...
public class VCFImporter {
	...
	public ConversionResults importVCF(Project proj, Set<String> samples, final String vcfFile, boolean createMarkerPositions) {
		...
		for (VariantContext vc : vcfFileReader) {
			...
			if (niceAllele.filter(vc).passed()) {
				...
				for (int i = 0; i < vcSamples.length; i++) {
		...
		for (int i = 0; i < vcSamples.length; i++) {
			vcSamples[i].dump(proj, allOutliers, fingerprint, log);
		}
		log.reportTimeWarning(numSkipped + " non-biallelic or ambigous varaints of " + total + " total variants were skipped");
		...
		if (samples == null) {
			...
			for (String desiredSample : samples) {
				if (ext.indexOfStr(desiredSample, samplesVCF, true, true) >= 0) {
					...
					proj.getLog().reportTimeWarning("Could not find sample " + desiredSample + " in vcf, skipping");
				}
			}
			if (samplesMissing.size() > 0) {
				try {
					PrintWriter writer = new PrintWriter(new FileWriter(proj.PROJECT_DIRECTORY.getValue() + "missing_samples.txt"));
					for (String missingSample : samplesMissing) {
						writer.print(missingSample);
					}
					writer.close();
				} catch (Exception e) {
					log.reportError("Error writing to " + proj.PROJECT_DIRECTORY.getValue() + "missing_samples.txt");
	...
	private static class ConversionResults {
		private Hashtable<String, Float> allOutliers;
	...
	private static class VCFImporterWorker implements Callable<ConversionResults> {
		...
		private boolean createMarkerPositions;

		public VCFImporterWorker(Project proj, String vcf, Set<String> samples, boolean createMarkerPositions) {
			super();
			this.proj = proj;
			this.vcf = vcf;
	...
	public static void test2(Project proj, String vcf, int numRounds, int numThreads, int numDecompressThreads) {
		PREPPED_SAMPLE_TYPE[] types = PREPPED_SAMPLE_TYPE.values();
		for (int i = 0; i < types.length; i++) {
			// if(types[i] ==PREPPED_SAMPLE_TYPE.NORMALIZED)
			try {
				test2(proj, vcf, numRounds, numThreads, numDecompressThreads, types[i]);
			} catch (Exception e) {
				System.exit(1);
			}
	...
	public static void test2(Project proj, String vcf, int numRounds, int numThreads, int numDecompressThreads, PREPPED_SAMPLE_TYPE type) {
		...
		// GcModel gcModel = GcAdjustor.GcModel.populateFromFile(proj.getFilename(Project.GC_MODEL_FILENAME, false, true), false, proj.getLog());
		// Files.copyFile(proj.getFilename(Project.MARKER_POSITION_FILENAME), projNorm.getFilename(Project.MARKER_POSITION_FILENAME));
		// // Files.copyFile(proj.getFilename(Project.SAMPLELIST_FILENAME), projNorm.getFilename(Project.SAMPLELIST_FILENAME));
		// Markers.orderMarkers(null, projNorm.getFilename(Project.MARKER_POSITION_FILENAME), projNorm.getFilename(Project.MARKERSET_FILENAME, true, true), projNorm.getLog());
		// //
		// VCFSamplePrepWorker vPrepWorker = new VCFSamplePrepWorker(proj, projNorm.getDir(Project.SAMPLE_DIRECTORY, true, true), type, gcModel);
		...
		// Files.writeSerial(allNewOutliers, projNorm.getDir(Project.SAMPLE_DIRECTORY) + "outliers.ser");
		// }
		...
		// CentroidCompute.computeAndDumpCentroids(projNorm, centFile, builder, numThreads, numDecompressThreads);
		// projNorm.saveProperties();
		...
		// //MitoPipeline.catAndCaboodle(projNorm, numDecompressThreads, "0.95", proj.getFilename(Project.TARGET_MARKERS_FILENAME), 10, "PCs" + type, false, true, 0.98, proj.getFilename(Project.SAMPLE_SUBSET_FILENAME), null, null, true, true, false, true);
		// TODO uncomment
		TransposeData.transposeData(projNorm, 2000000000, false);
		// String[] markers = HashVec.loadFileToStringArray(proj.getFilename(Project.MARKER_POSITION_FILENAME), true, new int[] { 0 }, true);

		MAF.computeMAF(projNorm);

		projNorm.saveProperties();
		// Files.writeList(markers, projNorm.getProjectDir() + "shadowMarkers.txt");
		// PennCNVPrep.exportSpecialPennCNV(projNorm, "shadowPrep" + type + "/", 10, projNorm.getProjectDir() + "shadowMarkers.txt", 6, 1, false, false, false, 2);
		// PennCNVPrep.exportSpecialPennCNV(projNorm, "shadowPrep" + type + "/", 10, projNorm.getProjectDir() + "shadowMarkers.txt", 6, 1, false, true, false, 2);

		// for (int i = 0; i < 10; i++) {
		// double qual = (double) i / 10;
		// builder.gcThreshold(qual);
		// CentroidCompute.computeAndDumpCentroids(projNorm, projNorm.getProjectDir() + ext.rootOf(vcf) + type + "_" + qual + ".cent", builder, numThreads, numDecompressThreads);
		// projNorm.setProperty(Project.CUSTOM_CENTROIDS_FILENAME, ext.rootOf(vcf) + type + "_" + qual + ".cent");
		// projNorm.saveProperties();
		// }
	}
	...
	public static void test(Project proj, String vcf, String gc5Base, PREPPED_SAMPLE_TYPE type, int numRounds, int numThreads) {
		...
		if (proj.getSamples() != null && proj.getSamples().length != samples.length) {
			...
			for (int i = 0; i < samples.length; i++) {
				if (ext.indexOfStr(samples[i], proj.getSamples()) < 0) {
					proj.getLog().reportTimeError("Did not find vcf sample " + samples[i] + "in the sample directory");
					haveAll = false;
				}
			}
			if (!haveAll) {
				...
				proj.getLog().reportTimeInfo("Detected all " + samples.length + " samples have been parsed");
				try {
		...
		MitoPipeline.catAndCaboodle(projNorm, numThreads, pretendMedian, 100, projNorm.PROJECT_DIRECTORY.getValue() + "VCF_PCS", true, true, 0.98, useFile, null, null, null, true, true, true, false, true, false, null, -1, -1, GENOME_BUILD.HG19, MitoPipeline.DEFAULT_PVAL_OPTS,null, false);
	...
	public static void main(String[] args) {
		...
		usage += "   (3) number of rounds to process the vcf file in  (i.e. numRounds=" + numRounds + " (default))\n" + "";
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				...
				numArgs--;
			} else if (args[i].startsWith("numRounds")) {
				numRounds = ext.parseIntArg(args[i]);
				numArgs--;
...
// // private final VariantContext vch = new VariantContextBuilder(vc).make().fullyDecode(header, true);
//
//
// @Override
