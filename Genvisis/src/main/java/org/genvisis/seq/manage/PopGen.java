...
public class PopGen {
  ...
  public static void filterForPopGen(String directory, String outputVCF, String vcfSuffix,
    ...
    final VariantContextFilter vcfFilter = new VariantContextFilter(fullPopFilters,
                                                                    ...
                                                                    null, null, log);
    final VariantContextFilter superPopFilter = new VariantContextFilter(new VARIANT_FILTER_DOUBLE[] {VARIANT_FILTER_DOUBLE.HWE,
    ...
    for (int i = 0; i < vcfs.length; i++) {
      ...
      for (final VariantContext variantContext : tmp) {
        ...
        VariantContextFilterPass superPopPass = new VariantContextFilterPass(true,
...
// private static int ReportPassingVariants(Logger log, String vcfs, VCFFileReader tmp,
// VariantContextWriter writer, int pass, CallableBufferSet<VariantContext, VariantContext[]>
// bufferSet) {
...
// return pass;
// }
// private static PopGenBuffer[] initBuffers(int numThreads, int bufferSize, VariantContextFilter
// vcfFilter, VariantContextFilter hweFilter, VCFOps.VcfPopulation hwePopTests, VCFHeader header,
// Logger log) {
// PopGenBuffer[] popGenBuffers = new PopGenBuffer[numThreads];
// for (int i = 0; i < numThreads; i++) {
// popGenBuffers[i] = new PopGenBuffer(bufferSize, vcfFilter, hweFilter, hwePopTests, new
// ArrayList<VariantContext>(), header, log);
// }
// return popGenBuffers;
// }
//
// private static class PopGenBuffer extends CallableBufferSet.CallableBuffer<VariantContext,
// VariantContext[]> {
...
// private Logger log;
//
// public PopGenBuffer(int bufferSize, VariantContextFilter vcfFilter, VariantContextFilter
// hweFilter, VCFOps.VcfPopulation hwePopTests, ArrayList<VariantContext> vcBuffer, VCFHeader
// header, Logger log) {
...
// VariantContextFilterPass hwefp = new VariantContextFilterPass(true, "Did not need to test super
// populations");
// if (vcfp.passed() && hwePopTests != null) {
...
// passes[i] = new VariantContextFilterPass(vcfp.passed() && hwefp.passed(), "All Sample Filter: " +
// vcfp.getTestPerformed() + " SuperPopFilter: " + hwefp.getTestPerformed());
// }
...
// PopGenBuffer[] buffers = initBuffers(numThreads, 2000, vcfFilter, hweFilter, hwePopTests, header,
// log);
// CallableBufferSet<VariantContext, VariantContext[]> bufferSet = new
// CallableBufferSet<VariantContext, VariantContext[]>(numThreads, 10, buffers, log);
//
// log.reportTimeInfo("Finished initializing reader for " + vcfs[i]);
// long time = System.currentTimeMillis();
// PopGenBuffer popGenBuffer = new PopGenBuffer(50000, vcfFilter, hweFilter, hwePopTests, new
// ArrayList<VariantContext>(), tmp.getFileHeader(), log);
// PopGenBuffer popGenBuffer2 = new PopGenBuffer(50000, vcfFilter, hweFilter, hwePopTests, new
// ArrayList<VariantContext>(), tmp.getFileHeader(), log);
...
// popGenBuffer = new PopGenBuffer(50000, vcfFilter, hweFilter, hwePopTests,
// popGenBuffer.getBuffer(), tmp.getFileHeader(), log);
// popGenBuffer2 = new PopGenBuffer(50000, vcfFilter, hweFilter, hwePopTests,
// popGenBuffer2.getBuffer(), tmp.getFileHeader(), log);
//
