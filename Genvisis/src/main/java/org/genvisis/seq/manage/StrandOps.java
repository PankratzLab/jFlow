		...
		if (ext.indexOfStr(alleles[0], VALID_ALLELES) >= 0 && ext.indexOfStr(alleles[1], VALID_ALLELES) >= 0) {
			...
			if (Array.booleanArraySum(nullChecks) == 1) {
				...
				if (referenceAlleles[0] == null) {
					...
					return index == 0 ? CONFIG.STRAND_CONFIG_SAME_ORDER_SAME_STRAND : CONFIG.STRAND_CONFIG_OPPOSITE_ORDER_SAME_STRAND;
				} else if (referenceAlleles[1] == null) {
					if (alleles[index].equals(referenceAlleles[0])) {
						...
						if (flipped[0].equals(referenceAlleles[0])) {
							return index == 0 ? CONFIG.STRAND_CONFIG_SAME_ORDER_FLIPPED_STRAND : CONFIG.STRAND_CONFIG_OPPOSITE_ORDER_FLIPPED_STRAND;
						} else {
							return CONFIG.STRAND_CONFIG_DIFFERENT_ALLELES;
						}
					}
					...
					if (alleles[index].equals(referenceAlleles[0])) {
						...
						return index == 1 ? CONFIG.STRAND_CONFIG_SAME_ORDER_SAME_STRAND : CONFIG.STRAND_CONFIG_OPPOSITE_ORDER_SAME_STRAND;
					} else {
						flipped = new String[] { Sequence.flip(alleles[index]) };
						if (flipped[0].equals(referenceAlleles[0])) {
							return index == 0 ? CONFIG.STRAND_CONFIG_SAME_ORDER_FLIPPED_STRAND : CONFIG.STRAND_CONFIG_OPPOSITE_ORDER_FLIPPED_STRAND;
						} else if (flipped[0].equals(referenceAlleles[1])) {
							return index == 1 ? CONFIG.STRAND_CONFIG_SAME_ORDER_FLIPPED_STRAND : CONFIG.STRAND_CONFIG_OPPOSITE_ORDER_FLIPPED_STRAND;
						} else {
							return CONFIG.STRAND_CONFIG_DIFFERENT_ALLELES;
						}
					}
