...
public class VariantFilterSample {
	...
	private VariantContextFilter vCFiltAltGeno;
	private VariantContextFilter vCFiltHomRefGeno;
	private VariantContextFilter vCFiltAllGeno;
	private VariantContextFilter vCFiltIndel;
	private VariantContextFilter vCFiltWholeVariant;
	private Logger log;
	...
	public VariantFilterSamplePass filter(VariantContext vc, FILTER_METHOD method) {
		...
		if (tmp == null || tmp.passed()) {
			...
			if (samplesTested != vc.getSampleNames().size()) {
				throw new IllegalStateException("Not all samples passed through the filters, internal error ,tested " + samplesTested + " and should have tested " + vc.getSampleNames().size());
			}
			switch (method) {
			case REMOVE_FROM_CONTEXT:// keep passing only
				vcPass.setPassingContext(VCOps.getSubset(vc, passingSamples, VC_SUBSET_TYPE.SUBSET_STRICT, false));
				break;
			...
			case REMOVE_FROM_CONTEXT:// remove all
				vcPass.setPassingContext(VCOps.getSubset(vc, new HashSet<String>(), VC_SUBSET_TYPE.SUBSET_STRICT, false));
				break;
	...
	public enum FILTER_METHOD {
		 ...
		 */
		REMOVE_FROM_CONTEXT, /**
		 * Samples that do not pass the filters will be set to missing in the final {@link VariantContext}
		 */
		SET_TO_MISSING;
	}
	...
	public static class VariantFilterSamplePass {
		...
		public VariantFilterSamplePass() {
			super();
			this.vcfps = new ArrayList<VariantContextFilterPass>();
		}
	...
	public static class Builder {
		...
		/**
		 * @param vCFiltAltGeno
		 *            this filter will be applied to alternate calls only
		 * @return
		 */
		...
		/**
		 * @param vCFiltHomRefGeno
		 *            this filter will be applied to reference (Hom Ref) calls only
		 * @return
		 */
		...
		/**
		 * @param vCFiltAllGeno
		 *            this filter will be applied to all genotypes
		 * @return
		 */
		...
		/**
		 * @param vCFiltIndel
		 *            this filter will be applied to Indels only
		 * @return
		 */
		...
		/**
		 * @param vCFiltWholeVariant
		 *            this filter will be applied to the whole variant (Annotations, biallelic, etc)
		 * @return
		 */
	...
	private VariantFilterSample(Builder builder, String filterName, Logger log) {
		...
		this.vCFiltAltGeno = builder.vCFiltAltGeno;
		this.vCFiltHomRefGeno = builder.vCFiltHomRefGeno;
		this.vCFiltAllGeno = builder.vCFiltAllGeno;
		this.vCFiltIndel = builder.vCFiltIndel;
		this.vCFiltWholeVariant = builder.vCFiltWholeVariant;
		this.log = log;
	}
}
