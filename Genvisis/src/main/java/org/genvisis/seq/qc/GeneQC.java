...
public class GeneQC {
  private static class GeneSummary {
    // private static final String[] SUMMARY = new String[] { "Gene", "NumExons", "numIsoForms",
    // "TotalMrna", "TotalMrnaMinusUTR", "CoveredMrna", "CoveredMrnaNoUtrs" };
    ...
    //
    // public GeneSummary(String geneName, int numExons, int numIsoForms, int totalMrna, int
    // mrnaNoUtrs) {
    // super();
  ...
  private final String bamQCFile;

  private final LocusSet<Segment> utrs;

  private final LocusSet<GeneData> genes;

  private final GeneSummary[] geneSummaries;
  ...
  public GeneQC(LocusSet<GeneData> genes, String bamQCFile, LocusSet<Segment> utrs, Logger log) {
    super();
    this.genes = genes;
    geneSummaries = new GeneSummary[genes.getLoci().length];
  ...
  // for (int i = 0; i < geneTrack.getGenes().length; i++) {
  // for (int j = 0; j < geneTrack.getGenes()[i].length; j++) {
  // GeneQCResult geneQCResult = new GeneQCResult(geneTrack.getGenes()[i][j].getGeneName(),
  // geneTrack.getGenes()[i][j], geneTrack.getGenes()[i][j].getExonBoundaries().length);
  // int mrnaLen = 0;
  // for (int k = 0; k < geneTrack.getGenes()[i][j].getExonBoundaries().length; k++) {
  //
  // Segment exon = new Segment(geneTrack.getGenes()[i][j].getChr(),
  // geneTrack.getGenes()[i][j].getExonBoundaries()[k][0],
  // geneTrack.getGenes()[i][j].getExonBoundaries()[k][1]);
  ...
  private void initializeSummaries() {
    for (int i = 0; i < genes.getLoci().length; i++) {
      ...
      // geneSummaries[i] = new GeneSummary(genes.getLoci()[i].getGeneName(),
      // genes.getLoci()[i].getExonBoundaries().length,
      // genes.getLoci()[i].getNcbiAssessionNumbers().length, totalMrna, mrnaNoUtrs);
    }
  }
  ...
  public void qcByGene() {
    ...
    try {
      ...
      while (reader.ready()) {
        ...
        if (gIndices != null) {
          for (int gIndice : gIndices) {
            String gene = genes.getLoci()[gIndice].getGeneName();
            int numExons = genes.getLoci()[gIndice].getExonBoundaries().length;
  ...
  // Segment curExon = new Segment(genes.getLoci()[gIndices[i]].getChr(),
  // genes.getLoci()[gIndices[i]].getExonBoundaries()[j][0],
  // genes.getLoci()[gIndices[i]].getExonBoundaries()[j][1]);
  // if (seqSeg.overlaps(curExon)) {
