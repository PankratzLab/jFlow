...
public class BlastContamination {
	private WorkerTrain<BlastResultsSummary[]> train;
	private Hashtable<String, Integer> taxonCounts;

	public BlastContamination(String currentDB, WorkerTrain<BlastResultsSummary[]> train, Logger log) {
		super();
		this.train = train;
		this.taxonCounts = new Hashtable<String, Integer>();
	}
	...
	public void runContam() {
		while (train.hasNext()) {
			...
			for (int i = 0; i < summaries.length; i++) {
				Set<String> hits = summaries[i].getHitCounts().keySet();
				for (String hit : hits) {
					if (!taxonCounts.containsKey(hit)) {
						taxonCounts.put(hit, 0);
					}
					int cur = taxonCounts.get(hit);
					taxonCounts.put(hit, cur + summaries[i].getHitCounts().get(hit));
				}
			}
		}
	}
	...
	public static void setup(String dbDir, String fastaqDir, String fileOfFastqs, int numReads, int numReadsPerThread, int blastWordSize, int reportWordSize, int numSampThreads, String outputDir, String outputRoot, int numBatches, int memoryInMB, int wallTimeInHours, Logger log) {
		...
		if (numBatches > 1) {
			batchIt(dbDir, numReads, numReadsPerThread, blastWordSize, reportWordSize, numSampThreads, outputDir, outputRoot, numBatches, memoryInMB, wallTimeInHours, log, fastaqs);

		} else {
			String outputFile = outputDir + outputRoot;
			Logger newLog = new Logger(ext.rootOf(outputFile, false) + ".log");
			runContams(fastaDbs, numReads, fastaqs, numSampThreads, numReadsPerThread, blastWordSize, reportWordSize, outputFile, newLog);
		}

	}

	private static void runContams(String[] fastaDbs, int numReads, String[] fastaqs, int numSampThreads, int numReadsPerThread, int blastWordSize, int reportWordSize, String outputFile, Logger log) {
		...
		for (int i = 0; i < fastaqs.length; i++) {
			...
			for (int j = 0; j < fastaDbs.length; j++) {
				log.reportTimeInfo("Currently processing file " + ext.removeDirectoryInfo(fastaqs[i]) + " (" + i + " of " + fastaqs.length + ") .fastqs; (" + j + " of " + fastaDbs.length + ") dbs");
	...
	private static BlastContamination runContam(String fastaDb, int numReads, String fastaq, int numSampThreads, int numReadsPerThread, int blastWordSize, int reportWordSize, Logger log) {
		BlastFastaq bFastaq = new BlastFastaq(fastaq, numReads, fastaDb, blastWordSize, reportWordSize, numReadsPerThread, log);
		WorkerTrain<BlastResultsSummary[]> train = new WorkerTrain<BlastResultsSummary[]>(bFastaq, numSampThreads, 10, log);
	...
	private static void batchIt(String dbDir, int numReads, int numReadsPerThread, int blastWordSize, int reportWordSize, int numSampThreads, String outputDir, String outputRoot, int numBatches, int memoryInMB, int wallTimeInHours, Logger log, String[] fastaqs) {
		...
		command += "\njava -cp " + PSF.Java.GENVISIS + " -Xmx" + memoryInMB + "m seq.qc.contamination.BlastContamination" + PSF.Ext.SPACE;
		...
		Files.qsub("Contam" + outputRoot, command, batches, memoryInMB, wallTimeInHours, numSampThreads);
	}
	...
	public static void main(String[] args) {
		...
		usage += "   (1) full path to directory containing nt* database files (i.e. " + DB_DIR + dbDir + " (default))\n" + "";
		usage += "   (2) full path to directory containing *.fastq files (i.e. " + FASTQ_DIR + " (no default))\n" + "";
		usage += "   (3) number of reads to blast from each *.fastq file (i.e. " + NUM_READS + numReads + " (default))\n" + "";
		usage += "   (4) number of reads to blast on each thread (i.e. " + NUM_READS_THREAD + numReadsPerThread + " (default))\n" + "";
		usage += "   (5) blast word size to intialize search (i.e. " + BLAST_WORD_SIZE + blastWordSize + " (default))\n" + "";
		usage += "   (6) blast word size to report (i.e. " + REPORT_WORD_SIZE + reportWordSize + " (default))\n" + "";
		usage += "   (7) full path to output directory (i.e. " + OUTPUT_DIR + outputDir + " (default))\n" + "";
		usage += "   (8) root output (relative to output directory) (i.e. " + OUTPUT_ROOT + outputRoot + " (default))\n" + "";
		usage += "   (9) number of batches to break up the job into (i.e. " + NUM_BATCHES + numBatches + " (default))\n" + "";
		...
		usage += "   (13) full path to a file listing full paths of fastqs to use (i.e. " + FASTQ_LIST_FILE + fileOfFastqs + " (default))\n" + "";
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				System.err.println(usage);
				System.exit(1);
			} else if (args[i].startsWith(DB_DIR)) {
				dbDir = ext.parseStringArg(args[i], "");
				numArgs--;
			} else if (args[i].startsWith(FASTQ_DIR)) {
				fastaqDir = ext.parseStringArg(args[i], "");
				numArgs--;
			} else if (args[i].startsWith(OUTPUT_ROOT)) {
				outputRoot = ext.parseStringArg(args[i], "");
				numArgs--;
			} else if (args[i].startsWith(FASTQ_LIST_FILE)) {
				fileOfFastqs = ext.parseStringArg(args[i], "");
				numArgs--;
			} else if (args[i].startsWith(OUTPUT_DIR)) {
				outputDir = ext.parseStringArg(args[i], "");
				numArgs--;
			} else if (args[i].startsWith(NUM_READS)) {
				numReads = ext.parseIntArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith(NUM_READS)) {
				numReads = ext.parseIntArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith(NUM_READS_THREAD)) {
				numReadsPerThread = ext.parseIntArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith(BLAST_WORD_SIZE)) {
				blastWordSize = ext.parseIntArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith(REPORT_WORD_SIZE)) {
				reportWordSize = ext.parseIntArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith(NUM_BATCHES)) {
				numBatches = ext.parseIntArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith(PSF.Ext.NUM_THREADS_COMMAND)) {
				numSampThreads = ext.parseIntArg(args[i]);
				numArgs--;
		...
		try {
			setup(dbDir, fastaqDir, fileOfFastqs, numReads, numReadsPerThread, blastWordSize, reportWordSize, numSampThreads, outputDir, outputRoot, numBatches, memoryInMB, wallTimeInHours, new Logger());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
// String[] fastaDbs = Files.toFullPaths(Files.list(dbDir, "nt.", "nsq", true, false), dbDir);
// String fastaqDir = "/panfs/roc/data_release/2/bmgc/tsaim/hiseq/141126_SN261_0580_AC5R7WACXX/Project_Tsai_Project_021/";
...
// Hashtable<String, Integer> allCounts = new Hashtable<String, Integer>();
// for (int j = 0; j < fastaDbs.length; j++) {
// log.reportTimeInfo(i + " of " + fastaqs.length + ".fastqs, " + j + " of " + fastaDbs.length + " dbs");
// Hashtable<String, Integer> curCounts = runContam(ext.rootOf(fastaDbs[j], false), fastaqs[i], 10, log).getTaxonCounts();
// allCounts = ext.addHashCounts(allCounts, curCounts);
// }
// popCounts.add(allCounts);
// }
