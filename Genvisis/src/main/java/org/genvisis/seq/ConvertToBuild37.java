...
//		long time;
		...
		for (int i = 0; i < files.length; i++) {
			lines = HashVec.loadFileToStringArray(dir+files[i], false, false, new int[] {0}, true, false, ",");
		...
		if (new File(dir+"liftOver.in").exists() && new File(dir+"liftOver.bed").exists()) {
			...
			conversions = HashVec.loadFileToStringArray(dir+"liftOver.bed", false, new int[] {0}, false);
		...
		for (int i = 0; i < files.length; i++) {
			try {
				reader = new BufferedReader(new FileReader(dir+files[i]));
				writer = new PrintWriter(new FileWriter(dir+"inputs/"+files[i].substring(0, files[i].lastIndexOf("_"))+"_input.txt"));
				while (reader.ready()) {
					temp = reader.readLine();
					line = temp.trim().split("[\\s]+");
					if (!line[0].startsWith("#")) {
						if (line.length < 5) {
							System.err.println("Error - invalid line in '"+files[i]+"': "+temp);
						}
						loc = Positions.getUCSCformat(new String[] {line[1], line[2], line[2]});
						...
						if (err.containsKey(loc)) {
							...
							writer.println("chr"+pos[0]+"\t"+pos[1]+"\t"+pos[2]+"\t"+line[3]+"\t"+line[4]);
						}
					}
				}
				reader.close();
