...
public class AIMsPicker {
	private static void pick(String filename, int minDistance, double minScore, Logger log) {
		...
		try {
			...
			ext.checkHeader(line, new String[] {"Cluster", "Variant", "region", "gene", "pval", "Chr", "Position", "Score"}, true);
			while (reader.ready()) {
				line = reader.readLine().trim().split("[\\s]+");
		...
		while (count != picks.length && round < 100) {
			for (int i = 0; i < picks.length; i++) {
				if (picks[i] == null) {
					...
					chr = Byte.parseByte(line[5]);
					position = Integer.parseInt(line[6]);
					picks[i] = line[1]+"\t"+chr+"\t"+position+"\t"+line[7]+"\t"+line[0]+"\t"+(pick+1)+" of "+cluster.size();
					chrs[i] = chr;
