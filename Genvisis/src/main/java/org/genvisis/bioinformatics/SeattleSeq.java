...
//	public static final String[] BAD = {"missense", "stop-gained", "stop-lost", "missense-near-splice", "splice-donor", "splice-acceptor"};
	...
	// there are triallelic markers on the exome chip, hence the Hashtable, but that doesn't necessarily capture all, still missing a little over a hundred
...
//			if (temp.startsWith("#"));
	...
	public static Hashtable<String, String[]> loadAllBadAnnotationInDir(String directory, Logger log) {
		BufferedReader reader;
		String[] files, line;
...
//		Hashtable<String, String[]> hash = loadAllAnnotationInDir("D:/Logan/SuperNovo/SeattleSeq/", new Logger());
...
//		System.exit(1);
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				...
				prep = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("prepFreq=")) {
				prepFreq = args[i].split("=")[1];
				numArgs--;
