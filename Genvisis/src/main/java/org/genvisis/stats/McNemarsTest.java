...
public class McNemarsTest {
	...
	public McNemarsTest(int[][] matrix) {
		...
		for (int i = 0; i<matrix.length; i++) {
			if (matrix[i][0] == -1 || matrix[i][1] == -1) {
				count++;
			} else if (matrix[i][0] == 1 && matrix[i][1] == 1) {
				a++;
			} else if (matrix[i][0] == 1 && matrix[i][1] == 0) {
				b++;
			} else if (matrix[i][0] == 0 && matrix[i][1] == 1) {
				c++;
	...
	public double compute() {
		...
		if (b+c < 20) {
			System.err.println("Error - Since b ("+b+") + c ("+c+") is less than 20"+(variable==null?"":" for variable "+variable)+", a sign test should be used instead of McNemar's test (unfortunately not yet implemented)");
		}
		return Math.pow((double)Math.abs(b-c)-(continuity?1:0), 2)/(double)(b+c);
	}
	...
	public double getPvalue() {
		return (b+c<THRESHOLD?getPvalueFromBinomialDistribution():getPvalueFromMcNemarTestNoMatterWhat()); 
	}
	...
	public static void batch(String dir, String pairs, String database, String[] analysisVariables) {
        ...
        for (int i = 0; i<pairings.length; i++) {
        	for (int j = 0; j<2; j++) {
        		...
        		if (trav == null) {
        			System.err.println("Error - UniqueID '"+pairings[i][j]+"' was not found in the file '"+database+"'");
        		} else {
        ...
        try {
	        ...
	        for (int i = 0; i<analysisVariables.length; i++) {
	    		...
	    		writer.print(analysisVariables[i]+"\t"+count+"\t"+Array.toStr(mt.getABCD())+"\t"+mt.getDirectionOfRisk()+"\t"+mt.getChiSq()+"\t"+mt.getPvalue());
