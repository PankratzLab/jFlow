...
public class Nonparametric {
	...
	public static double runWilcoxonRankSumTest(int[] groupings, double[] values, Logger log) {
		return runWilcoxonRankSumTest(Ttest.splitOut(groupings, values, Array.min(groupings)), Ttest.splitOut(groupings, values, Array.min(groupings)+1), log);
	}
...
//		System.out.println("p1: "+p);
		...
		for (int i = 0; i < counts.length; i++) {
			if (counts[i] > 1) {
				extra += (Math.pow(counts[i], 3)-counts[i])/12;
			}
		}
...
//		System.out.println("p2: "+p);
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(runWilcoxonRankSumTest(Array.toDoubleArray(HashVec.loadFileToStringArray("file1.txt", false, new int[] {0}, false)), Array.toDoubleArray(HashVec.loadFileToStringArray("file2.txt", false, new int[] {0}, false)), log));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
