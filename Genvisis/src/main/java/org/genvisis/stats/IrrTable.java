...
/**
 * Forms the basis of several Inter-rater reliability stats (Percent Agreement,Cohen�s Kappa,Fleiss'
 * kappa, etc) <br>
...
public class IrrTable {
	...
	private final int[][] ratings;// judge,ratings of judge
	private final Hashtable<Integer, Integer>[] judgeTotals;// stores the number of a particular
																													// rating given by a judge
	private final HashSet<Integer> uniqRatings;
	private final Hashtable<String, String> rated;// stores subjects judged already
	...
	/**
	 * @param ratingCategory the actual category, not its index
	 * @return
	 */
	public double getPercentAgreementFor(int ratingCategory) {
		...
		if (totalRatings > 0) {
			percentAgreement = agreements * 2 / totalRatings;
		}
	...
	public boolean parseAgreement() {
		...
		if (totalRated != shouldHave) {
			...
			for (int i = 0; i < ratings[0].length; i++) {// for each sample
				...
				for (int[] rating : ratings) {// check if all judges agree
					if (rating[i] != current) {
						agree = false;
					}
				}
	...
	/**
	 * @param judge
	 * @param judgedRatings must be the same length as initilized values for the number of samples
