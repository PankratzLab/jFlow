...
public class FishersExact {
					...
					for(int a3=0; a3<=c[0]-a1-a2; a3++) {
						...
						for(int a4=0; a4<=c[0]-a1-a2-a3; a4++) {
							...
							for(int a5=0; a5<=c[0]-a1-a2-a3-a4; a5++) {
								work[4][0] = a5;
								work[5][0] = c[0]-work[0][0]-work[1][0]-work[2][0]-work[3][0]-work[4][0];
								for(int b1=0; b1<=c[1]; b1++) {
									work[0][1] = b1;
									for(int b2=0; b2<=c[1]-b1; b2++) {
										work[1][1] = b2;
										for(int b3=0; b3<=c[1]-b1-b2; b3++) {
											...
											for(int b4=0; b4<=c[1]-b1-b2-b3; b4++) {
												...
												for(int b5=0; b5<=c[1]-b1-b2-b3-b4; b5++) {
													work[4][1] = b5;
													work[5][1] = c[1]-work[0][1]-work[1][1]-work[2][1]-work[3][1]-work[4][1];
	...
	public static double workUp(int[][] work, double[] facts, double num, double pex, double[] lookup) {
		double denom, temp;
...
//				0.06782864732521461,
				0.005591506952405223,
				0.027958060654154904,
				0.0019136629015811507,
				0.008846883022512499,
				0.009796226182780747,
				0.007435141888923397,
				0.9887934356789729,
				0.9887934356789694,
		};
		int[][][] tests = {
...
//				{{26,70,52}, {21,119,114}},
//				{{1, 2, 6}, {8, 7, 3}},
//				{{14, 19, 12}, {13, 27, 8}, {40, 22, 9}}, // 0.005591506952405223 time=7.37 sec, then 2.0 sec, now 0.72 sec and then 0.62
...
//		System.out.println("Generic FishersExact");
        ...
        time = new Date().getTime();
			...
			System.out.print("\tExact: "+d+"\t"+(Math.abs(d-probs[i])< 0.000000000000001?"checks out":"FAILED!!!"));
			System.out.print(" in "+ext.getTimeElapsed(time));
			System.out.println();
			System.out.println("Chi Sq: "+ProbDist.ChiDist(ContingencyTable.ChiSquare(matrix, false), (matrix.length-1)*(matrix[0].length-1)));
        }
    }
}
