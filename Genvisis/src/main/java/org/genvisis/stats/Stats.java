...
public class Stats {
	...
	public static double PearsonChiSquare(double a, double b, double c, double d) {
		...
		return Math.pow(a-aExp, 2.0)/aExp+Math.pow(b-bExp, 2.0)/bExp+Math.pow(c-cExp, 2.0)/cExp+Math.pow(d-dExp, 2.0)/dExp;
	}
	...
	public static double FishersExact(double a, double b, double c, double d, boolean oneTailed) {
		...
		for (int i = 1; i<=a+b+c+d; i++) {
			LogFactorial[i] = LogFactorial[i-1]+Math.log((double)i);
		}
		if (a*d>b*c) {
			a = a+b;
			b = a-b;
