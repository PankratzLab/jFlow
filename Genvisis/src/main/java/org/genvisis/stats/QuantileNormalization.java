		...
		if (dataToNorm == null || dataToNorm.length < 2) {
			...
			for (int i = 0; i < dataToNorm[0].length; i++) {
				...
				if (numNan != 0 && numNan != tmp.length) {
					String error = i
												 + "\tAll indices must either have complete (non-NaN) data, or all NaN data "
