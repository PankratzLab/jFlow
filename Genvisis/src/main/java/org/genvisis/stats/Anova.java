...
public class Anova {
	...
	private double Q05_3g = 3.31; // if n > 200 then Q is fixed at 3.31 for 3
	...
	public void oneway() throws IOException {
		...
		for (int i = 0; i<data.length; i++) {
			...
			groupMeans[i] = ((double)groupSums[i])/((double)data[i].length);
			groupCounts[i] = data[i].length;
			calc = ((double)groupSums[i])*((double)groupSums[i])/(double)data[i].length;
			SSb += calc;
			SSw += ((double)groupSumSquares[i])-calc;
			sumOfSums += groupSums[i];
			totalN += data[i].length;

		}
		SSb -= ((double)sumOfSums)*((double)sumOfSums)/((double)totalN);
		dfN = data.length-1;
		dfD = totalN-data.length;
		Fstat = (SSb/dfN)/(SSw/dfD);
		MSE = SSw/dfD;
		LSD05 = ProbDist.TDistReverse(0.05, totalN)*Math.sqrt(2*MSE)/Math.sqrt(totalN/data.length);
		LSD01 = ProbDist.TDistReverse(0.01, totalN)*Math.sqrt(2*MSE)/Math.sqrt(totalN/data.length);
	...
	public String report() throws IOException {
		...
		if (!analysis_complete) {
			...
			list += "f = "+ext.formDeci(Fstat, 5, true)+" with "+dfN+" df in numerator, "+dfD+" df in denominator\n";
			...
			for (int i = 0; i<data.length; i++) {
				for (int j = i+1; j<data.length; j++) {
					list += "Mean diff b/w "+groupNames.elementAt(i)+" & "+groupNames.elementAt(j)+" = "+ext.formDeci(groupMeans[i]-groupMeans[j], 2, true)+((Math.abs(groupMeans[i]-groupMeans[j])>LSD05)?"*":"")+((Math.abs(groupMeans[i]-groupMeans[j])>LSD01)?"*":"")+"\n";
				}
			}
