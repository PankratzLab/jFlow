...
public class ContingencyTable {
	...
	public static double[] relativeRiskCI(double[][] data, double alpha) { // for
		...
		p1 = (double)data[0][0]/(double)(data[0][0]+data[0][1]);
		p2 = (double)data[1][0]/(double)(data[1][0]+data[1][1]);
		n1 = Array.sum(data[0]);
		n2 = Array.sum(data[1]);
	...
	public static double oddsRatioSE(double[][] data) {
		...
		return Math.sqrt(1/(double)data[0][0]+1/(double)data[0][1]+1/(double)data[1][0]+1/(double)data[1][1]);
	}
	...
	public static double ChiSquare(double[][] data, boolean verbose, boolean possibleYatesCorrection) {
		double[][] expecteds;
		double chi;
		...
		if ((double)numCellsLT5/(double)numCells>=0.20) {
			if (verbose) {
				System.out.println("FYI, "+numCellsLT5+" cells ("+ext.formDeci((double)numCellsLT5/(double)numCells, 1, true)+"%) have expected count less than 5. The minimum expected count is "+ext.formDeci(minExpected, 2, true)+". Might consider using Fisher's exact test"+(possibleYatesCorrection?"; in the meantime using a Yates continuity correction":""));
			}
			useYates = true;
		}
		...
		for (int i = 0; i<data.length; i++) {
			for (int j = 0; j<data[i].length; j++) {
				diff = Math.abs((data[i][j]-expecteds[i][j])) - (useYates&&possibleYatesCorrection?0.5:0);
	...
	public static double[][] residuals(double[][] data) {
		...
		for (int i = 0; i<data.length; i++) {
			for (int j = 0; j<data[i].length; j++) {
				residuals[i][j] = (data[i][j]-expecteds[i][j])/Math.sqrt(expecteds[i][j]*(1-rowSums[i]/total)*(1-colSums[j]/total));
			}
		}
	...
	public static double binomialCoeff(int a, int b) {
		...
		d = BinomialDistribution.fact(a)/(BinomialDistribution.fact(b)*BinomialDistribution.fact(a-b));
...
//				{38.51666667, 29.5}};
		...
		System.out.println("Group 1"+ext.formStr(data[0][0]+"", 10)+"\t"+ext.formStr(data[0][1]+"", 10));
		System.out.println("Group 2"+ext.formStr(data[1][0]+"", 10)+"\t"+ext.formStr(data[1][1]+"", 10));

		System.out.println("p1 = "+ext.formDeci((double)data[0][0]/(double)(data[0][0]+data[0][1]), 4, true));
		System.out.println("p2 = "+ext.formDeci((double)data[1][0]/(double)(data[1][0]+data[1][1]), 4, true));
		System.out.println();
		ci = diffCI(data);
		System.out.println("The difference of proportions is "+ext.formDeci(diffProportions(data), 4, true)+" ("+ext.formDeci(ci[0], 4, true)+", "+ext.formDeci(ci[1], 4, true)+")");
		System.out.println();
		ci = relativeRiskCI(data);
		System.out.println("The relative risk is "+ext.formDeci(relativeRisk(data), 4, true)+" ("+ext.formDeci(ci[0], 4, true)+", "+ext.formDeci(ci[1], 4, true)+")");
		System.out.println();
		ci = oddsRatioCI(data);
		System.out.println("The odds ratio is "+ext.formDeci(oddsRatio(data), 4, true)+" ("+ext.formDeci(ci[0], 4, true)+", "+ext.formDeci(ci[1], 4, true)+")");
		System.out.println();
		...
		System.out.println("The Pearson (score) chi-square statistic is "+ext.formDeci(ChiSquare(data), 4, true)+" (p="+ProbDist.ChiDist(ChiSquare(data), (data.length-1)*(data[0].length-1))+")");
		System.out.println("The Likelihood-Ratio chi-square statistic is "+ext.formDeci(likelihoodRatioStatistic(data), 4, true)+" (p="+ProbDist.ChiDist(likelihoodRatioStatistic(data), (data.length-1)*(data[0].length-1))+")");
		System.out.println();
		System.out.println("The residuals are:");
		residuals = residuals(data);
		for (int i = 0; i<residuals.length; i++) {
			for (int j = 0; j<residuals[0].length; j++) {
				System.out.print((j==0?"":"\t")+ext.formDeci(residuals[i][j], 3));
			}
			System.out.println();
		}
		System.out.println();
		...
		System.out.println("The linear trend test statistic is "+ext.formDeci(linearTrendStatistic(data), 3)+" (p="+ext.prettyP(ProbDist.ChiDist(linearTrendStatistic(data), 1))+")");
		System.out.println("Using midranks:");
		System.out.println(Array.toStr(midRanks(computeColSums(data))));
		System.out.println("The linear correlation is "+ext.formDeci(linearCorrelation(data, midRanks(computeRowSums(data)), midRanks(computeColSums(data))), 5));
		System.out.println("The linear trend test statistic is "+ext.formDeci(linearTrendStatistic(data, midRanks(computeRowSums(data)), midRanks(computeColSums(data))), 3)+" (p="+ext.prettyP(ProbDist.ChiDist(linearTrendStatistic(data, midRanks(computeRowSums(data)), midRanks(computeColSums(data))), 1))+")");
		System.out.println();
	...
	public static double ChiSquareOptimizedSignificance(double[][] data, boolean twosided, boolean verbose) {
		double[][] expecteds;
		int numCellsLT5, numCells;
		double minExpected = 5;
		boolean fishy = false;
		...
		numCells = data.length*data[0].length;
		if ((double)numCellsLT5/(double)numCells>=0.20) {
			System.out.print(numCellsLT5+" cell"+(numCellsLT5>1?"s have":"  has ")+" an expected count less than 5 (min="+ext.formDeci(minExpected, 2, true)+"); ");
			if (numCells==4) {
