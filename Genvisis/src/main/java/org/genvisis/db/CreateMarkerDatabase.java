...
public class CreateMarkerDatabase {
	...
	public static final String[] FIELDS = {"!UniqueID", "!FamID", "!IndID", "!Source", ".AOO", "!AgeAtExam", ".Dx", ".VPD", "!Affected", "!Male", "!Caucasian", ".AffParent", "0parkin", "1noKnownHomozygousParkinMutation", "0G2019S", "0AnyLRRK2", "0CausativeMutation", "!Use", "!Comment"};
	public static final String[] DATABASES = {"C:\\Documents and Settings\\npankrat\\My Documents\\1_CRFdb\\crf_db.dat", "C:\\Documents and Settings\\npankrat\\My Documents\\tWork\\Global PD files\\control.db.xln"};
	public static final String[] POST_HOCS = {"Age:AOO|AgeAtExam|AgeAtExam|AgeAtExam:Use=1", "VPD_Analysis:VPD|1|1|1:Use=1", "ALL_Analysis:Affected|1|1|1:Use=1"};
	public static final String UNIQUEID = "UniqueID";

	public static void createFromParameters(String filename, Logger log) {
		Vector<String> paramV;

		paramV = Files.parseControlFile(filename, "db", new String[] {"file1.csv ColumnHeaderForUniqueID ColumnHeader1 ColumnHeader3 ColumnHeader5", "file2.csv ColumnHeaderForUniqueID ColumnHeader2 ColumnHeader3	ColumnHeader9","file3.csv ColumnHeaderForUniqueID ColumnHeader9","..."}, log);
		if (paramV != null) {
	...
	public static void create(String outfile, Vector<String> infoV) {
		...
		for (int i = 0; i<info.length; i++) {
			...
			try {
				...
				if (header.length==1) {
					System.err.println("Error - CreateMarkerDatabase currently requires comma delimited files");
					System.exit(1);
				}
				if (info[i].length==1) {
					...
					if (index==-1) {
						System.err.println("Error - "+UNIQUEID+" was not found as a column header in "+markerName);
					}
				...
				for (int j = 0; j<indices.length; j++) {
					trav = info[i][j+1].contains("|")?info[i][j+1].substring(0, info[i][j+1].indexOf("|")):info[i][j+1];
					indices[j] = ext.indexOfStr(trav, header);
					if (indices[j]==-1) {
						System.err.println("Error - '"+trav+"' is not a column header in file "+markerName);
						System.exit(1);
					}
				}
				while (reader.ready()) {
					...
					if (data[i]!=null) {
						System.err.println("Error - duplicate data in file '"+markerName+"' for "+line[indices[0]]);
					}
					data[i] = dataline;
				}
				reader.close();
		...
		try {
			...
			for (int i = 0; i<info.length; i++) {
				markerName = info[i][0].substring(0, info[i][0].lastIndexOf("."));
				for (int j = 2; j<info[i].length; j++) {
					trav = info[i][j].contains("|")?info[i][j].substring(info[i][j].indexOf("|")):info[i][j];
			...
			for (int j = 0; j<POST_HOCS.length; j++) {
				writer.print("\t"+POST_HOCS[j].split(":")[0]);
			}
			writer.println();
				...
				try {
					...
					for (int j = 0; j<fields.length; j++) {
						...
						if (required[j]&&indices[j]==-1) {
							System.err.println("Error - '"+fields[j]+"' is not a column header in file "+DATABASES[i]+" but is declared as being required");
							flag = true;
						}
					}
					if (flag) {
					...
					while (reader.ready()) {
						...
						if (hash.containsKey(line[indices[0]])) {
							data = hash.get(line[indices[0]]);
							if (data.length==0) {
								System.err.println("Error - multiple refrences to '"+line[indices[0]]+"' in "+DATABASES[i]+" and/or earlier databases");
							} else {
								...
								for (int j = 0; j<data.length; j++) {
									if (data[j]==null) {
										...
										writer.print("\t"+Array.toStr(data[j]));
									}
								}
								if (flag) {
									System.err.println("Warning - '"+line[indices[0]]+"' is missing data for at least one of the markers.");
								}
								for (int j = 1; j<FIELDS.length; j++) {
									writer.print("\t"+(indices[j]==-1||(line[indices[j]].equals(".")&&!FIELDS[j].substring(0, 1).equals("!"))?FIELDS[j].substring(0, 1):line[indices[j]]));
								}
								for (int j = 0; j<POST_HOCS.length; j++) {
									postHoc = POST_HOCS[j].trim().split(":");
									requirement = postHoc[2].trim().split("=");
									if (ext.indexOfStr(requirement[0], header)==-1) {
										System.err.println("Error - '"+requirement[0]+"' is a requirement for posthoc "+postHoc[0]+", but is not in header of "+DATABASES[i]);
										System.exit(1);
									}
