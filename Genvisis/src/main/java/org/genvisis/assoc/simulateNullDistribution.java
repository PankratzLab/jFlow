...
// make sure to trim it to only contain the individuals you want to permute (i.e. VPD, no caustitive mutation, no missing data)
...
public class simulateNullDistribution {
	...
	public simulateNullDistribution(String filename, String outputfile, int reps, boolean controlFreqs, int famCol, int indCol, int affCol, int allCol, String targets) {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (!line[allCol+0].equals(".")&&!line[allCol+0].equals("0")&&!line[allCol+1].equals(".")&&!line[allCol+1].equals("0")) {
		...
		try {
			...
			while (!done) {
				...
				if (done||!line[0].equals(prev)) {
					if (vips.containsKey(prev)) {
						...
						for (int i = 0; i<v.size(); i++) {
							...
							writer.println(Array.toStr(trav, "\t")+"\t"+(affs.containsKey(trav[1])?(Integer.parseInt(affs.get(trav[1]))+1)+"\t1\t1":"0\t0\t0")+"");
						}
					}
				...
				pre.add(line[0]+"\t"+line[1]+"\t"+line[4]+"\t"+line[5]+"\t"+(line[2].equals("F")?"2":(line[2].equals("M")?"1":"0")));
				prev = line[0];
			}
			fams = HashVec.getKeys(vips);
			for (int i = 0; i<fams.length; i++) {
				v = vips.get(fams[i]);
				affs = phenos.get(fams[i]);
				if (v.size()>1) {
					System.err.println("Error - more than one individual found in a family not found in ninfo2: "+fams[i]+"\n"+"        The following indivuals will be listed as unrelated: "+Array.toStr(Array.toStringArray(v), ",")+"");
				}
				for (int j = 0; j<v.size(); j++) {
					writer.println(fams[i]+"\t"+v.elementAt(j)+"\t0\t0\t2\t"+(affs.containsKey(v.elementAt(j))?(Integer.parseInt(affs.get(v.elementAt(j)))+1)+"\t1\t1":"0\t9\t9")+"");
				}
		...
		System.out.println("Allele frequencies were permuted one per family ("+numParticipants+" from "+fams.length+" families)");
		...
		new LinkageMap(1, Array.stringArraySequence(REP_STEP, "Rep"), alleleFreqs, Array.doubleArray(REP_STEP, 50), false, false).createFile("map.dat");
		...
		for (int i = 0; i<reps/REP_STEP; i++) {
			...
			try {
				...
				while (reader.ready()) {
					...
					for (int j = 0; j<temp.length(); j++) {
						writer.print((int)temp.charAt(j)==32?"\t":((int)temp.charAt(j)<10?(char)((int)temp.charAt(j)+48):temp.charAt(j)));
					}
					writer.println();
				}
				reader.close();
				...
				System.err.println("Error: file \""+outputfile+".1"+"\" not found in current directory");
				System.exit(1);
			} catch (IOException ioe) {
			...
			try {
				...
				System.err.println("Error: file \""+"tr_"+outputfile+".1"+"\" not found in current directory");
				System.exit(1);
			} catch (IOException ioe) {
	...
	public static Hashtable<String,Vector<String>> cloneHash(Hashtable<String,Vector<String>> hash) {
		...
		for (int i = 0; i<keys.length; i++) {
			clone.put(keys[i], hash.get(keys[i]));
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i<args.length; i++) {
			if (args[i].equals("-h")||args[i].equals("-help")||args[i].equals("/h")||args[i].equals("/help")) {
				...
				famIDcol = Integer.parseInt(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("ind=")) {
				indIDcol = Integer.parseInt(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("aff=")) {
				affCol = Integer.parseInt(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("allele=")) {
				alleleCol = Integer.parseInt(args[i].split("=")[1]);
				numArgs--;
			} else if (args[i].startsWith("targets=")) {
				targets = args[i].split("=")[1];
				numArgs--;
			}
		}
		if (numArgs!=0) {
		...
		try {
			new simulateNullDistribution(filename, outputfile, numReps, controlFreqs, famIDcol-1, indIDcol-1, affCol-1, alleleCol-1, targets);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
