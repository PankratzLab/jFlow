...
public class haplostats {
	...
	public haplostats(String filename) throws IOException {
		...
		while (reader.ready()) {
			...
			if (data.length%2!=0) {
				System.err.println("Error - Expecting "+(line.length-6)+" columns (including affection status) followed by an even number of alleles");
				System.exit(4);
			}
			score = 0;
			...
			if (score==8&&line[5].equals("2")) {
				...
				dude = new haploguy(""+(Integer.valueOf(line[0]).intValue()*1000+Integer.valueOf(line[1]).intValue()), line[4], data);
				v.add(dude);
				hash.put(line[0], v);
			}
		}
