...
public class haploGatherPerms {
	public haploGatherPerms(int numReps) throws IOException {
			...
			if (new File("bintests/"+i+"_bintest.out").exists()) {
				...
				if (line.length<6) {
					...
					if (df==-1) {
						...
						df = Integer.valueOf(line[5].substring(0, line[5].length()-1)).intValue();
						System.err.println("Uh oh - ned to redo reps 1-"+i+", cause they have less df than subsequent runs.");
					}
				}
				reader.close();
			} else {
		...
		System.out.println();
		System.err.println();
		System.err.println("Warning - there are "+count+" missing replicates. Run gatherAdditionalPerms to rerun just these replicates.");
		System.err.println("Warning - there are "+df_probs+" replicates, that dropped haplotypes. Run gatherAdditionalPerms to rerun just these replicates.");
		...
		temp = "Mean stat: "+ext.formDeci(average, 3, true)+" with df="+df+" (p="+ProbDist.ChiDist(average, df)+")\n";
	...
	public void specialPerm(int i) {
		...
		writer.println("bintest."+i+"<-haplo.score(affstat."+i+", geno, trait.type=\"binomial\")");
	...
	public static void main(String[] args) throws IOException {
		...
		String usage = "\n"+"park.haploGatherPerms requires 1 arguments:\n"+"   (1) the number of replicates to perform (i.e. reps="+numReps+" (default))\n"+"";
