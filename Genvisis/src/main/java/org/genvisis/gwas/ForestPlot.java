    ...
    try {
      ...
      if (studies == null) {
        ...
        for (String[] studie : studies) {
          if (ext.indexOfStr(studie[0], roots) == -1) {
            log.reportError("Error - there is no root called '" + studie[0] + "' in the datafile");
            problem = true;
          }
        }
        if (problem) {
        ...
        if (markers == null || markers.contains(line[markerIndex])) {
          ...
          try {
            ...
            for (int i = 0; i < studies.length; i++) {
              ...
              //// +"\t"+Array.toStr(Array.subArray(line,
              // indices[i])));(studies[i].length==1||studies[i][1]==null?studies[i][0]:studies[i][1])+"\t"+line[markerIndex]+"\t"+Array.toStr(Array.subArray(line,
              // indices[i])));
              // writer.println((studies[i].length==1||studies[i][1]==null?studies[i][0]:studies[i][1])+"\t"+line[markerIndex]+"\t"+Array.toStr(Array.subArray(line,
              // indices[i])));
              // writer.println((studies[i].length==1||studies[i][1]==null?studies[i][0]:studies[i][1])+"\t"+line[markerIndex]+"\t"+Array.toStr(Array.subArray(line,
              // indices[i])));
