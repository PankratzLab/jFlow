			...
			try {
				...
				if (errorMsg.equals("")) {
					if (indices[1] == -1 || indices[2] == -1) {
						...
						MapSNPsAndGenes.procSNPsToGenes(ext.parseDirectoryOfFile(filename), ext.rootOf(filename, true) + ".snps", MapSNPsAndGenes.DEFAULT_WIGGLE_ROOM, (byte) 37, log, true, false, false, null, null, null, false); // TODO should run SnpEff too?
