...
public class GeneScorePipeline {
      ...
      if (dataSources.size() == 0) {
        ...
        if (dataSources.size() > 1) {
          for (int i = 1; i < dataSources.size(); i++) {
            ...
            d0 = DosageData.combine(d0, d1, DosageData.COMBINE_OP.EITHER_IF_OTHER_MISSING, log);
            System.gc();
          }
        }
        if (d0.isEmpty()) {
  ...
  private static HashMap<String, Double> get1000GFreq(HashMap<String, int[]> markerMap) {
    ...
    for (Integer chr : mkrsByChr.keySet()) {
      ...
      for (String nonRSMkr : nonRS) {
        ...
        String mkrNm =
            pos > 0 && pos < posRS.length && null != posRS[pos] && !"".equals(posRS[pos]) ? posRS[pos]
                : "";
        plinkFreqs.put(nonRSMkr,
            "".equals(mkrNm) ? 0.0 : Double.valueOf(rsToFullData.get(mkrNm).split("\t")[1]));
      }
  ...
  public static void preprocessDataFiles(String[] files) {
    ...
    factors =
        new String[][] {Aliases.MARKER_NAMES, Aliases.CHRS, Aliases.POSITIONS, Aliases.PVALUES,
            Aliases.ALLELE_FREQS, Aliases.EFFECTS};
    for (String filename : files) {
      try {
        ...
        if (indices[5] == -1) {
          ...
          errorMsg =
              errorMsg.equals("") ? "ERROR - no Beta/Effect column found" : errorMsg
                  + "; no Beta/Effect column found";
        }
        if (indices[3] == -1) {
          // NO PVALUES! COMPLETE FAIL
          errorMsg =
              errorMsg.equals("") ? "ERROR - no P-Value column found" : errorMsg
        ...
        if (errorMsg.equals("")) {
          if (indices[1] == -1 || indices[2] == -1) {
            ...
            snps =
                HashVec.loadFileToStringArray(filename, false, true, new int[] {indices[0]}, false);// fileData.keySet().toArray(new
            ...
            MapSNPsAndGenes.procSNPsToGenes(ext.parseDirectoryOfFile(filename),
                ext.rootOf(filename, true) + ".snps", MapSNPsAndGenes.DEFAULT_WIGGLE_ROOM,
                (byte) 37, log, true, false, false, null, null, null, false); // TODO
                                                                              // should
                                                                              // run
                                                                              // SnpEff
                                                                              // too?
            ...
            for (String element : data) {
              ...
              markerMap.put(line[0],
                  new int[] {Positions.chromosomeNumber(line[1]),
                      ext.isMissingValue(line[2]) ? -1 : Integer.parseInt(line[2])});
            }
          } else {
          ...
          if (indices[4] == -1) {
            ...
            if (markerMap.isEmpty()) {
              fileData =
                  HashVec.loadFileToHashVec(filename, indices[0],
                      new int[] {indices[1], indices[2]}, "\t", true, false);
              for (String key : fileData.keySet()) {
                markerMap.put(key,
                    new int[] {Positions.chromosomeNumber(fileData.get(key).get(0).split("\t")[0]),
          ...
          while ((temp = reader.readLine()) != null) {
            ...
            String freq =
                indices[4] == -1 ? ""
                    + (freqs == null || freqs.isEmpty() || freqs.get(snp) == null ? 0.0 : freqs
                        .get(snp)) : line[indices[4]];
   ...
   */float[] indexThresholds, int[] windowMins, float[] windowExtThresholds, Logger log) {
    this.log = log;
      ...
      for (int m : windowMinSizePerSides) {
        for (float w : windowExtensionThresholds) {
          ...
          prefixSB.append(ext.formSciNot(i, 4, false)).append("_")
              .append(ext.formSciNot(m, 4, false)).append("_").append(ext.formSciNot(w, 4, false));
  ...
  private void runMetaHitWindowsAndLoadData() {
    String[][] factors =
        new String[][] {Aliases.MARKER_NAMES, Aliases.EFFECTS, Aliases.ALLELE_FREQS,
            Aliases.PVALUES};
    ...
    for (String dFile : dataFiles) {
      ...
      for (java.util.Map.Entry<String, Constraint> filePrefix : analysisConstraints.entrySet()) {
        ...
        if (hitMkrSet.isEmpty()) {
          String[] mkrs =
              HashVec.loadFileToStringArray(metaDir + dFile, true, new int[] {0}, false);
          if (mkrs == null) {
        ...
        if (!hitMkrSet.isEmpty()) {
          ...
          try {
            ...
            int[] indices =
                ext.indexFactors(factors, temp, false, false, true, true, new Logger(), false);
            while ((line = reader.readLine()) != null) {
            ...
            for (Study study : studies) {
              ...
              HashSet<String> bimMkrSet =
                  study.retrieveMarkers(dataFile + "\t" + filePrefix.getKey(), hitMkrSet);
    ...
    try {
      for (int i = 0; i < study.dataSources.size(); i++) {
        ...
        while ((temp = reader.readLine()) != null) {
          ...
          String affLine =
              line[0] + "\t" + line[1] + "\t"
                  + (ext.isMissingValue(line[5]) ? "." : -1 * (Integer.parseInt(line[5]) - 2))
                  + "\t" + line[4];
  ...
  private double getBetaFreqScore(HashMap<String, double[]> markerMap) {
    ...
    for (java.util.Map.Entry<String, double[]> entry : markerMap.entrySet()) {
      double score =
          2 * (entry.getValue()[0] * entry.getValue()[0]) * entry.getValue()[1]
              * (1 - entry.getValue()[1]);
      sum += score;
    }
    return sum;
    ...
    for (String dFile : dataFiles) {
      ...
      for (java.util.Map.Entry<String, Constraint> constraintEntry : analysisConstraints.entrySet()) {
        String crossFilterFile =
            study.studyDir + dataFile + "/" + constraintEntry.getKey() + "/"
                + CROSS_FILTERED_DATAFILE;
        if ((new File(crossFilterFile).exists())) {
          log.report(ext.getTime() + "]\tCross-filtered data file already exists! [ --> '"
              + crossFilterFile + "']");
          study.hitSnpCounts.get(constraintEntry.getKey()).put(dataFile,
              Files.countLines(crossFilterFile, 1));
          continue;
        }
        ...
        SnpMarkerSet markerSet =
            study.data.get(dataFile + "\t" + constraintEntry.getKey()).getMarkerSet();
        int cntAmbig = 0;
        ...
        while ((line = dataReader.readLine()) != null) {
          ...
          if (mkrsBim.containsKey(parts[0])
              && (ind == -1 || (ext.isValidDouble(parts[ind]) && Double.parseDouble(parts[ind]) < constraintEntry
                  .getValue().indexThreshold))) {
  ...
  private void runHitWindows(Study study) {
    for (String dFile : dataFiles) {
      ...
      for (java.util.Map.Entry<String, Constraint> filePrefix : analysisConstraints.entrySet()) {
        ...
        String[][] results =
            HitWindows.determine(crossFilterFile, filePrefix.getValue().indexThreshold,
                filePrefix.getValue().windowMinSizePerSide,
                filePrefix.getValue().windowExtensionThreshold, DEFAULT_ADDL_ANNOT_VAR_NAMES, log);
        if (results == null) {
  ...
  private void extractHitMarkerData(Study study) {
    for (String dFile : dataFiles) {
      ...
      for (java.util.Map.Entry<String, Constraint> filePrefix : analysisConstraints.entrySet()) {
        ...
        String hitsFile = prefDir + "/hits_" + filePrefix.getKey() + ".out";
        String[] hitMarkers =
            HashVec.loadFileToStringArray(hitsFile, true, new int[] {hitsMkrIndex}, false);
        ...
        PrintWriter writer =
            Files.getAppropriateWriter(prefDir + "/subsetData_" + filePrefix.getKey() + ".xln");
  ...
  private void runScore(Study study) {
    for (String dFile : dataFiles) {
      ...
      for (java.util.Map.Entry<String, Constraint> filePrefix : analysisConstraints.entrySet()) {
        ...
        // marker, allele1, beta
        HashMap<String, String[]> markerAlleleBeta =
            study.markerData.get(dFile + "\t" + filePrefix.getKey());
        ...
        try {
          ...
          for (int i = 0; i < study.phenoFiles.size(); i++) {
            ...
            RegressionModel baseModel =
                RegressionModel.determineAppropriate(Doubles.toArray(depData), baseCovars, false,
                    true);
            RegressionModel model =
                RegressionModel.determineAppropriate(Doubles.toArray(depData), covars, false, true);
      ...
      for (String dFile : dataFiles) {
        ...
        for (java.util.Map.Entry<String, Constraint> filePrefix : analysisConstraints.entrySet()) {
          HashMap<String, RegressionResult> phenoResults =
              study.regressions.get(filePrefix.getKey()).get(dataFile);

          String resultPrefix =
              study.studyName + "\t" + dataFile + "\t"
          ...
          String middle =
              (new StringBuilder()).append(dataCounts.get(dFile).get(filePrefix.getKey()))
                  .append("\t").append(study.hitSnpCounts.get(filePrefix.getKey()).get(dataFile))
                  .append("\t").append(study.hitWindowCnts.get(filePrefix.getKey()).get(dataFile))
                  .append("\t").append(study.scores.get(filePrefix.getKey()).get(dataFile)[0])
                  .append("\t").append(study.scores.get(filePrefix.getKey()).get(dataFile)[1])
                  .append("\t").toString();
          if (study.phenoFiles.isEmpty()) {
            ...
            String pvalExcl =
                rr.num == 0 ? "." : (rr.logistic ? "=NORMSDIST(" + Math.sqrt(rr.stats) + ")"
                    : "=TDIST(" + Math.abs(rr.stats) + "," + rr.num + ",2)");

            StringBuilder sb =
                new StringBuilder(resultPrefix)
                    .append("--")
                    .append("\t")
                    .append(rr.baseRSq)
                    .append("\t")
                    ...
                    .append("\t")
                    .append(
                        (Double.isNaN(rr.rsq) ? Double.NaN : (Double.isNaN(rr.baseRSq) ? rr.rsq
                            : (new BigDecimal(rr.rsq + ""))
                                .subtract(new BigDecimal(rr.baseRSq + ""))))).append("\t")
                    .append(rr.pval).append("\t").append(rr.beta).append("\t").append(rr.se)
                    .append("\t").append(rr.num).append("\t").append(middle).append(pvalExcl);
            ...
            for (String pheno : study.phenoFiles) {
              ...
              StringBuilder sb =
                  new StringBuilder(resultPrefix)
                      ...
                      .append(
                          (Double.isNaN(rr.rsq) ? Double.NaN : (Double.isNaN(rr.baseRSq) ? rr.rsq
                              : (new BigDecimal(rr.rsq + "")).subtract(new BigDecimal(rr.baseRSq
                                  + ""))))).append("\t").append(rr.pval).append("\t")
                      .append(rr.beta).append("\t").append(rr.se).append("\t").append(rr.num)
                      .append("\t").append(middle).append("\t").append(pvalExcl);
  ...
  public static void main(String[] args) {
    ...
    /* threads, runPlink, regress, writeHist, */iT, mZ, wT, log);
    gsp.runPipeline();
  }
