			...
			for (int i = 1; i <= 24; i++) {
				...
				if (foundGenos && foundSnpInfo) {
					rCode = "library(\"seqMeta\")\n"
							...
							+ "    "+cohort + "_chr" + i + " <- prepCox(Z=mGeno, formula(formu), SNPInfo=SNPInfo, snpNames=\"SNP\", aggregateBy=\"SKATgene\", data=mPheno)\n"
							+ "} else {\n"
							+ "    message(\"using traditional prepScores method\")\n"
							+ "    "+cohort + "_chr" + i + " <- prepScores(Z=mGeno, formula(formu), SNPInfo=SNPInfo, snpNames=\"SNP\", aggregateBy=\"SKATgene\", data=mPheno)\n"
							+ "}\n"
...
//				Files.qsub("checkObject", dir, -1, commands, iterations, qsubMem, qsubWalltime);
				Files.qsub(batchDir + "run_" + cohort, batchDir, -1, commands, iterations, qsubMem, qsubWalltime, queue);
	...
	public static void batchMany(String cohort, String genos, String phenosCommaDelimited, String racesCommaDelimited, String snpInfo, int qsubMem, double qsubWalltime, String queue) {
		...
		for (int i = 0; i < phenos.length; i++) {
			for (int j = 0; j < races.length; j++) {
				try {
					batch(cohort+"_"+races[j]+"_"+phenos[i], ext.replaceAllWith(genos, "[%race]", races[j]), ext.pwd()+cohort+"_"+races[j]+"_"+phenos[i]+".csv", snpInfo, qsubMem, qsubWalltime, queue);
				} catch (Exception e) {
...
//			Files.qsubMultiple(jobNamesWithAbsolutePaths, jobSizes, batchDir, batchDir+"chunk_"+cohort, 8, true, 2000, 1);
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i<args.length; i++) {
			...
			} else if (args[i].startsWith("queue=")) {
				queue = ext.parseStringArg(args[i], null);
			} else {
