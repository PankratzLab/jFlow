	...
	public static void convertToMDRformat(String dir, String prefix) {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (!line[5].equals("0")) {
					for (int i = 0; i<markers.size(); i++) {
						if (line[6+i*2+0].equals("0")||line[6+i*2+0].equals(".")||line[6+i*2+1].equals("0")||line[6+i*2+1].equals(".")) {
							writer.print(".\t");
						} else {
							count = 0;
							for (int j = 0; j<2; j++) {
			...
			System.err.println("Error: file \""+dir+prefix+".ped"+"\" not found in current directory");
			System.exit(1);
