	...
	public static String[] inverseVarianceWeighting(String[] lines, Logger log) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (line.length == 1 && line[0].equals("")) {
				...
				if (line.length < 2) {
					log.reportError("Line # "+(i+1)+" did not have at least two columns with beta and standard error data: "+Array.toStr(line, " / "));
				} else if (line.length != numColumns) {
					log.reportError("Line # "+(i+1)+" had a different number of columns (n="+line.length+") than the previous columns (n="+numColumns+"): "+Array.toStr(line, " / "));
				}
				if (line[1].startsWith("(") && line[1].endsWith(")")) {
					...
					if (line.length > 2) {
						...
						line[1] = (Double.parseDouble(line[1]) / Math.sqrt((double)n))+"";
						totalN += n;
						sdConversion = true;
					}
				}
				if (ext.isValidDouble(line[0]) && ext.isValidDouble(line[1])) {
					...
					if (countInvalids < 5) {
						log.reportError("Line # "+(i+1)+" had an invalid double: "+Array.toStr(line, " / "));
					}
					countInvalids++;
				}
			}
		...
		if (parentheses) {
			return new String[] {ext.formDeci(result[0], 9) + " ("+ext.formDeci(result[1], 9)+")"+(sdConversion?"\t"+totalN:"")};
		} else {
	...
	public static double[] inverseVarianceWeighting(double[][] betaStderrs, Logger log) {
		...
		for (int i = 0; i < betaStderrs.length; i++) {
			if (Double.isNaN(betaStderrs[i][0]) || Double.isNaN(betaStderrs[i][1])) {
				log.reportError("Entry # "+(i+1)+" had an invalid beta or stderr: "+Array.toStr(Array.toStringArray(betaStderrs[i]), " / "));
			} else {
