...
public class Ancestry {
  ...
  public static void checkHomogeneity(String dir, String putativeWhitesFile,
    ...
    CmdLine.runDefaults(
        "plink2 --bfile " + projectPlinkRoot + " --keep " + putativeWhitesFile + " --hardy",
        homoProjDir, log);
    CmdLine
        .runDefaults(
            "plink2 --bfile " + hapMapPlinkRoot + " --keep "
  ...
  public static void imputeRace(String dir, Project proj) {
    if (!Files.exists(dir + RACE_IMPUTATIONAS_FILENAME)) {
      String[][] pcResults = HashVec.loadFileToStringMatrix(
          dir + "combo_fancy_postnormed_eigens.xln", true, new int[] {0, 1, 2, 3}, false);
      ...
      Hashtable<String, Hashtable<String, String>> hapmaps =
          HashVec.loadFileToHashHash(sd, ext.indexOfStr("FID", sdHeader, false, true),
              ...
              ext.indexOfStr("Class=HapMap;1=CEU;2=YRI;3=CHB;4=JPT", sdHeader, false, true), true);
      ...
      PCImputeRace pcir = new PCImputeRace(proj, fidiids, pc1, pc2, Ints.toArray(europeans),
          Ints.toArray(africans), Ints.toArray(asians), proj.getLog());
  ...
  public static void main(String[] args) {
    ...
    String usage = "\n" + "gwas.Ancestry requires 3+ arguments\n"
        ...
        + hapMapPlinkRoot + " (default))\n" + "   (3) Logfile (i.e. log=" + "ancestry.log"
        + " (default))" + "  AND"
  ...
  public static void mergeHapMap(String dir, String projectPlinkRoot, String hapMapPlinkRoot,
    ...
    if (!Files.exists(dir + "unrelateds.txt")) {
      log.reportError(
          "Error - need a file called unrelateds.txt with FID and IID pairs before we can proceed");
      return;
    }
    if (!Files.exists(dir + "plink.bim_unambiguous.txt")) {
      log.report(ext.getTime() + "]\tGenerating list of unambiguous SNPs");
      new SnpMarkerSet(dir + "plink.bim", true, log)
          .listUnambiguousMarkers(dir + "plink.bim_unambiguous.txt", dropMarkersFile, true);
    }

    if (!Files.exists(dir + "unambiguousHapMap.bed")) {
      log.report(ext.getTime() + "]\tExtracting unambiguous SNPs for HapMap founders");
      CmdLine.runDefaults(
          "plink2 --bfile " + hapMapPlinkRoot
    ...
    if (!Files.exists(dir + "overlap.txt")) {
      ...
      Files.writeVector(
          HashVec.loadFileToVec(dir + "unambiguousHapMap.bim", false, new int[] {1}, false, false),
          dir + "overlap.txt");
    }
    ...
    if (!Files.exists(dir + "unambiguous.bed")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile plink --extract overlap.txt --make-bed --out unambiguous --noweb", dir,
          log);
    }
    ...
    if (!Files.exists(dir + "combo.missnp")) {
      ...
      CmdLine.runDefaults(
          "plink --bfile unambiguous --bmerge unambiguousHapMap.bed unambiguousHapMap.bim unambiguousHapMap.fam --out combo --noweb",
          dir, log);
    }
    ...
    if (Files.exists(dir + "combo.missnp")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile unambiguous --flip combo.1.missnp --make-bed --out unambiguousFlipped --noweb",
          dir, log);
      CmdLine.runDefaults(
          "plink --bfile unambiguousFlipped --bmerge unambiguousHapMap.bed unambiguousHapMap.bim unambiguousHapMap.fam --make-bed --out combo --noweb",
          dir, log);
      if (Files.exists(dir + "combo.missnp")) {
        ...
        CmdLine.runDefaults(
            "plink2 --bfile unambiguousFlipped --exclude combo.2.missnp --make-bed --out unambiguousFlippedDropped --noweb",
            dir, log);
        CmdLine.runDefaults(
            "plink2 --bfile unambiguousHapMap --exclude combo.2.missnp --make-bed --out unambiguousDroppedHapMap --noweb",
            dir, log);
        CmdLine.runDefaults(
            "plink --bfile unambiguousFlippedDropped --bmerge unambiguousDroppedHapMap.bed unambiguousDroppedHapMap.bim unambiguousDroppedHapMap.fam --make-bed --out combo --noweb",
            dir, log);
      }
    }
    ...
    if (!Files.exists(dir + "finalSNPs.txt")) {
      ...
      Files.writeVector(
          HashVec.loadFileToVec(dir + "combo.bim", false, new int[] {1}, false, false),
          dir + "finalSNPs.txt");
    }
  }
