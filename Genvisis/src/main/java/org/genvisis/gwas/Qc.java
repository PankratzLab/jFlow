...
public class Qc {
  ...
  public static String[][] FILES_CREATED = {
      {"plink.bed", "freq.frq", "missing.imiss",
          ...
          "mishap.missing.hap", "gender.assoc", "gender.missing", "miss_drops.dat"},
  ...
  public static void exportFullGamut(String dir, String plinkPrefix,
    ...
    cmds.append("\t").append("plink2 --bfile ").append(plink)
        .append(
            " --maf 0 --geno 1 --mind 1 --pheno plink.fam --mpheno 3 --assoc --out gender --noweb")
        .append(";\n");
    cmds.append("fi;\n");
    cmds.append("if [ ! -f gender.missing ] ; then").append("\n");
    cmds.append("\t").append("plink2 --bfile ").append(plink)
        .append(
            " --maf 0 --geno 1 --mind 1 --pheno plink.fam --mpheno 3 --test-missing --out gender --noweb")
        .append(";\n");
    cmds.append("fi;\n");
  ...
  public static void fullGamut(String dir, String plinkPrefix,
    ...
    if (!Files.exists(dir + "marker_qc/" + plink + "_geno20.bed")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile " + plink + " --geno 0.2 --make-bed --noweb --out ./" + plink + "_geno20",
          dir + "marker_qc/", log);
    }
    ...
    if (!Files.exists(dir + "marker_qc/" + plink + ".bed")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile " + plink + "_geno20 --mind 0.1 --make-bed --noweb --out ./" + plink,
          dir + "marker_qc/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    ...
    if (!Files.exists(dir + "marker_qc/freq.frq")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile " + plink + " --geno 1 --mind 1 --freq --out freq --noweb",
          dir + "marker_qc/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    ...
    if (!Files.exists(dir + "marker_qc/missing.imiss")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile " + plink + " --geno 1 --mind 1 --missing --out missing --noweb",
          dir + "marker_qc/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    ...
    if (!Files.exists(dir + "marker_qc/hardy.hwe")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile " + plink + " --geno 1 --mind 1 --hardy --out hardy --noweb",
          dir + "marker_qc/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    ...
    if (!Files.exists(dir + "marker_qc/mishap.missing.hap")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile " + plink + " --geno 1 --mind 1 --test-mishap --out mishap --noweb",
          dir + "marker_qc/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    ...
    if (!Files.exists(dir + "marker_qc/gender.assoc")) {
      ...
      CmdLine.runDefaults("plink2 --bfile " + plink + " --geno 1 --mind 1 --pheno " + plink
          + ".fam --mpheno 3 --assoc --out gender --noweb", dir + "marker_qc/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    }
    if (!Files.exists(dir + "marker_qc/gender.missing")) {
      ...
      CmdLine.runDefaults("plink2 --bfile " + plink + " --geno 1 --mind 1 --pheno " + plink
          + ".fam --mpheno 3 --test-missing --out gender --noweb", dir + "marker_qc/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    }
    ...
    if (!Files.exists(dir + "marker_qc/miss_drops.dat")) {
      ...
      Files.writeList(
          Array.addStrToArray("dir=" + dir + "marker_qc/", HashVec
              .loadFileToStringArray(dir + "marker_qc/miss.crf", false, new int[] {0}, false), 1),
          dir + "marker_qc/miss.crf");
    ...
    if (!Files.exists(dir + "ld_pruning/" + plink + ".bed")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile ../sample_qc/" + plink + " --mind 0.05 --make-bed --noweb --out " + plink,
          dir + "ld_pruning/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    ...
    if (!Files.exists(dir + "ld_pruning/" + plink + ".prune.in")) {
      ...
      CmdLine.runDefaults(
          "plink2 --noweb --bfile " + plink + " --indep-pairwise 50 5 0.3 --out " + plink,
          dir + "ld_pruning/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    ...
    if (!Files.exists(dir + "genome/" + plink + ".bed")) {
      ...
      CmdLine.runDefaults("plink2 --bfile ../ld_pruning/" + plink + " --extract ../ld_pruning/"
          + plink + ".prune.in --make-bed --noweb --out " + plink, dir + "genome/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    }
    if (!Files.exists(dir + "genome/" + plink + ".genome")) {
      ...
      CmdLine.runDefaults(
          "plink2 --noweb --bfile " + plink + " --genome"
    ...
    if (!keepGenomeInfoForRelatedsOnly && !Files.exists(dir + "genome/mds20.mds")) {
      ...
      CmdLine.runDefaults("plink2 --bfile " + plink + " --read-genome " + plink
          + ".genome --cluster --mds-plot 20 --out mds20 --noweb", dir + "genome/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    }
    if (!Files.exists(dir + "genome/" + plink + ".genome_keep.dat")) {
      ...
      Plink.flagRelateds(dir + "genome/" + plink + ".genome", dir + "genome/" + plink + ".fam",
          dir + "marker_qc/missing.imiss", lrrFile, Plink.FLAGS, Plink.THRESHOLDS, 4, false);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
    }
    ...
    if (!Files.exists(dir + "ancestry/" + plink + ".bed")) {
      ...
      CmdLine.runDefaults(
          "plink2 --bfile ../genome/" + plink + " --make-bed --noweb --out " + plink,
          dir + "ancestry/", log);
    }
    if (Thread.currentThread().isInterrupted()) {
      throw new RuntimeException(new InterruptedException());
