...
public class ResultsPackager {
  ...
  public static final String[] STANDARD_OUTPUT_FORMAT = {"MarkerName", "Chr", "Position",
      "Effect_allele", "Reference_allele", "Effect_allele_frequency", "N", "BETA", "SE", "P-value"};
  ...
  public static final String[] EMIM_OUTPUT_FORMAT_EMIM_RESULTS = {"freq", "C_lnR1", "C_se_lnR1",
      "C_lnR2", "C_se_lnR2", "C_lnS1", "C_se_lnS1", "C_lnS2", "C_se_lnS2", "CM_lnR1", "CM_se_lnR1",
      "CM_lnR2", "CM_se_lnR2", "CM_lnS1", "CM_se_lnS1", "CM_lnS2", "CM_se_lnS2", "M_lnR1",
      "M_se_lnR1", "M_lnR2", "M_se_lnR2", "M_lnS1", "M_se_lnS1", "M_lnS2", "M_se_lnS2"};
  public static final String[] EMIM_OUTPUT_FORMAT_EMIM_PVALS =
      {"pVal_C_df2", "pVal_C_df2_Excel", "pVal_C_df1", "pVal_C_df1_Excel", "pVal_M_df2",
          "pVal_M_df2_Excel", "pVal_M_df1", "pVal_M_df1_Excel", "pVal_CM-C_df2",
          "pVal_CM-C_df2_Excel", "pVal_CM-C_df1", "pVal_CM-C_df1_Excel", "pVal_CM-M_df2",
          "pVal_CM-M_df2_Excel", "pVal_CM-M_df1", "pVal_CM-M_df1_Excel"};
  ...
  public static final String[] EMIM_REQS = {"snpID", "freq", "lnR1", "sd_lnR1", "lnR2", "sd_lnR2",
      "lnS1", "sd_lnS1", "lnS2", "sd_lnS2", "lnliknull", "lnlikfull"};
  ...
  public static void createFromParameters(String filename, Logger log) {
    ...
    params = Files.parseControlFile(filename, "results",
        new String[] {"dir=", "results=plink.assoc.linear or plink.assoc.logistic",
            "type=plink, gwaf, sol, emim, or  forest", "map=plink.map", "freq=plink.frq",
            "customFreq=", "list=specificSNPs.txt # leave blank for all",
  ...
  public static void getForestPlotParameterFile(String[][] fullPathStatResults,
      String fullPathMarkerList, String markerColumnName, String[] analyses,
      String[][] columnNamesOfAnalyses, String[][] columnDisplayNames, String fullPathOutFile,
      Logger log) {
    Double beta, se;
        ...
        for (int k = 0; k < statResults.length; k++) {
          ...
          if (statResults[k][markerIndex][0] == null) {
            ...
            for (int l = 2; l < columnNamesOfAnalyses[i].length; l++) {
              ...
              if (k == mainIndex) {
                if (l == 2) {
                  out2[out2index] += " " + columnDisplayNames[i][l] + "=" + ext.prettyP(
                      Double.parseDouble(statResults[mainIndex][markerIndex][columnIndex]));
                } else {
    ...
    Files.writeList(out2,
        ext.parseDirectoryOfFile(fullPathOutFile) + ext.rootOf(fullPathOutFile) + ".input");
  }

  private static String getOutputString(Hashtable<Long, String> snpList,
      Hashtable<String, String[]> mendelErrors, Hashtable<String, String[]> hwe,
      double hweThreshold, String freq, String[] lineC, Map<String, Integer> indicesC,
      String[] lineM, Map<String, Integer> indicesM, String[] lineCM,
      Map<String, Integer> indicesCM, Hashtable<String, String[]> tdtResults, double[] pvals,
    ...
    index = Long.parseLong(
        lineC[indicesC.get("snpID")].substring(0, lineC[indicesC.get("snpID")].indexOf(".")));
    if (snpList.containsKey(index)) {
      ...
      if (tdtResults != null) {
        ...
        try {
          ...
          try {
            se = ((Math
                .abs(Math.log(Double.parseDouble(tmp2[ext.indexOfStr("L95", TDT_REQS)])) - beta)
                + Math.abs(
                    Math.log(Double.parseDouble(tmp2[ext.indexOfStr("U95", TDT_REQS)])) - beta))
                / 2) / 1.96;
  ...
  private static String[] getPvalEquations(String logLikilihood_null_C, String logLikilihood_full_C,
      String logLikilihood_null_M, String logLikilihood_full_M, String logLikilihood_full_CM,
      Logger log) {

    // Lines up with odd entries in EMIM_OUTPUT_FORMAT_EMIM_PVALS
    return new String[] {
        "=1-CHISQ.DIST(2 * (" + logLikilihood_full_C + "-" + logLikilihood_null_C + "),2,TRUE)",
  ...
  private static double[] getPvalues(double logLikilihood_null_C, double logLikilihood_full_C,
      double logLikilihood_null_M, double logLikilihood_full_M, double logLikilihood_full_CM,
      Logger log) {
  ...
  public static String[][] loadFile(String fullPathStatResults, String nameOfMarkerColumn,
      String[] markersToBeLoaded, String[] columnNamesToBeLoaded, Logger log) {
  ...
  public static void main(String[] args) {
    ...
    String usage = "\n" + "gwas.ResultsPackager requires 0-1 arguments\n"
        ...
        + "   (4) name of original freq file used to generate the gwaf files (i.e. freq=" + freqFile
    ...
    try {
      ...
      if (type.equalsIgnoreCase("gwaf")) {
        ...
        String[][] analysisNms = {{"tdt_OR", "tdt_U95", "tdt_P"},
            {"C_lnR1", "C_se_lnR1", "pVal_C_df1"}, {"CM_lnS1", "CM_se_lnS1", "pVal_CM-C_df1"}};
    ...
    for (String type : types) {
      for (String pheno : phenos) {
        ...
        parseIBCFormatFromGWAF(dir, pheno + "." + type + ".results.csv", "plink.map", "plink.frq",
            null, "list.txt", 1, "BOSS-EHLS_" + pheno + "_" + type + "_072512.txt", new Logger());
  ...
  public static void parseEmimFormat(String childResultsFile, String momResultsFile,
      String childMomResultsFile, String tdtResultsFile, String mapFile, String mendelErrorFile,
      String hweFile, String frqFile, double pValueThreshold, String outfile, Logger log) {
    ...
    try {
      ...
      while (reader1.ready()) {
        ...
        snpList.put(index,
            lineC[0] + "\t" + lineC[3] + "\t" + lineC[1] + "\t" + lineC[4] + "\t" + lineC[5]);
        index++;
      }
      reader1.close();
      ...
      writer.println(Array.toStr(EMIM_OUTPUT_FORMAT_SNPS)
          + (mendelErrorFile == null ? "" : ("\t" + Array.toStr(EMIM_OUTPUT_FORMAT_MENDEL_ERRORS)))
      ...
      while (reader1.ready()) {
        ...
        lineC = reader1.readLine()
            .replaceAll("\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*", "            NaN").trim()
            .split(delimiter1, -1);
        String snp = snpList.get(Long.parseLong(
            lineC[indicesC.get("snpID")].substring(0, lineC[indicesC.get("snpID")].indexOf("."))))
            .split("\t")[2];
        String freq;
        if (freqs == null) {
        ...
        lineM = reader2.readLine()
            .replaceAll("\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*", "            NaN").trim()
            .split(delimiter2, -1);
        lineCM = reader3.readLine()
            .replaceAll("\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*\\*", "            NaN").trim()
            .split(delimiter3, -1);
        ...
        if (!lineC[indicesC.get("snpID")].equals(lineM[indicesM.get("snpID")])
          ...
          log.reportError("Error - SNP ID in the files are not lined up. Child SNP ID: "
              + lineC[indicesC.get("snpID")] + "; Mom SNP ID: " + lineM[indicesM.get("snpID")]
              + "; ChildMom SNP ID: " + lineCM[indicesCM.get("snpID")] + ".");
          return;
        }
        ...
        // }
        if (pValueThreshold >= 1 || ((!temp.equals("NA")
            && pvalsPass(Array.addDoubleToArray(Double.parseDouble(temp), pvals), pValueThreshold))
            && Double.parseDouble(freq) >= .01)) {
          pvalEquations = getPvalEquations(lineC[indicesC.get("lnliknull")],
              lineC[indicesC.get("lnlikfull")], lineM[indicesM.get("lnliknull")],
              lineM[indicesM.get("lnlikfull")], lineCM[indicesCM.get("lnlikfull")], log);
          ...
          writer.println(getOutputString(snpList, mendelErrors, hwe, hweThreshold, freq, lineC,
              indicesC, lineM, indicesM, lineCM, indicesCM, tdtResults, pvals, pvalEquations, log));
        }
      }
      reader1.close();
      reader3.close();
  ...
  public static void parseIBCFormatFromGWAF(String dir, String resultsFile, String mapFile,
      String originalFrqFile, String customFrqFile, String markersToReport, double filter,
    ...
    originalFreqHash = HashVec.loadFileToHashString(dir + originalFrqFile, new int[] {1},
        new int[] {2, 3}, false, "\t", false, false, false); // add 4 if you want global frequency

    if (customFrqFile != null) {
      System.err.println(
          "Warning - use of custom freq file has not been tested properly; if it works then remove this warning");
      customFreqHash = HashVec.loadFileToHashString(originalFrqFile, new int[] {1},
          new int[] {2, 3, 4}, false, "\t", false, false, false); // add 4 if you want global
    ...
    try {
      ...
      while (reader.ready()) {
        ...
        trav = line[0];
        if ((markerHash == null || markerHash.contains(trav)) && !line[3].equals("") && (filter >= 1
          ...
          if (customFreqHash != null) {
            if (customFreqHash.containsKey(trav)) {
              ...
              freq = Alleles.getAlleleFreqForA1(alleles[0],
                  customFreqHash.get(ext.replaceAllWith(trav, ".", "-")).split("\t"))[2];
            } else {
  ...
  public static void parseSOLformat(String dir, String resultsFile, String mapFile, String freqFile,
      String markersToReport, double filter, double callRateThreshold, String outfile, Logger log) {
    BufferedReader reader;
    PrintWriter writer;
    String[] line;
    String temp, trav;
    ...
    try {
      ...
      if (indices[4] == -1 && indices[5] == -1) {
        log.reportError(
            "Error - results file did not contain a column for 'OR' (logistic) or 'BETA' (linear); aborting");
        return;
      ...
      if (logistic) {
        log.reportError(
            "Logistic parsing has not yet been implemented; check the column order etc, parse it properly and re-run");
        return;
      }

      if (indices[6] == -1) {
        log.reportError(
            "Warning - results file did not contain a column for 'StdErr/SE'; values will be set to NA");
      }
      ...
      while (reader.ready()) {
        ...
        if ((markerHash == null || markerHash.contains(trav)) && (callrate > callRateThreshold)
          ...
          writer.println(
              "\t" + (indices[2] == -1 ? "NA" : line[indices[2]]) + "\t" + line[indices[3]]); // se,
                                                                                              // pval
        }
      }
      reader.close();
  ...
  public static void parseStdFormatFromPlink(String dir, String resultsFile, String test,
      String mapFile, String freqFile, String markersToReport, double filter, String outfile,
      Logger log) {
    BufferedReader reader;
    PrintWriter writer;
    String[] line;
    ...
    if (markersToReport != null) {
      if (!Files.exists(dir + markersToReport)) {
        log.reportError(
            "Error: could not find markers to report file '" + dir + markersToReport + "'");
        return;
      }
      markerHash = HashVec.loadFileToHashSet(dir + markersToReport, false);
    } else {
    ...
    try {
      ...
      if (indices[4] == -1 && indices[5] == -1) {
        ...
        log.reportError(
            "Error - results file contain a column for both 'OR' (logistic) and 'BETA' (linear); aborting");
        return;
      } else if (indices[4] != -1) {
        logistic = true;
      } else {
      ...
      while (reader.ready()) {
        ...
        if ((markerHash == null || markerHash.contains(trav)) && !line[3].equals("")
          ...
          if (line[indices[1]].equals("NA")) {
            ...
            if (freqHash.containsKey(trav)) {
              writer.print("\t" + Array.toStr(
                  Alleles.getAlleleFreqForA1(line[indices[1]], freqHash.get(trav).split("\t")))); // a1,
                                                                                                  // a2,
                                                                                                  // a1_freq
            } else if (freqHash.containsKey(ext.replaceAllWith(trav, ".", "-"))) {
              writer.print("\t" + Array.toStr(Alleles.getAlleleFreqForA1(line[indices[1]],
                  freqHash.get(ext.replaceAllWith(trav, ".", "-")).split("\t")))); // a1, a2,
                                                                                   // a1_freq
            } else {
              log.reportError("Error - no frequency for " + trav);
              writer.print("\t" + line[indices[1]] + "\t.\t."); // a1, null, null
          ...
          writer.println(
              "\t" + (indices[6] == -1 ? "NA" : line[indices[6]]) + "\t" + line[indices[7]]); // se,
                                                                                              // pval
        }
      }
      reader.close();
