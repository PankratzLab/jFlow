...
public class Plink {
  ...
  public static final String[] FLAGS = new String[] {"duplicate", "parent-offspring", "sibling",
      "avuncular,gg", "first cousins,halfsibs", "second cousins"};
  ...
  public static void addToGenome(String root, int threads) {
    ...
    try {
      ...
      System.err.println(
          "Sorry, I can't trust you any more, I need to see a 'plink.imiss' file to proceed");
      return;
    ...
    if (listCount == 0) {
      System.err.println(
          "Error - cannot add to genome if there are no previous tmp.list### files to work from...");
      return;
    }
    ...
    for (String ind : inds) {
      ...
      if (!imiss.containsKey(line[0] + "\t" + line[1])) {
        System.err
            .println("Error - 'plink.imiss' does not contain individual" + line[0] + "," + line[1]);
        return;
    ...
    if (count > 0) {
      System.err.println(
          "Batch files will not include the " + count + " indiviudals that are not genotyped");
    }
    ...
    System.out.println(
        "Reminder to either code HapMap samples as controls or use the -noFilter flag for unphenotyped (but still genotyped) samples");
    System.out.println(
        "Also not a bad idea to run --missing to check for low-genotyped indiviudals that will crash the scripts");
  }
  ...
  public static void batchGenome(String root, int threads, double minPiHatToKeep) {
    ...
    try {
      ...
      writer.println(
          "plink --bfile ../../plink --keep ../cluster.genome_keep.dat --make-bed --noweb");
      writer.close();
    } catch (Exception e) {
      System.err.println("Error writing to " + "master");
      e.printStackTrace();
  ...
  public static void batchLD(String root, double minR2ToKeep) {
    Files.qsub("runLD", 1, 22,
        "cd " + ext.pwd() + "\n~/bin/plink --noweb --bfile " + root
            + " --r2 --chr # --ld-window 99999 --ld-window-r2 " + minR2ToKeep + " --out chr#",
        2000, 2);
  }
          ...
          while (reader.ready()) {
            ...
            if (rep == 1) {
              ...
              System.err.println("Error - marker mismatch in " + filename + "; expecting "
                  + markerNames[count] + " at line " + (count + 2) + ", found " + line[1]);
            }
    ...
    if (rep > 0) {
      try {
        ...
        for (int i = 0; i < markerNames.length; i++) {
          writer.println(
              markerNames[i] + "\t" + ((double) counts[i][0] + 1) / ((double) numRepsTotal + 1)
  ...
  public static int flagRelateds(String genomeFile, String famFile, String iMissFile,
      String lrrFile, String[] flags, double[][] thresholds, int removeOutTo, boolean kill) {
    BufferedReader reader;
    PrintWriter writer;
    ...
    if (iMissFile == null) {
      log.report(
          "Warning - no .imiss file specified, indiviudals will not be preferentially selected based on call rate");
      callrates = new Hashtable<String, String>();
    } else if (!Files.exists(iMissFile, false)) {
      log.reportError(
          "Error - specified .imiss file missing , indiviudals will not be preferentially selected based on call rate");
      callrates = new Hashtable<String, String>();
    } else {
    ...
    if (lrrFile == null) {
      log.report(
          "Warning - no LRR_SD file specified, indiviudals will not be preferentially selected based on LRR_SD");
      ...
      if (properHeader) {
        ...
        log.reportError(
            "Error - specified LRR_SD file has an improper header (looking for \"SAMPLE\" and \"LRR_SD\" as the first two columns of file.");
    ...
    try {
      ...
      writer.println(
          "FID1\tIID1\tCALLRATE1\tLRR_SD1\tFID2\tIID2\tCALLRATE2\tLRR_SD2\tP(IBD=0)\tP(IBD=1)\tP(IBD=2)\tPI_HAT\tType\tFID_KEEP\tIID_KEEP\tFID_DROP\tIID_DROP");
        ...
        for (int i = 0; i < removeOutTo; i++) {
          if (probs != null && probs[0] >= thresholds[i][0] && probs[1] >= thresholds[i][1]
            ...
            writer.print(fidIid1 + "\t" + metrics[0] + "\t" + metrics[1] + "\t" + fidIid2 + "\t"
                + metrics[2] + "\t" + metrics[3] + "\t" + line[4] + "\t" + line[5] + "\t" + line[6]
                + "\t" + line[7] + "\t" + flags[i]);
    ...
    for (int i = 0; i < 2; i++) {
      try {
        writer = new PrintWriter(
            new FileWriter(genomeFileRoot + "_" + (i == 0 ? "keep" : "drop") + ".dat"));
        trav = i == 0 ? in : out;
        ...
        System.err.println(
            "Error writing to " + genomeFileRoot + "_" + (i == 0 ? "keep" : "drop") + ".dat");
        e.printStackTrace();
      }
    }
  ...
  public static void main(String[] args) throws IOException {
    ...
    String usage = "\n" + "gwas.Plink requires 0-1 arguments\n"
        ...
        + famFile + " (default))\n"
        + "   (3) (optional) select sample to keep based on call rate (i.e. imiss=" + iMissFile
        + " (default))\n"
        + "   (4) (optional) select on LRR_SD if call rates are within 0.2% (i.e. lrr_sd=" + lrrFile
        + " (default; needs to have format FID\tIID\tLRR_SD))\n"
    ...
    for (int i = 0; i < FLAGS.length; i++) {
      usage += "           level " + (i + 1) + ": " + FLAGS[i] + "\tP(IBD=0)>=" + THRESHOLDS[i][0]
          + "\tP(IBD=1)>=" + THRESHOLDS[i][1] + "\tP(IBD=2)>=" + THRESHOLDS[i][2] + "\tPI_HAT>="
          + THRESHOLDS[i][3] + "\n";
    }
