...
public class Rvtests {
	...
	public static void renameToNamingScheme(String resultsDir, String[] filenameConversionDictionary,
		...
		for (String filename : filenames) {
			...
			for (int j = 0; j < indices.length; j++) {
				if (indices[j] < 0) {
					newFilename += ("_" + filenameAdditionalSegments.elementAt(-indices[j] - 1)
																													.replaceAll("'", ""));
				} else {
			...
			new File(resultsDir, filename).renameTo(new File(resultsDir, newFilename));
		}
	}
	...
	public static void main(String[] args) {
		...
		String dirOfRvtestsCommands, dirOfPedFiles, fullpathToVcf, rScriptDir, resultsDir,
				filenameConversionTable, filenameSegmentIndices;
		String commandIsScript, commandRvExecutable, commandPedDir, commandVcf, commandResultsDir,
				commandScriptDir, commandIsRename;
		Logger log;
		...
		for (String arg : args) {
			if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
				...
				System.exit(1);
			} else if (arg.startsWith(commandIsScript)) {
				isScripts = true;
				numArgs--;
			} else if (arg.startsWith(commandRvExecutable)) {
				dirOfRvtestsCommands = arg.split("=")[1];
				numArgs--;
			} else if (arg.startsWith(commandPedDir)) {
				dirOfPedFiles = arg.split("=")[1];
				numArgs--;
			} else if (arg.startsWith(commandVcf)) {
				fullpathToVcf = arg.split("=")[1];
				numArgs--;
			} else if (arg.startsWith(commandResultsDir)) {
				resultsDir = arg.split("=")[1];
				numArgs--;
			} else if (arg.startsWith(commandScriptDir)) {
				rScriptDir = arg.split("=")[1];
				numArgs--;
			} else if (arg.startsWith(commandIsRename)) {
				isRename = true;
				numArgs--;
			} else if (arg.startsWith(filenameSegmentIndices)) {
				filenameSegmentIndices = arg.split("=")[1];
				numArgs--;
			} else {
