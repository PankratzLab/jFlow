  ...
  public static void batch(String dir, String phenoFile, String pheno, String[] covars,
    ...
    try {
      ...
      while (new File(ext.replaceAllWith(dir + geneticDataTemplate, "#", count + "")).exists()
        ...
        writer = Files.openAppropriateWriter(dir + ext.insertNumbers(rootTemplate, count) + ".R");
        if (versionOfGWAF == 2) {
          writer.println("library(kinship2)");
        } else {
          writer.println("library(kinship)");
      ...
      if (numBatches < 1) {
        if (nodesToUse == null) {
          v = ArrayUtils.toStringVector(Qsub.qsub(dir, rootTemplate, startAt, count,
    ...
    try {
      ...
      while (trav < last + 20) {
        if (new File(ext.replaceAllWith(outfileTemplate, "#", trav + "")).exists()) {
          if (iv.size() > 0) {
            log.reportError("Skipping over file"
                            + (iv.size() > 1 ? "s"
                                             : "")
