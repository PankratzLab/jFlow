        ...
        try {
	        ...
	        while (reader.ready()) {
	        	...
	        	for (int i = 0; i<ages.length; i++) {
	        		if (line[2+i*8+3].equals("") || (line[2+i*8+6].equals("") && line[2+i*8+7].equals("TRUE"))) {
	        			ages[i] = -1;
	        		} else {
	    	        	dates = new String[line[2+i*8+7].equals("TRUE")?2:1];
	        			for (int j = 0; j<dates.length; j++) {
	        				dates[j] = (line[2+i*8+1+j*3+0].equals("")?"6":line[2+i*8+1+j*3+0])+"/"+
	        						   (line[2+i*8+1+j*3+1].equals("")?"15":line[2+i*8+1+j*3+1])+"/"+
	        						   line[2+i*8+1+j*3+2];
                        }
	        			ages[i] = ext.calcDays(ext.parseDate(dates[0]), dates.length==1?new Date():ext.parseDate(dates[1]))/365.25;
	        		}
                }
	        	...
	        	if (affected < 0 || Array.min(ages) < 0 || ages[affected]-ages[FLIP[affected]] > DIFF_AGE_THRESHOLD) {
	        		use = false;
	        	}
	        	writer.println(line[0]+"\t"+line[1]+"\t"+(tools.isAffected(dx, motherID)?1:0)+"\t"+ages[0]+"\t"+(tools.isAffected(dx, fatherID)?1:0)+"\t"+ages[1]+"\t"+(use?1:0));
	        }
            writer.close();
	        reader.close();
