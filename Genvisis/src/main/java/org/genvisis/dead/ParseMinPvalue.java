	...
	public static void parseMinPvalue(String dir, String filename) {
		...
		for (int i = 0; i<METHODS.length; i++) {
			try {
				...
				System.err.println("Error: file \""+dir+METHODS[i]+".xls"+"\" not found in current directory");
				System.exit(1);
		...
		for (int i = 0; i<markers.size(); i++) {
			if (hash.containsKey(markers.elementAt(i))) {
				writer.println(markers.elementAt(i)+"\t"+hash.get(markers.elementAt(i))+"\t"+-1*Math.log10(Double.parseDouble(hash.get(markers.elementAt(i)))));
				count++;
			}
		}
		writer.close();
