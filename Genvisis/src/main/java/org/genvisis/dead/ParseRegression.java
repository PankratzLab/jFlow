...
public class ParseRegression {
	...
	// public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gwas\\firstRun\\"};
	// public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gwas\\hits\\"};
	// public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gwas\\jewish\\"};
	// public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gwas\\results\\"};
	// public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gwas\\merged\\results\\unadjusted_aoo_results\\"};
	// public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gwas\\merged\\results\\JustIU\\"};
	// public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gwas\\merged\\results\\"};
	// public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gaw\\results\\allMarkers\\"};
//	public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\gaw\\results\\goodMarkers\\"};
//	public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\ADNI\\results\\"};
//	public static final String[] DEFAULT_DIRS = {"C:\\Documents and Settings\\npankrat\\My Documents\\LOAD\\results\\"};
...
//	public static final String[] MODELS = {"AdditiveWith", "AdditiveWithout", "Dominant", "Recessive"};
//	public static final String[] MODELS = {"Additive", "APOE_Risk", "APOEcounts", "APOEcountsNoAgeNoSex"};
		...
		for (int meth = 0; meth<METHODS.length; meth++) {
			...
			for (int i = 0; i<dirs.length; i++) {
				for (int j = 0; j<MODELS.length; j++) {
					try {
						reader = new BufferedReader(new FileReader(dirs[i]+MODELS[j].toLowerCase()+".assoc."+METHODS[meth]));
						System.out.println("Parsing "+dirs[i]+MODELS[j].toLowerCase()+".assoc."+METHODS[meth]);
						reader.readLine();
						count = 0;
						while (reader.ready()) {
							line = reader.readLine().trim().split("[\\s]+");
							if (line.length==1&&line[0].equals("25")) {
								System.err.println("Error - That weird thing with the last line of "+dirs[i]+MODELS[j].toLowerCase()+".assoc."+METHODS[meth]);
								continue;
							}
							...
							if (line[4+MODEL_OFFSETS[j]].equals(MODEL_ABBREVS[j])&&Integer.parseInt(line[0])<26) {
								if (j==0) {
									snpNames.add(line[1]);
									...
									if (!line[1].equals(snpNames.elementAt(count))) {
										System.err.println("Error - out of sync in "+dirs[i]+MODELS[j].toLowerCase()+".assoc."+METHODS[meth]+" - expecting "+snpNames.elementAt(count)+", found "+line[1]);
										System.exit(1);
									}
									data = allSNPs.get(line[1]);
								}
								data[j][0] = line[6+MODEL_OFFSETS[j]].equals("NA")?-999:Double.parseDouble(line[6+MODEL_OFFSETS[j]]);
								data[j][1] = line[7+MODEL_OFFSETS[j]].equals("NA")?-999:Double.parseDouble(line[7+MODEL_OFFSETS[j]]);
								data[j][2] = line[8+MODEL_OFFSETS[j]].equals("NA")?-999:Double.parseDouble(line[8+MODEL_OFFSETS[j]]);
						...
						System.err.println("Error: file \""+dirs[i]+MODELS[j].toLowerCase()+".assoc."+METHODS[meth]+"\" not found in current directory");
						System.exit(1);
					} catch (IOException e) {
						System.err.println("Error reading file \""+dirs[i]+MODELS[j].toLowerCase()+".assoc."+METHODS[meth]+"\"");
						e.printStackTrace();
						System.exit(2);
					}
				}
				...
				if (map == null) {
					System.err.println("Error - could not find a .MAP or .BIM file with any of the following roots: "+Array.toStr(MAP_ROOTS, ", "));
					System.exit(1);
				}
				...
				try {
					...
					writer.println("SNP\tChr\tPosition\tAllele\t"+Array.toStr(MODELS, "\tstatistic\tp-value\t")+"\tstatistic\tp-value");
					for (int j = 0; j<markerNames.length; j++) {
						if (allSNPs.containsKey(markerNames[j])) {
							writer.print(markerNames[j]+"\t"+chrs[j]+"\t"+positions[j]+"\t"+alleles.get(markerNames[j]));
							used.put(markerNames[j], data = allSNPs.remove(markerNames[j]));
							for (int k = 0; k<MODELS.length; k++) {
								writer.print("\t"+(data[k][0]==-999?".":ext.formDeci(data[k][0], 3, true))+"\t"+(data[k][1]==-999?".":ext.formDeci(data[k][1], 4, true))+"\t"+(data[k][2]==-999?".":ext.prettyP(data[k][2], 2, 5, 4, true)));
							}

							writer.println();
						} else {
							if (extras==null) {
								extras = new PrintWriter(new FileWriter(dirs[i]+"markers in map file but without results.out"));
							}
					...
					if (allSNPs.size()>0) {
						extras = new PrintWriter(new FileWriter(dirs[i]+"marke with results file but not in map file.out"));
				...
				try {
					...
					for (int j = 0; j<topSNPs.size(); j++) {
						...
						for (int k = 0; k<MODELS.length; k++) {
							writer.print("\t"+(data[k][0]==-999?".":ext.formDeci(data[k][0], 3, true))+"\t"+(data[k][2]==-999?".":data[k][2]));
						}
						writer.println();
					}
					writer.close();
				} catch (FileNotFoundException fnfe) {
					System.err.println("Error: file \""+dirs[i]+METHODS[meth]+"_topHits.xls"+"\" is currently in use");
					System.exit(1);
