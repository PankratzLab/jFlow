...
//
//	public static void findOptimalSet(String dir, String filename, String outputRoot, int numSNPs, float pval_threshold, String ldRoot, float r2_threshold, String directoryOfIlluminaScores, float scoreThreshold, float scoreDiffThreshold, float scoreClassBump, String filteringDataset, String forceBeforeFile, String forceAfterFile, String forceRegardlessFile) {
...
//	                if (!reader.ready()) {
//	                	System.err.println("Error - failed to parse '"+files[i]+"'; no line started with 'Locus_Name'");
//	                } else if (indices[1] == -1 && indices[2] == -1) {
//		                System.err.println("Error - Illumina files must have one of the following columns: "+Array.toStr(Array.subArray(ILLUMINA_TARGET_COLUMNS, 1), ", "));
...
//	                System.err.println("Error: file \""+directoryOfIlluminaScores+files[i]+"\" not found in current directory");
//	                System.exit(1);
...
//            }
//        	if (files.length == 0) {
//        		System.err.println("Error - no illumina scores files found in score directory; no warnings will be provided");
...
//			System.out.println("Forcing those SNPs in '"+forceBeforeFile+"' to be included as tags if p-value met");
//			forceBefore = HashVec.loadFileToHashNull(dir+forceBeforeFile, false);
//		} else {
//			System.out.println((forceBeforeFile==null?"No ":"No file named '"+forceBeforeFile+"'; no ")+"SNPs will be forced to be included as tags");
//			forceBefore = new Hashtable<String,String>();
//		}
//		if (forceAfterFile != null && new File(dir+forceAfterFile).exists()) {
//			System.out.println("Forcing those SNPs in '"+forceAfterFile+"' to be included as tags if p-value met");
//			forceAfter = HashVec.loadFileToHashNull(dir+forceAfterFile, false);
//		} else {
//			System.out.println((forceAfterFile==null?"No ":"No file named '"+forceAfterFile+"'; no ")+"SNPs will be forced to be included as tags after tagging");
//			forceAfter = new Hashtable<String,String>();
//		}
//		if (forceRegardlessFile != null && new File(dir+forceRegardlessFile).exists()) {
//			System.out.println("Forcing those SNPs in '"+forceRegardlessFile+"' to be included as tags regardless of p-value");
//			forceRegardless = HashVec.loadFileToHashNull(dir+forceRegardlessFile, false);
//			forcedKeys = HashVec.getKeys(forceRegardless);
//		} else {
//			System.out.println((forceRegardlessFile==null?"No ":"No file named '"+forceRegardlessFile+"'; no ")+"additional SNPs will be forced regardless of p-value");
...
//			if (directoryOfIlluminaScores == null || score > scoreThreshold || (pvalues[iv.elementAt(j)] < 1E-4 && score > 0.4)) {
//	        	trav = chrHash.get(superset[j]);
//				if (trav == null) {
////					System.err.println("Error - the chrHash derived from the marker file does not contain marker "+markerNames[ivs[i].elementAt(j)]);
...
//    	                				System.err.println("Error - missing LD info for "+subset[index]+"/"+subset[index+k*offset]+" pair");
//    	                			}
...
//        			pvals[index] = 2;
//            	}
//            	tags.add(subset[index]+"\t"+chr+"\t"+pval+"\t"+getScore(subset[index], scores, merges, missingIlluminaValues));
//            	
//            	for (int j = 0; j<2; j++) {
//                	offset = j==0?-1:1;
//                	for (int k = 1; index+k*offset >= 0 && 
...
//            		HashVec.addIfAbsent(subset[j]+"\t"+forceAfter.get(subset[j])+"\t"+getScore(subset[j], scores, merges, missingIlluminaValues), tags);
//            	}
//            }
//        }
//        
//        for (int j = 0; j<forcedKeys.length; j++) {
//        	HashVec.addIfAbsent(forcedKeys[j]+"\t"+forceRegardless.get(forcedKeys[j])+"\t"+getScore(forcedKeys[j], scores, merges, missingIlluminaValues), tags);
//        }
//        if (missingMarkers.size() > 0) {
//        	System.err.println("Error - Missing "+missingMarkers.size()+" markers for threshold "+pval_threshold);
...
//	}
//	
//	public static float getScore(String element, Hashtable<String,Float> scores, Hashtable<String,String> merges, Vector<String> missingValues) {
...
//				System.err.println("Error - "+element+" merged with "+merges.get(element)+"; so we'll need to request info for it");
//				return -9;
//			}
//		} else {
//			HashVec.addIfAbsent(merges.get(element), missingValues);
...
//        		System.err.println("Error - Haploview .TAGS file has changed format and needs to be addressed");
//        		System.exit(1);
//        	}
...
//	}
//	
//	public static void compareSelectionParameters(int numSNPs, String dir, String filename, float pval_threshold, String ldRoot, float r2_threshold, String directoryOfIlluminaScores, String filteringDataset, String forceBeforeFile, String forceAfterFile, String forceRegardlessFile) {
//        PrintWriter writer;
//        String[] line;
//        
...
//    			findOptimalSet(dir, filename, params[i][0]+"_"+params[i][1]+"_"+params[i][2], numSNPs, pval_threshold, ldRoot, r2_threshold, directoryOfIlluminaScores, params[i][0], params[i][1], params[i][2], filteringDataset, forceBeforeFile, forceAfterFile, forceRegardlessFile);
...
//		        }
//
//		        findOptimalSet("", resultsFile, outputRoot, numSNPs, pval_threshold, ldRoot, r2_threshold, directoryOfIlluminaScores, scoreThreshold, scoreDiffThreshold, scoreClassBump, filteringDataset, forceBeforeFile, forceAfterFile, forceRegardlessFile);
//	        }
//	        reader.close();
//        } catch (FileNotFoundException fnfe) {
...
//	    "   (7) (optional) file of SNPs to force before tagging [if they meet minimum p-value] (i.e. before="+forceBeforeFile+" (default))\n"+
//	    "   (8) (optional) file of SNPs to force after tagging [if they meet minimum p-value] (i.e. after="+forceAfterFile+" (default))\n"+
//	    "   (9) (optional) file of SNPs to force regardless of p-value (i.e. regardless="+forceRegardlessFile+" (default))\n"+
...
//	    try {
//	    	if (compare) {
//	    		compareSelectionParameters(numSNPs, dir, filename, pval_threshold, ldRoot, r2_threshold, dirIlluminaScores, filteringDataset, forceBeforeFile, forceAfterFile, forceRegardlessFile);
//	    	} else {
//	    		findOptimalSet(dir, filename, outputRoot, numSNPs, pval_threshold, ldRoot, r2_threshold, dirIlluminaScores, scoreThreshold, scoreDiffThreshold, scoreClassBump, filteringDataset, forceBeforeFile, forceAfterFile, forceRegardlessFile);
//	    	}
//	    } catch (Exception e) {
//		    e.printStackTrace();
//	    }
