...
public class LDdatabase implements Serializable {
	...
	public Hashtable<String, String> getChrHash() {
		if (unchanged) {
			...
			if (chrHash == null) {
				chrHash = SnpMarkerSet.loadSnpMarkerSetToChrHash(root+".bim", SnpMarkerSet.PLINK_BIM_FORMAT);
			}
			return chrHash;
		}
	}	
...
//        StringLDdb chrLDdb;
		...
		if (listName != null) {
			if (Files.exists(dir+listName+".list", false)) {
				check = HashVec.loadFileToStringArray(dir+listName+".list", false, new int[] {0}, false);
		...
		if (missing.containsKey("0") && missing.get("0").size() > 0) {
			v = missing.get("0");
			log.reportError("Warning - the following "+v.size()+" marker"+(v.size()>1?"s":"")+" were not found in the "+root+" dataset:");
        	...
        	if (ivs[chr].size() > 2) {
	    		...
	    		try {
		    		...
		    		for (int i = 0; i<subset.length; i++) {
		    			for (int j = i+1; j<subset.length; j++) {
		    				if (Math.abs(positions[i] - positions[j]) < BP_LIMIT && chrLDdb.get(subset[i], subset[j]) == MISSING_INFO) {
		    					HashVec.addIfAbsent(subset[i], v);
		    					HashVec.addIfAbsent(subset[j], v);
		    					if (writer == null) {
		    		                writer = new PrintWriter(new FileWriter(dir+"pairs.xln"));
		    					}
		    					writer.println(subset[i]+"\t"+subset[j]+"\t"+Math.abs(positions[i] - positions[j]));
		    				}
		                }
		            }
		    		if (writer != null) {
	    		...
	    		if (v.size() == 0) {
		        	...
		        	if (!new File(root+".chr"+chr+".bed").exists()) {
			        	CmdLine.run("plink --bfile "+root+" --chr "+chr+" --noweb --make-bed --out "+root+".chr"+chr, "./");
		        	}
		        	CmdLine.run("plink --bfile "+root+".chr"+chr+" --noweb --extract "+dir+LDDB_TARGETS+"_snplist.txt --recode --out "+dir+LDDB_TARGETS, "./");
		        	CmdLine.run("plink --file "+dir+LDDB_TARGETS+" --noweb --freq --out "+dir+"freqCheck", "./");
		        	try {
		                ...
		                log.reportError("Error: file \""+dir+"freqCheck.frq"+"\" not found in current directory");
		                System.exit(1);
		            ...
		            if (new File(HAPLOVIEW_LOC).exists()) {
			        	CmdLine.run("java -jar "+HAPLOVIEW_LOC+" -nogui -pedfile "+dir+LDDB_TARGETS+".ped -info "+dir+LDDB_TARGETS+".info -dprime -chromosome "+Positions.chromosomeNumberInverse(chr)+" -maxDistance 500 -hwcutoff 0 -minGeno 0 -missingCutoff 1", "./");
		            } else {
		        	...
		        	try {
		                ...
		                log.reportError("Error: file \""+dir+LDDB_TARGETS+".ped.LD"+"\" not found in current directory");
		                System.exit(1);
	                } catch (IOException ioe) {
	...
	public static void multiUpdate(LDdatabase[] lddbs, String[] markers, String listName) {
		for (int i = 0; i<lddbs.length; i++) {
			lddbs[i].updateWithTheseMarkers(markers, listName);
        }
	}
	...
	public static float multiCheck(LongLDdb[] chrLDdbs, String marker1, String marker2, int compType, Logger log) {
		float[] r2s;
		float r2;
		...
		case COMP_MIN:
			for (int i = 0; i<r2s.length; i++) {
				if (r2 == MISSING_INFO || (r2 < 0 && r2s[i] >= 0)) { 
					r2 = r2s[i];
				} else if (r2s[i] >= 0) {
					r2 = Math.min(r2, r2s[i]);
				}
            }
			break;
		case COMP_MAX:
	...
	public static String multiHashCheck(Hashtable<String,Hashtable<String,String>> chrHashes, String markerName, int reportType, Logger log) {
		String[] poslar;
		String pos;
			...
			for (int i = 0; i<poslar.length; i++) {
				if (pos == null) {
					...
					if (reportType == REPORT_VERBOSE) {
						log.reportError("Error - inconsistent position for marker "+markerName+": "+pos+" and "+poslar[i]);
					}
