...
public class DosageData implements Serializable {
  ...
  public static final int[][] PARAMETERS = {
      ...
      {MACH_ID_TYPE, 2, INDIVIDUAL_DOMINANT_FORMAT, 1, 0, 0, -1, -1, -1, -1, 0, 5, 3, 3}, // .dose
                                                                                          // (MINIMAC)
      {SEPARATE_FILE_ID_TYPE, 5, MARKER_DOMINANT_FORMAT, 3, 0, 1, 3, 4, CHR_INFO_IN_FILENAME, 2, 0,
          1, 3, 3}, // .impute2
  ...
  public DosageData(String dosageFile, String idFile, String mapFile, int[] parameters,
      ...
      Logger log) {
    this(dosageFile, idFile, mapFile, parameters, regionsToUseFile == null ? null : loadRegions(
        regionsToUseFile, log), markersToUseFile == null ? null
        : loadMarkers(markersToUseFile, log), markerNamePrepend, verbose, log);
  }
  ...
  public DosageData(String dosageFile, String idFile, String mapFile, int[] parameters,
    ...
    if (parameters[2] == PLINK_BINARY_FORMAT) {
      DosageData dd =
          loadPlinkBinary(ext.parseDirectoryOfFile(dosageFile), regions, markers,
        ...
        if (dominanceFormat == INDIVIDUAL_DOMINANT_FORMAT) {
          for (int i = 0; i < markerNames.length; i++) {
            if (!markerNames[i].equals(line[beginDataCol + numDataCols * i])) {
              String msg =
                  "Error - mismatched name at marker " + (i + 1) + " of " + dosageFile
                      + "; expecting " + markerNames[i] + " given map file " + mapFile + ", found "
                      + line[beginDataCol + numDataCols * i];
              log.reportError(msg);
              reader.close();
              return;
            }
          ...
          if (numDataCols != 2) {
            if (verbose) {
              String msg =
                  "Warning - ignoring the header with IDs in file " + dosageFile
            ...
            for (int i = 0; i < ids.length; i++) {
              if (!ids[i][0].equals(line[beginDataCol + numDataCols * i + 0])
                ...
                String msg =
                    "Error - mismatched IDs at individual " + (i + 1) + " of " + dosageFile
                        + "; expecting " + ids[i][0] + "," + ids[i][1] + " given id file " + idFile
                        + ", found " + line[beginDataCol + numDataCols * i + 0] + ","
      ...
      if (dominanceFormat == MARKER_DOMINANT_FORMAT) {
        ...
        for (int i = firstMkr; i < markerNames.length; i++) {
          ...
          if (indexA1 != -1) {
            if (line[indexA1].length() > 1 || !Sequence.validAllele(line[indexA1])) {
              if (verbose) {
                String msg =
                    "Warning - invalid allele ('" + line[indexA1] + "') at marker "
                        + markerNames[i];
                log.reportError(msg);
              }
            }
          ...
          if (indexA2 != -1) {
            if (line[indexA2].length() > 1 || !Sequence.validAllele(line[indexA2])) {
              if (verbose) {
                String msg =
                    "Warning - invalid allele ('" + line[indexA2] + "') at marker "
                        + markerNames[i];
                log.reportError(msg);
              }
            }
          ...
          if (indexChr >= 0) {
            try {
              ...
              if (!invalids.containsKey(line[indexChr])) {
                if (verbose) {
                  String msg =
                      "Warning - invalid chromosome number ('" + line[indexChr]
          ...
          if (indexPos != -1) {
            try {
              ...
              if (!invalids.containsKey(line[indexPos])) {
                if (verbose) {
                  String msg =
                      "Warning - invalid genome position ('" + line[indexPos] + "') for marker "
                          + markerNames[i];
                  log.reportError(msg);
                }
          ...
          if (line.length - beginDataCol != ids.length * numDataCols) {
            String msg =
                "Error - mismatched number of elements in line " + (i + 1 + indexHeaderRow)
                    + " of " + dosageFile + "; expecting " + ids.length + "*" + numDataCols + "+"
                    + beginDataCol + "[=" + (ids.length * numDataCols + beginDataCol) + "], found "
                    + line.length;
            log.reportError(msg);
            System.exit(1);
          }
          if (numDataCols == 1) {
            for (int j = 0; j < ids.length; j++) {
              dosageValues[index][j] =
                  ext.isMissingValue(line[beginDataCol + j]) ? Float.NaN : Float
                      .parseFloat(line[beginDataCol + j]);
            }
          } else {
            for (int j = 0; j < ids.length; j++) {
              genotypeProbabilities[index][j][0] =
                  ext.isMissingValue(line[beginDataCol + j * numDataCols + 0]) ? Float.NaN : Float
                      .parseFloat(line[beginDataCol + j * numDataCols + 0]);
              genotypeProbabilities[index][j][1] =
                  ext.isMissingValue(line[beginDataCol + j * numDataCols + 1]) ? Float.NaN : Float
                      .parseFloat(line[beginDataCol + j * numDataCols + 1]);
              if (numDataCols == 3) {
                genotypeProbabilities[index][j][2] =
                    ext.isMissingValue(line[beginDataCol + j * numDataCols + 2]) ? Float.NaN
                        : Float.parseFloat(line[beginDataCol + j * numDataCols + 2]);
                if (Math
                    .abs((1 - genotypeProbabilities[index][j][1] - genotypeProbabilities[index][j][0])
        ...
        for (int i = 0; i < ids.length; i++) {
          ...
          if (line.length - beginDataCol != markerNames.length * numDataCols) {
            String msg =
                "Error - mismatched number of elements in line " + (i + 1 + indexHeaderRow)
                    + " of " + dosageFile + "; expecting " + markerNames.length + "*" + numDataCols
                    + "+" + beginDataCol + "[=" + (markerNames.length * numDataCols + beginDataCol)
                    + "], found " + line.length;
            log.reportError(msg);
            System.exit(1);
          }
              ...
              if (numDataCols == 3) {
                genotypeProbabilities[index][i][2] =
                    ext.isMissingValue(line[beginDataCol + j * numDataCols + 2]) ? Float.NaN
                        : Float.parseFloat(line[beginDataCol + j * numDataCols + 2]);
                if (Math
                    .abs((1 - genotypeProbabilities[index][i][1] - genotypeProbabilities[index][i][0])
  ...
  public void analyze(String phenoFile, String phenoMissingValue, String snpList, boolean verbose,
    ...
    hash =
        HashVec.loadFileToHashString(phenoFile, new int[] {0, 1},
    ...
    try {
      ...
      writer =
          new PrintWriter(new FileWriter(ext.rootOf(phenoFile, false) + ".results."
      ...
      for (int i = 0; i < markerNames.length; i++) {
        if (analyze[i]) {
          ...
          model =
              logistic ? new LogisticRegression(deps, indeps, false, false) : new LeastSquares(
                  deps, indeps, false, false);
          ...
          if (betas.length != traits.length + 1) {
            ...
            w2.println(markerNames[i] + "\t" + alleles[i][0] + "\t" + alleles[i][1] + "\t"
                ...
                + ext.prettyP(pvals[1], sigfig, 4, 3, true) + "\t"
                + ext.formDeci(betas[1], 6, true) + "\t" + ext.formDeci(stderrs[1], 6, true));
            for (int j = 1; j < traits.length; j++) {
              writer.println(chrs[i] + "\t" + markerNames[i] + "\t" + positions[i] + "\t"
  ...
  public void writeToFile(String filename, String mapOut, String extractMarkers,
      String regionsFile, boolean allowIncompleteList, boolean writeNaNsAsPeriods, int format,
      Logger log) {
    String[] markersToKeep =
        extractMarkers == null ? null : HashVec.loadFileToStringArray(extractMarkers, false, false,
    ...
    if (regionsFile == null) {
      ...
      String[] rgns =
          HashVec
              .loadFileToStringArray(regionsFile, false, false, new int[] {0}, true, false, "\t");
              ...
              for (int j = 0; j < ids.length; j++) {
                writer.print(delimiter
                    + (Float.isNaN(genotypeProbabilities[i][j][0]) ? (writeNaNsAsPeriods ? "."
                        : Float.NaN) : ext.formDeci(genotypeProbabilities[i][j][0], parameters[13],
                        parameters[12] == parameters[13])));
                writer.print(delimiter
                    + (Float.isNaN(genotypeProbabilities[i][j][1]) ? (writeNaNsAsPeriods ? "."
                        : Float.NaN) : ext.formDeci(genotypeProbabilities[i][j][1], parameters[13],
                        parameters[12] == parameters[13])));
                if (parameters[3] == 3) {
                  if (genotypeProbabilities[i][j].length > 2) {
                    writer.print(delimiter
                        + (Float.isNaN(genotypeProbabilities[i][j][2]) ? (writeNaNsAsPeriods ? "."
                            : Float.NaN) : ext.formDeci(genotypeProbabilities[i][j][2],
                    ...
                    writer
                        .print(delimiter
                            + (Float.isNaN(genotypeProbabilities[i][j][0])
                                || Float.isNaN(genotypeProbabilities[i][j][1]) ? (writeNaNsAsPeriods ? "."
                                : Float.NaN)
                                : ext.formDeci(1 - genotypeProbabilities[i][j][1]
                                    - genotypeProbabilities[i][j][0], parameters[13],
                                    parameters[12] == parameters[13])));
                  }
                }
              }
              ...
              if ((markersToKeep == null && regionsToKeep == null)
                ...
                writer.print(delimiter
                    + (Float.isNaN(genotypeProbabilities[j][i][0]) ? (writeNaNsAsPeriods ? "."
                        : Float.NaN) : ext.formDeci(genotypeProbabilities[j][i][0], parameters[13],
                        parameters[12] == parameters[13])));
                writer.print(delimiter
                    + (Float.isNaN(genotypeProbabilities[j][i][1]) ? (writeNaNsAsPeriods ? "."
                        : Float.NaN) : ext.formDeci(genotypeProbabilities[j][i][1], parameters[13],
                        parameters[12] == parameters[13])));
                if (parameters[3] == 3) {
                  if (genotypeProbabilities[j][i].length > 2) {
                    writer.print(delimiter
                        + (Float.isNaN(genotypeProbabilities[j][i][2]) ? (writeNaNsAsPeriods ? "."
                            : Float.NaN) : ext.formDeci(genotypeProbabilities[j][i][2],
                            parameters[13], parameters[12] == parameters[13])));
                  } else {
                    writer
                        .print(delimiter
                            + (Float.isNaN(genotypeProbabilities[j][i][0])
                                || Float.isNaN(genotypeProbabilities[j][i][1]) ? (writeNaNsAsPeriods ? "."
                                : Float.NaN)
                                : ext.formDeci(1 - genotypeProbabilities[j][i][1]
                                    - genotypeProbabilities[j][i][0], parameters[13],
                                    parameters[12] == parameters[13])));
                  }
                }
              }
  ...
  public void writeToPlinkBinary(String dir, String plinkRoot, Logger log) {
    ...
    try {
      ...
      for (String[] id : ids) {
        ...
        String[] parts =
            {id.length > 2 ? id[2] : "0", id.length > 3 ? id[3] : "0", id.length > 4 ? id[4] : "0",
                id.length > 5 ? id[5] : "-9",};
    ...
    try {
      ...
      for (int j = 0; j < markerNames.length; j++) {
        out.write(PlinkData.encodePlinkBedBytesForASingleMarkerOrSample(Array
            .toByteArray(dosageValues[j])));
      }
  ...
  public static DosageData combine(DosageData dd1, DosageData dd2, COMBINE_OP onDupeOp, Logger log) {
    ...
    System.gc();
    ddNew.genotypeProbabilities =
        ddNew.genotypeProbabilities == null ? null : Sort.putInOrder(ddNew.genotypeProbabilities,
            keys);
    System.gc();
    ddNew.dosageValues =
        ddNew.dosageValues == null ? null : Sort.putInOrder(ddNew.dosageValues, keys);
    System.gc();
  ...
  public static void convert(String dosageFile, String idFile, String mapFile,
      int[] fromParameters, String outfile, String mapOut, String extract, int[] toParameters,
            ...
            for (int i = 0; i < markerNames.length; i++) {
              if (!markerNames[i].equals(line[fromParameters[1] + fromParameters[3] * i])) {
                log.reportError("Error - mismatched name at marker " + (i + 1) + " of "
                    + dosageFile + "; expecting " + markerNames[i] + " given map file " + mapFile
        ...
        if (fromParameters[2] == MARKER_DOMINANT_FORMAT) {
          for (int i = 0; i < markerNames.length; i++) {
              ...
              try {
                ...
                if (!invalids.containsKey(line[fromParameters[8]])) {
                  log.reportError("Warning - invalid chromosome number ('"
                      + line[fromParameters[8]] + "'), first seen at marker " + markerNames[i]);
            ...
            if (extract == null || keeps.contains(markerNames[i])) {
              lead =
                  LEADS[toParameters[11]] == null ? new String[toParameters[1]]
              ...
              for (int j = 0; j < ids.length; j++) {
                ...
                if (toParameters[3] == 1) {
                  ...
                  writer.print(delimiter
                      + ext.formDeci(dosageValue, toParameters[13],
                          toParameters[12] == toParameters[13]));
                } else {
                  writer.print(delimiter
                      + ext.formDeci(genotypeProbabilities[0], toParameters[13],
                          toParameters[12] == toParameters[13]));
                  writer.print(delimiter
                      + ext.formDeci(genotypeProbabilities[1], toParameters[13],
                          toParameters[12] == toParameters[13]));
                  if (toParameters[3] == 3) {
                    if (fromParameters[3] == 3) {
                      writer.print(delimiter
          ...
          for (int i = 0; i < ids.length; i++) {
            ...
            if (line.length - fromParameters[1] != markerNames.length * fromParameters[3]) {
              log.reportError("Error - mismatched number of elements in line "
                  ...
                  + markerNames.length + "*" + fromParameters[3] + "+" + fromParameters[1]
                  + ", found " + line.length);
              System.exit(1);
            }

            lead =
                LEADS[toParameters[11]] == null ? new String[toParameters[3]]
                    : LEADS[toParameters[11]];
  ...
  public static DosageData loadPlinkBinary(String dir, int[][] regionsToKeep,
      String[] markersToKeep, String plinkRoot, String markerNamePrepend, boolean loadMissingAsNaN) {
    DosageData dd = new DosageData();

    String[][] bimData =
        HashVec.loadFileToStringMatrix(dir + plinkRoot + ".bim", false,
    ...
    dd.ids =
        HashVec.loadFileToStringMatrix(dir + plinkRoot + ".fam", false,
            new int[] {0, 1, 2, 3, 4, 5}, false);
    ...
    for (int i = 0; i < bimData.length; i++) {
      ...
      markerNames[index] =
          (null == markerNamePrepend ? "" : (markerNamePrepend.endsWith("_") ? markerNamePrepend
              : markerNamePrepend + "_")) + bimData[i][1];
  ...
  private static String[] loadMarkers(String markersToUseFile, Logger log) {
    ...
    if (markersToUseFile != null && !"".equals(markersToUseFile)) {
      if (Files.exists(markersToUseFile)) {
        markers =
            HashVec.loadFileToStringArray(markersToUseFile, false, false, new int[] {0}, true,
                false, "\t");
      }
    } else {
  ...
  private static int[][] loadRegions(String regionsToUseFile, Logger log) {
    ...
    if (regionsToUseFile != null && !"".equals(regionsToUseFile)) {
      if (Files.exists(regionsToUseFile)) {
        String[] rgns =
            HashVec.loadFileToStringArray(regionsToUseFile, false, false, new int[] {0}, true,
                false, "\t");
        regions = new int[rgns.length][];
        for (int i = 0; i < rgns.length; i++) {
          regions[i] = Positions.parseUCSClocation(rgns[i]);
