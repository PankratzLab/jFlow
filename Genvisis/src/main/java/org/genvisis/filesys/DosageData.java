...
public class DosageData implements Serializable {
        ...
        if (dominanceFormat == INDIVIDUAL_DOMINANT_FORMAT) {
          for (int i = 0; i < markerNames.length; i++) {
            if (!markerNames[i].equals(line[beginDataCol + numDataCols * i])) {
              String msg = "Error - mismatched name at marker " + (i + 1) + " of " + dosageFile + "; expecting " + markerNames[i] + " given map file " + mapFile + ", found " + line[beginDataCol + numDataCols * i];
              log.reportError(msg);
              reader.close();
              return;
            }
          ...
          if (numDataCols != 2) {
            ...
            for (int i = 0; i < ids.length; i++) {
              if (!ids[i][0].equals(line[beginDataCol + numDataCols * i + 0]) || !ids[i][1].equals(line[beginDataCol + numDataCols * i + 1])) {
                String msg = "Error - mismatched IDs at individual " + (i + 1) + " of " + dosageFile + "; expecting " + ids[i][0] + "," + ids[i][1] + " given id file " + idFile + ", found " + line[beginDataCol + numDataCols * i + 0] + "," + line[beginDataCol + numDataCols * i + 1];
                log.reportError(msg);
                reader.close();
                return;
              }
      ...
      if (dominanceFormat == MARKER_DOMINANT_FORMAT) {
        ...
        for (int i = firstMkr; i < markerNames.length; i++) {
          ...
          if (!markerNames[i].equals(line[indexMkrIID])) {
            if (verbose) {
              String msg = "Error - mismatched name at marker " + (i + 1) + " of " + dosageFile + "; expecting " + markerNames[i] + " given map file " + mapFile + ", found " + line[indexMkrIID];
              log.reportError(msg);
            }
            reader.close();
            return;
          ...
          if (indexA1 != -1) {
            if (line[indexA1].length() > 1 || !Sequence.validAllele(line[indexA1])) {
              if (verbose) {
                String msg = "Warning - invalid allele ('" + line[indexA1] + "') at marker " + markerNames[i];
                log.reportError(msg);
              }
            }
          ...
          if (indexA2 != -1) {
            if (line[indexA2].length() > 1 || !Sequence.validAllele(line[indexA2])) {
              if (verbose) {
                String msg = "Warning - invalid allele ('" + line[indexA2] + "') at marker " + markerNames[i];
                log.reportError(msg);
              }
            }
          ...
          if (indexChr >= 0) {
            try {
              ...
              if (!invalids.containsKey(line[indexChr])) {
                if (verbose) {
                  String msg = "Warning - invalid chromosome number ('" + line[indexChr] + "'), first seen at marker " + markerNames[i];
                  log.reportError(msg);
                }
          ...
          if (indexPos != -1) {
            try {
              ...
              if (!invalids.containsKey(line[indexPos])) {
                if (verbose) {
                  String msg = "Warning - invalid genome position ('" + line[indexPos] + "') for marker " + markerNames[i];
                  log.reportError(msg);
                }
          ...
          if (line.length - beginDataCol != ids.length * numDataCols) {
            String msg = "Error - mismatched number of elements in line " + (i + 1 + indexHeaderRow) + " of " + dosageFile + "; expecting " + ids.length + "*" + numDataCols + "+" + beginDataCol + "[=" + (ids.length * numDataCols + beginDataCol) + "], found " + line.length;
            log.reportError(msg);
            System.exit(1);
          }
          if (numDataCols == 1) {
            for (int j = 0; j < ids.length; j++) {
              dosageValues[index][j] = ext.isMissingValue(line[beginDataCol + j]) ? Float.NaN : Float.parseFloat(line[beginDataCol + j]);
            }
          } else {
            for (int j = 0; j < ids.length; j++) {
              genotypeProbabilities[index][j][0] = ext.isMissingValue(line[beginDataCol + j * numDataCols + 0]) ? Float.NaN : Float.parseFloat(line[beginDataCol + j * numDataCols + 0]);
              genotypeProbabilities[index][j][1] = ext.isMissingValue(line[beginDataCol + j * numDataCols + 1]) ? Float.NaN : Float.parseFloat(line[beginDataCol + j * numDataCols + 1]);
              if (numDataCols == 3) {
                genotypeProbabilities[index][j][2] = ext.isMissingValue(line[beginDataCol + j * numDataCols + 2]) ? Float.NaN : Float.parseFloat(line[beginDataCol + j * numDataCols + 2]);
                if (Math.abs((1 - genotypeProbabilities[index][j][1] - genotypeProbabilities[index][j][0]) - genotypeProbabilities[index][j][2]) > 0.01) {
                  String msg = "Error: P(BB) does not equal [ 1 - P(AA) - P(AB) ] for individual " + ids[j][0] + "," + ids[j][1] + " at marker " + markerNames[i] + " which is line " + (i + 1 + indexHeaderRow) + " of " + dosageFile + ": " + line[beginDataCol + j * numDataCols + 0] + " " + line[beginDataCol + j * numDataCols + 1] + " " + line[beginDataCol + j * numDataCols + 2];
                  log.reportError(msg);
                }
              }
            }
        ...
        for (int i = 0; i < ids.length; i++) {
          ...
          if (line.length - beginDataCol != markerNames.length * numDataCols) {
            String msg = "Error - mismatched number of elements in line " + (i + 1 + indexHeaderRow) + " of " + dosageFile + "; expecting " + markerNames.length + "*" + numDataCols + "+" + beginDataCol + "[=" + (markerNames.length * numDataCols + beginDataCol) + "], found " + line.length;
            log.reportError(msg);
            System.exit(1);
          }
          if (numDataCols == 1) {
            ...
            for (int j = 0; j < markerNames.length; j++) {
              ...
              index++;
              dosageValues[index][i] = ext.isMissingValue(line[beginDataCol + j]) ? Float.NaN : Float.parseFloat(line[beginDataCol + j]);
              if (index == keepTotal - 1) {
                break;
              }
            }
            ...
            for (int j = 0; j < markerNames.length; j++) {
              ...
              index++;
              genotypeProbabilities[index][i][0] = ext.isMissingValue(line[beginDataCol + j * numDataCols + 0]) ? Float.NaN : Float.parseFloat(line[beginDataCol + j * numDataCols + 0]);
              genotypeProbabilities[index][i][1] = ext.isMissingValue(line[beginDataCol + j * numDataCols + 1]) ? Float.NaN : Float.parseFloat(line[beginDataCol + j * numDataCols + 1]);
              if (numDataCols == 3) {
                genotypeProbabilities[index][i][2] = ext.isMissingValue(line[beginDataCol + j * numDataCols + 2]) ? Float.NaN : Float.parseFloat(line[beginDataCol + j * numDataCols + 2]);
                if (Math.abs((1 - genotypeProbabilities[index][i][1] - genotypeProbabilities[index][i][0]) - genotypeProbabilities[index][i][2]) > 0.01) {
                  String msg = "Error: P(BB) does not equal [ 1 - P(AA) - P(AB) ] for individual " + ids[i][0] + "," + ids[i][1] + " at marker " + markerNames[j] + " which is line " + (i + 1 + indexHeaderRow) + " of " + dosageFile + ": " + line[beginDataCol + j * numDataCols + 0] + " " + line[beginDataCol + j * numDataCols + 1] + " " + line[beginDataCol + j * numDataCols + 2];
                  log.reportError(msg);
                }
              }
              if (index == keepTotal - 1) {
  ...
  public void writeToFile(String filename, String mapOut, boolean allowIncompleteList, boolean writeNaNsAsPeriods, int[] parameters, Logger log) {
    writeToFile(filename, mapOut, null, null, allowIncompleteList, writeNaNsAsPeriods, parameters, log);
  }
  ...
  public void writeToFile(String filename, String mapOut, String extractMarkers, String regionsFile, boolean allowIncompleteList, boolean writeNaNsAsPeriods, int format, Logger log) {
    ...
    writeToFile(filename, mapOut, markersToKeep, regions, allowIncompleteList, writeNaNsAsPeriods, PARAMETERS[format], log);
  }
  ...
  public void writeToFile(String filename, String mapOut, String[] markersToKeep, int[][] regionsToKeep, boolean allowIncompleteList, boolean writeNaNsAsPeriods, int[] parameters, Logger log) {
    PrintWriter writer;
      ...
      if (parameters[2] == MARKER_DOMINANT_FORMAT) {
        for (int i = 0; i < markerNames.length; i++) {
          if ((markersToKeep == null && regionsToKeep == null) || keeps.contains(markerNames[i])) {
            ...
            if (parameters[3] == 1) {
              for (int j = 0; j < ids.length; j++) {
                writer.print(delimiter + (Float.isNaN(dosageValues[i][j]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(dosageValues[i][j], parameters[13], parameters[12] == parameters[13])));
              }
            } else {
              if (genotypeProbabilities == null) {
              ...
              for (int j = 0; j < ids.length; j++) {
                writer.print(delimiter + (Float.isNaN(genotypeProbabilities[i][j][0]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(genotypeProbabilities[i][j][0], parameters[13], parameters[12] == parameters[13])));
                writer.print(delimiter + (Float.isNaN(genotypeProbabilities[i][j][1]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(genotypeProbabilities[i][j][1], parameters[13], parameters[12] == parameters[13])));
                if (parameters[3] == 3) {
                  if (genotypeProbabilities[i][j].length > 2) {
                    writer.print(delimiter + (Float.isNaN(genotypeProbabilities[i][j][2]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(genotypeProbabilities[i][j][2], parameters[13], parameters[12] == parameters[13])));
                  } else {
                    writer.print(delimiter + (Float.isNaN(genotypeProbabilities[i][j][0]) || Float.isNaN(genotypeProbabilities[i][j][1]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(1 - genotypeProbabilities[i][j][1] - genotypeProbabilities[i][j][0], parameters[13], parameters[12] == parameters[13])));
                  }
                }
              }
            }
          ...
          if (parameters[3] == 1) {
            for (int j = 0; j < markerNames.length; j++) {
              if ((markersToKeep == null && regionsToKeep == null) || keeps.contains(markerNames[j])) {
                writer.print(delimiter + (Float.isNaN(dosageValues[j][i]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(dosageValues[j][i], parameters[13], parameters[12] == parameters[13])));
              }
            }
          } else {
            for (int j = 0; j < markerNames.length; j++) {
              if ((markersToKeep == null && regionsToKeep == null) || keeps.contains(markerNames[j])) {
                writer.print(delimiter + (Float.isNaN(genotypeProbabilities[j][i][0]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(genotypeProbabilities[j][i][0], parameters[13], parameters[12] == parameters[13])));
                writer.print(delimiter + (Float.isNaN(genotypeProbabilities[j][i][1]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(genotypeProbabilities[j][i][1], parameters[13], parameters[12] == parameters[13])));
                if (parameters[3] == 3) {
                  if (genotypeProbabilities[j][i].length > 2) {
                    writer.print(delimiter + (Float.isNaN(genotypeProbabilities[j][i][2]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(genotypeProbabilities[j][i][2], parameters[13], parameters[12] == parameters[13])));
                  } else {
                    writer.print(delimiter + (Float.isNaN(genotypeProbabilities[j][i][0]) || Float.isNaN(genotypeProbabilities[j][i][1]) ? (writeNaNsAsPeriods ? "." : Float.NaN) : ext.formDeci(1 - genotypeProbabilities[j][i][1] - genotypeProbabilities[j][i][0], parameters[13], parameters[12] == parameters[13])));
                  }
                }
              }
            }
      ...
      if (onDupeOp == COMBINE_OP.EITHER_IF_OTHER_MISSING_ALL) {
        ...
        for (String s : duplicatedIDs) {
          ...
          for (String m : duplicatedMarkersAndIndices.keySet()) {
            ...
            boolean valid1 = (dd1.genotypeProbabilities != null && !checkMissing(dd1.genotypeProbabilities[mkrInd1][idInd1], missingGeno))
                             || (dd1.dosageValues != null && !checkMissing(dd1.dosageValues[mkrInd1][idInd1], missingDosage));
            if (valid1) {
              missingAll1 = false;
              break;
            }
          ...
          for (String m : duplicatedMarkersAndIndices.keySet()) {
            ...
            boolean valid2 = missingAll2 && (dd2.genotypeProbabilities != null && !checkMissing(dd2.genotypeProbabilities[mkrInd2][idInd2], missingGeno))
                             || (dd2.dosageValues != null && !checkMissing(dd2.dosageValues[mkrInd2][idInd2], missingDosage));
    ...
    if (ddNewNumGeno > 1) {
      ...
      while (markerIter.hasNext()) {
        ...
        for (int s = 0; s < ddNew.ids.length; s++) {
          ...
          float[] dd1Data = (mkrIn1 && idIn1) ? dd1.genotypeProbabilities[dd1MarkersAndIndices.get(mkr)][dd1IdsAndIndices.get(id)] : null;
          float[] dd2Data = (mkrIn2 && idIn2) ? dd2.genotypeProbabilities[dd2MarkersAndIndices.get(mkr)][dd2IdsAndIndices.get(id)] : null;
...
//              valid data in dd1 and missing data in dd2
              if (onDupeOp == COMBINE_OP.EITHER_IF_OTHER_MISSING) {
                ...
                // should have died before this point if onDupeOp is FAIL, and wouldn't reach this check if onDupeOp is DROP
              }
            } else if (!miss1 && miss2) {
//            valid data in dd1 and missing data in dd2
              if (onDupeOp == COMBINE_OP.EITHER_IF_OTHER_MISSING) {
      ...
      while (markerIter.hasNext()) {
        ...
        for (int s = 0; s < ddNew.ids.length; s++) {
          ...
          float dd1Data = (mkrIn1 && idIn1) ? dd1.dosageValues[dd1MarkersAndIndices.get(mkr)][dd1IdsAndIndices.get(id)] : null;
          float dd2Data = (mkrIn2 && idIn2) ? dd2.dosageValues[dd2MarkersAndIndices.get(mkr)][dd2IdsAndIndices.get(id)] : null;
...
//            valid data in dd1 and missing data in dd2
  ...
  private static boolean checkMissing(float f, float missingDose) {
    ...
    if (missNaN && !Float.isNaN(f))
      return false;
    if (Float.compare(f, missingDose) != 0)
      return false;
    return true;
  }
  ...
  private static boolean checkMissing(float[] fs, float missingGeno) {
    ...
    for (float f : fs) {
      if (missNaN && !Float.isNaN(f))
        return false;
      if (Float.compare(f, missingGeno) != 0)
        return false;
    }
    return true;
  }
                ...
                if (fromParameters[3] == 1) {
                  ...
                  if (fromParameters[3] == 3) {
                    ...
                    if (Math.abs((1 - genotypeProbabilities[1] - genotypeProbabilities[0]) - genotypeProbabilities[2]) > 0.01) {
                      log.reportError("Error: P(BB) does not equal [ 1 - P(AA) - P(AB) ] for individual " + ids[j][0] + "," + ids[j][1] + " at marker " + markerNames[i] + " which is line " + (i + 1 + fromParameters[4]) + " of " + dosageFile + ": " + line[fromParameters[1] + j * fromParameters[3] + 0] + " " + line[fromParameters[1] + j * fromParameters[3] + 1] + " "
              ...
              if (extract == null || keeps.contains(markerNames[j])) {
                if (fromParameters[3] == 1) {
                  ...
                  if (fromParameters[3] == 3) {
                    genotypeProbabilities[2] = Float.parseFloat(line[fromParameters[1] + j * fromParameters[3] + 2]);
                    if (Math.abs((1 - genotypeProbabilities[1] - genotypeProbabilities[0]) - genotypeProbabilities[2]) > 0.01) {
                      log.reportError("Error: P(BB) does not equal [ 1 - P(AA) - P(AB) ] for individual " + ids[i][0] + "," + ids[i][1] + " at marker " + markerNames[j] + " which is line " + (i + 1 + fromParameters[4]) + " of " + dosageFile + ": " + line[fromParameters[1] + j * fromParameters[3] + 0] + " " + line[fromParameters[1] + j * fromParameters[3] + 1] + " "
                          + line[fromParameters[1] + j * fromParameters[3] + 2]);
                    }
                  }
                }
