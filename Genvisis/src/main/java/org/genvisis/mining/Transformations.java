...
public class Transformations {
	...
	public static double[] rankTransform(double[] array) {
		...
		while (count<array.length) {
			plus = 0;
			while (count+plus+1<array.length&&array[order[count]]==array[order[count+plus+1]]) {
				plus++;
			}
			for (int i = 0; i<plus+1; i++) {
				newData[order[count+i]] = ((double)count*2+(double)plus)/2+1;
			}
			count += plus+1;
		}
	...
	public static void transformFile(String filename, String outfile, boolean ignoreFirstLine, int column, boolean commaDelimited, boolean replace, int type, Logger log) {
		...
		try {
			...
			if (ignoreFirstLine) {
				...
				if (!replace) {
					line = Array.insertStringAt(line[column]+"_"+Transformations.LABELS[type], line, column+1);
				}
	...
	public static void main(String[] args) {
		...
		"   (3) ignore first [header] line (i.e. ignoreFirstLine=" + ignoreFirstLine + " (default))\n" + 
		...
		"   (7) type of transformation (see below for options) (i.e. type=" + type + " (default))\n" + 
		"";
		for (int i = 0; i < LABELS.length; i++) {
			usage += "       "+i+"="+LABELS[i];
		}
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				...
				numArgs--;
			} else if (args[i].startsWith("ignoreFirstLine=")) {
				ignoreFirstLine = ext.parseBooleanArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("col=")) {
				col = ext.parseIntArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("comma=")) {
				commaDelimited = ext.parseBooleanArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("replace=")) {
				replace = ext.parseBooleanArg(args[i]);
				numArgs--;
			} else if (args[i].startsWith("type=")) {
				type = ext.parseIntArg(args[i]);
				numArgs--;
...
//			
			...
			transformFile(filename, outfile, ignoreFirstLine, col, commaDelimited, replace, type, new Logger(logfile));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
