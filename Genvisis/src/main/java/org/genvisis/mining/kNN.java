...
public class kNN {
	...
	public kNN(double[] newTargets, double[][] newPredictors, int[][] newPartitions) throws IOException {
		this();
		targets = newTargets;
	...
	public kNN calc() {
		...
		if (partitions[1].length==0) {
			...
			for (int i = startK; i<=stopK; i++) {
				for (int j = 0; j<3; j++) {
					errorRates[i][j] = calcError(targetsFromPartition(j), score(predictorsFromPartition(j), i, false));
				}
	...
	public double[] score(double[][] indeps, int K, boolean raw) {
		...
		for (int i = 0; i<indeps.length; i++) {
			...
			for (int j = 0; j<K; j++) {
				predicteds[i] += (quant&&weighted?(1/distances[keys[j]])/sum:1/(double)K)*targets[partitions[0][keys[j]]];
			}
	...
	public void report() {
		...
		try {
			...
			for (int i = startK; i<=stopK; i++) {
				writer.println(i+"\t"+ext.formDeci(errorRates[i][0], 4, true)+"\t"+ext.formDeci(errorRates[i][1], 4, true)+(partitions[2].length==0?"":"\t"+ext.formDeci(errorRates[i][2], 4, true))+(i==bestK?"\t<-- Best K":""));
			}
			writer.println();
			writer.println();
			if (!quant) {
				for (int set = 0; set<3; set++) {
					if (partitions[set].length>0) {
						...
						writer.println((set==0?"Training":(set==0?"Validation":"Test"))+" Data scoring - Summary Report (for k="+bestK+")");
						...
						writer.println("1\t"+(counts[0][0]+counts[0][1])+"\t\t"+counts[0][1]+"\t\t"+ext.formDeci(((double)counts[0][1]/(double)(counts[0][0]+counts[0][1]))*100, 2, true)+"%");
						writer.println("0\t"+(counts[1][0]+counts[1][1])+"\t\t"+counts[1][0]+"\t\t"+ext.formDeci(((double)counts[1][0]/(double)(counts[1][0]+counts[1][1]))*100, 2, true)+"%");
						writer.println();
			...
			for (int i = 0; i<predicteds.length; i++) {
				writer.println((quant?"":(predicteds[i]>=cutoff?1:0)+"\t")+actuals[i]+"\t"+predicteds[i]);
			}
	...
	public kNN(String filename) {
		...
		IntVector[] partitionIndices = new IntVector[] {new IntVector(), new IntVector(), new IntVector()};
		int count = 0;

		try {
			reader = new BufferedReader(new FileReader(filename));
			output = (filename.indexOf(".")>=0?filename.substring(0, filename.lastIndexOf(".")):filename)+"-report.out";
			header = reader.readLine().split("\t", -1);
			System.out.println("Assuming the last column ("+header[header.length-1]+") is the dependent variable");
			if (manualPartition = header[0].equals("Partition")) {
				System.out.println("Assuming the first column ("+header[0]+") will be used to partition the data");
			} else {
				System.out.println("Assuming there is no provided 'Partition' column; using defaults ("+PARTITION_DEFAULTS[0]+":"+PARTITION_DEFAULTS[1]+":"+PARTITION_DEFAULTS[2]+")");
			}
			while (reader.ready()) {
				line = reader.readLine().split("\t");
				if (line.length!=header.length) {
					System.err.println("Error - number of columns for row "+(count+1)+" does not match that for header");
					System.exit(3);
				}
				...
				for (int i = 0; i<line.length; i++) {
					try {
						if (i==line.length-1) {
							...
							if (line[0].length()!=1||(!line[0].equals("T")&&!line[0].equals("V")&&!line[0].equals("S"))) {
								System.err.println("Error - '"+line[0]+"' is not a  valid partition setting (use T for Training, V for Validation, and S for test set)");
								System.exit(4);
							}
						...
						System.err.println("Error - '"+line[i]+"' is not a valid integer, and we're not currently not set up to process missing fields");
						System.exit(5);
					}
				}
