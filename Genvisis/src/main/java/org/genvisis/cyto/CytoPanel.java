...
public class CytoPanel extends JPanel implements ActionListener {
	...
	public CytoPanel(Project proj, CytoGUI cytoGUI, String startDir, Logger log) {
		...
		this.log = (log == null ? new Logger(proj.PROJECT_DIRECTORY.getValue() + ext.getTimestampForFilename() + PANEL_LOG) : log);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source.equals(selectFilesButton)) {
			FileChooser fileChooser = new FileChooser(this, proj.PROJECT_DIRECTORY.getValue(), false, true, SELECT_TITLE, log);
...
//				CytoCompare.compare(proj, workBenchFiles, proj.getFilename(proj.COMMON_CNP_FILENAME), proj.getFilename(proj.REPORTED_CNP_FILENAME), proj.getFilename(proj.UNREPORTED_CNP_FILENAME), CytoCompare.DEFAULT_SCORE_THRESHOLD, startDir, true, new Logger(startDir + ext.getTimestampForFilename() + "_" + CytoCompare.DEFAULT_LOG));
				CytoCompare.compare(proj, workBenchFiles, proj.COMMON_CNP_FILENAME.getValue(), proj.REPORTED_CNP_FILENAME.getValue(), proj.UNREPORTED_CNP_FILENAME.getValue(), CytoCompare.DEFAULT_SCORE_THRESHOLD, startDir, true, new Logger(startDir + ext.getTimestampForFilename() + "_" + CytoCompare.DEFAULT_LOG));
			}
		} catch (Exception exc) {
...
//			if (!linuxSafeRootEquals(proj.getFilename(proj.COMMON_CNP_FILENAME),files[i]) && !linuxSafeRootEquals(proj.getFilename(proj.REPORTED_CNP_FILENAME),files[i]) && !linuxSafeRootEquals(proj.getFilename(proj.UNREPORTED_CNP_FILENAME),files[i])) {
			if (!linuxSafeRootEquals(proj.COMMON_CNP_FILENAME.getValue(), files[i]) && !linuxSafeRootEquals(proj.REPORTED_CNP_FILENAME.getValue(), files[i]) && !linuxSafeRootEquals(proj.UNREPORTED_CNP_FILENAME.getValue(), files[i])) {
				filter.add(files[i]);
			}
...
//		convertCNPfiles(proj, proj.getFilename(proj.COMMON_CNP_FILENAME), cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.getFilename(proj.COMMON_CNP_FILENAME)), true) + CytoCNVariant.CNV_EXT, log);
//		convertCNPfiles(proj, proj.getFilename(proj.REPORTED_CNP_FILENAME), cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.getFilename(proj.REPORTED_CNP_FILENAME)), true) + CytoCNVariant.CNV_EXT, log);
//		convertCNPfiles(proj, proj.getFilename(proj.UNREPORTED_CNP_FILENAME), cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.getFilename(proj.UNREPORTED_CNP_FILENAME)), true) + CytoCNVariant.CNV_EXT, log);
		convertCNPfiles(proj, proj.COMMON_CNP_FILENAME.getValue(), cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.COMMON_CNP_FILENAME.getValue()), true) + CytoCNVariant.CNV_EXT, log);
		convertCNPfiles(proj, proj.REPORTED_CNP_FILENAME.getValue(), cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.REPORTED_CNP_FILENAME.getValue()), true) + CytoCNVariant.CNV_EXT, log);
		convertCNPfiles(proj, proj.UNREPORTED_CNP_FILENAME.getValue(), cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.UNREPORTED_CNP_FILENAME.getValue()), true) + CytoCNVariant.CNV_EXT, log);
	}
...
//		proj.setProperty(proj.CNV_FILENAMES, proj.getProperty(proj.CNV_FILENAMES) + (proj.getProperty(proj.CNV_FILENAMES).length() > 0 ? ";" : "") + cnvFile);
...
//		if (Files.exists(proj.getFilename(proj.SAMPLELIST_FILENAME)) && proj.getSampleList() != null && proj.getSampleList().getSamples().length > 0) {
//			new File(proj.getFilename(proj.SAMPLELIST_FILENAME)).delete();
		if (Files.exists(proj.SAMPLELIST_FILENAME.getValue()) && proj.getSampleList() != null && proj.getSampleList().getSamples().length > 0) {
	...
	private boolean checkAlreadyParsed() {
		...
		if (importFiles != null && importFiles.length > 0) {
			for (int i = 0; i < importFiles.length; i++) {
				String file = proj.SOURCE_DIRECTORY.getValue(false, true) + ext.rootOf(importFiles[i], true) + proj.getProperty(proj.SOURCE_FILENAME_EXTENSION);
...
//		return verifySampleDataIssues(issues.toArray(new String[issues.size()]), proj.getFilename(proj.SAMPLE_DATA_FILENAME));
		return verifySampleDataIssues(issues.toArray(new String[issues.size()]), proj.SAMPLE_DATA_FILENAME.getValue());
	}
...
//			String dialoge = "Could not find the Common CNP file " + proj.getFilename(proj.COMMON_CNP_FILENAME);
...
//			String dialoge = "Could not find the Reported CNP file " + proj.getFilename(proj.REPORTED_CNP_FILENAME);
		if (parseResponse(response) && !Files.exists(proj.REPORTED_CNP_FILENAME.getValue())) {
			String dialoge = "Could not find the Reported CNP file " + proj.REPORTED_CNP_FILENAME.getValue();
...
//			String dialoge = "Could not find the  Un - Reported CNP file " + proj.getFilename(proj.UNREPORTED_CNP_FILENAME);
		if (parseResponse(response) && !Files.exists(proj.COMMON_CNP_FILENAME.getValue())) {
			String dialoge = "Could not find the  Un - Reported CNP file " + proj.UNREPORTED_CNP_FILENAME.getValue();
			response = JOptionPane.showOptionDialog(this, dialoge, "Error", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
		}
	...
	public class FileBox extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (source.equals(selectButton)) {
				FileChooser fileChooser = new FileChooser(this, proj.PROJECT_DIRECTORY.getValue(), false, false, SELECT_TITLE, log);
