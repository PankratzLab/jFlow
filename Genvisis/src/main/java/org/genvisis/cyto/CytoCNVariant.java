 ...
 * Data can look like:
 * <p>
 * 1.1 p36.11 chr1 A_18_P10061443 25599273 25599326 ref|Homo sapiens Rh blood group, D antigen
 * (RHD), transcript variant 2, mRNA. RHD 0.31111443 0.45292675 -0.46087968 0 -0.096791215 0
 * 0.2777727 0
 */
public class CytoCNVariant extends CNVariant {
  ...
  private static int[][] determineSampleIndices(String[] header, Logger log) {
    ...
    for (String element : header) {
      if (element.equals(CYTO_SAMPLE_HEADER[0])) {
        count++;
      }
    }
    ...
    try {
      ...
      do {
        ...
        count++;
          ...
          && (ext.indexFactors(CYTO_ABERATION_HEADER, line, false, true, false, false)[0] == -1));
      if (!reader.ready()) {
        log.reportError(
            "Error - did not find the neccesary column headers in file " + cytoCNVariantFile);
        return sampleNames;
      }
      reader.close();

      reader = Files.getAppropriateReader(cytoCNVariantFile);
      // the sample names are located right above the data header, so we re-scan the reader to that
      // location
      do {
        line = reader.readLine().trim().split(SPLITS[0], -1);
        scan++;
   ...
   * <p>
   * If it is adding to a previous aberation (same logRatio score, we update the cytoCNVariant to
   * reflect the new info (new stop position, add any new markers/genes)
   * <p>
   ...
   * @param tmps temporary variants corresponding to one per sample as we scan the rows. Assuming no
   *        amplification/duplication combinations
  ...
  private static void parseInputLine(String[] sampleNames, String[] line, int[] indicesCommon,
      int[][] indicesSamples, CytoCNVariant[] tmps, ArrayList<CytoCNVariant> cytoCNVariant,
      Logger log) {
    ...
    try {
      ...
      for (int i = 0; i < indicesSamples.length; i++) {
        ...
        if (ext.indexOfStr(sampAmp, NOTHINGS, true, true) == -1
          ...
          log.reportError(
              "Error - found an amplification and a deletion on line " + Array.toStr(line));
          return;
        }
        if (ext.indexOfStr(sampAmp, NOTHINGS, true, true) == -1) {
          ...
          if (tmps[i] != null) { // if one existed on the previous line
            if (tmps[i].isSame(logRatio) && tmps[i].isSameChr(chr)) {// if it is the same aberration
              ...
              tmps[i] = new CytoCNVariant("" + (i + 1),
                  ext.replaceWithLinuxSafeCharacters(sampleNames[i], true), chr, start, stop, CN,
                  DEFAULT_SCORE, 1, 0, cytoBand, logRatio, probe, gene);
            }
  ...
  public static void test(String cytoCNVariantFile, Logger log) {
    ...
    for (CytoCNVariant test : tests) {
      log.report(test.getMyReport());
    }
  }
   ...
   * <p>
   * Warning - this method assumes that FID\tIID ids are unique to a sample, this is not always the
   * case
   * <p>
  ...
  public static CytoCNVariant[][] toIndividuals(CytoCNVariant[] cytoCNVariants, Logger log) {
    ...
    for (CytoCNVariant cytoCNVariant : cytoCNVariants) {
      ...
      track.get(key).add(cytoCNVariant);
    }
  ...
  /**
   * @param CytoCNVariant CytoCNVariant[] to write
  ...
  private final double avgLogRatio;
  ...
  public CytoCNVariant(String familyID, String individualID, byte chr, int start, int stop, int cn,
      double score, int numMarkers, int source, String cytoBand, double avgLogratio,
      String firstProbe, String firstGene) {
   ...
   */
  public CytoCNVariant(String familyID, String individualID, byte chr, int start, int stop, int cn,
      double score, int numMarkers, int source, String cytoBand, String[] probeNames,
      String[] genes, double avgLogratio) {
    super(familyID, individualID, chr, start, stop, cn, score, numMarkers, source);
    this.cytoBand = cytoBand;
  ...
  /**
   * Call this method to clear ArrayLists and convert to arrays. Assigns number of probes found.
   * Used for loading from file when unknown number of probes are being added
   */
  public void finalizeVariant() {
    probeNames = probeNamestmp.toArray(new String[probeNamestmp.size()]);
    probeNamestmp.clear();
    genes = genestmp.toArray(new String[genestmp.size()]);
    genestmp.clear();
    numMarkers = probeNames.length;
    formatCytoBand();
    cytoBandtmp.clear();
  }
  ...
  public String getInterpretation() {
    ...
    if (getCN() == 1) {
      ...
      System.err.println("Error - invalid copy number " + getCN()
          + " for CytoCNVariants, copy number must be " + Array.toStr(CN_NUMBER, ", or"));
      return "";
    }
  ...
  public boolean update(String newCytoBand, byte newChr, int newstop, String newProbeName,
    ...
    if (newstop <= stop) {
      log.reportError(
          "Error - the current stop being added is less than the previous stop position, this should not happen");
      updated = false;
    }
    if (newstop <= start) {
      log.reportError(
          "Error - the current stop being added is less than the start position, this should not happen");
      updated = false;
    }
    if (!isSame(newAvgLogratio)) {
      log.reportError(
          "Error - the current logRatio does not equal the previous, this should not happen");
      updated = false;
    }
    if (updated) {
