 ...
 * 
 * https://cran.r-project.org/web/packages/wbs/wbs.pdf
 * 
 * Port of https://github.com/cran/wbs/blob/8919e7c35389b92e27b6948572271e0843b5f808/src/wbs.c
 */
public class WBS {
	...
	private static void binarySegmentationRecursive(double[] x, int n, int s, int e, double[] res,
																									double[] iplus,
																									double[] iminus,
																									double[] ipres, double minth, int scale) {
	...
	private static void wbsIntegratedRecursive(double[] x, int n, int s, int e, double[] res,
																						 double[] iplus,
																						 double[] iminus,
																						 double[] ipres, double[] wbsres, int[] index,
																						 int indexn, int M,
																						 double minth,
																						 int scale) {
		int len = e - s + 1;
	 ...
	 * https://github.com/PankratzLab/Analysis/blob/master/WBS/WBS_TestClip.md
	 * 
	 * types taken from R code
	 * 
	 * Note for why the int/augmented version
	 * 
	 * The optional augmentation of Ms,e by {0} is done to ensure that the algorithm also exam- ines
	 * the entire current interval [s, e], and not only its randomly drawn subintervals, in case [s,e]
	 * only contains one change-point and hence it is optimal to examine [s,e] in its entirety. We
	 * note that unlike the BS procedure, the WBS algorithm (in the case without the optional
	 * augmentation) returns estimated change-points in the order corresponding to decreasing maxima
	 * of |X ̃b |, which is due to the maximisation over m. There is no corresponding sm ,em
	 * maximisation in the BS procedure, which means that the maxima of the CUSUM statis- tics
	 * corresponding to estimated change-points in the latter procedure are not necessarily arranged
