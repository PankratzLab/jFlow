...
class ChangePoints {
	 ...
	 */
	static ChangePoints changepointsSbs(List<ChangePoint> wlist, double[] x,
																			double thConst, int kmax) {
	...
	static ChangePoints changepointsWbs(List<ChangePoint> wlist, double[] x,
																			double thConst, List<Penalty_type> penaltyTypes, int kmax) {
	...
	static double[] computeMeansBetweenChangepoints(double[] x, List<ChangePoint> wlist) {
		...
		assignMean(means, x, x.length,
							 wlist.get(wlist.size()
												 - 1)
										.getChangePoint());

		for (int i = 0; i < wlist.size() - 1; i++) {
			assignMean(means, x, wlist.get(i + 1)
																.getChangePoint(),
								 wlist.get(i)
											.getChangePoint());
		}
	...
	private static void assignMean(double[] means, double[] x, int stop, int start) {
		double mean = ArrayUtils.mean(ArrayUtils.subArray(x,
																											start, stop));
