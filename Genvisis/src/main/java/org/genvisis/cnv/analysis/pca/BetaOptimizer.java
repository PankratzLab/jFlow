...
public class BetaOptimizer {
  private static class BetaCorrelationResult {
    ...
    public BetaCorrelationResult(int comparisonIndex, double correlPearsonSigned,
        double pPearsonSigned, double correlSpearmanSigned, double pSpearmanSigned,
        double correlPearsonUnSigned, double pPearsonUnSigned, double correlSpearmanUnSigned,
  ...
  private static class BetaWorker implements Callable<BetaCorrelationResult[]> {
    ...
    private BetaCorrelationResult getResult(double[] data) {
      ...
      for (int i = 0; i < genotypes.length; i++) {
        if (i % 2000 == 0) {
          log.reportTimeInfo("computing betas for genotype " + (i + 1) + " of " + genotypes.length
              + " for index " + comparisonIndex + " on " + Thread.currentThread().getName());
      ...
      BetaCorrelationResult result = new BetaCorrelationResult(comparisonIndex, pearsonSigned[0],
          pearsonSigned[1], spearmanSigned[0], spearmanSigned[1], pearsonUnSigned[0],
          pearsonUnSigned[1], spearmanUnSigned[0], spearmanUnSigned[1], dataBetas.size());
      return result;
    }
    ...
    boolean flipBetas() {
      if (!validMatch) {
        throw new IllegalArgumentException(
            "Did not have valid rs id match, this method should not be used");
      }
  ...
  private static void analyze(Project proj, String pcFile, String singleRaceSamples,
      MarkerSet markerSet, ABLookup abLookup, String dbsnpVCF, String[] namesToQuery,
      String outpuDir, List<String> betaFiles, double[] pvals, double markerCallRate, int maxPCs,
      int numthreads, String usedInPCFile, int pvalRefineCutoff, Logger log) {
    ...
    for (String betaFile : betaFiles) {
      ...
      if (!Files.exists(bigSummaryOut)) {
        ...
        try {
          ...
          if (singleRaceSamples != null && Files.exists(singleRaceSamples)) {
            ...
            log.reportTimeInfo(
                "Loaded " + sampsForMods.length + " samples from " + singleRaceSamples);
            ...
            log.reportTimeWarning(
                "A file of unrelated and single race samples was not provided for optimization");
          }
          for (double pval : pvals) {
              ...
              for (RUN_TYPES runtype : RUN_TYPES.values()) {
                ...
                for (ORDER_TYPE oType : ORDER_TYPE.values()) {
                  ...
                  if (Files.exists(file)) {
                    ...
                    if (markersRemoved > 0) {
                      log.reportTimeWarning(
                          "Removed " + markersRemoved + " markers for pval cut-off " + pval
                      ...
                      try {
                        ...
                        while (train.hasNext()) {
                          ...
                          writer.println(Array.toStr(results[0].getSummary()) + "\t"
                              ...
                              + method + "\t" + pval + "\t" + numSamps + "\t" + method + "_" + pval
                              + "\t" + markerCallRate);
                        }
      ...
      for (double pval : pvals) {
        ...
        methodPU.setTitle(
            "ABS Betas from " + ext.rootOf(betaFile) + "\nMarker Call rate >= " + markerCallRate);
        ...
        methodPSNat.setTitle(
            "Betas from " + ext.rootOf(betaFile) + "\nMarker Call rate >= " + markerCallRate);
        ...
        RScatter methodPUInv = new RScatter(bigSummaryOut, methodProotUnsignedInv + ".rscript",
            ...
            new String[] {"inv_correl_pearsonSigned"}, "Method", SCATTER_TYPE.POINT, log);
        ...
        RScatter methodPSNatInv = new RScatter(bigSummaryOut, methodProotsignednatInv + ".rscript",
            ext.rootOf(methodProotsignednatInv), methodProotsignednatInv + ".jpeg", "PC",
            new String[] {"inv_correl_spearmanSigned"}, "Method", SCATTER_TYPE.POINT, log);
      ...
      RScatters rscScattersBetas = new RScatters(rScattersBetas, rootOutBetas + ".rscript",
          rootOutBetas + ".pdf", COLUMNS_MULTIPLOT.COLUMNS_MULTIPLOT_1, PLOT_DEVICE.PDF, log);
      rscScattersBetas.execute();

      RScatters rscScattersInvBetas = new RScatters(rScattersInvBetas, rootOutInvBetas + ".rscript",
          rootOutInvBetas + ".pdf", COLUMNS_MULTIPLOT.COLUMNS_MULTIPLOT_1, PLOT_DEVICE.PDF, log);
  ...
  private static void analyzeAll(Project proj, String pcFile, String samplesToBuildModels,
      MarkerSet markerSet, ABLookup abLookup, String dbsnpVCF, String[] namesToQuery,
      String outpuDir, String[] betas, double[] pvals, double markerCallRate, int maxPCs,
      int numthreads, String usedInPCFile, int pvalRefineCutoff, Logger log) {
  ...
  private static ArrayList<MetaBeta> loadBetas(ArrayList<MarkerRsFormat> markerRsFormats,
    ...
    if (Array.countIf(indices, -1) > 0) {
      log.reportTimeError(
          "Did not detect proper header in " + betaFile + ", requires " + Array.toStr(BETA_HEADER));
      return null;
    } else {
              ...
              if (Numbers.isFinite(beta) && Numbers.isFinite(p) && p < minPval) {
                ...
                if (useConfig(strandConfig) && current.isValidMatch()) {
                  ...
                  switch (strandConfig) {
                    ...
                    default:
                      throw new IllegalArgumentException(
                          "Should not have strand config " + strandConfig + " in current");
                  }
  ...
  public static ArrayList<MarkerRsFormat> mapToRsIds(Project proj, ABLookup abLookup,
      String dbsnpVCF, String[] namesToQuery, String outSer, Logger log) {
    MarkerSet markerSet = proj.getMarkerSet();
    ...
    if (proj.GENOME_BUILD_VERSION.getValue() != GENOME_BUILD.HG19) {
      if (proj.ARRAY_TYPE.getValue() == ARRAY.AFFY_GW6
        ...
        proj.getLog().reportTimeInfo(
            "Attempting to use " + GENOME_BUILD.HG19.getBuild() + " positions for rsID lookup");
        Resource affyhg19 =
        ...
        throw new IllegalArgumentException(
            "Genome version must be " + GENOME_BUILD.HG19.getBuild());
      }
    }
    ...
    for (int i = 0; i < posIndices.length; i++) {
      if (posIndices[i] >= 0) {
        segs[i] = new Segment(markerSet.getChrs()[posIndices[i]],
            markerSet.getPositions()[posIndices[i]], markerSet.getPositions()[posIndices[i]] + 1);
      } else {
    ...
    try {
      ...
      log.reportTimeInfo(
          "Attempting to look up " + namesToQuery.length + " markers as rsIds from " + dbsnpVCF);
      for (int i = 0; i < segs.length; i++) {
        ...
        MarkerRsFormat markerRsFormat = new MarkerRsFormat(namesToQuery[i], current.getStart(),
            indices[i], "NA", new String[] {"N", "N"}, allelesMarker, CONFIG.STRAND_CONFIG_UNKNOWN,
            SITE_TYPE.UNKNOWN);
        ...
        if (Array.countIf(allelesMarker, "N") == 0) {
          ...
          while (!foundGood && vcIter.hasNext()) {
            ...
            if (!vc.isPointEvent() || current.getStart() == vc.getStart()
                || Integer.parseInt(
                    VCOps.getAnnotationsFor(new String[] {"RSPOS"}, vc, "-1")[0]) == current
                        .getStart()) {
              ...
              for (String[] element : allelesVC) {
                if (!foundGood) {
                  ...
                  markerRsFormat = new MarkerRsFormat(namesToQuery[i], current.getStart(),
                      indices[i], vc.getID(), element, allelesMarker, config,
                      vc.isBiallelic() ? SITE_TYPE.BIALLELIC : SITE_TYPE.TRIALLELIC);
                  switch (config) {
  ...
  public static void optimize(Project proj, String pcFile, String outDir, String betaLoc,
      String unRelatedFile, String pcSamps, double[] pvals, int maxPCs, double markerCallRate,
      int pvalRefineCutoff, int numthreads) throws IllegalStateException {
    ...
    if (!Files.exists(proj.AB_LOOKUP_FILENAME.getValue())) {
      if (Files.exists(proj.AB_LOOKUP_FILENAME.getValue())) {
        ...
        if (proj.ARRAY_TYPE.getValue() == ARRAY.AFFY_GW6
            || proj.ARRAY_TYPE.getValue() == ARRAY.AFFY_GW6_CN) {
          Resource abAffy = ARRAY_RESOURCE_TYPE.AFFY_SNP6_ABLOOKUP
              .getResource(proj.GENOME_BUILD_VERSION.getValue());
          if (abAffy.isAvailable(proj.getLog())) {
            ...
            throw new IllegalStateException(
                "Could not retrieve required AB lookup for affymetrix array, halting");
          }
        }
        if (!Files.exists(proj.AB_LOOKUP_FILENAME.getValue())) {
          abLookup.parseFromAnnotationVCF(proj);
  ...
  public static ArrayList<MetaBeta> prep(Project proj, MarkerSet markerSet, ABLookup abLookup,
      String dbsnpVCF, String[] namesToQuery, String outpuDir, String betaFile, double minPval,
      Logger log) {
