...
public class PennCNVPrep {
  ...
  public PennCNVPrep(Project proj, PrincipalComponentsResiduals principalComponentsResiduals,
                     ...
                     String[] markers, int numComponents, String dir, LS_TYPE lType, int numThreads,
  ...
  public Hashtable<String, Float> exportSpecialSamples(String[] fileNamesOfMarkerDataInOrder,
    ...
    String[] subSamples = ArrayUtils.subArray(proj.getSamples(), samplesToExport);
    proj.getLog()
        .report("Info - checking for existing files in " + correctedSampleDirectory + "...");
    boolean allExist = true;
    for (int i = 0; i < subSamples.length; i++) {
      if (!Files.exists(correctedSampleDirectory + subSamples[i] + Sample.SAMPLE_FILE_EXTENSION)) {
        allExist = false;
      }
    }
    ...
    if (!allExist) {// if all files exist we skip this export
      ...
      for (int i = 0; i < shadowSamples.length; i++) {
        ...
        if (i % 200 == 0) {
          proj.getLog()
              .report(ext.getTime()
                      + "\tData loaded = " + Math.round(((double) i / shadowSamples.length) * 100.0)
      ...
      for (ShadowSample shadowSample : shadowSamples) {
        shadowSample.writeShadow(proj.getLog(), correctedSampleDirectory,
                                 proj.getMarkerSet().getFingerprint(),
                                 proj.getArrayType().getCanXYBeNegative(), allOutliers);
      }
    } else {
      proj.getLog()
          .report("Info - detected that all " + subSamples.length + " shadow samples exist in "
                  + correctedSampleDirectory + " for the current batch, skipping export...");
    }
    return allOutliers;
  }
  ...
  public static class ShadowSample {
    ...
    public Hashtable<String, Float> writeShadow(Logger log, String dir, long fingerprint,
      ...
      if (!valid) {
        log.reportError("Error data was not valid");
        return null;
      }
      new File(dir).mkdirs();
  ...
  public static final String getCorrectedProjectDirectory(Project proj, int numComponents,
                                                          CORRECTION_TYPE correctionType,
                                                          CHROMOSOME_X_STRATEGY sexStrategy) {
    return proj.PROJECT_DIRECTORY.getValue() + "pcCorrected_" + numComponents + "PCs_"
           + correctionType.name() + "_" + sexStrategy.name() + "/";
  }
  ...
  public static final String getCorrectedProjectProperties(Project proj, int numComponents,
    ...
    return projectPropsDir + ext.rootOf(proj.getPropertyFilename(), true) + "_pcCorrected_"
           + numComponents + "PCs" + correctionType.name() + "_" + sexStrategy.name()
           + ".properties";
  }
  ...
  public static void exportSpecialPennCNV(Project proj, String dir, String tmpDir,
    ...
    String projectDirectory = getCorrectedProjectDirectory(proj, numComponents, correctionType,
                                                           sexStrategy);
    ...
    if (shadowSamples) {
      ...
      boolean[][] batches = ArrayUtils.splitUpStringArrayToBoolean(proj.getSamples(),
  ...
  private static class WorkerShadow implements Callable<Hashtable<String, Float>> {
    ...
    private final String correctedSamplesDir;
    private final Logger log;

    public WorkerShadow(PennCNVPrep specialPennCNVFormat, String[] sortedFileNames, boolean[] batch,
                        int batchIndex, boolean forceLoadFromFiles,
                        String correctedSamplesDirectory, Logger log) {
      super();
      ...
      this.correctedSamplesDir = correctedSamplesDirectory;
      this.log = log;
    }
    ...
    public Hashtable<String, Float> call() {
      ...
      Hashtable<String, Float> outliers = specialPennCNVFormat.exportSpecialSamples(sortedFileNames,
                                                                                    correctedSamplesDir,
                                                                                    batch,
                                                                                    forceLoadFromFiles);
      return outliers;
  ...
  public static void main(String[] args) {
    ...
    usage += "   (7) a full path to a file listing markers to export in the current batch, (i.e. markers="
             + numComponents + " (default))\n" + "";
    usage += "   (8) create PennCNV files from the tempory markerData files, (i.e. -create ( not the default))\n"
             + "";
    usage += "   (9) set this up for a batch run, which is recommended. Set to 0 if batch is not wanted (i.e. batch="
             + batch + " (default))\n" + "";
    ...
    usage += "   (12) export shadow samples for quickly comparing the current sample data to corrected data(i.e. -shadow ( not the default))\n"
             + "";
    usage += "   (12) if exporting shadow samples, the number of chunks to export at once (this many samples*the number of threads will be held in memory). A value <= 0 will make chunking determined automatically. (i.e. sampleChunks="
    ...
    usage += "   (15) if using a large number of PCs (>150) use a svd regression method (i.e. -svd (not the default))\n"
             + "";
    usage += "   (16) number of threads for a single marker (correction within a marker) (i.e. numThreads="
             + numThreads + " (default))\n" + "";
    usage += "   (17) number of threads for between a marker  (correction between a marker)(i.e. numMarkerThreads="
             + numThreads + " (default))\n" + "";
