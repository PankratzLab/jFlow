...
public class CNVBurdenIterator {
	...
	// boolean collapseCNVTypes, boolean binaryCNVcounts) {
	// int males = 0, females = 0;
	...
	// }
	//
	// dataTable = HashVec.loadFileToHashString(dataFile, ID_COLS, DATA_COLS, false, "\t", true,
	// false, true);
	...
	// (collapseCNVTypes ? indiv.getValue().getCNVs(CNV_SIZES[sz], CNV_FILTERS[cn]-1) : 0);
	// if (binaryCNVcounts) {
	...
	//
	// resultsB[sex][cn][sz] = regression.getBetas().length > 4 ? regression.getBetas()[4] :
	// Double.NaN;
	...
	// header + "\t\t" + labels[sex] + header + "\t\t" + labels[sex] + header + "\tn=" +
	// VAR_SIZES[sex]);
	// for (int sz = 0; sz < CNV_SIZES.length; sz++) {
	...
	public void runProgram(	String cnvFile, String dataFile, String famFile, String idsFile,
		...
		binaryCNVCounts = binaryCNVcounts;
	...
	public static void main(String[] args) {
		...
		String idsFile = "";// "D:/SIDS and
												// IQ/ids_with_high_quality_CNV_data_from_blood_cluster.unrelateds_keep.dat";
		...
		for (String arg : args) {
			if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
				...
				numArgs--;
			} else if (arg.startsWith("idsFile=")) {
				idsFile = arg.split("=")[1];
				numArgs--;
			} else if (arg.startsWith("genderColumn=")) {
				genCol = Integer.parseInt(arg.split("=")[1]);
				numArgs--;
			} else if (arg.startsWith("idColumns=")) {
				String[] cols = arg.split("=")[1].split(",");
				idsCols = new int[cols.length];
				for (int j = 0; j < cols.length; j++) {
					idsCols[j] = Integer.parseInt(cols[j]);
				}
				numArgs--;
			} else if (arg.startsWith("dataColumns=")) {
				String[] cols = arg.split("=")[1].split(",");
				...
				numArgs--;
			} else if (arg.startsWith("dependColumn=")) {
				depCol = Integer.parseInt(arg.split("=")[1]);
				numArgs--;
				...
				logisticRun = true;
				numArgs--;
			} else if (arg.startsWith("-stepwise")) {
				stepwise = true;
				numArgs--;
			}
		}
