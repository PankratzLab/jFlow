...
public class PCA {
   ...
   *        {@link PrincipalComponentsApply} object returned will only have the extrapolated pc file
   *        populated
   */
  public static PrincipalComponentsApply applyLoadings(Project proj, int numComponents,
      String singularFile, String markerLoadingFile, String useFile, boolean excludeSamples,
      boolean imputeMeanForNaN, boolean recomputeLRR, String output, GcAdjustorParameters params) {
    ...
    PrincipalComponentsApply pcApply = new PrincipalComponentsApply(proj, numComponents,
        singularFile, markerLoadingFile, samplesToUse, imputeMeanForNaN, recomputeLRR);
   ...
   *        already exist, the {@link PrincipalComponentsCompute} object returned will only have the
   *        existing filenames populated. The U,V,and W matrices will not be computed again
   */
  public static PrincipalComponentsCompute computePrincipalComponents(Project proj,
    ...
    return PrincipalComponentsCompute.getPrincipalComponents(proj, excludeSamples, numComponents,
        printFullData, center, reportMarkerLoadings, reportSingularValues, imputeMeanForNaN,
        recomputeLRR, useFile, output, parameters);
  }
  ...
  public static PrincipalComponentsResiduals computeResiduals(Project proj, String pcFile,
      String markersToAssessFile, int numComponents, boolean printFull, float gcThreshold,
      boolean homozygousOnly, boolean recomputeLRR, String output, GcAdjustorParameters params) {
    PrincipalComponentsResiduals pcResids =
        new PrincipalComponentsResiduals(proj, pcFile, markersToAssessFile, numComponents,
            printFull, gcThreshold, homozygousOnly, recomputeLRR, ext.rootOf(output));
  ...
  public static PrincipalComponentsApply generateFullPCA(Project proj, int numComponents,
      String outputBase, boolean recomputeLRR_PCs, boolean imputeMeanForNaN,
    ...
    PrincipalComponentsCompute pcs = PCA.computePrincipalComponents(proj, false, numComponents,
        false, false, true, true, imputeMeanForNaN, recomputeLRR_PCs,
        proj.PROJECT_DIRECTORY.getValue() + outputBase + PCA_SAMPLES, outputBase, params);
    ...
    PrincipalComponentsApply pcApply = PCA.applyLoadings(proj, numComponents,
        pcs.getSingularValuesFile(), pcs.getMarkerLoadingFile(), null, false, imputeMeanForNaN,
  ...
  public static void main(String[] args) {
    ...
    String usage = // String usage = "\n"+
        "cnv.analysis.PCA requires 1 argument\n"
            ...
            + "   (4) supply a file with a list of samples (one per line) to use for PCA computation (i.e. useFile="
            + useFile + " (default , use all samples))\n"
            + "   (5) number of principal components to compute  (i.e. components=" + numComponents
            + " (default))\n"
            ...
            + "   (4) a marker loading file to use (i.e. markerLoadingFile=" + markerLoadingFile
            + " (default))\n" + "   (5) a file containing singular values (i.e. singularValueFile="
            + singularValueFile + " (default))\n"
            + "   (6) supply a file with a list of samples (one per line) to use for PCA extrapolation (i.e. useSampleFile="
            + useFile + " (default , apply to all samples))\n"
            + "   (7) number of principal components (using marker loadings to apply)   (i.e. components="
            ...
            + "   (5) supply a file with a list of markers (one per line) to use to compute median LRR values and residuals from PCs (i.e. markers="
            + markersToassessFile + " (default))\n"
            + "   (6) pc file to use for residuals  (i.e. pcFile=" + pcFile + " (default))\n"
            + "   (7) number of principal components to use for residuals (i.e. components="
            + numComponents + " (default))\n"
            + "   (8) gcThreshold filter for computing median LRR values (i.e. gcThreshold="
            + gcThreshold + " (default, no filtering))\n"
    ...
    for (String arg : args) {
      if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
        ...
        excludeSamples = true;
        numArgs--;
      } else if (arg.startsWith("-printFull")) {
        printFullData = true;
        numArgs--;
        ...
        numComponents = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("gcThreshold=")) {
        gcThreshold = Float.parseFloat(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("useSampleFile=")) {
        useFile = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("markers=")) {
        markersToassessFile = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("markerLoadingFile=")) {
        markerLoadingFile = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("singularValueFile=")) {
        singularValueFile = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("useApplyfile=")) {
        useApplyfile = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("-impute")) {
        imputeMeanForNaN = true;
        numArgs--;
      } else if (arg.startsWith("-center")) {
        center = true;
        numArgs--;
      } else if (arg.startsWith("-homozygousOnly")) {
        homozygousOnly = true;
        numArgs--;
    ...
    if (applyLoadings) {
      applyLoadings(proj, numComponents, singularValueFile, markerLoadingFile, useFile,
          excludeSamples, imputeMeanForNaN, recomputeLRR, evalOut, null);
    } else if (computeResiduals) {
      computeResiduals(proj, pcFile, markersToassessFile, numComponents, printFullData, gcThreshold,
      ...
      PrincipalComponentsCompute pc =
          computePrincipalComponents(proj, excludeSamples, numComponents, printFullData, center,
              true, true, imputeMeanForNaN, recomputeLRR, useFile, evalOut, null);
