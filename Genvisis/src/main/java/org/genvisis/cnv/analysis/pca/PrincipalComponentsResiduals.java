...
public class PrincipalComponentsResiduals implements Cloneable, Serializable {
  ...
  private static PrintWriter[] getNWriters(Project proj, String[] fileOuts) {
    ...
    for (int i = 0; i < fileOuts.length; i++) {
      ...
      try {
        writers[i] = Files.openAppropriateWriter(proj.PROJECT_DIRECTORY.getValue() + fileOuts[i]);
      } catch (FileNotFoundException fnfe) {
   ...
   *          NOTE: the pc object does not have to be in project order, or contain all samples, but
   *          it should
  ...
  public CrossValidation getCorrectedDataAt(double[] data, double[][] extraIndeps,
    ...
    if (samplesTobuildModel != null && samplesTobuildModel.length != numSamples) {
      proj.getLog()
          .reportError("Error - boolean definitions of samples to build the regression model must be provided for every sample in the project, only found  "
    ...
    if (data.length != numSamples) {
      proj.getLog()
          .reportError("Error - data points must be provided for every sample in the project, only found "
                       + data.length);
   ...
   * @param log
   * @return the {@link StatsCrossTabRank } array for all data requested NOTE: the data is only
   *         ranked against individual PCs, not
   */
