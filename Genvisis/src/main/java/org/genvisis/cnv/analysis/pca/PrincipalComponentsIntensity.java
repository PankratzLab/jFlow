...
public class PrincipalComponentsIntensity extends PrincipalComponentsResiduals {
  ...
  public static class PcCorrectionProducer extends AbstractProducer<PrincipalComponentsIntensity> {
    ...
    public PcCorrectionProducer(PrincipalComponentsResiduals pcResiduals, int correctAt,
        int[] sampleSex, boolean[] samplesToUseCluster, LS_TYPE lType, int numCorrectionThreads,
        int numDecompressThreads, String[] markersToCorrect) {
      super();
          ...
          new Callable<PrincipalComponentsIntensity>() {
            ...
            public PrincipalComponentsIntensity call() throws Exception {
              PrincipalComponentsIntensity principalComponentsIntensity =
                  new PrincipalComponentsIntensity(pcResiduals, markerData, true, sampleSex,
                      samplesToUseCluster, 1, 0, null, true, lType, 2, 5, DEFAULT_RESID_STDV_FILTER,
                      DEFAULT_CORRECTION_RATIO, numCorrectionThreads, false, null);
  ...
  private static class WorkerRegression implements Callable<CrossValidation> {
    ...
    public WorkerRegression(PrincipalComponentsResiduals principalComponentsResiduals,
        double[] data, boolean[] samplesTobuildModel, int clusterComponent, LS_TYPE lType,
        String title, boolean verbose, Logger log) {
      super();
    ...
    public CrossValidation call() {// acts like run
      return principalComponentsResiduals.getCorrectedDataAt(data, samplesTobuildModel,
          clusterComponent, lType, title, verbose);
  ...
  private static int getProperComponent(int atComponent, int numGenoCounts, int geno,
      double correctionRatio, String markerName, boolean verbose, Logger log) {
  ...
  // private CrossValidation getRegression(double[] data, boolean[] samplesTobuildModel, int
  // clusterComponent,boolean sv) {
  // CrossValidation cvalX = getCorrectedDataAt(data, samplesTobuildModel, clusterComponent,
  // svdRegression, "Genotype cluster: " + i + " X values");
  // return cvalX;
  // }
  ...
  private static CentroidCompute prepareProperCentroid(ARRAY array, MarkerData markerData,
      int[] sampleSex, boolean[] samplesToUseCluster, double missingnessThreshold,
      double confThreshold, ClusterFilterCollection clusterFilterCollection, boolean medianCenter,
      Logger log) {
  ...
  public static void test(Project proj, String pcFile, int numComponents, boolean recomputeLRR,
    ...
    // String[] markers =
    // HashVec.loadFileToStringArray(proj.getFilename(Project.DISPLAY_MARKERS_FILENAME), false, new
    // int[] { 0 }, false);
    // MarkerDataLoader markerDataLoader =
    // MarkerDataLoader.loadMarkerDataFromListInSeparateThread(proj, markers);
    // for (int i = 0; i < markers.length; i++) {
    // MarkerData markerData = markerDataLoader.requestMarkerData(i);
    // PrincipalComponentsIntensity pcIntensity = new PrincipalComponentsIntensity(testResids,
    // markerData, recomputeLRR, sampleSex, samplesToUseCluster, missingnessThreshold,
    // confThreshold, clusterFilterCollection, medianCenter, svdRegression, 1, 5, 2, true, output);
    ...
    // corrected += markerData.getXs()[j] + "\t" + markerData.getYs()[j] + "\t" +
    // pcIntensity.getCorrectedXFull()[j] + "\t" + pcIntensity.getCorrectedYFull()[j] + "\n";
    // }
    //
  ...
  private static float toCartesianX(float[] centroid) {
    return (float) (centroid[1]
        / (1 + Math.sin(centroid[0] * Math.PI / 2) / Math.cos(centroid[0] * Math.PI / 2)));
  }

  private static float toCartesianY(float[] centroid) {
    return (float) (centroid[1]
        / (1 + Math.cos(centroid[0] * Math.PI / 2) / Math.sin(centroid[0] * Math.PI / 2)));
  }
  ...
  public PrincipalComponentsIntensity(
      final PrincipalComponentsResiduals principalComponentsResiduals, final MarkerData markerData,
      boolean recomputeLRR, int[] sampleSex, boolean[] samplesToUseCluster,
      double missingnessThreshold, double confThreshold,
      ClusterFilterCollection clusterFilterCollection, boolean medianCenter, LS_TYPE lType,
      int correctionMethod, int nStage, double residStandardDeviationFilter, double correctionRatio,
      int numThreads, boolean verbose, String output) {
    ...
    centroid = prepareProperCentroid(getProj().getArrayType(), markerData, sampleSex,
        samplesToUseCluster, missingnessThreshold, confThreshold, clusterFilterCollection,
      ...
      if (centroid.getMarkerData().getAbGenotypes() != null) {
        ...
        for (int i = 0; i < abGenotypes.length; i++) {
          if (!samplesToUse[i]
            ...
            byte nearestThetaGenotype = findNearestTheta(genotypeThetaCenters, Centroids.calcTheta(
                centroid.getMarkerData().getXs()[i], centroid.getMarkerData().getYs()[i]));
            if (nearestThetaGenotype >= 0) {
              assignTrueAt(i, nearestThetaGenotype, genoSampleClusters);
              genoClusterCounts[nearestThetaGenotype]++;
            } else {
              assignAllFalseAt(i, genoSampleClusters);
              proj.getLog().reportError("Warning - marker "
                  + centroid.getMarkerData().getMarkerName() + " for sample " + samples[i]
      ...
      if (hasSomeGenotypes()) {
        ...
        proj.getLog()
            .reportError("Error - correction has failed for marker due to not having any genotypes "
                + centroid.getMarkerData().getMarkerName()
                + ", returning orgiginal x, y, and genotype values");
        setOriginal();
      }
    } else {
      proj.getLog()
  ...
  private void correctXYRegression(int atComponent) {
    CrossValidation[][] cvals =
        threadIt(atComponent, Array.toDoubleArray(centroid.getMarkerData().getXs()),
    ...
    for (int i = 0; i < genoSampleClusters.length; i++) {
      int clusterComponent = getProperComponent(atComponent, genoClusterCounts[i], i,
          correctionRatio, centroid.getMarkerData().getMarkerName(), verbose, proj.getLog());
      if (cvals[i] != null && cvals[i][1] != null && cvals[i][0] != null
        ...
        correctedXCluster[i] =
            Array.toFloatArray(Array.subArray(cvals[i][0].getResiduals(), genoSampleClusters[i]));
        correctedYCluster[i] =
            Array.toFloatArray(Array.subArray(cvals[i][1].getResiduals(), genoSampleClusters[i]));
      } else {
        if (cvals[i] == null || cvals[i][1] == null || cvals[i][0] == null
            || cvals[i][0].analysisFailed() || cvals[i][1].analysisFailed()) {
          if (verbose) {
          ...
          if (!forceThisCluster[j]) {// this cluster has two few inds, we won't use it for new
            ...
            if (fullXPredicteds[j] != null && fullYPredicteds[j] != null
              ...
              if (!Double.isNaN(fullXPredicteds[j][i]) && !Double.isNaN(fullYPredicteds[j][i])) {
                if (Double.compare(residStandardDeviationFilter, 0) == 0 || (Math
                    .abs(residY[j][i]) < residStandardDeviationFilter * residstdevY[j]
                    && Math.abs(residX[j][i]) < residStandardDeviationFilter * residstdevX[j])) {
                  double tmpDist =
                      cartDistance(Xs[i], Ys[i], fullXPredicteds[j][i], fullYPredicteds[j][i]);
                  if (tmpDist < minDist) {
                    minDist = tmpDist;
                    tmpGenotype = (byte) j;
                  }
  ...
  public float[][] getCorrectedIntensity(String type, boolean originalGenotypes) {
    ...
    byte[] genotypesToUse =
        originalGenotypes ? centroid.getMarkerData().getAbGenotypes() : getGenotypesUsed();
    MarkerData tmpMarkerData = new MarkerData(centroid.getMarkerData().getMarkerName(),
        centroid.getMarkerData().getChr(), centroid.getMarkerData().getPosition(),
        centroid.getMarkerData().getFingerprint(), centroid.getMarkerData().getGCs(), null, null,
        correctedXFull, correctedYFull, null, null, null, null, genotypesToUse, genotypesToUse);
  ...
  private void mergeCorrectedClusters() {
    ...
    for (int i = 0; i < numTotalSamples; i++) {
      ...
      for (int j = 0; j < genoSampleClusters.length; j++) {
        if (genoSampleClusters[j][i] && centroid.getCentroid()[j] != null) {
          ...
          correctedXFull[i] = Math.max(0,
              correctedXCluster[j][genoIndices[j]] + toCartesianX(centroid.getCentroid()[j]));
          correctedYFull[i] = Math.max(0,
              correctedYCluster[j][genoIndices[j]] + toCartesianY(centroid.getCentroid()[j]));
          genoIndices[j]++;
          hasCorrection = true;
    ...
    if (!hasAnyCorrection) {
      proj.getLog().reportTimeWarning(
          "Marker " + centroid.getMarkerData().getMarkerName() + " was unable to be corrected...");
    }
  }
  ...
  // principalComponentsResiduals, final MarkerData markerData, boolean recomputeLRR, int[]
  // sampleSex, boolean[] samplesToUseCluster, double missingnessThreshold, double confThreshold,
  // ClusterFilterCollection clusterFilterCollection, boolean medianCenter, boolean svdRegression,
  // int correctionMethod, int nStage, double residStandardDeviationFilter, double correctionRatio,
  // int numThreads, boolean verbose, String output) {
  // super(principalComponentsResiduals);// we hijack the loading of the PC file and tracking of
  // samples etc ...
  ...
  // this.verbose = verbose;
  // this.nStage = nStage;
  // this.fail = (markerData.getXs() == null || markerData.getYs() == null ||
  // markerData.getAbGenotypes() == null);
  // this.centroid = prepareProperCentroid(markerData, sampleSex, samplesToUseCluster,
  // missingnessThreshold, confThreshold, clusterFilterCollection, medianCenter, proj.getLog());
  ...
  private CrossValidation[][] threadIt(int atComponent, double[] Xs, double[] Ys) {
    ...
    for (int i = 0; i < genoSampleClusters.length; i++) {
      ...
      if (validClusterComponent(genoClusterCounts[i], clusterComponent, numTotalSamples)) {
        ...
        WorkerRegression workerX =
            new WorkerRegression(this, Xs,
                genoSampleClusters[i], clusterComponent, lType, "Genotype cluster: " + i
                    + " X values for Marker " + centroid.getMarkerData().getMarkerName(),
                verbose, log);
        tmpResults.put(keyX, executor.submit(workerX));
        WorkerRegression workerY =
            new WorkerRegression(this, Ys,
                genoSampleClusters[i], clusterComponent, lType, "Genotype cluster: " + i
                    + " Y values for Marker " + centroid.getMarkerData().getMarkerName(),
                verbose, log);
    ...
    for (int i = 0; i < genoSampleClusters.length; i++) {
      ...
      if (validClusterComponent(genoClusterCounts[i], clusterComponent, numTotalSamples)) {
        ...
        try {
          ...
          proj.getLog().reportError("Error - could not correct "
              + centroid.getMarkerData().getMarkerName() + " regression has failed");
...
// */
// private void estimateNewGenotypes(int atComponent) {
// double[] xMatchedPCs = Array.toDoubleArray(Array.subArray(centroid.getMarkerData().getXs(),
// samplesToUse));
// double[] yMatchedPCs = Array.toDoubleArray(Array.subArray(centroid.getMarkerData().getYs(),
// samplesToUse));
// double[][] fullXPredicteds = new double[3][];
// double[][] fullYPredicteds = new double[3][];
// double[][] residX = new double[3][];
...
// for (int i = 0; i < genoSampleClusters.length; i++) {
// int clusterComponent = getProperComponent(atComponent, genoClusterCounts[i], i,
// centroid.getMarkerData().getMarkerName(), verbose, proj.getLog());
// if (validClusterComponent(genoClusterCounts[i], clusterComponent, numTotalSamples)) {
// double[][] train_indeps = genoPCsClusters[i].returnUpToNumComponents(clusterComponent);// train
// on pcs within a cluster
// long time =System.currentTimeMillis();
// CrossValidation cvalXTest = getCorrectedDataAt(testXs, genoSampleClusters[i], clusterComponent,
// svdRegression, "Genotype cluster: " +i+" X values");
// CrossValidation cvalYTest = getCorrectedDataAt(testYs, genoSampleClusters[i], clusterComponent,
// svdRegression, "Genotype cluster: " +i+" X values");
// System.out.println(ext.getTimeElapsed(time) + " for New method");
// time=System.currentTimeMillis();
// CrossValidation cvalX = getPredictedValues(genoXClusters[i], train_indeps, xMatchedPCs,
// getTrimmedPreppedPCs(clusterComponent,false), svdRegression, proj.getLog());
// CrossValidation cvalY = getPredictedValues(genoYClusters[i], train_indeps, yMatchedPCs,
// getTrimmedPreppedPCs(clusterComponent,false), svdRegression, proj.getLog());
// System.out.println(ext.getTimeElapsed(time) + " for OLD method");
//
// System.out.println("Are Xs equal" + Arrays.equals(cvalX.getResiduals(),
// cvalXTest.getResiduals()));
// System.out.println("Are Ys equal" + Arrays.equals(cvalY.getResiduals(),
// cvalYTest.getResiduals()));
//
//
...
// residX[i] = cvalX.getResiduals();
// residstdevX[i] = Array.stdev(Array.subArray(residX[i], genoSampleClusters[i]), true);// compute
// standard deviation only from members of that cluster
// } else {
// residX[i] = null;
// residstdevX[i] = Double.NaN;
// }
...
// if (fullXPredicteds[j] != null && fullYPredicteds[j] != null && genoPCsClusters[j].getNumStored()
// > 1) {// must have predicteds, and at more than one individual
// if (residStandardDeviationFilter == 0 || (Math.abs(residY[j][i]) < residStandardDeviationFilter *
// residstdevY[j] && Math.abs(residX[j][i]) < residStandardDeviationFilter * residstdevX[j])) {
// double tmpDist = cartDistance(xMatchedPCs[i], yMatchedPCs[i], fullXPredicteds[j][i],
// fullYPredicteds[j][i]);
// if (tmpDist < minDist) {
// minDist = tmpDist;
// tmpGenotype = (byte) j;
...
// if (forceThisCluster[i]) {
// if (genoSampleClusters[i][j]) {// if a sample belongs to this cluster
// estimatedGenotypes[j] = centroid.getMarkerData().getAbGenotypes()[j]; // which will always be the
// original at this point
// }
// } else if (estimatedGenotypes[j] == -1) {
// estimatedGenotypes[j] = centroid.getMarkerData().getAbGenotypes()[j]; // didnt get a new cluster,
// set to original
// } else {
// // new genotype!
// }
...
//
//
// private static CrossValidation getPredictedValues(double[] train_deps, double[][] train_indeps,
// double[] val_deps, double[][] val_indeps, boolean svdRegression, Logger log) {
// CrossValidation cval = new CrossValidation(train_deps, train_indeps, val_deps, val_indeps, true,
// svdRegression, log);
// cval.train();
...
// genoPCsClusters[i].addToNextIndex(preppedPCs[samplesInPc.get(samples[j])]);// add all the
// components, matched by the index in the pc file
// }
// }
// }
// for (int i = 0; i < genoPCsClusters.length; i++) {
// if (!genoPCsClusters[i].allFull()) {
// proj.getLog().reportError("Error - could not match all individuals with PCs to their respective
// genotypes");
// }
// }
// }
