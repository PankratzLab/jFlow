...
public class LRRBDevHetOutlierClassifier {
  ...
  static final class AnalysisData {
    ...
    public void addIndivData(String idStr, IndividualData individualData) {
      dataMap.put(idStr, individualData);
    }
    ...
    public int getBdevCol() {
      return bdevCol;
    }

    public String getBdevHdr() {
      return bdevHdr;
    }

    public int getLrrCol() {
      return lrrCol;
    }

    public String getLrrHdr() {
      return lrrHdr;
    }

    public void setBdevHdr(String bdevHdr) {
      this.bdevHdr = bdevHdr;
    }

    public void setLrrHdr(String lrrHdr) {
      this.lrrHdr = lrrHdr;
    }
  ...
  static final class PopulationData {
    ...
    public PopulationData(double bdevMean, double lrrMean, double bdevSD, double lrrSD,
        double bdevMed, double lrrMed) {
      stdDevBDev = bdevSD;
      stdDevLRR = lrrSD;
      meanBDev = bdevMean;
      meanLRR = lrrMean;
  ...
  ////
  // // 4.0 derived through examination of results from other measures - outliers classified by
  // other measures all exceed 4.0 on this test.
  ...
  // public boolean testIndiv(PopulationData popData, IndividualData indivDatum) {
  // if (indivDatum.dataBDev >= 0.1
  // && (indivDatum.dataLRR >= (popData.meanLRR + (popData.stdDevLRR * 2)) || indivDatum.dataLRR <=
  // (popData.meanLRR - (popData.stdDevLRR * 2)))) {
  // return true;
  // }
  // return false;
  // }
  ...
  public static void main(String[] args) {
    ...
    for (String arg : args) {
      if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
        ...
        numArgs--;
      } else if (arg.startsWith("fileRoot=")) {
        fileRoot = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("vfile=")) {
        validationFilename = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("proj=")) {
        proj = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("excl=")) {
        exclFile = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("id=")) {
        idCol = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("lrr=")) {
        lrrCol = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("bdev=")) {
        bdevCol = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("lrrHdr=")) {
        lrrHdr = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("bdevHdr=")) {
        bdevHdr = arg.split("=")[1];
        numArgs--;
        ...
        runValidation = true;
        numArgs--;
      } else if (arg.equals("-validateOnly")) {
        validationOnly = true;
        numArgs--;
      } else if (arg.equals("-scores")) {
        writeScores = true;
        numArgs--;
        ...
        noHeader = true;
        numArgs--;
      } else if (arg.equals("-splitResults")) {
        splitResults = true;
        numArgs--;
      } else {
    ...
    if (runValidation) {
      try {
        validateResults(validationFilename, filename,
            ext.rootOf(filename, false) + "_validation.txt");
      } catch (IOException e) {
        e.printStackTrace();
      }
      return;
    ...
    try {
      ...
      if (!validationOnly) {
        classifier.writeResults(outFile, outliersOnly, validationFilename != null, false,
            writeScores, !noHeader, !splitResults);
      }
      classifier.dispose();
    } catch (Exception e) {
      e.printStackTrace();
   ...
   * @throws IOException
   */
  private static void validateResults(String validationFile, String resultsFile, String outFile)
      throws IOException {
          ...
          if (!excludeList.contains(line[idCol])) {
            ...
            for (AnalysisData analysis : analyses) {
              analysis.addIndivData(id,
                  new IndividualData(id, Double.parseDouble(line[analysis.lrrCol]),
    ...
    if (reader != null) {
      try {
        ...
        while ((temp = reader.readLine()) != null) {
          String[] line = temp.split("\t");
          if (!excludeList.contains(line[idCol])) {
            analysis.addIndivData(line[idCol], new IndividualData(line[idCol],
                Double.parseDouble(line[lrrCol]), Double.parseDouble(line[bdevCol])));
            popCnt++;
          }
        }
        reader.close();
  ...
  private void writeResults(String outFile, boolean writeOutliersOnly, boolean recodeOutliers,
      boolean recodeResults, boolean writeScores, boolean writeHeader, boolean combine) {
    ...
    if (combine) {
      ...
      for (AnalysisData analysis : analyses) {
        ...
        if (writeHeader) {
          writer.println(
              "ID\tOutliers" + (writeScores ? "\tScore" : "") + "\tN=" + analyses.get(0).popCount);
        }
