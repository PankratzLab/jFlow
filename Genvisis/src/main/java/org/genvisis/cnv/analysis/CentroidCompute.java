...
public class CentroidCompute {
  ...
  private boolean intensityOnly;
  private final boolean sexSpecific;
  private boolean failed;
  private boolean hasCentroid;
  private final boolean medianCenter;
  private double[] centerThetas, centerRs;
  private int[] counts, sampleSex;
  private final double missingnessThreshold, gcThreshold;
   ...
   * @param samplesToUse (optional, can be null)an array representing samples to use for the
   *        clustering
   * @param intensityOnly perform an intensity only clustering, useful for cnv-type probesets, we do
   *        not auto-detect cnv-probesets here
   * @param missingnessThreshold threshold for marker call rate: set to 1 for no filtering
   * @param confThreshold threshold for call confidence: set to 0 for no filtering
   * @param clusterFilterCollection (optional, can be null) if provided, these cluster filters will
   *        be applied
   * @param medianCenter computes the centroids using median values as opposed to average values.
   * @param log
   */
  ...
  public CentroidCompute(MarkerData markerData, int[] sampleSex, boolean[] samplesToUse,
    ...
    sexSpecific = sexSpecific(markerData.getChr());
    ...
    gcThreshold = confThreshold;
    this.medianCenter = medianCenter;
    centerThetas = new double[5];// 0=all,1=missing,2=AA,3=AB,4=BB
    centerRs = new double[5];
    counts = new int[5];
    centroid = null;
    alternateGenotypes = null;
    this.log = log;
  ...
  private void checkData() {
    ...
    if (sampleSex == null && sexSpecific) {
      // log.report("Warning - marker " + markerData.getMarkerName() + " is on chromosome " + chr +
      // " and sample sex was not provided, cluster centers may be inaccurate");
  ...
  private void assignGenotypeCentroid() {
    centroid = assignRegularCentroid(centerThetas, centerRs, counts, missingnessThreshold);// we
                                                                                           // first
                                                                                           // assign
                                                                                           // all
                                                                                           // the
  ...
  private void assignIntensityCentroid() {
    centroid = new float[3][2];
    centroid[0] = new float[] {(float) centerThetas[2], (float) centerRs[2]};// median/mean of the
                                                                             // log base 2 values
    centroid[1] = new float[] {(float) centerThetas[3], (float) centerRs[3]};// mean/median of the
                                                                             // log base 2 values
    centroid[2] = new float[] {(float) centerThetas[4], (float) centerRs[4]};// median/mean of the
                                                                             // actual X values
  }
  ...
  /**
   * We compute the center for thetas and Rs according to samplesToUse (if available), sex (if
   * available and required), confidence threshold, and clusterfilters.
   */
  ...
  private static class ArraySpecialLists {
    private final ArraySpecialList[] arraySpecialLists;

    public ArraySpecialLists(int num, int initCapacity) {
      arraySpecialLists = new ArraySpecialList[num];
      init(initCapacity);
    }
  ...
  /**
   * @param sampleSex int 1=male, 2=female
   * @param chr
  ...
  /**
   * In the case where one of the genotype clusters has 0 calls in the samples used to compute them,
   * we attempt to assign an alternate R value
   * <p>
  ...
  public static void test(Project proj) {
    ...
    for (int i = 0; i < markers.length; i++) {
      ...
      // System.out.println("LRR:" + Array.toStr(markerData.compareLRRs(cent.getCentroid())) + "\t"
      // + markers[i]);
      // System.out.println(cent.computeBAF()[0] + "\tBAF:\t" +
      // Array.toStr(stats.Correlation.Pearson(Array.toDoubleArray(cent.computeBAF()),
      // Array.toDoubleArray(markerData.getBAFs()))));
  ...
  public static class CentroidBuilder {
    ...
    /**
     * @param samplesToUse only cluster on these samples
     * @return
     */
    ...
    /**
     * @param intensityOnly this is an intensity only cluster
     * @return
     */
    ...
    /**
     * @param medianCenter cluster about the median value
     * @return
     */
    ...
    /**
     * @param sampleSex sex of each individual
     * @return
     */
    ...
    /**
     * @param missingnessThreshold call rate for clustering
     * @return
     */
    ...
    /**
     * @param gcThreshold gc threshold for clustering
     * @return
     */
   ...
   * @param fullpathToCentFile where the centroids will be stored
   * @param builder a centroid builder to use
   * @param numCentThreads number of threads for computing centroids
  ...
  private static class CentroidProducer extends AbstractProducer<CentroidCompute[]> {
    ...
    public CentroidProducer(Project proj, String[] markers, CentroidBuilder[] builders,
      ...
      mdl = new MDL(proj, proj.getMarkerSet(), markers, numDecompressThreads, 100);
