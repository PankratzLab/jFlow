...
public class CorrectionIterator implements Serializable {
  ...
  private static class IterationResult implements Serializable {
     ...
     */
    private static final long serialVersionUID = 1L;
    private static final String[] HERIT_ADDITIONS =
        new String[] {"NUM_PC", "MERLIN_PROPORTION_HERITABLITY", "SOLAR_PROPORTION_HERITABLITY",
            "SOLAR_PVAL", "SOLAR_ST_ERROR", "SOLAR_KURT", "SOLAR_KURT_WARNING"};
    ...
    public BooleanClassifier[] estimateBoxPlots(Project proj, String[] sampleDataStratCats,
        String[] numericStratCats, ArrayList<RScatter> scatters, Logger log) {
      ...
      if (numericStratCats != null) {
        int[] indices = ext.indexFactors(numericStratCats,
            Files.getHeaderOfFile(proj.SAMPLE_DATA_FILENAME.getValue(), proj.getLog()), true,
            false);
      ...
      try {
        ...
        for (int i = 0; i < sampleDataStratCats.length; i++) {
          BooleanClassifier booleanClassifier = Array.classifyStringsToBoolean(
              parser.getStringDataForTitle(sampleDataStratCats[i]), new String[] {"NaN"});
          ...
          try {
            ...
            writer.println();
            for (int sampleIndex =
                0; sampleIndex < evaluationResults[0].getEstimateData().length; sampleIndex++) {
              for (int groupIndex =
                  0; groupIndex < booleanClassifier.getClassified().length; groupIndex++) {
                int n = 0;
            ...
            plotSkips(sampleDataStratCats, log, boxDir, rScatters, i, evaluationResults, outputBox,
                pcYs);
            RScatter rScatterFirstLast =
                plotFirstLast(sampleDataStratCats, log, boxDir, i, outputBox, pcYs);
            ...
            if (numericStratCats != null) {
              ...
              for (String numericStratCat : numericStratCats) {
                ...
                String outputBoxSub =
                    ext.addToRoot(outputBox, sampleDataStratCats[i] + "_" + numericStratCat);
                ...
                if (data != null) {
                  ...
                  writerSub.println();
                  for (int sampleIndex = 0; sampleIndex < evaluationResults[0]
                      .getEstimateData().length; sampleIndex++) {
                    for (int groupIndex =
                        0; groupIndex < booleanClassifier.getClassified().length; groupIndex++) {
                      int n = 0;
    ...
    private void plotFirst(String[] sampleDataStratCats, Logger log, String dir,
        ArrayList<RScatter> rScatters, int i, EvaluationResult[] evaluationResults,
      ...
      RScatter rScatterSubset = new RScatter(outputBox, ext.addToRoot(outputBox, "sub.rscript"),
          ...
          dir + ext.removeDirectoryInfo(outputBox) + "sub.pdf", sampleDataStratCats[i],
          Array.subArray(Array.toStringArray(pcYs), 0, Math.min(20, evaluationResults.length)),
          SCATTER_TYPE.BOX, log);
    ...
    private RScatter plotFirstLast(String[] sampleDataStratCats, Logger log, String dir, int i,
        String outputBox, ArrayList<String> pcYs) {
      RScatter rScatterFirstLast =
          new RScatter(outputBox, ext.addToRoot(outputBox, "subFirstlast.rscript"),
              ext.removeDirectoryInfo(outputBox) + "subFirstlast",
              dir + ext.removeDirectoryInfo(outputBox) + "subFirstlast.pdf", sampleDataStratCats[i],
              new String[] {pcYs.get(0), pcYs.get(pcYs.size() - 1)}, SCATTER_TYPE.BOX, log);
      ...
      if (pedFile != null) {
        ...
        if (!Files.exists(heritSummary)
          ...
          try {
            ...
            int[] indices =
                ext.indexFactors(toExtract, Files.getHeaderOfFile(tmpHerit, log), true, false);
        ...
        try {
          ...
          double[] tmp = Array.toDoubleArray(
              HashVec.loadFileToStringArray(heritSummary, true, new int[] {1}, false));
          if (tmp.length > 0) {
            estimates = tmp;
          }
        } catch (Exception e) {
        ...
        double[] pvals = Array
            .toDoubleArray(HashVec.loadFileToStringArray(heritSummary, true, new int[] {3}, false));
        double[] solarHerit = Array
            .toDoubleArray(HashVec.loadFileToStringArray(heritSummary, true, new int[] {2}, false));
        double[] solarStError = Array
            .toDoubleArray(HashVec.loadFileToStringArray(heritSummary, true, new int[] {4}, false));
        String[] solarWarnKurt =
            HashVec.loadFileToStringArray(heritSummary, true, new int[] {6}, false);

        ArrayList<GeomText> sigGTexts = new ArrayList<GeomText>();
        for (int i = 0; i < pvals.length; i++) {
          if (pvals[i] < 0.05) {
            GeomText geomText =
                new GeomText(i, Math.min(solarHerit[i] + solarStError[i], 1), 0, "*", 12);
            sigGTexts.add(geomText);
          }
          if (solarWarnKurt[i].equals("NORMAL")) {
            GeomText geomText =
                new GeomText(i, Math.max(solarHerit[i] - solarStError[i], 0), 0, "N", 12);
            sigGTexts.add(geomText);
          }
        }

        RScatter rScatter = new RScatter(heritSummary, heritRscript, ext.rootOf(heritSummary),
            heritPlot, HERIT_ADDITIONS[0], new String[] {HERIT_ADDITIONS[1], HERIT_ADDITIONS[2]},
            SCATTER_TYPE.POINT, log);
        ErrorBars errorBars =
            new ErrorBars(new String[] {HERIT_ADDITIONS[2]}, new String[] {HERIT_ADDITIONS[4]});
    ...
    public RScatter plotRank(Logger log) {
      RScatter rScatter = new RScatter(outputRank, rankRscript, ext.rootOf(outputRank), rankplot,
          "OriginalOrder", new String[] {"Stat"}, SCATTER_TYPE.POINT, log);
      rScatter.setyRange(new double[] {0, 1});
      rScatter.setxLabel("PC (" + oType + " - sorted)");
    ...
    private void plotSkips(String[] sampleDataStratCats, Logger log, String dir,
      ...
      RScatter rScatterSkip = new RScatter(outputBox, ext.addToRoot(outputBox, "skips.rscript"),
          ...
          dir + ext.removeDirectoryInfo(outputBox) + "skips.pdf", sampleDataStratCats[i],
          Array.toStringArray(skips), SCATTER_TYPE.BOX, log);
    ...
    public RScatter plotSummary(String[] dataColumns, String index,
        EvaluationResult evaluationResult, int modelN, double[] trimRangeX, double[] trimRangeY,
        Logger log) {
      RScatter rScatter =
          new RScatter(outputSummary, evalRscript, ext.rootOf(outputSummary) + "_" + index,
              ext.rootOf(ext.addToRoot(evalPlot, index + ""), false) + ".jpeg", "Evaluated",
              dataColumns, SCATTER_TYPE.POINT, log);
      ...
      for (int i = 0; i < availableYs.length; i++) {
        if (availableYs[i].equals("Rsquare_correction")) {
          ...
          String toSearch =
              availableYs[i].replaceAll("PEARSON_CORREL_", "").replaceAll("SPEARMAN_CORREL_", "");
      ...
      RScatter rScatterTrim = new RScatter(outputSummary, evalRscript,
          ...
          ext.rootOf(ext.addToRoot(evalPlot, index + "" + "_" + index), false) + ".jpeg",
  ...
  private static class IterSummary {
    ...
    private final MODEL_BUILDER_TYPE mBuilder_TYPE;
    private final ITERATION_TYPE iType;
    private final ORDER_TYPE otType;
  ...
  public static class IterSummaryProducer extends AbstractProducer<IterSummary> {
    ...
    public IterSummaryProducer(Project proj, CorrectionIterator[] cIterators,
        String[][] plotTitlesForSummary, String[] stratCats, String[] numericStratCats,
        String pedFile, double[] trimRangeX, double[] trimRangeY, boolean plot) {
      super();
      this.proj = proj;
      ...
      Callable<IterSummary> callable = new Callable<IterSummary>() {
        ...
        public IterSummary call() throws Exception {
          ...
          String outputRoot = typedOut + iterationResult.getbType() + "_"
              + iterationResult.getoType() + "_" + iterationResult.getiType() + "_finalSummary";
          new File(typedOut).mkdirs();
          EvaluationResult[] evaluationResults =
              EvaluationResult.readSerial(originalSer, proj.getLog());
          ...
          if (plot) {
            ...
            int modelN =
                Array.booleanArraySum(iterationResult.getBasicPrep().getSamplesForModels());
            for (int i = 0; i < plotTitlesForSummary.length; i++) {
              RScatter rScatterSummary = iterationResult.plotSummary(plotTitlesForSummary[i],
                  i + "", evaluationResults[0], modelN, trimRangeX, trimRangeY, proj.getLog());
            ...
            String[] fullHeader =
                Files.getHeaderOfFile(iterationResult.getOutputSummary(), proj.getLog());
            for (String element : fullHeader) {
              if (element.startsWith("SPEARMAN_CORREL_EVAL_DATA_")
                  && element.endsWith("NO_STRAT")) {
                String title =
                    element.replaceAll("SPEARMAN_CORREL_EVAL_DATA_", "").replaceAll("NO_STRAT", "");
                RScatter rScatterSummary = iterationResult.plotSummary(new String[] {element},
                    title, evaluationResults[0], modelN, trimRangeX, trimRangeY, proj.getLog());
                rScatterSummary.setSeriesLabeler(null);
                rScatterSummary.setgTexts(null);
                rScatterSummary.execute();
              }
            ...
            if (pedFile != null) {
              ...
              if (subsetDataHeritability == null) {
                ...
                scatters.add(iterationResult
                    .plotHeritability(proj, pedFile,
                        iterationResult.getBasicPrep().getSamplesToEvaluate(), null, null, log)
                    .getrScatter());
                  ...
                  for (int i = 0; i < subsetDataHeritability.length; i++) {
                    if (!subsetDataHeritability[i].toLowerCase().contains("sex")) {
                      ...
                      if (index >= 0) {
                        ...
                        String newSer = newDir + ext.removeDirectoryInfo(ext.addToRoot(originalSer,
                            "." + subsetDataHeritability[i] + ".summary"));
                        ...
                        RScatter tmpherit =
                            tmpR.plotHeritability(proj, pedFile, currentModel, null, null, log)
                                .getrScatter();
                        tmpherit.setTitle(subsetDataHeritability[i] + " - matched samples n="
                            + Array.booleanArraySum(currentModel));
                        scatters
                            .add(tmpR.plotHeritability(proj, pedFile, currentModel, null, null, log)
                                .getrScatter());
                      } else {
            ...
            RScatters finalScattersTyped =
                new RScatters(scatters.toArray(new RScatter[scatters.size()]),
                    outputRoot + ".rscript", outputRoot + ".pdf",
                    COLUMNS_MULTIPLOT.COLUMNS_MULTIPLOT_2, PLOT_DEVICE.PDF, proj.getLog());
          ...
          return new IterSummary(scatters.toArray(new RScatter[scatters.size()]),
              iterationResult.getbType(), iterationResult.getiType(), iterationResult.getoType());
        }
        ...
        private void classifyHerit(final CorrectionIterator tmp, final Logger log,
            ...
            ArrayList<RScatter> scatters, String subsetDataHeritability, boolean[] subsetMask,
            double[] otherData) {
          if (classifiers != null) {
            String originalSer = iterationResult.getOutputSer();
            for (int classifyIndex = 0; classifyIndex < classifiers.length; classifyIndex++) {
              if (classifiers[classifyIndex].getTitles().length < 10
                ...
                boolean[] allModel = Array.booleanArray(
                    iterationResult.getBasicPrep().getSamplesToEvaluate().length, false);
                for (int titleIndex =
                    0; titleIndex < classifiers[classifyIndex].getTitles().length; titleIndex++) {
                  IterationResult tmpR = tmp.getIterationResult();
                  String newDir =
                      ext.parseDirectoryOfFile(originalSer) + stratCats[classifyIndex] + "_herit"
                  ...
                  boolean[] currentModel =
                      new boolean[iterationResult.getBasicPrep().getSamplesToEvaluate().length];
                  for (int j = 0; j < currentModel.length; j++) {
                    if (iterationResult.getBasicPrep().getSamplesToEvaluate()[j]
                  ...
                  tmpherit.setTitle(classifiers[classifyIndex].getTitles()[titleIndex]
                      + " - matched samples n=" + Array.booleanArraySum(currentModel));
                  tmpherit.execute();
                  ...
                  if (otherData != null) {
                    newSer = newDir + ext.removeDirectoryInfo(ext.addToRoot(originalSer,
                    ...
                    HeritPlot heritPlotRaw = tmpR.plotHeritability(proj, pedFile, currentModel,
                        otherData, subsetDataHeritability, log);

                    GeomText geomTextRaw =
                        new GeomText(0, heritPlotRaw.getEstimates()[0], 0, stratCats[classifyIndex]
                            + classifiers[classifyIndex].getTitles()[titleIndex] + "_qpcr", 3);
                ...
                if (otherData != null) {
                  ...
                  String newSer = newDir + ext.removeDirectoryInfo(ext.addToRoot(originalSer,
                      ".rawAll" + (subsetDataHeritability == null ? "" : subsetDataHeritability)
                          + ".summary"));
                  Files.copyFileUsingFileChannels(new File(iterationResult.getOutputSer()),
                      new File(newSer), log);
                  ...
                  HeritPlot heritPlotRaw = tmpFullModelRSub.plotHeritability(proj, pedFile,
                      allModel, otherData, subsetDataHeritability, log);
                ...
                String stratCatHerit = stratCats[classifyIndex]
                    + (subsetDataHeritability == null ? "" : subsetDataHeritability);
                String comboHerit =
                    ext.rootOf(originalSer, false) + stratCatHerit + ".heritability.summary";
                ...
                String newDir =
                    ext.parseDirectoryOfFile(originalSer) + stratCats[classifyIndex] + "_herit"
                        + (subsetDataHeritability == null ? "" : subsetDataHeritability) + "/";
                new File(newDir).mkdirs();
                String newSer = newDir + ext.removeDirectoryInfo(
                    ext.addToRoot(originalSer, "." + stratCatHerit + ".summary"));
                ...
                HeritPlot all =
                    tmpFullModelR.plotHeritability(proj, pedFile, allModel, null, null, log);

                adds[adds.length - 1] =
                    stratCatHerit + "_ALL_nInd_" + all.getNumSamps() + "_numFam_" + all.getNumFam();
                ...
                for (int i = 0; i < adds.length - 1; i++) {
                  String add = classifiers[classifyIndex].getTitles()[i] + "_nInd_"
                      + hPlots.get(i).getNumSamps() + "_numFam_" + hPlots.get(i).getNumFam();
                  adds[i] = add;
                }
                ...
                String[][] columns = Files.paste(Array.toStringArray(currentClass), comboHerit,
                    new int[] {0, 1, 2, 3, 4, 5, 6}, 0, adds, null, log);
                ...
                RScatter classHerit = new RScatter(comboHerit, comboHerit + ".rscript",
                    ext.removeDirectoryInfo(comboHerit), comboHerit + ".pdf", columns[0][0],
                    heritColumns, SCATTER_TYPE.POINT, log);
                classHerit.setTitle(
                    all.getrScatter().getTitle() + " \nHeritability by " + stratCats[classifyIndex]
                        + (subsetDataHeritability == null ? "" : " and " + subsetDataHeritability));
                ...
                classHerit
                    .setgTexts(otherDataGeomTexts.toArray(new GeomText[otherDataGeomTexts.size()]));
                classHerit.execute();
                ...
                log.reportTimeWarning(
                    "Skipping " + stratCats[classifyIndex] + " due to size of categories");
              }
            }
          }
        }
  ...
  private static CorrectionIterator[] getIterations(Project proj, String markesToEvaluate,
      String samplesToBuildModels, String outputDir, LS_TYPE lType, boolean recomputeLRR,
      ...
      for (int j = 0; j < ORDER_TYPE.values().length; j++) {
        for (int j2 = 0; j2 < MODEL_BUILDER_TYPE.values().length; j2++) {
          cIterators.add(new CorrectionIterator(proj, markesToEvaluate, samplesToBuildModels,
              ITERATION_TYPE.values()[i], ORDER_TYPE.values()[j], MODEL_BUILDER_TYPE.values()[j2],
  ...
  private static void printStatSummary(PrintWriter writer, CorrectionIterator correctionIterator,
      String type, EvaluationResult[] evaluationResults, MODEL_BUILDER_TYPE mBuilder_TYPE,
      ORDER_TYPE oType, ITERATION_TYPE iType, ArrayList<double[]> statsAL, ArrayList<ICC> statsICC,
    ...
    for (int i = 0; i < (statsAL == null ? statsICC.size() : statsAL.size()); i++) {
      ...
      int numSamplesModel = Array.booleanArraySum(
          correctionIterator.getIterationResult().getBasicPrep().getSamplesForModels());
  ...
  public static CorrectionIterator[] runAll(Project proj, String markesToEvaluate,
      String samplesToBuildModels, String outputDir, String pcFile, String pedFile, LS_TYPE lType,
      boolean recomputeLRR, double pcPercent, boolean plot, int numthreads) {
    if (pcFile != null) {
    ...
    for (String customPlotFile : customPlotFiles) {
      ...
      for (int j = 0; j < groups.length; j++) {
        arrayStringList[Integer.parseInt(groups[j]) - 1]
            .add("SPEARMAN_CORREL_EVAL_DATA_" + names[j] + "_CUSTOM_PHENO_TAG_NO_STRAT");
      }
    ...
    IterSummaryProducer producer = new IterSummaryProducer(proj, cIterators, plotTitlesForSummary,
        CorrectionEvaluator.INDEPS_CATS, null, pedFile, new double[] {0, 150},
        new double[] {-.5, 1}, plot);
    ...
    WorkerTrain<IterSummary> summaryTrain =
        new WorkerTrain<IterSummary>(producer, numthreads, numthreads, proj.getLog());
    ...
    try {
      ...
      for (CorrectionIterator correctionIterator : cIterators) {
        EvaluationResult[] evaluationResults = EvaluationResult
            .readSerial(correctionIterator.getIterationResult().getOutputSer(), log);
    ...
    if (rScatters.size() > 0) {
      RScatters finalScatters = new RScatters(rScatters.toArray(new RScatter[rScatters.size()]),
          outputRoot + ".rscript", outputRoot + ".pdf", COLUMNS_MULTIPLOT.COLUMNS_MULTIPLOT_2,
  ...
  public CorrectionIterator(Project proj, String markesToEvaluate, String samplesToBuildModels,
      ITERATION_TYPE iType, ORDER_TYPE oType, MODEL_BUILDER_TYPE bType, String outputDir,
      LS_TYPE lType, boolean recomputeLRR, double pcPercent, int numthreads) {
    super();
    this.proj = proj;
  ...
  private IterationResult run(Project proj, String markesToEvaluate, String samplesToBuildModels,
    ...
    switch (bType) {
      ...
      case WITH_QC_BUILDERS:
        if (!Files.exists(samplesToBuildModels)) {
          ...
          log.reportTimeInfo(
              "Loaded " + sampsForMods.length + " model builders from " + samplesToBuildModels);
        ...
        log.reportTimeInfo(
            Array.booleanArraySum(samplesForModels) + " model builders from after QC filtering");
    ...
    GcAdjustorParameters params = GcAdjustorParameters
        .readSerial(proj.GC_CORRECTION_PARAMETERS_FILENAMES.getValue()[0], proj.getLog());
    ...
    if (valid) {
      ...
      if (precomputed == null) {
        sTabRank = pcResiduals.getStatRankFor(pcResiduals.getMedians(), extraIndeps,
            samplesForModels, "RAW_MEDIANS", STAT_TYPE.LIN_REGRESSION, VALUE_TYPE.STAT,
            oType == ORDER_TYPE.STEPWISE_RANK_R2, numthreads, proj.getLog());
      ...
      if (order != null && oType == ORDER_TYPE.STEPWISE_RANK_R2) {
        String out =
            outputDir + iType + "_" + oType + "_" + bType + "_PC_" + oType + "_Selection.txt";
      ...
      cEvaluator = new CorrectionEvaluator(proj, pcResiduals, precomputed, order,
          new boolean[][] {samplesForModels, samplesToEvaluate}, extraIndeps, lType);
      BasicPrep basicPrep = new BasicPrep(cEvaluator.getParser().getNumericData(),
          cEvaluator.getParser().getNumericDataTitles(), samplesToEvaluate, samplesForModels, null);
      ...
      try {
        ...
        WorkerTrain<EvaluationResult> train =
            new WorkerTrain<EvaluationResult>(cEvaluator, numthreads, numthreads, proj.getLog());
        int index = 0;
        while (train.hasNext()) {
