...
public class CorrectionIterator implements Serializable {
  ...
  public enum ITERATION_TYPE {
                              /**
                               * The evaluation happens with the addition of other independent
                               * variables in addition to PCs
                               */
                              WITHOUT_INDEPS,
    /**
  ...
  public enum MODEL_BUILDER_TYPE {
                                  /**
                                   * We load the additional sample (union with not excluded) file
                                   * for model building
                                   */
                                  WITH_QC_BUILDERS,
    /**
  ...
  public enum ORDER_TYPE {
                          /**
                           * PCS are ranked by the amount of variance explained in the estimate of
                           * interest
                           */
                          // RANK_R2,
                          /**
                          ...
                          NATURAL,
                          /**
                           * PCS are ranked by the amount of variance explained within a stepwise
                           * regression
                           */
                          STEPWISE_RANK_R2,
  ...
  private IterationResult run(Project proj, String markesToEvaluate, String samplesToBuildModels,
    ...
    GcAdjustorParameters params =
        GcAdjustorParameters.readSerial(proj.GC_CORRECTION_PARAMETERS_FILENAMES.getValue()[0],
                                        proj.getLog());
    ...
    if (valid) {
      ...
      if (precomputed == null) {
        sTabRank =
            pcResiduals.getStatRankFor(pcResiduals.getMedians(), extraIndeps, samplesForModels,
                                       "RAW_MEDIANS", STAT_TYPE.LIN_REGRESSION, VALUE_TYPE.STAT,
  ...
  private static class IterationResult implements Serializable {
    ...
    public RScatter plotRank(Logger log) {
      RScatter rScatter =
          new RScatter(outputRank, rankRscript, ext.rootOf(outputRank), rankplot, "OriginalOrder",
                       new String[] {"Stat"}, SCATTER_TYPE.POINT, log);
            ...
            if (numericStratCats != null) {
              ...
              for (String numericStratCat : numericStratCats) {
                ...
                if (data != null) {
                  ...
                  writerSub.close();
                  RScatter rScatterNumeric =
                      new RScatter(outputBoxSub, ext.addToRoot(outputBoxSub, "sub.rscript"),
                                   ext.removeDirectoryInfo(outputBoxSub) + "sub",
                                   boxDir + ext.removeDirectoryInfo(outputBoxSub) + "sub.pdf",
                                   sampleDataStratCats[i],
                                   new String[] {numericStratCat, pcYsub.get(0),
                                                 pcYsub.get(pcYsub.size() - 1)},
    ...
    private void plotFirst(String[] sampleDataStratCats, Logger log, String dir,
      ...
      RScatter rScatterSubset =
          new RScatter(outputBox, ext.addToRoot(outputBox, "sub.rscript"),
    ...
    private void plotSkips(String[] sampleDataStratCats, Logger log, String dir,
      ...
      RScatter rScatterSkip =
          new RScatter(outputBox, ext.addToRoot(outputBox, "skips.rscript"),
                       ext.removeDirectoryInfo(outputBox) + "skips",
                       dir + ext.removeDirectoryInfo(outputBox) + "skips.pdf",
                       sampleDataStratCats[i], Array.toStringArray(skips), SCATTER_TYPE.BOX, log);
    ...
    private RScatter plotFirstLast(String[] sampleDataStratCats, Logger log, String dir, int i,
      ...
      RScatter rScatterFirstLast =
          new RScatter(outputBox, ext.addToRoot(outputBox, "subFirstlast.rscript"),
                       ...
                       sampleDataStratCats[i],
                       new String[] {pcYs.get(0), pcYs.get(pcYs.size() - 1)}, SCATTER_TYPE.BOX,
                       log);
    ...
    public RScatter plotSummary(String[] dataColumns, String index,
      ...
      RScatter rScatter =
          new RScatter(outputSummary, evalRscript, ext.rootOf(outputSummary) + "_" + index,
                       ext.rootOf(ext.addToRoot(evalPlot, index + ""), false) + ".jpeg",
      ...
      RScatter rScatterTrim =
          new RScatter(outputSummary, evalRscript,
                       ext.rootOf(outputSummary) + "_" + index + "_"
                                                   + index,
                       ext.rootOf(ext.addToRoot(evalPlot, index + "" + "_" + index), false)
                                                            + ".jpeg",
                       "Evaluated", dataColumns, SCATTER_TYPE.POINT, log);
    ...
    public HeritPlot plotHeritability(Project proj, String pedFile, boolean[] samplesToEvaluate,
      ...
      if (pedFile != null) {
        ...
        double[] pvals = Array.toDoubleArray(HashVec.loadFileToStringArray(heritSummary, true,
                                                                           new int[] {3}, false));
        double[] solarHerit =
            Array.toDoubleArray(HashVec.loadFileToStringArray(heritSummary, true, new int[] {2},
                                                              false));
        double[] solarStError =
            Array.toDoubleArray(HashVec.loadFileToStringArray(heritSummary, true, new int[] {4},
                                                              false));
        ...
        RScatter rScatter =
            new RScatter(heritSummary, heritRscript, ext.rootOf(heritSummary), heritPlot,
                         HERIT_ADDITIONS[0], new String[] {HERIT_ADDITIONS[1], HERIT_ADDITIONS[2]},
  ...
  public static CorrectionIterator[] runAll(Project proj, String markesToEvaluate,
    ...
    CorrectionIterator[] cIterators =
        getIterations(proj, markesToEvaluate, samplesToBuildModels, outputDir, lType, recomputeLRR,
                      pcPercent, numthreads);
    ArrayList<RScatter> rScatters = new ArrayList<RScatter>();
    ...
    IterSummaryProducer producer =
        new IterSummaryProducer(proj, cIterators, plotTitlesForSummary,
                                CorrectionEvaluator.INDEPS_CATS, null, pedFile,
                                new double[] {0, 150}, new double[] {-.5, 1}, plot);
    ...
    try {
      ...
      for (CorrectionIterator correctionIterator : cIterators) {
        EvaluationResult[] evaluationResults =
            EvaluationResult.readSerial(correctionIterator.getIterationResult().getOutputSer(),
                                        log);
    ...
    if (rScatters.size() > 0) {
      RScatters finalScatters =
          new RScatters(rScatters.toArray(new RScatter[rScatters.size()]), outputRoot + ".rscript",
                        outputRoot + ".pdf", COLUMNS_MULTIPLOT.COLUMNS_MULTIPLOT_2, PLOT_DEVICE.PDF,
                        proj.getLog());
  ...
  private static void printStatSummary(PrintWriter writer, CorrectionIterator correctionIterator,
    ...
    for (int i = 0; i < (statsAL == null ? statsICC.size() : statsAL.size()); i++) {
      ...
      int numSamplesModel =
          Array.booleanArraySum(correctionIterator.getIterationResult().getBasicPrep()
                                                  .getSamplesForModels());
    ...
    public Callable<IterSummary> next() {
      ...
      Callable<IterSummary> callable = new Callable<IterSummary>() {
        ...
        public IterSummary call() throws Exception {
          ...
          String outputRoot =
              typedOut + iterationResult.getbType() + "_" + iterationResult.getoType() + "_"
                              + iterationResult.getiType() + "_finalSummary";
          ...
          if (plot) {
            ...
            for (int i = 0; i < plotTitlesForSummary.length; i++) {
              RScatter rScatterSummary =
                  iterationResult.plotSummary(plotTitlesForSummary[i], i + "", evaluationResults[0],
                                              modelN, trimRangeX, trimRangeY, proj.getLog());
            ...
            for (String element : fullHeader) {
              if (element.startsWith("SPEARMAN_CORREL_EVAL_DATA_")
                ...
                RScatter rScatterSummary =
                    iterationResult.plotSummary(new String[] {element}, title, evaluationResults[0],
                                                modelN, trimRangeX, trimRangeY, proj.getLog());
                rScatterSummary.setSeriesLabeler(null);
                rScatterSummary.setgTexts(null);
                rScatterSummary.execute();
                    ...
                    if (!subsetDataHeritability[i].toLowerCase().contains("sex")) {
                      ...
                      if (index >= 0) {
                        ...
                        boolean[] currentModel =
                            new boolean[iterationResult.getBasicPrep()
                        ...
                        String newSer =
                            newDir + ext.removeDirectoryInfo(ext.addToRoot(originalSer,
            ...
            RScatters finalScattersTyped =
                new RScatters(scatters.toArray(new RScatter[scatters.size()]),
                              ...
                              COLUMNS_MULTIPLOT.COLUMNS_MULTIPLOT_2, PLOT_DEVICE.PDF,
                              proj.getLog());
          ...
          if (classifiers != null) {
            ...
            for (int classifyIndex = 0; classifyIndex < classifiers.length; classifyIndex++) {
              if (classifiers[classifyIndex].getTitles().length < 10
                ...
                boolean[] allModel =
                    Array.booleanArray(iterationResult.getBasicPrep().getSamplesToEvaluate().length,
                                       false);
                for (int titleIndex =
                  ...
                  if (otherData != null) {
                    ...
                    HeritPlot heritPlotRaw =
                        tmpR.plotHeritability(proj, pedFile, currentModel, otherData,
                ...
                if (otherData != null) {
                  ...
                  HeritPlot heritPlotRaw =
                      tmpFullModelRSub.plotHeritability(proj, pedFile, allModel, otherData,
                                                        subsetDataHeritability, log);
                ...
                new File(newDir).mkdirs();
                String newSer =
                    newDir + ext.removeDirectoryInfo(ext.addToRoot(originalSer, "." + stratCatHerit
                                                                                + ".summary"));
                ...
                String[][] columns =
                    Files.paste(Array.toStringArray(currentClass), comboHerit,
                ...
                RScatter classHerit =
                    new RScatter(comboHerit, comboHerit + ".rscript",
                                 ext.removeDirectoryInfo(comboHerit), comboHerit + ".pdf",
                                 columns[0][0], heritColumns, SCATTER_TYPE.POINT, log);
