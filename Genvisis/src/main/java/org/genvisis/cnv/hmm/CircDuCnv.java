...
//
/// **
// * @author lane0212 Genvisis version of Circular binary segmentation: might be applicable to both
// array and NGS...
// *
// *
// */
...
// public CircDuCnv(String inputBinPath, String dataType, int idxScore, Hashtable<String, int[]>
// startByChr, Hashtable<String, int[]> endByChr, Hashtable<String, double[]> scoreByChr,
// SegmentSplitUndo undoMethod, String forbiddenIntervalBedPath) {
// super();
...
// GenomeSegmentationResults results = new GenomeSegmentationResults(new Hashtable<String,
// Segment[]>());
// return results;
// }
//
...
// // private void CBS(uint nPerm = 10000, string pMethod = "hybrid", int minWidth = 2, int kMax =
// 25,
// // uint nMin = 200, double eta = 0.05, uint[] sbdry = null, double trim = 0.025,
// // double undoPrune = 0.05, double undoSD = 3, int verbose = 1)
// // {
//
// private void CBS(int nPerm, String pMethod, int minWidth, int kMax, int nMin, double eta, int[]
// sbdry, double trim, double undoPrune, double undoSD, int verbose) {
...
// System.out.println("ONfdsf chsdfsdfr " + key + "\t" +
// ina.length+"\t"+ScoreByChr.get(key).length);
//
// double[] scores = null;
...
// Hashtable<String, MersenneTwister> perChromosomeRandom = new Hashtable<String,
// MersenneTwister>();
...
//
// lengthSeg = CBSUtils.ChangePoints(ScoreByChr.get(chr), sbdry, lengthSeg, segmentMeans,
// perChromosomeRandom.get(chr),
//
// DataType, DefaultAlpha, nPerm, pMethod, minWidth, kMax, nMin, trimmedSD, UndoMethod, undoPrune,
// undoSD, verbose, 100, 1E-6);
//
...
// segments[i] = new SegmentCBS(chr, StartByChr.get(chr)[start], EndByChr.get(chr)[end],
// lengthSeg[i], 0);
...
//
// }
// CircDuCnv circDuCnv = new CircDuCnv(null, DEFAULT_DATA_TYPE, 3, StartByChr, EndByChr, ScoreByChr,
// SegmentSplitUndo.None, null);
// circDuCnv.CBS(nPerm, pMethod, minWidth, kMax, nMin, eta, sbdry, trim, undoPrune, undoSD,
// verbose);
//
// // private void CBS(int nPerm, String pMethod, int minWidth, int kMax, int nMin, double eta,
// int[] sbdry, double trim, double undoPrune, double undoSD, int verbose) {
//
// }
...
// // {
// // for (int segmentIndex = 0; segmentIndex < SegmentationResults.SegmentByChr[chr].Length;
// segmentIndex++)
// // {
...
// // }
// //
// // Dictionary<string, List<GenomicBin>> ExcludedIntervals = new Dictionary<string,
// List<GenomicBin>>();
...
// // int excludeIndex = 0; // Points to the first interval which *doesn't* end before our current
// position
...
// // {
// // while (excludeIndex < excludeIntervals.Count && excludeIntervals[excludeIndex].Stop <
// previousBinEnd) excludeIndex++;
// // if (excludeIndex < excludeIntervals.Count)
// // {
// // // Note: forbiddenZoneMid should never fall inside a bin, becuase these intervals were already
// excluded
// // // from consideration during the call to CanvasBin.
// // int forbiddenZoneMid = (excludeIntervals[excludeIndex].Start +
// excludeIntervals[excludeIndex].Stop) / 2;
// // if (previousBinEnd < forbiddenZoneMid && end >= forbiddenZoneMid) newSegment = true;
// // }
// // }
// // if (newSegment) segmentNum++;
// // writer.WriteLine(string.Format("{0}\t{1}\t{2}\t{3}\t{4}", chr, start, end,
// ScoreByChr[chr][pos], segmentNum));
// // previousBinEnd = end;
// // }
// // }
// // }
