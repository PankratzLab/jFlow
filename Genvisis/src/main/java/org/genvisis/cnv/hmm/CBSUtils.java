...
//
///**
// * @author lane0212 Utility class for the circular binary segmentation. Ported from C -> https://github.com/Illumina/canvas/ which was in turn ported from R/fortran ala https://www.bioconductor.org/packages/release/bioc/html/DNAcopy.html
// *
// */
//public class CBSUtils {
//	private static final double M_LN2 = 0.693147180559945309417;
//	private static final double DBL_EPSILON = 2.2204460492503131E-16; // dangerous...the smallest x such that 1 + x != 1
...
//			k = n1s - 2;
//			pExcd += Math.exp(Math.log(n1) + Math.log(n1 - 1.0) - Math.log(2.0) + CombinatoricsUtils.binomialCoefficientLog(n, k) - dlcnk) + Math.exp(Math.log(n1) + Math.log(n2 - n1) + CombinatoricsUtils.binomialCoefficientLog(n, k) - dlcnk);
//		}
//		if (n1s > 3) {
//			for (i = 4; i <= n1s; i++) {
...
//				k = n1s - i + 1;
//				pExcd += Math.exp(CombinatoricsUtils.binomialCoefficientLog(n1, k1) + CombinatoricsUtils.binomialCoefficientLog(n, k) - dlcnk) + Math.exp(CombinatoricsUtils.binomialCoefficientLog(n1, k2) + Math.log(n3 - n1) + CombinatoricsUtils.binomialCoefficientLog(n, k) - dlcnk) + Math.exp(CombinatoricsUtils.binomialCoefficientLog(n1, k3) + Math.log(n2 - n1) + Math.log(n3 - n2) + CombinatoricsUtils.binomialCoefficientLog(n, k) - dlcnk) + Math.exp(CombinatoricsUtils.binomialCoefficientLog(n1, k3) + Math.log(n2 - n1) - Math.log(2.0) + Math.log(n2 - n1 - 1.0) + CombinatoricsUtils.binomialCoefficientLog(n, k) - dlcnk);
//			}
//		}
//		return pExcd;
//	}
...
//	public static int[] ChangePoints(double[] genomeData, int[] sbdry, int[] lengthSeg, double[] segmentMeans, MersenneTwister rnd, String dataType, double alpha, int nPerm, String pMethod, int minWidth, int kMax, int nMin, double trimmedSD, SegmentSplitUndo undoSplits, double undoPrune, double undoSD, int verbose, int nGrid, double tol) {
...
//					iChangePoint = FindChangePoints(currentGenomeData, currentTSS, nPerm, alpha, nChangePoints, iChangePoint, dataType.equals("binary"), hybrid, minWidth, kMax, delta, nGrid, sbdry, tol, rnd);
//				}
//			} else {
//				nChangePoints = 0;
//			}
...
//	}
//
//	private static int[] ChangePointsSDUndo(double[] genomeData, int[] lengthSeg, double trimmedSD, double changeSD) {
//		changeSD *= trimmedSD;
//
...
//				{
//					segmentMedians[i] = Array.median(Array.subArray(genomeData, starts.get(i), changePointLocations.get(i)));
//
//				}
...
//	// }
//	private static int[] FindChangePoints(double[] genomeData, double totalSumOfSquares, int nPerm, double cutoffPValue, int nChangePoints, int[] iChangePoint, boolean isBinary, boolean hybrid, int al0, int hk, double delta, int nGrid, int[] sbdry, double tol, MersenneTwister rnd) {
...
//			}
//
//			java.lang.System.arraycopy(Array.Diff(scoresByChr.get(chr), 1), 0, diff, i, scoresByChr.get(chr).length - 1);
...
//	}
//
//	public static void TMaxO(double[] genomeData, double totalSumOfSquares, double[] sx, int[] iseg, double ostat, int al0, boolean isBinary) {
//		// look at the partial sums in blocks of size sqrt(n)
//		int ipsmin, ipsmax, ipsmin0, ipsmax0, nb, i, j, k, l, nb1, nb2, bi, bj, ilo, ihi, jlo, jhi, alenmax, i2j, sxmxi, alenlo, alenhi, tmaxi, tmaxj, ixlo, ixhi, nal0;
//		double psum, psmin, psmax, psmin0, psmax0, bssmax, bsslim, rn, rj, rjhi, rjlo, rnjov1, sij1, sij2, sijmx0, absx, sxmx, bijbss, rnov2, psdiff;
//
...
//	}
//
//	public static double ErrorSumOfSquares(int[] lengthSeg, double[] segmentSums, int k, int[] locations) {
...
//
//		y = x + a - 0.5;
//		integral = (8.0 * y) / (1.0 - 4.0 * Math.pow(y, 2)) + 2.0 * Math.log((1.0 + 2.0 * y) / (1.0 - 2.0 * y));
//		y = x - 0.5;
//		integral = integral - (8.0 * y) / (1.0 - 4.0 * Math.pow(y, 2)) - 2.0 * Math.log((1.0 + 2.0 * y) / (1.0 - 2.0 * y));
//		return integral;
//	}
//
//	public static double HTMaxP(int k, double totalSumOfSquares, double[] px, double[] sx, int al0, boolean isBinary) {
//		int i, j, nmj;
...
//	}
//
//	public static double TMaxP(double totalSumOfSquares, double[] px, double[] sx, int al0, boolean isBinary) {
//		// look at the partial sums in blocks of size sqrt(n)
//
//		int ipsmin, ipsmax, ipsmin0, ipsmax0, nb, i, j, k, l, nb1, nb2, bi, bj, ilo, ihi, jlo, jhi, alenmax, i2j, alenlo, alenhi, ixlo, ixhi, nal0;
//		double psum, psmin, psmax, psmin0, psmax0, bssmax, bsslim, rn, rj, rjhi, rjlo, rnjov1, sij1, sij2, sijmx0, absx, sxmx, bijbss, rnov2, psdiff;
//
//		// use local arrays for working within blocks
//		// block partial sum max and min
...
//	}
//
//	public static double TPermP(int n1, int n2, int n, double[] genomeData, int gDOffset, double[] px, int nPerm, MersenneTwister rnd) {
//		int np, i, m1, j, nrej;
...
//			// TODO????
//
//			HypergeometricDistribution hypergeometricDistribution = new HypergeometricDistribution((int) (n1s + dn), k, (int) (n1s + dn));
...
//	// public static double phyper(double x, double NR, double NB, double n, boolean lowerTail, boolean logP) {
