...
public class DumpSerializedObject {
  ...
  private static void dump(String projectPropertyFile, String filename, String logFile) {
    ...
    Object object;
    object = SerializedFiles.readSerial(filename, proj.getLog(), false, false);
    ...
    /**
     * Classes we should support now: outliers.ser --* AnnotationCollection --* Centroids --*
     * ClusterFilterCollection --* MarkerSet CNVariant --- would only write one cnvariant to one
     * file... --* SNPMarkerSet --* SegmentList --* SegmentLists
     */

    /**
     * Classes we might consider supporting in the future: BurdenMatrix ChromatinAccessibility
     * GenotypeMatrix LDdatabase SerialStringMatrix SuperNovo
     */
