 ...
 * external files This has not been heavily tested, so please verify and test before use...
 *
 *
 */
public class ExtProjectDataParser {
  /**
   * Builder for {@link ExtProjectDataParser}: the default options should handle a file with a
   * header containing DNA, and all other columns (with header) containing numeric data
   *
   */
  public static class ProjectDataParserBuilder {
     ...
     */
    public ExtProjectDataParser build(Project proj, String fullPathToDataFile)
        throws FileNotFoundException {
      return new ExtProjectDataParser(this, proj, typeBuilder, fullPathToDataFile);
    }

    /**
     * @param commentString lines starting with this string will be skipped when parsing the header
     * @return
     */
    ...
    /**
     * @param concatMultipleStringEntries if multiple entries are found for either a marker or
     *        sample, the results will be concatenated
     * @return
     */
    public ProjectDataParserBuilder concatMultipleStringEntries(
        boolean concatMultipleStringEntries) {
      this.concatMultipleStringEntries = concatMultipleStringEntries;
      return this;
    }

    /**
     * @param dataKeyColumnIndex this column will be matched to either the projects samples or
     *        markers
     * @return
     */
    ...
    /**
     * @param dataKeyColumnName name of the data to match column
     * @return
     */
    ...
    /**
     * @param firstEntryOnly if true, only the first entry of a duplicate will be loaded
     * @return
     */
    ...
    /**
     * @param hasHeader the file has a header
     * @return
     */
    ...
    /**
     * @param missingString for string data, set strings to this if a sample/marker is not found in
     *        the data file
     * @return
     */
    ...
    /**
     * @param numericDataTitles optional titles of the numeric data...usually if a header is not
     *        present
     * @return
     */
    ...
    /**
     * @param requireAll require all samples or markers to be present in the file
     * @return
     */
    ...
    /**
     * @param sampleBased the file will be matched to samples
     * @return
     */
    ...
    /**
     * @param separator data separator for the file being loaded
     * @return
     */
    ...
    /**
     * @param numericDataTitles optional titles of the string data...usually if a header is not
     *        present
     * @return
     */
    ...
    /**
     * @param skipUnFound if a sample or marker is not found, keep going and just skip it
     * @return
     */
    ...
    /**
     * @param stringDataIndices indices to extract for string data...
     * @return
     */
    ...
    /**
     * @param treatAllNumeric all columns will be treated as numeric except for the column to be
     *        matched
     * @return
     */
  ...
  private final TypedFileParser typedFileParser;
  private double[][] numericData;
  private String[][] stringData;
  private String[] numericDataTitles, stringDataTitles;
  private int dataKeyColumnIndex;
  private final String dataKeyColumnName;
  private String[] dataToLoad;
  private boolean[] dataPresent;
  private final boolean requireAll, verbose;
  private final boolean skipUnFound;
  private final boolean hasHeader;
  private final boolean treatAllNumeric;
  private final String fullPathToDataFile;
  private final String missingString;
  private final String commentString;
  private final String[] headerFlags;
  private final boolean sampleBased;
  private final boolean concatMultipleStringEntries;
  private final boolean setInvalidNumericToNaN;

  private final boolean firstEntryOnly;

  private ExtProjectDataParser(ProjectDataParserBuilder builder, Project proj,
      TypedFileParser.Builder typeBuilder, String fullPathToDataFile) throws FileNotFoundException {
    this.proj = proj;
    numericDataTitles = builder.numericDataTitles;
    stringDataTitles = builder.stringDataTitles;
    dataKeyColumnIndex = builder.dataKeyColumnIndex;
    dataKeyColumnName = builder.dataKeyColumnName;
    dataToLoad = builder.dataToLoad;
    dataPresent = builder.dataPresent;
    requireAll = builder.requireAll;
    skipUnFound = builder.skipUnFound;
    hasHeader = builder.hasHeader;
    treatAllNumeric = builder.treatAllNumeric;
    this.fullPathToDataFile = fullPathToDataFile;
    missingString = builder.missingString;
    sampleBased = builder.sampleBased;
    commentString = builder.commentString;
    verbose = builder.verbose;
    headerFlags = builder.headerFlags;
    concatMultipleStringEntries = builder.concatMultipleStringEntries;
    typedFileParser = typeBuilder.build(Files.getAppropriateReader(fullPathToDataFile));
    setInvalidNumericToNaN = builder.setInvalidNumericToNaN;
    firstEntryOnly = builder.firstEntryOnly;
  }
  ...
  public boolean determineIndicesFromTitles() {
    ...
    if (hasHeader) {
      String[] header;
      if (headerFlags == null) {
        header = Files.getHeaderOfFile(fullPathToDataFile, typedFileParser.getSeparator(),
            new String[] {commentString}, proj.getLog());
      } else {
        header = Files.getLineContaining(fullPathToDataFile, typedFileParser.getSeparator(),
            headerFlags, proj.getLog());
      }
      if (header != null) {
        ...
        if (determined && stringDataTitles != null && stringDataTitles.length > 0) {
          if (typedFileParser.getStringColumns() != null) {
            proj.getLog().reportTimeWarning(
                "String columns were already provided, skipping string column assignment");
          } else {
            ...
            if (!determined) {
              if (verbose) {
                proj.getLog().reportTimeError(
                    "Could not find all string indices in header " + Array.toStr(header));
              }
            } else {
        ...
        if (determined && numericDataTitles != null && numericDataTitles.length > 0) {
          if (typedFileParser.getNumericColumns() != null) {
            proj.getLog().reportTimeWarning(
                "Numeric columns were already provided, skipping numeric column assignment");
          } else {
            ...
            if (!determined) {
              proj.getLog().reportTimeError("Could not find all numeric indices"
                  + Array.toStr(numericDataTitles) + " in header " + Array.toStr(header));
            } else {
  ...
  private boolean determineSampleIndex(boolean determined, String[] header) {
    if (determined && !dataKeyColumnName.equals(DEFUALT_SAMPLE_NAME_COLUMN)) {
      ...
      if (dataKeyColumnIndex < 0) {
        proj.getLog()
            .reportTimeError("Header must contain sample name column " + dataKeyColumnName);
        determined = false;
  ...
  public double[] getNumericDataForTitle(String title) {
    ...
    if (index < 0 || numericData == null) {
      ...
      proj.getLog().reportTimeError(numericData == null ? "No Numeric data Titles available"
          : "Titles available: " + Array.toStr(numericDataTitles));
      if (numericData == null) {
  ...
  public String[] getStringDataForTitle(String title) {
    ...
    if (index < 0 || stringData == null) {
      proj.getLog().reportTimeError("Data for " + title + " was not found");
      proj.getLog().reportTimeError(stringDataTitles == null ? "No String data Titles available"
  ...
  private void init() {
    ...
    dataPresent = new boolean[dataToLoad.length];
    Arrays.fill(dataPresent, false);

    if (typedFileParser.getNumericColumns() != null
        && typedFileParser.getNumericColumns().length > 1) {
      proj.getLog().reportTimeError(
          "Currently this method only handles one dimensionional numeric extraction, even though its an int[][]");
      return;
    }
    if (typedFileParser.getStringColumns() != null
        && typedFileParser.getStringColumns().length > 1) {
      proj.getLog().reportTimeError(
          "Currently this method only handles one dimensionional string extraction, even though its an int[][]");
      return;
    }

    typedFileParser.addStringColumns(new int[] {dataKeyColumnIndex});// samples will be in first
                                                                     // array position

    if (typedFileParser.getNumericColumns() != null) {
      numericData = new double[typedFileParser.getNumericColumns()[0].length][dataToLoad.length];
      ...
      for (double[] element : numericData) {
        Arrays.fill(element, Double.NaN);
      }
    }
    if (typedFileParser.getStringColumns() != null
      ...
      for (String[] element : stringData) {
        Arrays.fill(element, missingString);
      }
    }
  }
  ...
  public void loadData() {
    ...
    proj.getLog().reportTimeInfo("Attempting to load "
        + (sampleBased ? "sample based" : "marker based") + " data from " + fullPathToDataFile);
    if (treatAllNumeric) {
      treatAllNumeric();
    }
    init();
    try {
      ...
      while (typedFileParser.ready()) {
        ...
        if (typedFileLine.isValidLine()) {
          ...
          if (dataIndex < 0) {
            if (skipUnFound) {
              if (verbose) {
                proj.getLog().reportTimeWarning("Did not find " + data + " in project's "
                    + (sampleBased ? "samples" : "markers") + ", skipping");
              }
            } else {
              proj.getLog().reportTimeError("Did not find " + data + " in project's "
                  + (sampleBased ? "samples" : "markers") + ", halting");
            ...
            if (!concat || !firstEntryOnly) {
              if (concat && (!concatMultipleStringEntries || typedFileLine.hasNumericData())) {
                if (typedFileLine.hasNumericData()) {
                  proj.getLog()
                      .reportTimeError("Multiple entries for the same "
                          + (sampleBased ? "sample" : "marker") + " " + data
                          + " were found and numeric indices were requested, cannot load...");

                } else {
                  proj.getLog().reportTimeError("Multiple entries for the same "
                      + (sampleBased ? "sample" : "marker") + " " + data
                ...
                for (int i = 0; i < numericData.length; i++) {
                  if (typedFileLine.getNumInvalidNumerics() > 0 && !setInvalidNumericToNaN) {
                    ...
                    proj.getLog().reportTimeInfo(
                        "Set the setInvalidNumericToNaN flag to avoid this shutdown");
                    return;
                  }
              ...
              if (typedFileLine.hasStringData() && stringData != null) {
                for (int i = 0; i < stringData.length; i++) {
                  stringData[i][dataIndex] = (concat
                      ? stringData[i][dataIndex] + typedFileParser.getSeparator()
                          + typedFileLine.getStringData()[1][i]
                      : typedFileLine.getStringData()[1][i]);// 0 is reserved for the samples
                }
              }
            }
          }
      ...
      if (requireAll && dataLoaded != dataToLoad.length) {
        proj.getLog()
            .reportTimeError("All data were required to be present, but did not see all of them");
        stringData = null;
        numericData = null;
      } else {
        proj.getLog().reportTimeInfo("Loaded data for " + dataLoaded + " "
            + (sampleBased ? "sample(s)" : "marker(s) ") + " of " + dataPresent.length);
        if (numInvalidNumerics > 0) {
          proj.getLog().reportTimeWarning(
              numInvalidNumerics + "numeric entries had invalid doubles and were set to NaN");
        }
      }
    } catch (IOException e) {
  ...
  private void scanToData() throws IOException {
    if (commentString != null) {
      proj.getLog().reportTimeWarning(
          "The has header option was flagged and comments were set, skipping header and comments");
      String line;
      do {
      ...
      String line;
      proj.getLog().reportTimeWarning(
          "The has header option was flagged and and header flags were set, skipping lines preceding header");
      do {
        line = typedFileParser.readLine();
        if (verbose) {
          proj.getLog().reportTimeInfo("Skipping line " + line);
        }
      } while (line != null && Array.countIf(ext.indexFactors(headerFlags,
          line.trim().split(typedFileParser.getSeparator()), true, proj.getLog(), false, false),
          -1) > 0);
    } else {

      proj.getLog().reportTimeWarning(
          "The has header option was flagged and comments were not set, skipping the first line");
  ...
  private void treatAllNumeric() {
    ...
    if (!hasHeader || determineSampleIndex(true, header)) {
      ...
      proj.getLog().reportTimeInfo(
          "If data titles are present, they are assumed to be in order of the column header");

      if (!hasHeader && numericDataTitles == null) {
        proj.getLog().reportTimeInfo(
            "Since no header or data titles are present, numeric titles will be the relative indices");
      } else if (numericDataTitles != null && numericDataTitles.length != header.length - 1) {
        proj.getLog().reportTimeError("The length of the data title (" + numericDataTitles.length
            + ") does not match the data in the file's length (" + (header.length - 1) + ")");
        return;
      }
      ...
      proj.getLog().reportTimeError(
          "Could not determine proper sample column index for treating all data as numeric");
    }
  }
}
