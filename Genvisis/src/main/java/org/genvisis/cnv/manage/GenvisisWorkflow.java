...
public class GenvisisWorkflow {
  ...
  public static abstract class STEP {
    ...
    public abstract boolean[][] checkRequirements(Project proj,
        HashMap<STEP, Boolean> stepSelections, HashMap<STEP, ArrayList<String>> variables);
    ...
    public boolean hasRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
        HashMap<STEP, ArrayList<String>> variables) {
  ...
  static final STEP S1I_CREATE_MKR_POS = new STEP("Create Marker Positions (if not already exists)",
    ...
    @Override
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
        HashMap<STEP, ArrayList<String>> variables) {
  ...
  static final STEP S2I_PARSE_SAMPLES = new STEP("Parse Illumina Sample Files", "",
      new String[][] {{"[Create Marker Positions] step must be selected and valid.",
          "Parsed markerPositions file must already exist."}, {"Number of Threads to Use"}},
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      return new boolean[][] {{
          stepSelections.get(S1I_CREATE_MKR_POS)
              && S1I_CREATE_MKR_POS.hasRequirements(proj, stepSelections, variables),
          Files.exists(ext.verifyDirFormat(variables.get(this).get(0))),}, {numThreads > 0}};
    }

    @Override
    public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      String projPropFile = proj.getPropertyFilename();
      StringBuilder kvCmd = new StringBuilder(Files.getRunString())
          .append(" cnv.filesys.Project proj=").append(projPropFile);
      ...
      command.append(Files.getRunString()).append(" cnv.manage.SourceFileParser proj=")
          .append(projPropFile).append(" threads=").append(numThreads);
  ...
  static final STEP S3_CREATE_SAMPLEDATA = new STEP("Create SampleData.txt File", "",
      new String[][] {
          ...
          {"Create a minimal SampleData.txt file from sample files",
              ...
              "A Sample_Map.csv file, with at least two columns having headers \""
                  + MitoPipeline.SAMPLEMAP_INPUT[1] + "\" and \"" + MitoPipeline.SAMPLEMAP_INPUT[2]
                  + "\""}},
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      STEP parseStep =
          stepSelections.containsKey(S2I_PARSE_SAMPLES) ? S2I_PARSE_SAMPLES : S2A_PARSE_SAMPLES;
      boolean checkStepParseSamples = stepSelections.get(parseStep)
          && parseStep.hasRequirements(proj, stepSelections, variables);
      return new boolean[][] {
          {checkStepParseSamples || (Files.exists(sampDir)
              && Files.list(sampDir, ".sampRAF", proj.JAR_STATUS.getValue()).length > 0)},
          {Boolean.parseBoolean(variables.get(this).get(0)),
              Files.exists(variables.get(this).get(1)), Files.exists(variables.get(this).get(2))}};
    }
    ...
    public Object[] getRequirementDefaults(Project proj) {
      String pedPreset =
          Files.exists(proj.PEDIGREE_FILENAME.getValue()) ? proj.PEDIGREE_FILENAME.getValue() : "";
      String sampMapPreset = pedPreset.equals("") ? getLocationOfSampleMap(proj) : null; // check
                                                                                         // for
                                                                                         // SampleMap
                                                                                         // only if
                                                                                         // we
  ...
  static final STEP S4_TRANSPOSE_TO_MDF = new STEP("Transpose Data into Marker-Dominant Files", "",
      new String[][] {
          {"[Parse Sample Files] step must have been run already or must be selected and valid."}},
      new RequirementInputType[][] {{RequirementInputType.NONE}}, S2I_PARSE_SAMPLES,
      S2A_PARSE_SAMPLES) {
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      boolean checkStepParseSamples = stepSelections.get(parseStep)
          && parseStep.hasRequirements(proj, stepSelections, variables);
      return new boolean[][] {{checkStepParseSamples || (Files.exists(sampDir)
          && Files.list(sampDir, ".sampRAF", proj.JAR_STATUS.getValue()).length > 0),}};
    }
    ...
    public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      return cmd.append(Files.getRunString())
          .append(
              " cnv.manage.TransposeData -transpose proj=" + projPropFile + " max=" + 2000000000)
          .toString();
    }
  ...
  static final STEP S5_COMPUTE_GCMODEL = new STEP("Compute GCMODEL File", "",
      new String[][] {{"A GC Base file must exist."}, {"GCModel output file must be specified."}},
      new RequirementInputType[][] {{RequirementInputType.FILE}, {RequirementInputType.FILE}}) {
    ...
    public Object[] getRequirementDefaults(Project proj) {
      if (Resources.GENOME_RESOURCE_TYPE.GC5_BASE.getResource(proj.GENOME_BUILD_VERSION.getValue())
        ...
        return new Object[] {
            Resources.GENOME_RESOURCE_TYPE.GC5_BASE
                .getResource(proj.GENOME_BUILD_VERSION.getValue()).getResource(proj.getLog()),
  ...
  static final STEP S6_SAMPLE_QC = new STEP("Run Sample QC Metrics", "",
    ...
    public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      cmd.append(Files.getRunString()).append(" cnv.qc.LrrSd").append(" proj=").append(projPropFile)
          .append(" threads=").append(numThreads).append(" projectMarkers=TRUE");
      return cmd.toString();
    }
    ...
    public void run(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      try {
        ...
        proj.getLog().report("Couldn't parse number of threads argument: {"
            + variables.get(this).get(0) + "}, using default of {" + numThreads + "}");
      }
  ...
  static final STEP S7_MARKER_QC = new STEP("Run Marker QC Metrics", "",
      new String[][] {
          ...
          {"Number of threads to use."}},
      new RequirementInputType[][] {{RequirementInputType.NONE},
          {RequirementInputType.NONE, RequirementInputType.FILE}, {RequirementInputType.NUMBER}},
      S2I_PARSE_SAMPLES) {
  ...
  static final STEP S8_SEX_CHECKS = new STEP("Run Sex Checks", "",
      ...
      new RequirementInputType[][] {{RequirementInputType.NONE}, {RequirementInputType.NONE},
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      return new boolean[][] {
          ...
          {stepSelections.get(S3_CREATE_SAMPLEDATA)
              && S3_CREATE_SAMPLEDATA.hasRequirements(proj, stepSelections, variables)
              || Files.exists(sampDataFile)},
          {true}, {useRValues, !useRValues && Files.exists(discriminatingMarkersFile), !useRValues
              && !Files.exists(discriminatingMarkersFile) && Files.exists(blastVCFFile)}};
    }
  ...
  static final STEP S9_GENERATE_ABLOOKUP = new STEP("Generate AB Lookup File", "",
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      boolean checkStepParseSamples = stepSelections.get(S2I_PARSE_SAMPLES)
          && S2I_PARSE_SAMPLES.hasRequirements(proj, stepSelections, variables);
      return new boolean[][] {
          {checkStepParseSamples || (Files.exists(mkrPosFile) && Files.exists(sampDir)
              && Files.list(sampDir, ".sampRAF", proj.JAR_STATUS.getValue()).length > 0)},};
    }
  ...
  static final STEP S10_RUN_PLINK = new STEP("Create PLINK Files", "",
    ...
    public boolean checkIfOutputExists(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      boolean pedCheck = Boolean.valueOf(variables.get(this).get(1))
          ? Files.exists(proj.PEDIGREE_FILENAME.getValue()) : true;
      return /* Files.exists(fileCheck1) && */ Files.exists(fileCheck2)
          /* && Files.exists(fileCheck3) */ /*
                                             * && Files.list(fileCheck3, ".bed", false).length > 0
                                             */ && pedCheck;
    }
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      return new boolean[][] {
          {checkStepParseSamples || (Files.exists(sampDir)
              && Files.list(sampDir, ".sampRAF", proj.JAR_STATUS.getValue()).length > 0)},
          {Files.exists(pedFile), Boolean.parseBoolean(variables.get(this).get(1))}};
    }
    ...
    public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      StringBuilder cmd = new StringBuilder();
      if (kvCmd.length() > 0) {
        cmd.append(Files.getRunString()).append(" cnv.filesys.Project proj=").append(projPropFile)
            .append(kvCmd).append("\n");
      }
      if (Boolean.valueOf(variables.get(this).get(1))) {
    ...
    public void run(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      boolean create =
          PlinkData.saveGenvisisToPlinkBedSet(proj, "plink/plink", null, null, -1, true);
      if (!create) {
        setFailed();
  ...
  static final STEP S11_GWAS_QC = new STEP("Run GWAS QC", "",
      new String[][] {
          {"[Create/Run PLINK Files] step must have been run already or must be selected and valid."},
          {"Keep genome info for unrelateds only?"},
          {"Skip ancestry checks", "File with FID/IID pairs of putative white samples",
    ...
    public boolean checkIfOutputExists(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      if (!skipAncestry) {
        if (!Files.exists(dir + Qc.ANCESTRY_DIR + "freqsByRace.xln")
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      boolean hapMapPlinkRootExists = Files.exists(hapMapPlinkRoot + ".bed")
          && Files.exists(hapMapPlinkRoot + ".bim") && Files.exists(hapMapPlinkRoot + ".fam");
      boolean files = Files.exists(dir);
      return new boolean[][] {
          {(stepSelections.get(S10_RUN_PLINK)
              && S10_RUN_PLINK.hasRequirements(proj, stepSelections, variables)) || files},
  ...
  static final STEP S12_MOSAIC_ARMS = new STEP("Create Mosaic Arms File", "",
    ...
    public boolean checkIfOutputExists(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      boolean outputCheck =
          Files.exists(proj.RESULTS_DIRECTORY.getValue(false, false) + "Mosaicism.xln");
      return outputCheck;
    }
  ...
  static final STEP S13_ANNOTATE_SAMPLE_DATA = new STEP("Annotate Sample Data File", "",
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      if (sampleQCFileExists && ext.indexOfStr("LRR_SD_Post_Correction",
      ...
      return new boolean[][] {{checkStepSampleQC || sampleQCFileExists},
          ...
          {!checkDuplicates,
              stepSelections.get(S11_GWAS_QC)
                  && S11_GWAS_QC.hasRequirements(proj, stepSelections, variables)
  ...
  //
  // static final STEP S14_CREATE_MT_CN_EST = new STEP("Create Mitochondrial Copy-Number Estimates
  // File",
  // "",
  // new String[][]{
  // {"[Transpose Data into Marker-Dominant Files] step must be selected and valid.", "Parsed marker
  // data files must already exist."},
  // {"[Create Principal Components File] step must be selected and valid.", "Extrapolated PCs file
  // must already exist."},
  ...
  //
  // @Override
  // public void setNecessaryPreRunProperties(Project proj, HashMap<STEP, ArrayList<String>>
  // variables) {
  // // not needed for step
  // }
  //
  ...
  // MitoPipeline.FILE_BASE;//variables.get(this).get(6);
  ...
  // params = GcAdjustorParameter.generate(proj, "GC_ADJUSTMENT/", refGenomeFasta, gAdjustorBuilder,
  // recomputeLRR_Median || recomputeLRR_PCs, bpGcModel, numThreads);
  ...
  // //
  // // proj.getLog().reportTimeError("GC adjustment was flagged, but could not generate neccesary
  // files");
  // // }
  // } else {
  // proj.getLog().reportTimeError("Can not gc correct values without a valid reference genome");
  // proj.getLog().reportTimeError("please supply a valid reference genome (full path) with the
  // \"ref=\" argument");
  // }
  // }
  //
  ...
  // PrincipalComponentsResiduals pcResids = PCA.computeResiduals(proj, extrapolatedPCsFile,
  // ext.removeDirectoryInfo(medianMarkers), numComponents, true, 0f, homozygousOnly,
  // recomputeLRR_Median, outputBase, params);
  ...
  // } catch (NumberFormatException e) {}
  // return new boolean[][]{
  // {(stepSelections.get(S4_TRANSPOSE_TO_MDF) && S4_TRANSPOSE_TO_MDF.hasRequirements(proj,
  // stepSelections, variables)), Files.exists(markerDir)},
  // {(stepSelections.get(S13_CREATE_PCS) && S13_CREATE_PCS.hasRequirements(proj, stepSelections,
  // variables)), Files.exists(extrapPCFile)},
  ...
  // @Override
  // public Object[] getRequirementDefaults(Project proj) {
  // return new String[]{proj.MARKER_DATA_DIRECTORY.getValue(false, false),
  ...
  // @Override
  // public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
  // return "## << Create Mitochondrial Copy-Number Estimates File >> Not Implemented For Command
  // Line Yet ##"; // TODO
  // }
  // };

  static final STEP S14_CREATE_PCS = new STEP(
      "Create Principal Components File and Mitochondrial Copy-Number Estimates File", "",
      new String[][] {
          {"[Transpose Data into Marker-Dominant Files] step must have been run already or must be selected and valid."},
          {"MedianMarkers file must exist."}, {"LRR SD threshold to filter samples."},
          {"Call rate threshold to filter markers."}, {"Compute PCs with samples passing QC only?"},
          {"Should impute mean value for NaN?"}, {"Should recompute Log-R ratio for PC markers?"},
          {"Should recompute Log-R ratio for median markers?"}, {"Homozygous only?"},
          {"Regression distance for the GC adjustment"}, {"Number of threads to use"},
      ...
      new RequirementInputType[][] {{RequirementInputType.NONE}, {RequirementInputType.FILE},
          {RequirementInputType.NUMBER}, {RequirementInputType.NUMBER}, {RequirementInputType.BOOL},
          {RequirementInputType.BOOL}, {RequirementInputType.BOOL}, {RequirementInputType.BOOL},
          {RequirementInputType.BOOL}, {RequirementInputType.NUMBER}, {RequirementInputType.NUMBER},
          {RequirementInputType.FILE}, {RequirementInputType.FILE},},
      S4_TRANSPOSE_TO_MDF) {
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      int numThreads = -1;
      try {
        lrrThresh = Double.parseDouble(variables.get(this).get(1));
      } catch (NumberFormatException e) {
      }
      try {
        callrate = Double.parseDouble(variables.get(this).get(2));
      } catch (NumberFormatException e) {
      }
      try {
        regressionDistance = Integer.parseInt(variables.get(this).get(8));
      } catch (NumberFormatException e) {
      }
      try {
        numThreads = Integer.parseInt(variables.get(this).get(9));
      } catch (NumberFormatException e) {
      }
      return new boolean[][] {
          {(stepSelections.get(S4_TRANSPOSE_TO_MDF)
              && S4_TRANSPOSE_TO_MDF.hasRequirements(proj, stepSelections, variables))
              || Files.exists(markerDir)},
          {Files.exists(medianMkrs)}, {lrrThresh > 0}, {callrate > 0}, {true}, // TRUE or FALSE are
                                                                               // both valid
                                                                               // selections
          {true}, {true}, {true}, {true}, {regressionDistance > 0}, {numThreads > 0},
    ...
    public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      cmd.append(Files.getRunString()).append(" org.genvisis.cnv.manage.MitoPipeline")
          ...
          .append(sampLrr).append(" threads=").append(numThreads).append(" log=")
    ...
    public Object[] getRequirementDefaults(Project proj) {
      return new Object[] {"", proj.LRRSD_CUTOFF.getValue(),
          MitoPipeline.DEFAULT_MKR_CALLRATE_FILTER, "true", "true", "true", "true", "true",
          GcAdjustor.DEFAULT_REGRESSION_DISTANCE[0], proj.NUM_THREADS.getValue(), "", "",};
    }
    ...
    public void run(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      if (retCode == 42) { // TODO remove magic number
        MitoPipeline.estimateMtDNACN(proj, numThreads, medianMarkers, numComponents, outputBase,
            homozygousOnly, markerCallRateFilter, betaOptFile, pedFile, recomputeLRR_PCs,
            recomputeLRR_Median, sampLrr, imputeMeanForNaN, gcCorrect, refGenomeFasta, bpGcModel,
            regressionDistance, proj.GENOME_BUILD_VERSION.getValue(), pvalOpt, betaFile, plot,
            proj.getLog());
      } else {
        setFailed();
      }
      ...
      if (sampleLRRSdFilter < 0) {
        switch (proj.ARRAY_TYPE.getValue()) {
          ...
          case AFFY_GW6_CN:
            ...
            proj.getLog().reportTimeInfo("Setting " + proj.LRRSD_CUTOFF.getName()
  ...
  static final STEP S15_COMPUTE_PFB = new STEP("Compute Population BAF files", "",
    ...
    public boolean checkIfOutputExists(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      boolean pfbExists =
          Files.exists(pfbOutputFile) || Files.exists(ext.rootOf(subSampFile) + ".pfb");
      return pfbExists;
    }
    ...
    public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      return cmd.append(Files.getRunString()).append(" cnv.analysis.PennCNV -pfb proj="
          + proj.getPropertyFilename() + " log=" + proj.getLog().getFilename()).toString();
    }

    @Override
    public Object[] getRequirementDefaults(Project proj) {
      return new Object[] {proj.SAMPLE_SUBSET_FILENAME.getValue(),
          Files.exists(proj.SAMPLE_SUBSET_FILENAME.getValue())
              ? ext.rootOf(proj.SAMPLE_SUBSET_FILENAME.getValue()) + ".pfb"
  ...
  static final STEP S16_SEX_CENTROIDS_PFB_GCMODEL =
      new STEP("Create Sex-Specific Centroids; Filter PFB and GCMODEL Files", "",
          new String[][] {{"[" + S5_COMPUTE_GCMODEL.stepName + "] must be selected and valid.",
              "Full GC Model File."}, {"Number of threads to use."},},
          new RequirementInputType[][] {{RequirementInputType.NONE, RequirementInputType.FILE},
              {RequirementInputType.NUMBER}},
          S5_COMPUTE_GCMODEL) {
        @Override
        public boolean checkIfOutputExists(Project proj,
            HashMap<STEP, ArrayList<String>> variables) {
          String malePFB, femalePFB, centFilePathM, centFilePathF, newGCFile;
          ...
          return Files.exists(malePFB) && Files.exists(femalePFB) && Files.exists(centFilePathM)
              && Files.exists(centFilePathF) && Files.exists(newGCFile);
        }
        ...
        public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
          ...
          boolean checkStepGCModel = stepSelections.get(S5_COMPUTE_GCMODEL)
              && S5_COMPUTE_GCMODEL.hasRequirements(proj, stepSelections, variables);
          int numThreads = -1;
          try {
            numThreads = Integer.parseInt(variables.get(this).get(1));
          } catch (NumberFormatException e) {
        ...
        public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
          ...
          String mainCmd = Files.getRunString() + " cnv.filesys.Centroids proj="
              + proj.getPropertyFilename() + " threads=" + numThreads;
          String gcModelFile = variables.get(this).get(0);
        ...
        public void run(Project proj, HashMap<STEP, ArrayList<String>> variables) {
          ...
          Centroids.computeSexSpecificCentroids(proj,
              AnalysisFormats.getChromosomalMarkersOnly(proj), new String[] {malePFB, femalePFB},
              new String[] {centFilePathM, centFilePathF}, true, numThreads);
  ...
  static final STEP S17_CNV_CALLING = new STEP("Call CNVs", "",
      new String[][] {{"Hidden Markov Model File Must Exist"},
          ...
          {"Number of threads To use."}, {"Output filename."}},
      new RequirementInputType[][] {{RequirementInputType.FILE},
          {RequirementInputType.NONE, RequirementInputType.FILE},
          {RequirementInputType.NONE, RequirementInputType.FILE}, {RequirementInputType.NUMBER},
          {RequirementInputType.FILE},},
    ...
    public Object[] getRequirementDefaults(Project proj) {
      return new Object[] {proj.HMM_FILENAME.getValue(), proj.CUSTOM_PFB_FILENAME.getValue(),
          proj.GC_MODEL_FILENAME.getValue(), proj.NUM_THREADS.getValue(), "cnvs/genvisis.cnv"};
    }
    ...
    public void run(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      CNVCaller.callAutosomalCNVs(proj, output, proj.getSamples(), null, null,
          CNVCaller.DEFUALT_MIN_SITES, CNVCaller.DEFUALT_MIN_CONF, numThreads, 1);// TODO, sex
                                                                                  // specific
                                                                                  // centroids,etc
  ...
  static final STEP S18_SHADOW_SAMPLES = new STEP("Create 'Shadow' Project", "",
      new String[][] {
          ...
          {"Number of threads to use."},},
      new RequirementInputType[][] {{RequirementInputType.NONE}, {RequirementInputType.NUMBER},
          {RequirementInputType.STRING}, {RequirementInputType.NUMBER}, {RequirementInputType.BOOL},
          {RequirementInputType.FILE}, {RequirementInputType.NUMBER},},
      S2I_PARSE_SAMPLES, S2A_PARSE_SAMPLES) {
    @Override
    public boolean checkIfOutputExists(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      String outputBase = proj.PROJECT_DIRECTORY.getValue() + MitoPipeline.FILE_BASE;
    ...
    public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
      ...
      int numComponents = -1;
      try {
        numComponents = Integer.parseInt(variables.get(this).get(0));
      } catch (NumberFormatException e) {
      }
      variables.get(this).get(1);
      double markerCallRateFilter = -1;
      try {
        markerCallRateFilter = Double.parseDouble(variables.get(this).get(2));
      } catch (NumberFormatException e) {
      }
      Boolean.parseBoolean(variables.get(this).get(3));
      String tmpDir =
          "".equals(variables.get(this).get(4).trim()) ? null : variables.get(this).get(4);
      int totalThreads = -1;
      try {
        totalThreads = Integer.parseInt(variables.get(this).get(5));
      } catch (NumberFormatException e) {
      }
      String sampDir = proj.SAMPLE_DIRECTORY.getValue();
      STEP parseStep =
      ...
      return new boolean[][] {
          ...
          {markerCallRateFilter >= 0}, {true}, {tmpDir == null || Files.exists(tmpDir)},
          {totalThreads > 0},};
    }
  ...
  private static STEP[] ILLUMINA_STEPS = {S1I_CREATE_MKR_POS, S2I_PARSE_SAMPLES,
      S3_CREATE_SAMPLEDATA, S4_TRANSPOSE_TO_MDF, S5_COMPUTE_GCMODEL, S6_SAMPLE_QC, S7_MARKER_QC,
      S8_SEX_CHECKS, S9_GENERATE_ABLOOKUP, S10_RUN_PLINK, S11_GWAS_QC, S12_MOSAIC_ARMS,
      S13_ANNOTATE_SAMPLE_DATA, S14_CREATE_PCS, S15_COMPUTE_PFB,
      // S14_CREATE_MT_CN_EST,
      S16_SEX_CENTROIDS_PFB_GCMODEL, S17_CNV_CALLING, S18_SHADOW_SAMPLES,};
  private static STEP[] AFFY_STEPS =
      {S2A_PARSE_SAMPLES, S3_CREATE_SAMPLEDATA, S4_TRANSPOSE_TO_MDF, S5_COMPUTE_GCMODEL,
          S6_SAMPLE_QC, S7_MARKER_QC, S8_SEX_CHECKS, S9_GENERATE_ABLOOKUP, S10_RUN_PLINK,
          S11_GWAS_QC, S12_MOSAIC_ARMS, S13_ANNOTATE_SAMPLE_DATA, S14_CREATE_PCS, S15_COMPUTE_PFB,
          // S14_CREATE_MT_CN_EST,
          S16_SEX_CENTROIDS_PFB_GCMODEL, S17_CNV_CALLING, S18_SHADOW_SAMPLES,};
