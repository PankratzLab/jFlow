...
public class GenvisisWorkflow {
  ...
  public static abstract class STEP {
    ...
    public abstract boolean[][] checkRequirements(Project proj,
                                                  HashMap<STEP, Boolean> stepSelections,
                                                  HashMap<STEP, ArrayList<String>> variables);
  ...
  static final STEP S1I_CREATE_MKR_POS =
      new STEP("Create Marker Positions (if not already exists)", "",
  ...
  static final STEP S2I_PARSE_SAMPLES =
      new STEP("Parse Illumina Sample Files", "",
        ...
        public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
          ...
          StringBuilder kvCmd =
              new StringBuilder(Files.getRunString()).append(" cnv.filesys.Project proj=")
                                                     .append(projPropFile);
  ...
  static final STEP S3_CREATE_SAMPLEDATA =
      new STEP("Create SampleData.txt File", "",
               ...
               new RequirementInputType[][] {{RequirementInputType.NONE},
                                             {RequirementInputType.BOOL, RequirementInputType.FILE,
        ...
        public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
          ...
          boolean checkStepParseSamples =
              stepSelections.get(parseStep) && parseStep.hasRequirements(proj, stepSelections,
                                                                         variables);
  ...
  static final STEP S5_COMPUTE_GCMODEL =
      new STEP("Compute GCMODEL File", "",
  ...
  static final STEP S6_SAMPLE_QC = new STEP("Run Sample QC Metrics", "",
    ...
    public void run(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      ...
      try {
        ...
        proj.getLog()
            .report("Couldn't parse number of threads argument: {" + variables.get(this).get(0)
                    + "}, using default of {" + numThreads + "}");
      }
  ...
  static final STEP S7_MARKER_QC =
      new STEP("Run Marker QC Metrics", "",
               new String[][] {{"[Parse Sample Files] step must have been run already or must be selected and valid."},
  ...
  static final STEP S8_SEX_CHECKS =
      new STEP("Run Sex Checks", "",
               new String[][] {{"[Parse Sample Files] step must have been run already or must be selected and valid."},
        ...
        public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
          ...
          return new boolean[][] {{checkStepParseSamples
                                  ...
                                  {true},
                                  {useRValues,
                                   !useRValues && Files.exists(discriminatingMarkersFile),
  ...
  static final STEP S10_RUN_PLINK =
      new STEP("Create PLINK Files", "",
        ...
        public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
          ...
          StringBuilder cmd = new StringBuilder();
          if (kvCmd.length() > 0) {
            cmd.append(Files.getRunString()).append(" cnv.filesys.Project proj=")
  ...
  static final STEP S11_GWAS_QC =
      new STEP("Run GWAS QC", "",
        ...
        public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
          ...
          boolean hapMapPlinkRootExists =
              Files.exists(hapMapPlinkRoot + ".bed") && Files.exists(hapMapPlinkRoot + ".bim")
                                          && Files.exists(hapMapPlinkRoot + ".fam");
          boolean files = Files.exists(dir);
          return new boolean[][] {{(stepSelections.get(S10_RUN_PLINK)
                                    && S10_RUN_PLINK.hasRequirements(proj, stepSelections,
                                                                     variables))
                                   || files},
                                  {true},
                                  {skipAncestry, putativeWhitesExists && hapMapPlinkRootExists,
        ...
        private String getPlinkDir(Project proj, HashMap<STEP, ArrayList<String>> variables) {
          // This directory is also used in S10_ANNOTATE_SAMPLE_DATA, any changes should be
          // reflected
          // there
  ...
  static final STEP S12_MOSAIC_ARMS =
      new STEP("Create Mosaic Arms File", "",
               new String[][] {{"[Parse Sample Files] step must have been run already or must be selected and valid."},
                               {"Number of threads to use."}},
               new RequirementInputType[][] {{RequirementInputType.NONE},
                                             {RequirementInputType.NUMBER}},
  ...
  static final STEP S13_ANNOTATE_SAMPLE_DATA =
      new STEP("Annotate Sample Data File", "",
               new String[][] {{"[" + S6_SAMPLE_QC.stepName
                               ...
                               {"LRR SD Threshold"}, {"Callrate Threshold"},
        ...
        public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
                                             HashMap<STEP, ArrayList<String>> variables) {
          boolean checkStepSampleQC =
              stepSelections.get(S6_SAMPLE_QC) && S6_SAMPLE_QC.hasRequirements(proj, stepSelections,
                                                                               variables);
  ...
  static final STEP S14_CREATE_PCS =
      new STEP("Create Principal Components File and Mitochondrial Copy-Number Estimates File", "",
               new String[][] {{"[Transpose Data into Marker-Dominant Files] step must have been run already or must be selected and valid."},
                               ...
                               {"Homozygous only?"}, {"Regression distance for the GC adjustment"},
                               {"Number of threads to use"},
        ...
        public boolean[][] checkRequirements(Project proj, HashMap<STEP, Boolean> stepSelections,
          ...
          return new boolean[][] {{(stepSelections.get(S4_TRANSPOSE_TO_MDF)
                                  ...
                                  {Files.exists(medianMkrs)}, {lrrThresh > 0}, {callrate > 0},
                                  {true}, // TRUE or FALSE are
                                          // both valid
                                          // selections
        ...
        public String getCommandLine(Project proj, HashMap<STEP, ArrayList<String>> variables) {
          ...
          cmd.append(Files.getRunString()).append(" org.genvisis.cnv.manage.MitoPipeline")
             ...
             .append(markerCallRateFilter).append(" regressionDistance=").append(regressionDistance)
             .append(" sampLRR=").append(sampLrr).append(" threads=").append(numThreads)
        ...
        public void run(Project proj, HashMap<STEP, ArrayList<String>> variables) {
          ...
          boolean plot = false;
          int retCode =
              PCAPrep.prepPCA(proj, numThreads, outputBase, markerQC, markerCallRateFilter, useFile,
                              proj.getSampleList(), proj.getLog());
          if (retCode == 42) { // TODO remove magic number
            MitoPipeline.estimateMtDNACN(proj, numThreads, medianMarkers, numComponents, outputBase,
                                         ...
                                         pvalOpt, betaFile, plot, proj.getLog());
          } else {
            setFailed();
          }
        }
  ...
  static final STEP S15_COMPUTE_PFB =
      new STEP("Compute Population BAF files", "",
               new String[][] {{"[Parse Sample Files] step must have been run already or must be selected and valid.",
      ...
      new STEP("Create Sex-Specific Centroids; Filter PFB and GCMODEL Files", "",
        ...
        public void run(Project proj, HashMap<STEP, ArrayList<String>> variables) {
          ...
          Centroids.computeSexSpecificCentroids(proj,
                                                ...
                                                new String[] {centFilePathM, centFilePathF}, true,
                                                numThreads);
  ...
  static final STEP S17_CNV_CALLING =
      new STEP("Call CNVs", "",
  ...
  static final STEP S18_SHADOW_SAMPLES = new STEP("Create 'Shadow' Project", "",
                                                  new String[][] {{"[Parse Sample Files] step must have been run already or must be selected and valid."},
                                                                  ...
                                                                  {"Number of threads to use."},},
                                                  new RequirementInputType[][] {{RequirementInputType.NONE},
                                                                                {RequirementInputType.NUMBER}, {RequirementInputType.STRING}, {RequirementInputType.NUMBER}, {RequirementInputType.BOOL}, {RequirementInputType.FILE}, {RequirementInputType.NUMBER},},
                                                  S2I_PARSE_SAMPLES, S2A_PARSE_SAMPLES) {
    @Override
    public boolean checkIfOutputExists(Project proj, HashMap<STEP, ArrayList<String>> variables) {
      String outputBase = proj.PROJECT_DIRECTORY.getValue() + MitoPipeline.FILE_BASE;
  ...
  private static STEP[] ILLUMINA_STEPS =
      {S1I_CREATE_MKR_POS, S2I_PARSE_SAMPLES, S3_CREATE_SAMPLEDATA, S4_TRANSPOSE_TO_MDF,
       S5_COMPUTE_GCMODEL, S6_SAMPLE_QC, S7_MARKER_QC, S8_SEX_CHECKS, S9_GENERATE_ABLOOKUP,
       S10_RUN_PLINK, S11_GWAS_QC, S12_MOSAIC_ARMS, S13_ANNOTATE_SAMPLE_DATA, S14_CREATE_PCS,
       S15_COMPUTE_PFB,
  ...
  private static STEP[] AFFY_STEPS =
      {S2A_PARSE_SAMPLES, S3_CREATE_SAMPLEDATA, S4_TRANSPOSE_TO_MDF, S5_COMPUTE_GCMODEL,
       S6_SAMPLE_QC, S7_MARKER_QC, S8_SEX_CHECKS, S9_GENERATE_ABLOOKUP, S10_RUN_PLINK, S11_GWAS_QC,
       S12_MOSAIC_ARMS, S13_ANNOTATE_SAMPLE_DATA, S14_CREATE_PCS, S15_COMPUTE_PFB,
       // S14_CREATE_MT_CN_EST,
       S16_SEX_CENTROIDS_PFB_GCMODEL, S17_CNV_CALLING, S18_SHADOW_SAMPLES,};
