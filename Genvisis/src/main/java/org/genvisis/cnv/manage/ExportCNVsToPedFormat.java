...
public class ExportCNVsToPedFormat {
  ...
  public static void export(String cnvFilename, String pedFilename, String outputRoot,
    ...
    cnVector = CNVariant.loadPlinkFile(cnvFilename, cnvIsDNAKeyed ? dnaHashFromCnvOrPedData
    ...
    for (int startPosition = 0; startPosition < allChrPosSegs.length; startPosition += markersPerFile) {
      ...
      for (int i = cnvIndex; !done && i < cnvs.length; i++) {
        ...
        currentSampleIndex = sampleListHashFromCnvOrPedData.containsKey(cnvs[i].getFamilyID() + "\t"
                                                                        + cnvs[i].getIndividualID()) ? Integer.parseInt(sampleListHashFromCnvOrPedData.get(cnvs[i].getFamilyID() + "\t" + cnvs[i].getIndividualID())) : dnaHashFromCnvOrPedData.containsKey(cnvs[i].getFamilyID() + "\t" + cnvs[i].getIndividualID()) ? Integer.parseInt(dnaHashFromCnvOrPedData.get(cnvs[i].getFamilyID() + "\t" + cnvs[i].getIndividualID())) : -1;
      ...
      for (Entry<String, String> e : sampleListHashFromCnvOrPedData.entrySet()) {
        if (!fileFormat.equals(PLINK_TRANSPOSED_TEXT_FORMAT)
          ...
          finalSampleList[Integer.parseInt(e.getValue())] = ext.replaceAllWith(e.getKey(), "\t",
                                                                               "-");
        } else {
      ...
      // // dig into whether we need to convert this into MarkerData + an AB_Lookup object, or if
      // this can be modularized
      // } else {
      try {
        for (int i = 0; i < currentChrPosSegs.length; i++) {
          if (!excludeMonomorphicLoci || ArrayUtils.min(currentCNs[i]) < 0
              || ArrayUtils.max(currentCNs[i]) > 0
                 && !ArrayUtils.equals(currentCNs[i], previousCNs)) {
  ...
  private static char[] getPlinkGeno(byte[] bs) {
    ...
    for (byte element : bs) {
      if (mapping[1] == '0' && (element == 0 || element == 1)) {
        mapping[1] = PLINK_TEXT_CODES[element].charAt(0);
      } else if (mapping[0] == '0' && (element == 1 || element == 2)) {
        mapping[0] = PLINK_TEXT_CODES[element].charAt(1);
  ...
  private static void writeFamOrPed(byte[][] currentCNs, String pedFile, String outputRoot,
    ...
    for (int i = 0; i < idList.length; i++) {
      ...
      if (missingWriter != null && !iidMap.containsKey(idList[i])
          && !dnaMap.containsKey(fidiid[1])) {
  ...
  public static void main(String[] args) {
    ...
    for (String arg : args) {
      if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
        ...
        System.exit(1);
      } else if (arg.startsWith("cnv=")) {
        cnvFilename = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("ped=")) {
        pedFilename = ext.parseStringArg(arg, null);
        numArgs--;
      } else if (arg.startsWith("out=")) {
        outputFilename = arg.split("=")[1];
        numArgs--;
      } else if (arg.startsWith("del=")) {
        includeDele = Boolean.parseBoolean(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("dup=")) {
        includeDupl = Boolean.parseBoolean(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("ord=")) {
        ordered = Boolean.parseBoolean(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("coll=")) {
        collapsed = Boolean.parseBoolean(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("homozygousOnly=")) {
        homozygous = Boolean.parseBoolean(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("excludeMonomorphicLoci=")) {
        homozygous = Boolean.parseBoolean(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("lociPerFile=")) {
        lociPerFile = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("win=")) {
        window = Integer.parseInt(arg.split("=")[1]);
        numArgs--;
      } else if (arg.startsWith("-linux")) {
        endOfLine = "\n";
        numArgs--;
      } else if (arg.startsWith("format=")) {
        fileFormat = ext.parseStringArg(arg, MATRIX_FORMAT);
        numArgs--;
      } else {
    ...
    // cnvBySample(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) + "ordered",
    // true, true, true, false, markersPerFile, window);
    // cnvBySample(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) +
    // "ordered_collapsed", true, true, true, true, markersPerFile, window);
    // cnvBySample(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) + "del_only",
    // true, false, true, true, markersPerFile, window);
    // cnvBySample(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) + "dup_only",
    // false, true, true, false, markersPerFile, window);
    // cnvBySample(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) + "unordered",
    // true, true, false, true, markersPerFile, window);
    ...
    // endOfLine = "\n";
    // rfglsOutput = true;
    // export(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) +
    // "../results/deletionsAndDuplications", endOfLine, rfglsOutput, true, true, false, true,
    // false, true, lociPerFile, window);
    // export(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) +
    // "../results/deletionsOnly", endOfLine, rfglsOutput, true, false, false, true, false, true,
    // lociPerFile, window);
    // export(cnvFilename, pedFilename, ext.parseDirectoryOfFile(cnvFilename) +
    // "../results/homozygousDeletionsOnly", endOfLine, rfglsOutput, true, false, false, true, true,
    // true, lociPerFile, window);
    // System.exit(1);
