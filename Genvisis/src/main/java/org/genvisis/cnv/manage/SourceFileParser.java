...
public class SourceFileParser implements Runnable {
				...
				try {
					...
					while ((tmp = reader.readLine()) != null) {
						...
						if (count == 0) {
							...
							log.reportError("Found " + line[snpIndex] + " at marker #" + (count + 1)
															+ " in file "
						...
						try {
							for (int i1 = 0; i1 < indCols.length; i1++) {
								ind = i1;
								if (indCols[ind] != -1) {
									data[ind][key] = ext.isMissingValue(line[indCols[ind]])
																																					? Float.NaN
						...
						if (headerData.colGeno1 != -1 && headerData.colGeno2 != -1) {
							...
							if (genotypes[0][key] == -1) {
								if (proj.getArrayType() == ARRAY.ILLUMINA) {
									...
									genotypes[0][key] = (byte) ext.indexOfStr(line[headerData.colGeno1],
																														Sample.AB_PAIRS);
									if (genotypes[0][key] == -1 && ext.indexOfStr(line[headerData.colGeno1],
																																Sample.ALT_NULLS) == -1) {
										log.reportError("Error - failed to lookup " + line[headerData.colGeno1]
																		+ " for marker "
																		+ markerNames[count] + " of sample " + files[i]
						...
						if (headerData.colGenoAB1 != -1 && headerData.colGenoAB2 != -1) {
							...
							} else if (abLookup == null || headerData.colGeno1 == -1
												 || headerData.colGeno2 == -1
	...
	public static String determineFilename(String dir, String sampleName, long timeBegan,
		...
		if (versionToOverwrite != -1) {
			...
			if (new File(dir + SourceFileParser.OVERWRITE_OPTION_FILE).exists()) {
				...
				if (!Files.isWindows()) {
					log.reportError("Error - the same sample name '"
													+ sampleName
			...
			while (reader.ready()) {
				...
				if (markerNameHash.containsKey(trav)) {
					...
					if (idHeader.equals(SourceFileParser.FILENAME_AS_ID_OPTION)) {
						...
						if (!proj.LONG_FORMAT.getValue()) {
							log.reportError("... and the sample name changed from "
															+ sampleName
															+ " to "
															+ (parseAtAt
																					 ? line[sampIndex].substring(0,
						...
						log.reportError("Error - detected the same marker ('"
														+ trav
	...
	private static int checkForExistingFiles(Project proj, String dir, final String ext) {
		...
		if (foundFiles) {
			...
			if (GraphicsEnvironment.isHeadless()) {
				proj.getLog()
						.reportTimeWarning("Parsed data files were found in "
															 + dir
	...
	public static int createFilesFromLongFormat(Project proj, String[] files, String idHeader,
		...
		try {
			LongFileFormatProducer producer = new LongFileFormatProducer(proj, files, idHeader, fixes,
																																	 ...
																																	 headers, log);
			WorkerTrain<LongFormatParseResult> train = new WorkerTrain<SourceFileParser.LongFormatParseResult>(
																																																				 producer,
																																																				 numThreads,
																																																				 10,
																																																				 log);
	 ...
	 * @throws Elision
	 */
	private static LongFormatParseResult parseLongFormatFile(Project proj,
																													 String file,
																													 String idHeader,
																													 Hashtable<String, String> fixes,
																													 String delimiter,
		...
		try {
			...
			if (headerData == null) {
				// TODO error, missing header for source files
				log.report("WARNING - No parsed header object found for file "
									 + file
	...
	public static void main(String[] args) {
		...
		String usage = "\n"
									 ...
									 + " (default))\n"
									 + "   (2) number of threads to use (i.e. "
