...
public class TransposeData {
	...
	public static void transposeData(Project proj, long markerFileSizeSuggested, boolean keepAllSampleFilesOpen) {
		...
		if (new File(proj.PROJECT_DIRECTORY.getValue()).getFreeSpace() <= (allSampleNamesInProj.length * (long)allMarkerNamesInProj.length * numBytesPerSampleMarker)) {
			log.reportError("Not enough disk space for all the new data to be created. Available: " + ext.prettyUpSize(new File(proj.PROJECT_DIRECTORY.getValue()).getFreeSpace(), 1) + "; Required: " + ext.prettyUpSize(new File(proj.PROJECT_DIRECTORY.getValue()).getFreeSpace(), 1) + ").");
			return;
		}
		...
		while (!done) {
			...
			try {
				...
				backupCount = backupOlderFiles(proj.MARKER_DATA_DIRECTORY.getValue(true, false), new String[] {MarkerData.MARKER_DATA_FILE_EXTENSION, "outliers.ser"}, true);
				...
				for (int i=0; i<numFiles; i++) {
					markerFilenames[i] = proj.MARKER_DATA_DIRECTORY.getValue(false, true) + "markers." + i + MarkerData.MARKER_DATA_FILE_EXTENSION;
				...
				deleteOlderRafs(proj.MARKER_DATA_DIRECTORY.getValue(true, false), null, new String[] {MarkerData.MARKER_DATA_FILE_EXTENSION, "outliers.ser"}, false, null);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static void recreateMarkerLookup(Project proj) {
		...
		if (files==null) {
			System.err.println("Error - failed to create MarkerLookup -- marker data directory does not exist: "+proj.MARKER_DATA_DIRECTORY.getValue(false, true));
			...
			for (int i = 0; i<files.length; i++) {
				try {
					...
					currentFile = new RandomAccessFile(proj.MARKER_DATA_DIRECTORY.getValue(false, true) + files[i], "r");
...
//		boolean isCurrentOutFileComplete;
		...
		nullStatus = getNullstatusFromRandomAccessFile(proj.MARKER_DATA_DIRECTORY.getValue(false, true) + proj.getMarkerLookup().getFirstMarkerDataRafFilename(), false, false);
		numBytesPerSampleMarker = Sample.getNBytesPerSampleMarker(nullStatus);
		if (new File(proj.PROJECT_DIRECTORY.getValue()).getFreeSpace() <= (listOfAllSamplesInProj.length * (long)listOfAllMarkersInProj.length * numBytesPerSampleMarker)) {
			log.reportError("Not enough disk space for all the new data to be created. Available: " + ext.prettyUpSize(new File(proj.PROJECT_DIRECTORY.getValue()).getFreeSpace(), 1) + "; Required: " + ext.prettyUpSize(new File(proj.PROJECT_DIRECTORY.getValue()).getFreeSpace(), 1) + ").");
			return;
		}
...
//				timerLoadFiles = 0;
				...
				for(int i=0; i<numRoundsLoadingMarkerFiles; i++) {
					...
					for (int j=0; j<markerDataRafFilenames.length; j++) {
						timerTmp = new Date().getTime();
						markerFile = proj.MARKER_DATA_DIRECTORY.getValue(true, true) + markerDataRafFilenames[j];
