...
public class CNVComparison {
	...
	public CNVComparison(CNVariantQC[][] unfilteredcnvsQCs1, CNVariantQC[][] unfilteredcnvsQCs2, Hashtable<String, CNVSampleQC> cnvSampleQCHash, OptimizedQCThresholds qcThresholds, int filterType, Logger log) {
		this.totalCallsAvailable = getCallsAvailable(unfilteredcnvsQCs1, unfilteredcnvsQCs2);
		...
		this.goodCalls = getGoodCalls(filteredcnvQCs1, filteredcnvQCs2);
		this.averageCNPercent = averageCNPercents(goodCalls, filteredCallsAvailable);
	...
	private int[] getAllCalls(CNVariantQC[][] cnvQCs, int[] totalCallsAvailable) {
		for (int i = 0; i < cnvQCs.length; i++) {
			if ((cnvQCs[i] != null)) {
				if (cnvQCs[i].length > 0) {
					totalCallsAvailable = getCNNumbers(cnvQCs[i], totalCallsAvailable);
				} 
			}
		}
		return totalCallsAvailable;
	...
	private boolean passesSampleQC(CNVariantQC unfilteredcnvQC, int numCNVs) {
		String lookup = unfilteredcnvQC.getCnVariant().getFamilyID() + "\t" + unfilteredcnvQC.getCnVariant().getIndividualID();
	...
	private boolean goodScore(int numMarkers, double height) {
		if (Double.isNaN(qcThresholds.getAlpha()) || Double.isNaN(qcThresholds.getBeastConfCutoff())) {
			...
			return score(numMarkers, qcThresholds.getAlpha(), height) >= qcThresholds.getBeastConfCutoff();
		}
	}
...
//	}
	...
	private int[][] countMatches(CNVariantQC[] filteredcnvQCs1, CNVariantQC[] filteredcnvQCs2, int[][] counts) {
		...
		int match;
		int CN = 5;
		for (int a = 0; a < filteredcnvQCs1.length; a++) {
			match = 0;
			for (int b = 0; b < filteredcnvQCs2.length; b++) {
				if (filteredcnvQCs2[b].getCnVariant().getCN() == filteredcnvQCs1[a].getCnVariant().getCN()) {
					if (filteredcnvQCs1[a].getCnVariant().equals(filteredcnvQCs2[b].getCnVariant())) {
						match = 3;
						filteredcnvQCs2[b].getCnVariant().setSource(99);
						CN = filteredcnvQCs2[b].getCnVariant().getCN();
					} else if (match < 2 && filteredcnvQCs1[a].getCnVariant().significantOverlap(filteredcnvQCs2[b].getCnVariant())) {
						match = 4;
						CN = filteredcnvQCs2[b].getCnVariant().getCN();

						// an overlap is not assumed in both directions
						// filteredcnvQCs2[b].getCnVariant().setSource(99);
					} else if (match < 2 && filteredcnvQCs1[a].getCnVariant().overlaps(filteredcnvQCs2[b].getCnVariant())) {
						match = 2;
						CN = filteredcnvQCs2[b].getCnVariant().getCN();
						filteredcnvQCs2[b].getCnVariant().setSource(99);
					}
		...
		for (int b = 0; b < filteredcnvQCs2.length; b++) {
			...
			for (int a = 0; a < filteredcnvQCs1.length; a++) {
				if (filteredcnvQCs2[b].getCnVariant().getCN() == filteredcnvQCs1[a].getCnVariant().getCN()) {
					if (filteredcnvQCs2[b].getCnVariant().getSource() != 99 && filteredcnvQCs2[b].getCnVariant().equals(filteredcnvQCs1[a].getCnVariant())) {
						match = 3;
						CN = filteredcnvQCs2[b].getCnVariant().getCN();
					} else if (match < 2 && filteredcnvQCs2[b].getCnVariant().getSource() != 99 && filteredcnvQCs2[b].getCnVariant().significantOverlap(filteredcnvQCs1[a].getCnVariant())) {
						match = 4;
						CN = filteredcnvQCs2[b].getCnVariant().getCN();
					} else if (match < 2 && filteredcnvQCs2[b].getCnVariant().getSource() != 99 && filteredcnvQCs2[b].getCnVariant().overlaps(filteredcnvQCs1[a].getCnVariant())) {
						match = 2;
						CN = filteredcnvQCs2[b].getCnVariant().getCN();
					}
				}
