...
public class TwoDPanel extends AbstractPanel implements MouseListener, MouseMotionListener {
  ...
  private void generateRectangles() {
    ...
    float binHalf = (float) (hist.getBins().length > 1
        ? ext.roundToSignificantFigures(minDiff / 2.0, sig + 1) : DEFAULT_HALF_BIN_SIZE);
    ...
    for (int i = 0; i < rectangles.length; i++) {
      ...
      if (swapAxes) {
        startX = 0f;
        startY = sig > 0
            ? (float) ext.roundToSignificantFigures((float) (hist.getBins()[i] - binHalf), sig)
  ...
  // }
  // float binHalf = (float) (currentData.size() > 1 ? ext.roundToSignificantFigures(minDiff / 2.0,
  // sig + 1) : DEFAULT_HALF_BIN_SIZE);
  //
  ...
  // if (swapAxes) {
  // startX = 0f;
  // startY = sig > 0 ? (float) ext.roundToSignificantFigures((float)(xAxisValue - binHalf), sig) :
  // (float)(xAxisValue - binHalf);
  // stopX = yAxisValue;
  // stopY = sig > 0 ? (float) ext.roundToSignificantFigures((float)(xAxisValue + binHalf), sig) :
  // (float)(xAxisValue + binHalf);
  // } else {
  // startX = sig > 0 ? (float) ext.roundToSignificantFigures((float)(xAxisValue - binHalf), sig) :
  // (float)(xAxisValue - binHalf);
  // startY = 0f;
  // stopX = sig > 0 ? (float) ext.roundToSignificantFigures((float)(xAxisValue + binHalf), sig) :
  // (float)(xAxisValue + binHalf);
  // stopY = yAxisValue;
  // }
  // rectangles[i] = new GenericRectangle(startX, startY, stopX, stopY, (byte) 5, true, false,
  // Byte.parseByte(line[3]), (byte) 2, (byte) 0);
  //
  // for (int j = 0; j < linkerData[i].length; j ++) {
  // linkerData[i][j] = line[j + index];
  // }
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    if (prox != null && prox.size() > 0) {
      ...
      for (int i = 0; i < maxNumPoints; i++) {
        ...
        if (linkKeyIndicies[TwoDPlot.DNA_INDEX_IN_LINKERS] >= 0 && Files.exists(
        ...
        if (sampleData != null && ids != null) {
          sampColor =
              colorScheme[sampleData.determineCodeFromClass(tdp.colorKeyPanel.getCurrentClass(),
                  (byte) 0, sampleData.getIndiFromSampleHash(ids[0]), (byte) 0, 0)];
        }
        if (sample != null) {
          if (region != null) {
  ...
  public void overrideAxisLabels(String xAxis, String yAxis) {
    overrideAxisLabels = xAxis != null && yAxis != null;
    if (overrideAxisLabels) {
      xAxisLabel = xAxis;
      yAxisLabel = yAxis;
    }
