...
public class CompareCalls_dev {
	...
	private int numPassingAndPresent;
	private double[] averagePercent;
	private int[] totalCallsAnalyzed;
	private String[] cnvFiles;
	private int[] exactMatches;
	private int[] olapMatches;
	private int[] sigOlapMatches;
	private double[] indPercentMatches;
	private int[][] indCNVCalls;
	private String[][] comparedIDs;

	public CompareCalls_dev(int numPassingAndPresent, double averagePercent[], int[] totalCallsAnalyzed, String[] cnvFiles, int[] exactMatches, int[] sigOlapMatches, int[] olapMatches, String[][] comparedIDs, int[][] indCNVCalls, double[] indPercentMatches) {
	...
	// iterate(0.2, 0.6, 20, 0.0, 100, 10, 0, 20, rootDir, cnvFile, LRR_lookup, "rootsANDdubs.compsmall.txt", markerFile);
	// iterate over parameters(lrr, conf, numMarkers) and compare results among duplicates
	public static void iterate(double lrrstart, double lrrstop, int numLrrBins, double confstart, double confstop, int numConfBins, int probestart, int probeStop, String rootDir, String cnvFile, String LRR_lookup, String compFile, String markerFile) {
		...
		try {
			...
			// comparedCalls = compare(rootDir, filterAndClean(rootDir, cnvFile, LRR_lookup, compFile, lrrValues[k], DEFAULT_CONF_FILTER, i, markerFile, DEFAULT_BP_FRACTION, DEFAULT_NUM_MARKER_FRACTION), compFile);
			// if (i == probestart && k == 0) {
			...
			for (int i = probestart; i < probeStop + 1; i += 5) {
				String iterType = "numMarkers";
				comparedCalls = compare(rootDir, filterAndClean(rootDir, cnvFile, LRR_lookup, compFile, DEFAULT_LRR_FILTER, DEFAULT_CONF_FILTER, i, markerFile, DEFAULT_BP_FRACTION, DEFAULT_NUM_MARKER_FRACTION), DEFAULT_COMP_LIST);
				if (i == probestart) {
					probeWriters = getWriters(rootDir, comparedCalls, iterType, probestart, probeStop, false);
				} else {
					probeWriters = getWriters(rootDir, comparedCalls, iterType, probestart, probeStop, true);
			...
			for (int i = 0; i < lrrValues.length; i++) {
				String iterType = "lrr";
				comparedCalls = compare(rootDir, filterAndClean(rootDir, cnvFile, LRR_lookup, compFile, lrrValues[i], DEFAULT_CONF_FILTER, DEFAULT_NUM_MARKERS_FILTER, markerFile, DEFAULT_BP_FRACTION, DEFAULT_NUM_MARKER_FRACTION), DEFAULT_COMP_LIST);
				if (i == 0) {
			...
			for (int i = 0; i < confValues.length; i++) {
				...
				comparedCalls = compare(rootDir, filterAndClean(rootDir, cnvFile, LRR_lookup, compFile, DEFAULT_LRR_FILTER, confValues[i], DEFAULT_NUM_MARKERS_FILTER, markerFile, DEFAULT_BP_FRACTION, DEFAULT_NUM_MARKER_FRACTION), DEFAULT_COMP_LIST);
				if (i == 0) {
	...
	// compare all combinations of files, where Ids are matched as defined in the comp file, copy number aware comparisons
	public static CompareCalls_dev[] compare(String rootDir, String[] files, String compFile) {
		...
		for (int i = 0; i < allPossibleCombinations.length; i++) {
			...
			if (inds1.length != inds2.length || inds1.length == 0 || inds2.length == 0) {
				System.err.println("Info - there are " + inds1.length + " ids  in " + files[allPossibleCombinations[i][0]] + " and " + inds2.length + " ids in " + files[allPossibleCombinations[i][1]]);
				if (inds1.length == 0 || inds2.length == 0) {
					System.err.println("Info - skipping the comparison between " + files[allPossibleCombinations[i][0]] + " and " + files[allPossibleCombinations[i][1]]);
				}
			}
			try {
				...
				for (int j = 0; j < inds1.length; j++) {
					for (int k = 0; k < inds2.length; k++) {
						if (defineCompHash.containsKey(inds1[j]) && defineCompHash.get(inds1[j]).containsKey(inds2[k])) {
							cnvs = getCompareCNVs(inds1[j], inds2[k], allPossibleCombinations[i], indFileCNVs);
							...
							comparedIDs[1][j] = inds2[k];
							indCNVCalls[0][j] = cnvs[0].length;
							indCNVCalls[1][j] = cnvs[1].length;
							// TODO john do copynumber aware call percent. OK I did it. now calculates copy number specific
							// percent matches
							if (cnvs[0].length > 0 || cnvs[1].length > 0) {
								...
								double callPercent = ((double) ((2 * exactMatches[j]) + sigOlapMatches[j]) / (cnvs[0].length + cnvs[1].length));
				...
				System.err.println("Info - " + numPassingAndPresent + " with valid comparison matched Ids were found and compared");
				comparedCalls[i] = new CompareCalls_dev(numPassingAndPresent, averageCNPercent, cnNumbers, files, exactMatches, sigOlapMatches, olapMatches, comparedIDs, indCNVCalls, indPercentMatches);
			} catch (Exception e) {
	...
	public static String cleanCNVs(String rootDir, String cnvfile, String markerFile, double bpFraction, double markerFraction) {
		PrintWriter writer;
		String[] inds;
		String file;
		CNVariant[] cnvs;
		...
		for (int i = 0; i < inds.length; i++) {
			CNVariant mergedCNV;
			cnvs = toCNVArray(allIndCNVs.get(inds[i]));
		...
		System.out.println("Info - " + changecount + " individual calls were combined into " + newCalls + " new calls,\t" + nochange + " Calls were Unchanged,\tA total " + totalCount + " of Calls Examined from " + inds.length + " individuals");
		writer.close();
		return file;
	}

	// do both filtering and cleaning
	public static String[] filterAndClean(String rootDir, String cnvFile, String LRR_lookup, String compFile, double lrrFilter, double confFilter, int numMarkers, String markerFile, double bpFraction, double markerFraction) {
		String[] filteredFiles, cleanedFiles;
		filteredFiles = filterCNVs(rootDir, cnvFile, LRR_lookup, compFile, lrrFilter, confFilter, numMarkers);
		cleanedFiles = new String[filteredFiles.length];
		for (int i = 0; i < filteredFiles.length; i++) {
			cleanedFiles[i] = cleanCNVs(rootDir, filteredFiles[i], markerFile, bpFraction, markerFraction);
		}
		return cleanedFiles;
	}

	public static String[] filterCNVs(String rootDir, String cnvFile, String LRR_lookup, String compFile, double lrrFilter, double confFilter, int numMarkers) {
		...
		try {
			...
			if (!ext.checkHeader(reader.readLine().trim().split("[\\s]+"), CNVariant.PLINK_CNV_HEADER, false)) {
				...
				for (int i = 0; i < maxRepNumber; i++) {
					files[i] = rootDir + "intermediateFiles/Filtered_LRR_" + lrrFilter + "_conf_" + confFilter + "_numMarkers_" + numMarkers + "_rep" + i + ".cnv";
				...
				for (int i = 0; i < maxRepNumber; i++) {
					...
					System.out.println("Info - Out of a total of " + totalCounts + " cnv calls in " + rootDir + cnvFile + ", a total of " + totalInCompFile + " calls had matched ids listed in " + compFile + " column " + i + ": " + filteredCounts[i] + " were filtered out, " + includeCounts[i] + " were included for comparison in " + files[i]);
				}
			}
			reader.close();
		} catch (FileNotFoundException fnfe) {
			System.err.println("Error: file \"" + rootDir + cnvFile + "\" not found in current directory");
			System.exit(1);
	...
	// computes average percent match for each copy number, and a total for all copy numbers
	private static double[] goodCallCN(CNVariant[] cnvs0, CNVariant[] cnvs1, int[][] counts, double[] goodCalls) {
		// CN state percents as defined here are ,0,1,2,3,4,5...where 2 is a total non-copy number aware count, and 5 is for non matching (overlap,exact,sigoverlap)
		int[] indCNNumbers = new int[6];
		indCNNumbers = getCNNumbers(cnvs0, cnvs1, indCNNumbers);
		for (int i = 0; i < counts.length; i++) {
			// System.out.println(i);
			if (indCNNumbers[i] > 0) {
				// System.out.println(cnvs0[0].getFamilyID() + "\t" + cnvs1[0].getFamilyID() + "\t" + counts[i][3] + "\t" + counts[i][4] + "\t" + indCNNumbers[i] + "\t" + i);
				goodCalls[i] += 2 * counts[i][3] + counts[i][4];
				// System.out.println(cnvs0[0].getFamilyID() + "\t" + cnvs1[0].getFamilyID() + "\t" + counts[i][3] + "\t" + counts[i][4] + "\t" + indCNNumbers[i] + "\t" + i + "\t" + ((double) ((2 * counts[i][3]) + counts[i][4]) / (indCNNumbers[i])));
	...
	private static String getHeader(String iterType) {
		return iterType + "\tCN=any;avgPercent\tCN=any;numCallsAnalyzed\tCN=0;avgPercent\tCN=0;numCallsAnalyzed\tCN=1;avgPercent\tCN=1;numCallsAnalyzed\tCN=3;avgPercent\tCN=3;numCallsAnalyzed\tCN=4;avgPercent\tCN=4;numCallsAnalyzed\tPairs of Individuals PassingFilters";
	}
	...
	private static String getPrintString(CompareCalls_dev comparedCall) {
		...
		printString = avePercents[2] + "\t" + totalCallsAnalyzed[2] + "\t" + avePercents[0] + "\t" + totalCallsAnalyzed[0] + "\t" + avePercents[1] + "\t" + totalCallsAnalyzed[1] + "\t" + avePercents[3] + "\t" + totalCallsAnalyzed[3] + "\t" + avePercents[4] + "\t" + totalCallsAnalyzed[4] + "\t" + comparedCall.getnumPassingAndPresent();

		return printString;
	}

	private static PrintWriter[] getWriters(String rootDir, CompareCalls_dev[] comparedCalls, String compareType, int start, int stop, boolean append) {
	...
	private static PrintWriter[] getWriters(String rootDir, CompareCalls_dev[] comparedCalls, String compareType, double start, double stop, boolean append) {
		PrintWriter[] writers = new PrintWriter[comparedCalls.length];
		for (int i = 0; i < comparedCalls.length; i++) {
			String[] comparisons = getComparisons(comparedCalls[i]);
			String file = getFileName(rootDir, compareType, start, stop, comparisons);
	...
	private static String getFileName(String rootDir, String compareType, int start, int stop, String[] comparisons) {
		String file;
		if (compareType.equals("numMarkers")) {
			file = rootDir + "test" + comparisons[0] + "_vs_" + comparisons[1] + "_" + compareType + "_Concordance_" + start + "_" + stop + "_LRR_" + DEFAULT_LRR_FILTER + "Conf_" + DEFAULT_CONF_FILTER + ".concord";
		} else {
	...
	private static String getFileName(String rootDir, String compareType, double start, double stop, String[] comparisons) {
		String file;
		if (compareType.equals("lrr")) {
			file = rootDir + "test" + comparisons[0] + "_vs_" + comparisons[1] + "_" + compareType + "_Concordance_" + start + "_" + stop + "_numMarkers_" + DEFAULT_NUM_MARKERS_FILTER + "_Conf_" + DEFAULT_CONF_FILTER + ".concord";
		} else if (compareType.equals("conf")) {
			file = rootDir + "test" + comparisons[0] + "_vs_" + comparisons[1] + "_" + compareType + "_Concordance_" + start + "_" + stop + "_lrr_" + DEFAULT_LRR_FILTER + "_numMarkers_" + DEFAULT_NUM_MARKERS_FILTER + ".concord";
		} else {
			System.err.println("invalid invalid Comparison type");
			file = "NA";
		}
	...
	private static CNVariant[][] getCompareCNVs(String ind1, String ind2, int[] combinations, Hashtable<Integer, Hashtable<String, ArrayList<CNVariant>>> indFileCNVs) {
		return new CNVariant[][] { toCNVArray(indFileCNVs.get(combinations[0]).get(ind1)), toCNVArray(indFileCNVs.get(combinations[1]).get(ind2)) };
	}
		...
		try {
			...
			while (reader.ready()) {
				...
				if (lrrs.containsKey(line[0])) {
					System.err.println("Warning - duplicate samples found in " + rootDir + LRR_lookup + " only using one of them");
				} else {
			...
			System.err.println("Error: file \"" + rootDir + LRR_lookup + "\" not found in current directory");
			System.exit(1);
		} catch (IOException ioe) {
	...
	private static Hashtable<String, Hashtable<String, Integer>> defineCompLists(String rootDir, String compFile) {
		Hashtable<String, Hashtable<String, Integer>> defineCompHash = new Hashtable<String, Hashtable<String, Integer>>();
		BufferedReader reader;
		String[] line;
		...
		try {
			...
			System.err.println("Error: file \"" + rootDir + compFile + "\" not found in current directory");
			System.exit(1);
		} catch (IOException ioe) {
	...
	private static boolean alreadyDefinedID(Hashtable<String, Hashtable<String, Integer>> defineCompHash, String[] line) {
		boolean alreadyDefined = false;
		for (int i = 0; i < line.length; i++) {
			// same id not allowed in same line
			if (defineCompHash.containsKey(line[i])) {
				System.err.println("Warning - duplicate IDs were detectected in the comparision file: " + line[i] + " was seen twice, only the first comparison will be used ");
				alreadyDefined = true;
			}
		}
	...
	private static void assignMax(Hashtable<String, Hashtable<String, Integer>> defineCompHash, int maxNumComparisions) {
	...
	private static void defineComparisons(Hashtable<String, Hashtable<String, Integer>> defineCompHash, String[] line) {
		Hashtable<String, Integer> defined = new Hashtable<String, Integer>();
		for (int i = 0; i < line.length; i++) {
			for (int j = 0; j < line.length; j++) {
				if (i != j && line[i] == line[j]) {
					System.err.println("Warning - duplicate IDs were detectected in the comparision file: " + line[i] + " in column " + i + "and " + line[j] + " in column " + j);
				}
	...
	private static Hashtable<Byte, List<Integer>> getMarkerLookup(String rootDir, String markerPositionsFile) {
	...
	private static boolean checkAll(double bpFraction, double markerFraction, Hashtable<Byte, List<Integer>> markers, CNVariant thisCNV, CNVariant nextCNV) {
		return checkChr(thisCNV, nextCNV) && bpGapCheck(thisCNV, nextCNV, bpFraction) && checkCN(thisCNV, nextCNV) && markerGapCheck(thisCNV, nextCNV, markers, markerFraction);
	}

	// new confidence value = old+old+..., as per PennCNV folks
	private static CNVariant mergeCNVs(CNVariant[] cnvs, int startCombine, int stopCombine, Hashtable<Byte, List<Integer>> markers) {
		CNVariant mergedCNV;
		double newConf = 0.0;
		int newNumMarkers = 0;
		...
		mergedCNV = new CNVariant(cnvs[startCombine].getFamilyID(), cnvs[startCombine].getIndividualID(), cnvs[startCombine].getChr(), cnvs[startCombine].getStart(), cnvs[stopCombine].getStop(), cnvs[startCombine].getCN(), newConf, newNumMarkers, cnvs[startCombine].getSource());
		return mergedCNV;
	}
	...
	public static void main(String[] args) {
		...
		// "   (7) the index of the transformation to export or -1 for all (i.e. transIndex= (default))\n"+
		// "  OR:\n"+
		...
		try {
			...
			// public static String[] filterAndClean(String rootDir, String cnvFile, String LRR_lookup, String compFile, double lrrFilter, double confFilter, int numMarkers, String markerFile, double bpFraction, double markerFraction) {
			filterAndClean(rootDir, cnvFile, LRR_lookup, "Full_SAMP.txt", lrrFilter, confFilter, numMarkers, markerFile, 0.3, 0.3);
			// // cleanCNVs(rootDir, filterCNVs(rootDir, cnvFile, LRR_lookup, DEFAULT_COMP_LIST, lrrFilter, confFilter, numMarkers)[0], markerFile, 0.2, 0.2);
			// compare(rootDir, filterAndClean(rootDir, cnvFile, LRR_lookup, DEFAULT_COMP_LIST, lrrFilter, confFilter, numMarkers, markerFile, 0.2, 0.2), DEFAULT_COMP_LIST);
			// defineCompLists(rootDir, DEFAULT_COMP_LIST);
			// getLRRs(rootDir, LRR_lookup);
			// iterate(0.2, 0.6, 30, 0, 100, 10, 0, 100, rootDir, cnvFile, LRR_lookup, DEFAULT_COMP_LIST, markerFile);
			// filterCNVs(rootDir, cnvFile, LRR_lookup, DEFAULT_COMP_LIST, lrrFilter, confFilter, numMarkers);
			// compare(rootDir, filterCNVs(rootDir, cnvFile, LRR_lookup, compFiles, lrrFilter, confFilter, numMarkers));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
// private static Hashtable<Integer, Hashtable<String, ArrayList<CNVariant>>> getCNVHash(String[] files) {
// BufferedReader reader;
// String[] line;
// ArrayList<CNVariant> v;
// Hashtable<Integer, Hashtable<String, ArrayList<CNVariant>>> hash = new Hashtable<Integer, Hashtable<String, ArrayList<CNVariant>>>();
...
// for (int i = 0; i < lrrValues.length; i++) {
// comparedCalls = compare(rootDir, filterCNVs(rootDir, cnvFile, LRR_lookup, compFiles, lrrValues[i], DEFAULT_CONF_FILTER, DEFAULT_NUM_MARKERS_FILTER));
// for (int j = 0; j < comparedCalls.getInds().length; j++) {
// if (comparedCalls.getIndCNVCalls()[0][j] > 0 && comparedCalls.getIndCNVCalls()[1][j] > 0) {
// lrrWriter.println(comparedCalls.getInds()[j] + "\t" + lrrValues[i] + "\t" + comparedCalls.getIndCNVCalls()[0][j] + "\t" + comparedCalls.getIndCNVCalls()[1][j] + "\t" + comparedCalls.getExactMatches()[j] + "\t" + comparedCalls.getSigOlapMatches()[j] + "\t" + comparedCalls.getindPercentMatches()[j] + "\t" + comparedCalls.getAveragePercent() + "\t" + comparedCalls.getnumPassingAndPresent() + "\t" + comparedCalls.getTotalCallsAnalyzed() + "\t" + Array.toStr(comparedCalls.getCnvFiles()) + "\t");
// }
// }
// // lrrWriter.println(lrrValues[i] + "\t" + comparedCalls.getAveragePercent() + "\t" + comparedCalls.getnumPassingAndPresent() + "\t" + comparedCalls.getTotalCallsAnalyzed() + "\t" + Array.toStr(comparedCalls.getCnvFiles()));
// }
// for (int i = 0; i < confValues.length; i++) {
// comparedCalls = compare(rootDir, filterCNVs(rootDir, cnvFile, LRR_lookup, compFiles, DEFAULT_LRR_FILTER, confValues[i], DEFAULT_NUM_MARKERS_FILTER));
// for (int j = 0; j < comparedCalls.getInds().length; j++) {
// if (comparedCalls.getIndCNVCalls()[0][j] > 0 && comparedCalls.getIndCNVCalls()[1][j] > 0) {
...
// }
//
// // confWriter.println(confValues[i] + "\t" + comparedCalls.getAveragePercent() + "\t" + comparedCalls.getnumPassingAndPresent() + "\t" + comparedCalls.getTotalCallsAnalyzed() + "\t" + Array.toStr(comparedCalls.getCnvFiles()));
// }
// // for (int j = 0; j < lrrValues.length; j++) {
// // for (int i = probestart; i < probeStop + 1; i++) {
// // comparedCalls = compare(rootDir, filterCNVs(rootDir, cnvFile, LRR_lookup, compFiles, lrrValues[j], DEFAULT_CONF_FILTER, i));
// // lrrProbeWriter.println(lrrValues[j] + "\t" + i + "\t" + comparedCalls.getAveragePercent() + "\t" + comparedCalls.getnumPassingAndPresent() + "\t" + comparedCalls.getTotalCallsAnalyzed() + "\t" + Array.toStr(comparedCalls.getCnvFiles()));
// // }
// // }

// confWriter = new PrintWriter(new FileWriter(rootDir + "confConcordance_" + confstart + "_" + confstop + "LRR" + DEFAULT_LRR_FILTER + "numMarkers" + DEFAULT_NUM_MARKERS_FILTER + ".concord"));
// lrrWriter = new PrintWriter(new FileWriter(rootDir + "lrrConcordance_" + lrrstart + "_" + lrrstop + "numMarkers" + DEFAULT_NUM_MARKERS_FILTER + "Conf" + DEFAULT_CONF_FILTER + ".concord"));

// if (isDefined(inds, allPossibleCombinations, defineCompHash, hash, allPossibleCombinations[i][0], allPossibleCombinations[i][1], j, k)) {
// // System.out.println("hhragdsafdsacomparing " + inds[j] + " and " + inds[k]);
// cnvs = new CNVariant[][] { CNVariant.toCNVariantArray(), CNVariant.toCNVariantArray(hash.get(inds[j]).get(allPossibleCombinations[i][1] + "")) };
// if (hash.get(inds[j]).containsKey(allPossibleCombinations[i][0] + "")) {
// cnvs = new CNVariant[][] { CNVariant.toCNVariantArray(hash.get(inds[j]).get(allPossibleCombinations[i][0] + "")), CNVariant.toCNVariantArray(hash.get(inds[k]).get(allPossibleCombinations[i][1] + "")) };
// }
//
...
// indPercentMatches[j] = Double.NaN;
// }
// // public CompareCalls_dev(int numPassingAndPresent, double averagePercent, int totalCallsAnalyzed, String[] cnvFiles, int[] exactMatches, int[] sigOlapMatches, String[] inds, int[][] indCNVCalls, double[] indPercentMatches) {
//
// }
...
// BufferedReader reader;
// String[] line;
// Hashtable<String, Hashtable<String, ArrayList<CNVariant>>> hash = new Hashtable<String, Hashtable<String, ArrayList<CNVariant>>>();
// Hashtable<String, ArrayList<CNVariant>> source = new Hashtable<String, ArrayList<CNVariant>>();
// try {
...
// private static boolean isDefined(String[] inds, int[][] allPossibleCombinations, Hashtable<String, Hashtable<String, Integer>> defineCompHash, Hashtable<Integer, Hashtable<String, ArrayList<CNVariant>>> hash, int combo1, int combo2, int j, int k) {
...
// }
//
// // return ((defineCompHash.containsKey(inds[j]) && defineCompHash.get(inds[j]).containsKey(inds[k]))||defineCompHash.containsKey(inds[j]) && defineCompHash.get(inds[j]).containsKey(inds[k])) && hash.containsKey(inds[j])&& hash.containsKey(inds[k]) && (hash.get(inds[j]).containsKey(allPossibleCombinations[i][0] + "") ) && hash.get(inds[k]).containsKey(allPossibleCombinations[i][1] + "")||hash.get(inds[j]).containsKey(allPossibleCombinations[i][0] + "") ) && hash.get(inds[k]).containsKey(allPossibleCombinations[i][1] + "");
// }
// for (int i = probestart; i < probeStop + 1; i++) {
// comparedCalls = compare(rootDir, filterAndClean(rootDir, cnvFile, LRR_lookup, compFile, DEFAULT_LRR_FILTER, DEFAULT_CONF_FILTER, i, markerFile, DEFAULT_BP_FRACTION, DEFAULT_NUM_MARKER_FRACTION), DEFAULT_COMP_LIST);
// if (i == probestart) {
...
// // + "\t" + 20 + "\t" + comparedCalls[j].getIndCNVCalls()[0][j] + "\t" + comparedCalls[j].getIndCNVCalls()[1][j] + "\t" + comparedCalls[j].getExactMatches()[j] + "\t" + comparedCalls[j].getSigOlapMatches()[j] + "\t" + comparedCalls[j].getindPercentMatches()[j] + "\t" + comparedCalls[j].getAveragePercent() + "\t" + comparedCalls[j].getnumPassingAndPresent() + "\t" + comparedCalls[j].getTotalCallsAnalyzed() + "\t" + Array.toStr(comparedCalls[j].getCnvFiles()) + "\t");
// }
