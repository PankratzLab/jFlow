...
public class CNVtools {
	...
	public static void create(String map) {
		...
		try {
			...
			while (mapReader.ready()) {
				...
				if ((map.endsWith(".map")&&line.length!=4)||(map.endsWith(".bim")&&line.length!=6)) {
				...
				if (!file.exists()) {
					System.err.println("Error - '"+line[1]+"' was not found in the expected directory (./"+subdir+"/)");
				} else {
					if (firstTime) {
						try {
						...
						for (int i = 0; i<ids.length; i++) {
							writer.print("\t"+ids[i]+".GType"+"\t"+ids[i]+".Log R Ratio"+"\t"+ids[i]+".B Allele Freq");
						}
						writer.println();
						firstTime = false;
						System.out.println("There are "+ids.length+" individuals (remember this in case you need to split up to the process in PennCNV)");
					}
					...
					try {
						...
						for (int i = 0; i<ids.length; i++) {
							...
							if (!line[0].equals(ids[i])) {
								System.err.println("Error - out of sync with first file (expecting '"+ids[i]+"'; found '"+line[0]+"')");
								System.exit(1);
							}
	...
	public static Hashtable<String,String> createMarkerLookup(String dir) {
		...
		for (int i = 0; i<files.length; i++) {
			try {
				...
				while (reader.ready()) {
					hash.put(reader.readLine(), files[i].getName().substring(0, files[i].getName().lastIndexOf(".")));
				}
				reader.close();
			} catch (FileNotFoundException fnfe) {
	...
	public static void main(String[] args) {
		...
		String usage = "\\n"+"cnv.PennCNV requires 0-1 arguments\n"+"   (1) map file (i.e. map="+mapfile+" (default))\n"+"   (2) errors and warnings capture (i.e. err="+warnings+" (default))\n"+"";
