...
public class CNVtools {
    ...
    try {
      ...
      while (mapReader.ready()) {
        ...
        if ((map.endsWith(".map") && line.length != 4)
          ...
          System.err.println(
              "Error - it looks like your map file might not be in plink format, which could pose a problem");
        }
        ...
        if (!file.exists()) {
          ...
          if (firstTime) {
            ...
            for (String id : ids) {
              writer.print(
                  "\t" + id + ".GType" + "\t" + id + ".Log R Ratio" + "\t" + id + ".B Allele Freq");
            }
            writer.println();
            firstTime = false;
  ...
  public static void main(String[] args) {
    ...
    String usage = "\\n" + "cnv.PennCNV requires 0-1 arguments\n" + "   (1) map file (i.e. map="
        + mapfile + " (default))\n" + "   (2) errors and warnings capture (i.e. err=" + warnings
        + " (default))\n" + "";
