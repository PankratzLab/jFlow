...
public class ReferenceMaps {
	...
	public static final String[] MCCAROLL_HEADER = {"CNP_id", "chr", "start_hg17", "end_hg17", "start_hg18", "end_hg18", "CN classes observed"};
	...
	public static void createMcCarollMap() {
		...
		try {
			...
			while (reader.ready()) {
				...
				hash.put(line[0], count+"\t"+line[1]+":"+ext.replaceAllWith(line[4], "\"", "")+"-"+ext.replaceAllWith(line[5], "\"", ""));
				count++;
			}
			reader.close();
		} catch (FileNotFoundException fnfe) {
			System.err.println("Error: file \""+MCCAROLL_DIR+MCCAROLL_MAP+"\" not found in current directory");
			System.exit(1);
		...
		for (int i = 0; i<MCCAROLL_POPULATIONS.length; i++) {
			...
			try {
				reader = new BufferedReader(new FileReader(MCCAROLL_DIR+"pedinfo2sample_"+MCCAROLL_POPULATIONS[i]+".txt"));
				while (reader.ready()) {
				...
				System.err.println("Error: file \""+MCCAROLL_DIR+"pedinfo2sample_"+MCCAROLL_POPULATIONS[i]+".txt"+"\" not found in current directory");
				System.exit(1);
			} catch (IOException ioe) {
				System.err.println("Error reading file \""+MCCAROLL_DIR+"pedinfo2sample_"+MCCAROLL_POPULATIONS[i]+".txt"+"\"");
				System.exit(2);
			}

			try {
				reader = new BufferedReader(new FileReader(MCCAROLL_DIR+"ng.238-S3_"+MCCAROLL_POPULATIONS[i]+".txt"));
				...
				System.err.println("Error: file \""+MCCAROLL_DIR+"ng.238-S3_"+MCCAROLL_POPULATIONS[i]+".txt"+"\" not found in current directory");
				System.exit(1);
			} catch (IOException ioe) {
				System.err.println("Error reading file \""+MCCAROLL_DIR+"ng.238-S3_"+MCCAROLL_POPULATIONS[i]+".txt"+"\"");
				System.exit(2);
			}
		...
		try {
			...
			for (int i = 0; i<MCCAROLL_POPULATIONS.length; i++) {
				for (int j = 0; j<=MAX_COPIES; j++) {
					writer.print("\t"+MCCAROLL_POPULATIONS[i]+"_"+j);
				}
			}
			writer.println();
