...
public class CountHets {
	...
	public static final String[] EXPECTED_HEADER = {"Sample", "FID", "IID", "CHR", "BP1", "BP2", "TYPE", "SCORE", "SITES"};
	public static final String[] CNV_SUFFIXES = {"Name", "Chr", "Position", ".GType", ".Log R Ratio", ".B Allele Freq"};
	...
	public static void count(String dir, String filename) {
		...
		try {
			...
			writer.println(Array.toStr(EXPECTED_HEADER)+"\t#Markers\t%CalledHet\t%0.15<BAF,0.85\t%0.25<BAF,0.75");
				...
				try {
					...
					for (int j = 0; j<CNV_SUFFIXES.length; j++) {
						if (!line[j].endsWith(CNV_SUFFIXES[j])) {
							System.err.println("Error - expecting column "+(j+1)+" to end in "+CNV_SUFFIXES[j]+" (found "+line[j]+")");
							problem = true;
						}
					}
					if (problem) {
					...
					System.err.println("Error: file \""+cnvDirectory+hashKeys.elementAt(i)+"\" not found in current directory");
					System.exit(1);
				} catch (IOException ioe) {
...
class CNVregion {
	...
	public String toString() {
		return sample+"\t"+famID+"\t"+indID+"\t"+chr+"\t"+start+"\t"+stop+"\t"+type+"\t"+countMarkers+"\t"+ext.formDeci((double)countHet/(double)countCalled, 3)+"\t"+ext.formDeci((double)baf15/(double)countValidBAF, 3)+"\t"+ext.formDeci((double)baf25/(double)countValidBAF, 3); // countHet+"\t"+countCalled+"\t"+
	}
}
