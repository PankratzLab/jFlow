...
public class ProjectPropertiesEditor extends JFrame {
	...
	private final InputValidator QQ_PLOT_VALIDATOR = new InputValidator() {
		...
		Object processNewValue(Object newValue, Object oldValue) {
			// general contract of InputValidator is that processNewValue is only called after
			// acceptNewValue has returned true
			return newValue;
		}
		...
		Object processNewValue(Object newValue, Object oldValue) {
			if (newValue instanceof File[]) {
				if (((File[]) newValue).length == 0
						|| (((File[]) newValue).length == 1 && ((File[]) newValue)[0].getPath().equals(""))) {
					return newValue;
				}
			...
			public Component getTableCellRendererComponent(	final JTable table, Object value,
				...
				if (value instanceof Boolean) {
					...
					if (((File) value).isDirectory()
							|| ((FileProperty) proj.getProperty(tempKey)).isDirectory()) {
