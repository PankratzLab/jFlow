...
public class MarkerData implements Serializable {
  ...
  public byte[] compress(int mkrIndLocal, byte nullStatus, Hashtable<String, Float> oorTable,
    ...
    // Xs
    for (int i = 0; i < numInd; i++) {
      boolean oor = canXYBeNegative ? !Compression.xyCompressAllowNegative(getXs()[i], mkrBuff,
                                                                           buffInd)
    ...
    // Ys
    for (int i = 0; i < numInd; i++) {
      boolean oor = canXYBeNegative ? !Compression.xyCompressAllowNegative(getYs()[i], mkrBuff,
                                                                           buffInd)
    ...
    // Genotypes
    for (int i = 0; i < numInd; i++) {
      mkrBuff[buffInd] = Compression.genotypeCompress(getAbGenotypes() == null ? -1
                                                                               : getAbGenotypes()[i],
