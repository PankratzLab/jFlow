...
public class CytoPanel extends JPanel implements ActionListener {
  ...
  private static void convertCNPfiles(Project proj, String cnvdir, Logger log) {
    ...
    // true) + CytoCNVariant.CNV_EXT, log);
    convertCNPfiles(proj, proj.COMMON_CNP_FILENAME.getValue(),
                    cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.COMMON_CNP_FILENAME.getValue()),
                                                                true)
                                                               + CytoCNVariant.CNV_EXT,
                    log);
    convertCNPfiles(proj, proj.REPORTED_CNP_FILENAME.getValue(),
                    cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.REPORTED_CNP_FILENAME.getValue()),
                                                                true) + CytoCNVariant.CNV_EXT,
                    log);
    convertCNPfiles(proj, proj.UNREPORTED_CNP_FILENAME.getValue(),
                    cnvdir + ext.replaceWithLinuxSafeCharacters(ext.rootOf(proj.UNREPORTED_CNP_FILENAME.getValue()),
                                                                true) + CytoCNVariant.CNV_EXT,
                    log);
  }
        ...
        try {
          ...
          for (Segment seg : segs) {
            writer.println(new CNVariant(ext.rootOf(output), seg.getUCSClocation(), seg.getChr(),
                                         seg.getStart(), seg.getStop(), 2, 10f, 10, 0)
                                                                                      .toPlinkFormat());
          }
          writer.close();
          converted = true;
  ...
  private static boolean verifySampleDataIssues(String[] issues, String sampleDataFile) {
    ...
    if (issues != null && issues.length > 0) {
      ...
      String dialoge = "Warning - the following samples were already present in the sample data file \""
                       + sampleDataFile + "\".\n";
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (source.equals(selectButton)) {
        ...
        response = JOptionPane.showOptionDialog(null,
                                                "Remove " + jFileNameTextField.getText() + "?",
