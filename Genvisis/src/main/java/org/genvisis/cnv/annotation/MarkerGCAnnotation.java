...
public class MarkerGCAnnotation extends LocusAnnotation implements AnnotationParser {
  ...
  public static AnnotationData getGCAnnotationDatas() {
    return new AnnotationData(VCFHeaderLineType.Float, null, 1,
        ...
        "The gc content of the marker, not including the interrogation position", DEFAULT_VALUE,
        DEFAULT_VALUE);
  }

  public static MarkerGCAnnotation[] initForMarkers(Project proj, String[] markers,
      MarkerSet markerSet, Hashtable<String, Integer> indices) {
    if (markerSet == null) {
    ...
    for (int i = 0; i < markerGCAnnotations.length; i++) {
      ...
      markerGCAnnotations[i] =
          new MarkerGCAnnotation(builder, markers[i], new Segment(markerSet.getChrs()[index],
              markerSet.getPositions()[index], markerSet.getPositions()[index]));
    }
