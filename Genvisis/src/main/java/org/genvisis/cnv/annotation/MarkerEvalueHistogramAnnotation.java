...
public class MarkerEvalueHistogramAnnotation extends HistogramAnnotation {
	...
	public void setDataToExactHistogram() {
		...
		for (int i = 0; i < indices.length; i++) {
			data.add(tmp[indices[i]] + "");
			data.add(exactHistogram.get(tmp[indices[i]] + "") + "");
		}
	...
	/**
	 * Not really a histogram since the values are not binned, but thats cuz I didn't know the best way to bin
	 *
	 */
	...
	public EvalueHistogram formatHistogram() {
		if (blastEvalueCounts == null || blastEvalues == null || blastEvalueCounts.size() != blastEvalues.size()) {
