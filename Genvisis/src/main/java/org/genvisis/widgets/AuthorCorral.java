...
public class AuthorCorral {
	...
	public static void run(	String inFile, String outFile, boolean rtfOutput, boolean printErr,
		...
		while ((temp = inReader.readLine()) != null) {
			...
			if (line.length < 3) {
				...
				// If middle name contains an exclamation point (!), then report as is. If full middle name
				// and no exclamation point then convert to initials. Adds periods if requested.
				String middleName;
				boolean warn = false;
	...
	public static void main(String[] args) {
		...
		String usage = "\n"	+ "one.AuthorCorral requires 2+ arguments\n"
										...
										+ "   (3) OPTIONAL output in RTF format (used for superscripts) (i.e. rtf="
										+ rtf + " (default))\n" + "   (4) OPTIONAL include errors in output (i.e. err="
										+ err + " (default))\n"
		...
		for (String arg : args) {
			if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
				...
				err = Boolean.valueOf(arg.split("=")[1]);
				numArgs--;
			} else if (arg.startsWith("periods=")) {
				periodsAfterInitials = Boolean.valueOf(arg.split("=")[1]);
				numArgs--;
			} else {
