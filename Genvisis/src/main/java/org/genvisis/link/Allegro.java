...
public class Allegro {
	public static void makeOptFile(int chr, String model, boolean unique) throws IOException {
		...
		PrintWriter writer = new PrintWriter(new FileWriter("useful"+(unique?chrome:"")+".opt"));
		...
		if (model.equals("NPL")) {
			writer.println("MODEL mpt lin all equal chrom"+chrome+".lin.out chromf"+chrome+".lin.out");
			writer.println("MODEL mpt exp all equal chrom"+chrome+".exp.out chromf"+chrome+".exp.out");
		} else {
	...
	public static void main(String[] args) {
		...
		String usage = "\\n"+"link.Allegro requires 0-1 arguments\n"+"   (1) chromosome (i.e. chr="+chr+" (default))\n"+"   (2) model (i.e. model="+model+" (default) options include: NPL, DOM, REC)\n"+"   (3) unique opt file (i.e. -unique ("+(unique?"":"not the ")+"default))\n"+"";
