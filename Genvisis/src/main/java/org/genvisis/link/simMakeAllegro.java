...
public class simMakeAllegro {
	...
	public simMakeAllegro(double het, int multiplier) throws IOException {
		...
		for (int i = 0; i<FILES.length; i++) {
			...
			optfile.println("% Read input in LINKAGE style format:\n"+"PREFILE "+file+".pre\n"+"DATFILE linkage.dat\n\n"+"% Simulate stroke reconstruction pedigrees\n"+"SIMULATE dloc:32.0 npre:"+NUM_REPS+" rep:"+FAMREPS[i]+" err:0.00 yield:1.0 het:"+ext.formDeci(het, 2, true)+"\n\n" // change
																																																																							// from
																																																																							// 8 to
			// 32 for denser map
			        // wtf? 32 is
	...
	public static void main(String[] args) throws IOException {
		if (args.length!=2) {
			...
			try {
				new simMakeAllegro(Double.valueOf(args[0]).doubleValue(), Integer.valueOf(args[1]).intValue());
				// stdin.readLine();
			} catch (Exception e) {
				e.printStackTrace();
			}
