...
public class Markers {
	...
	public static int[] order(String[] markers, String databaseFile, boolean shouldBeOnSameChromosome) {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (line[0].equals("")) {} else if (line[0].startsWith("chromosome")) {
			...
			if (shouldBeOnSameChromosome) {
				...
				for (int i = 0; i<keys.length; i++) {
					if (distances[keys[i]]==-1) {
						System.err.println("Error - marker '"+markers[i]+"' could not be found in the database");
					}
					if (chrs[keys[i]]!=chr) {
						System.err.println("Error - all markers were not on the same chromosome ('"+markers[keys[i]]+"' for instance)");
					}
					if (i>0) {
						diff = distances[keys[i]]-distances[keys[i-1]];
						writer.print(myFormatter.format(diff)+" ");
						output += "\n     "+myFormatter.format(diff)+"\n"+markers[keys[i]]+"    "+distances[keys[i]];
					}
				}
				writer.println();
