...
public class meld {
	public meld() throws IOException {
		...
		for (int chr = 1; chr<=23; chr++) {
			...
			try {
				...
				if (line1.length!=line2.length) {
					if (line1.length>line2.length) {
						System.err.println("Error - 1/chromosome"+chr+".dat has "+(line1.length-line2.length)+" more columns (markers?) than 2/chromosome"+chr+".dat");
					} else {
						System.err.println("Error - 2/chromosome"+chr+".dat has "+(line2.length-line1.length)+" more columns (markers?) than 1/chromosome"+chr+".dat");
					}
					System.exit(1);
				}
							...
							for (int k = 3; k<line1.length; k++) {
								if (line1[k].equals(line2[k])) {
									...
									if (offs.elementAt((k-3)/2).containsKey(off+"")) {
										offCount = Integer.valueOf(offs.elementAt((k-3)/2).get(off+"")).intValue()+1;
									} else {
										offCount = 1;
									}
				...
				for (int j = 0; j<key.length; j++) {
					writer.println(hash.get(individuals.elementAt(key[j])));
				}
				...
				for (int j = 0; j<store.length; j++) {
					if (store[j]<10) {
						...
						temp = (int)(100*(double)maxOffCount/(double)totalOffCount)+"% shifted "+(maxOff>0?"+"+maxOff:""+maxOff);
					}
		...
		for (int j = 0; j<key.length; j++) {
			if (differs.containsKey(individuals.elementAt(key[j]))) {
				mismatchGenotypes.println(individuals.elementAt(key[j])+"\t"+differs.get(individuals.elementAt(key[j])));
			}
		}

		System.err.println("Overall error rate was "+diffs+"/"+(sames+diffs)+" ("+((double)diffs/(double)(sames+diffs)*100)+"%)");
