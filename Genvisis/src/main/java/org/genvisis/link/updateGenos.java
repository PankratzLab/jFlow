	...
	public updateGenos(String filename) throws IOException {
		...
		while (reader.ready()) {
			...
			if (!allMarkers.containsKey(line[2])) {
				System.err.println("Error - cannot find "+line[2]+" in the marshfield database. Check spelling and/or the order of the file. Expecting:");
				System.err.println("FamID IndID marker_name old_genotype1 old_genotype2 new_genotype1 new_genotype2");
				System.exit(2);
			}
					...
					if (people.containsKey(line[1]+"\t"+line[2])) {
						...
						for (int j = 0; j<changes.size(); j++) {
							...
							if (!markerNames.contains(alteration[0])) {
								System.err.println("Error - marker "+alteration[0]+" was not found in chromosome"+chr+".dat");
								System.err.println("        Skipping alteration");
							} else {
								if (line[3+markerNames.indexOf(alteration[0])*2].equals(alteration[1])&&line[3+markerNames.indexOf(alteration[0])*2+1].equals(alteration[2])) {
									System.out.println("Congrats you found the right genotype ("+line[3+markerNames.indexOf(alteration[0])*2]+" & "+line[3+markerNames.indexOf(alteration[0])*2+1]+")");
									line[3+markerNames.indexOf(alteration[0])*2] = alteration[3];
									line[3+markerNames.indexOf(alteration[0])*2+1] = alteration[4];
									madeChange = true;
								} else if (line[3+markerNames.indexOf(alteration[0])*2].equals(alteration[2])&&line[3+markerNames.indexOf(alteration[0])*2+1].equals(alteration[1])) {
									System.out.println("Congrats you found the right genotype ("+line[3+markerNames.indexOf(alteration[0])*2]+" & "+line[3+markerNames.indexOf(alteration[0])*2+1]+") (albeit reversed)");
									line[3+markerNames.indexOf(alteration[0])*2] = alteration[3];
									line[3+markerNames.indexOf(alteration[0])*2+1] = alteration[4];
									madeChange = true;
								} else if (line[3+markerNames.indexOf(alteration[0])*2].equals(alteration[3])&&line[3+markerNames.indexOf(alteration[0])*2+1].equals(alteration[4])) {
									System.out.println("Genotype "+alteration[1]+"/"+alteration[2]+" has already been changed to "+alteration[3]+"/"+alteration[4]+" for "+line[1]+"-"+line[2]+" ("+alteration[0]+")");
								} else {
									System.err.println("Error - Wrong genotypes specified for "+line[1]+"-"+line[2]+" ("+alteration[0]+")");
									System.err.println("        Supposed to change "+alteration[1]+"/"+alteration[2]+" to "+alteration[3]+"/"+alteration[4]+" but found "+line[3+markerNames.indexOf(alteration[0])*2]+"/"+line[3+markerNames.indexOf(alteration[0])*2+1]);
								}
							}
						}
						writer.print(line[0]);
