...
public class sumOSA {
	...
	public sumOSA() throws IOException {
		...
		for (int i = 0; i<SUGGESTIONS.length; i++) {
			if (new File(SUGGESTIONS[i]).exists()) {
				runs.add(SUGGESTIONS[i]);
			}
		}
	...
	public sumOSA(Vector<String> runDirs) throws IOException {
		...
		for (int run = 0; run<runDirs.size(); run++) {
			...
			temp += "\t"+(run==0?"Direction":"")+"\tPosition\tmaxLOD\tbaseLOD\tIncrease\tSignificance\tFamsUsed\tTotalFams\tProportion";
		}
		writer.println();
		writer.println(temp);
		for (int i = 0; i<23; i++) {
			...
			for (int k = 0; k<2; k++) {
				...
				for (int run = 0; run<runs.size(); run++) {
					...
					if (osac[k][i]!=null) {
						writer.print("\t"+osac[k][i].maxPos+"\t"+ext.formDeci(osac[k][i].maxLOD, 2, true)+"\t"+ext.formDeci(osac[k][i].baseLOD, 2, true)+"\t"+ext.formDeci(osac[k][i].maxLOD-osac[k][i].baseLOD, 2, true)+"\t"+osac[k][i].pval+"\t"+osac[k][i].maxFams+"\t"+osac[k][i].totalFams+"\t"+ext.formDeci(((double)osac[k][i].maxFams/(double)osac[k][i].totalFams), 3, true)+"\t");
					} else {
						writer.print("\t\t\t\t\t\t\t\t\t");
					}
				}
	...
	public osaChrom[][] parseMapmaker(String base, PrintWriter batch, String plug) throws IOException {
		...
		for (int i = 0; i<2; i++) {
			...
			for (int j = 0; j<23; j++) {
				...
				osac[i][j] = new osaChrom(Integer.valueOf(cutLookup.elementAt(cutLookup.size()-1)).intValue());
				...
				if (new File(base+"chrom"+chrome+"/sims/perm"+chrome+".out").exists()) {
					reader = new BufferedReader(new FileReader(base+"chrom"+chrome+"/sims/perm"+chrome+".out"));
					while (reader.ready()) {
						line = reader.readLine().split("[\\s]+");
					...
					if (count>=20) {
						...
						osac[i][j].pval = "<"+ext.formDeci(1.0/(double)total, 4, true);
		...
		for (int j = 0; j<23; j++) {
			chr = j+1;
			chrome = (chr<10)?"0"+chr:""+chr;
			reps = (osac[0][j]!=null?osac[0][j].repsRequested:0)+(osac[1][j]!=null?osac[1][j].repsRequested:0);
			if (reps>0) {
				if (reps>200) {
					reps = 100;
				}
