...
public class createAspex {
	...
	public createAspex(int chromosome) throws IOException {
		...
		try {
			...
			try {
				map = new BufferedReader(new FileReader("/home/npankrat/park/00masters/map"+chrome+".dat"));
		...
		try {
			...
			try {
				genotype = new BufferedReader(new FileReader("/home/npankrat/park/00masters/mrkr"+chrome+".dat"));
				System.err.println("Could not find mrkr"+chrome+".dat in the current directory");
				System.err.println("  using the one in /home/npankrat/park/00masters/");
			} catch (Exception e2) {
				System.err.println("Could not find mrkr"+chrome+".dat in /home/npankrat/park/00masters/ or in the current directory");
				peachy = false;
			}
		...
		while (struct.ready()) {
			...
			writer.print(id+"\t"+st.nextToken()+"\t"+st.nextToken()+"\t"+st.nextToken()+"\t"+st.nextToken());
