...
public class gatherSimwalk2 {
		...
		for (int chromosome = 1; chromosome<=23; chromosome++) {
			...
			for (int i = start; i<=stop; i++) {
				try {
					...
					// max_pos = -1;
					if (chromosome<23) {
						reader = new BufferedReader(new FileReader("chr"+chromosome+"/Model"+i+"/SCORE-"+chrome+".ALL"));
						do {
					...
					System.err.println("Missed: chr"+chromosome+"/Model"+i+"/SCORE-"+chrome+".ALL");
				}
			}
			peaks.println();
			for (int i = start; i<=stop; i++) {
	...
	public static void main(String[] args) throws IOException {
		try {
			if (args.length==1) {
				new gatherSimwalk2(Integer.valueOf(args[0]).intValue(), Integer.valueOf(args[0]).intValue());
			} else if (args.length==2) {
				new gatherSimwalk2(Integer.valueOf(args[0]).intValue(), Integer.valueOf(args[1]).intValue());
			} else {
