...
public class Filesystem {
	...
	public static void create(int chr, boolean reportOutliers) throws IOException {
		...
		try {
			...
			System.err.println("chromosome"+chr+".dat was not found, failed to make mrkr"+chrome+".dat");
			System.exit(1);
		}
		...
		for (int i = 0; i<numMarkers; i++) {
			...
			if (key[i]==-1) {
				System.err.println("markers in data file are fubar at marker # "+i+" couldn't find "+orderedMarkers.elementAt(i));
			}
		}
			...
			for (int i = 0; i<numMarkers; i++) {
				...
				if (first.equals("0")||second.equals("0")) {
					if (!first.equals("0")) {
						...
						writer.print("   0   0");
						if (first.equals("0")&&second.equals("0")) {} else {
							error.println(IDed+" has one allele missing ("+ext.formStr(first, 4)+ext.formStr(second, 4)+") for marker number "+i+" on chromosome "+chr);
						}
					}
				} else {
					writer.print(ext.formStr(first, 4)+ext.formStr(second, 4));
		...
		for (int i = 0; i<numMarkers; i++) {
			...
			for (int j = numMissing; j<quickkey.length; j++) {
				alleleFreqs[i][j-numMissing] = (double)Integer.valueOf(handle.get(alleles.elementAt(quickkey[j]))).intValue()/(double)total;
				for (int k = 0; k<Integer.valueOf(handle.get(alleles.elementAt(quickkey[j]))).intValue(); k++) {
					allAlleles[alleleCount++] = Integer.valueOf(alleles.elementAt(quickkey[j])).intValue();
				}

			}
			if (alleleCount!=total) {
				System.err.println("Error - for some reason the number of genotypes ("+alleleCount+") did not reach the expected ("+total+")");
			}
			...
			while (gen.ready()) {
				...
				for (int i = 0; i<problemAlleles.length; i++) {
					for (int j = 0; j<problemAlleles[i].length; j++) {
						if (line[i*2+2].equals(problemAlleles[i][j]+"")||line[i*2+2+1].equals(problemAlleles[i][j]+"")) {
							if (!changes.containsKey(line[0]+"\t"+line[1]+"\t"+orderedMarkers.elementAt(i))) {
								writer.println(line[0]+"\t"+line[1]+"\t"+orderedMarkers.elementAt(i)+"\t"+line[i*2+2]+"\t"+line[i*2+2+1]+"\t\t");
	...
	public static int[] spreadCheck(int[] source, String header) {
		...
		for (int i = 0; i<missedOpps.length; i++) {
			System.err.print(missedOpps[i]+" "+(missedCounts.elementAt(i)==1?"":"("+missedCounts.elementAt(i)+"x) "));
		}
	...
	public int[] boxPlotOutliers(int[] source, String header) {
		...
		for (int i = 0; i<source.length; i++) {
			if (source[i]<lf||source[i]>uf) {
				System.err.println(source[i]+" is out of the expected range of "+lf+" - "+uf+" for "+header);
				missedOpportunities.add(source[i]+"");
			}
		}
