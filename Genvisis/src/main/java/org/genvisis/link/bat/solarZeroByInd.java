...
public class solarZeroByInd {
	...
	public solarZeroByInd(String fams, int start, int stop) throws IOException {
		...
		for (int chromosome = start; chromosome<=stop; chromosome++) {
			String bakFilename = Files.getBakFilename("solar.gtypes."+chromosome, super.getClass().getName());
	...
	public static void main(String[] args) throws IOException {
		if (args.length<1||args.length>2) {
			...
			try {
				...
				if (args.length==2) {
					new solarZeroByInd(args[0], Integer.valueOf(args[1]).intValue(), Integer.valueOf(args[1]).intValue());
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
