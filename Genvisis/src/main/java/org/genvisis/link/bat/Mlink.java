...
public class Mlink {
  ...
  public static void parseStreams(String[] markerNames, double[] cumulative_cM_positions, int chr,
    ...
    try {
      for (int i = 0; i < numFiles; i++) {
        reader = new BufferedReader(new FileReader(dir + "stream" + chrome + "-" + (i + 1)
                                                   + ".out"));
  ...
  public static int breakApart(int chr, String destination, boolean pedformat) {
    ...
    try {
      ...
      writers[numFiles - 1].println(ArrayUtils.toStr(ArrayUtils.stringArraySequence(numMarkers
                                                                                    - (numFiles - 1)
                                                                                      * MAX_MARKERS
                                                                                    + 1, ""),
                                                     " "));
      ...
      if (pedformat) {
        pedStruct2 = procPedStruct(new File("re_chrom" + chrome
                                            + ".ped").exists() ? "re_chrom" + chrome + ".ped"
                                                               : searchForPedStruct(),
                                   true);
      } else {
  ...
  public static void main(String[] args) {
    ...
    for (String arg : args) {
      if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
        ...
        pedformat = arg.split("=")[1].equalsIgnoreCase("true");
        numArgs--;
      } else {
        if (chr == -1) {
          try {
            chr = Integer.parseInt(arg);
          } catch (NumberFormatException nfe) {
            System.err.println("Error - invalid chromosome number: " + arg);
            System.exit(1);
          }
          numArgs--;
        } else {
