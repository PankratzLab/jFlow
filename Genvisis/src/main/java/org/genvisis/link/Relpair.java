...
public class Relpair {
	...
	public static final String[][] FILTER_PAIRS = { {"UN", "CO"}, {"UN", "AV"}, {"CO", "UN"}, {"CO", "AV"}};
	...
	public static void createFiles(String dir) {
		...
		for (int i = 0; i<chrs.length; i++) {
			try {
				readers[i] = new BufferedReader(new FileReader(dir+"re_chrom"+ext.chrome(chrs[i])+".pre"));
		...
		try {
			...
			readers[0].close();
			readers[0] = new BufferedReader(new FileReader(dir+"re_chrom"+ext.chrome(chrs[0])+".pre"));
		} catch (IOException ioe) {
			System.err.println("Error parsing first file: "+dir+"re_chrom"+ext.chrome(chrs[0])+".pre");
			ioe.printStackTrace();
		}
		fams = cv.getValues();
			...
			for (int chrI = 0; chrI<chrs.length; chrI++) {
				try {
					...
					for (int i = 0; i<markerNames.length; i++) {
						writer.println(ext.formStr((markerNames[i].length()<10?markerNames[i]:markerNames[i].substring(0, 8)+"-"), 10, true)+((chrs[chrI]<23)?"AUTOSOME":"X-LINKED")+" "+ext.formStr(alleleFreqs[i].length+"", 2, true)+ext.formStr(chrs[chrI]+"", 4, false)+"    "+ext.formDeci(cumulativePositions[i], 3, true));
						for (int j = 0; j<alleleFreqs[i].length; j++) {
							writer.println(ext.formStr((j+1)+"", 8, true)+ext.formStr(ext.formDeci(alleleFreqs[i][j], 6, true), 8, true));
						}
					}
		...
		try {
			...
			writer.println("(3A"+(maxindIDsize+1)+",2A1,A"+(MAX_NUM_DIGITS_IN_ALLELE*2+1)+","+Array.sum(numMarkers)+"(1X,A"+(MAX_NUM_DIGITS_IN_ALLELE*2+1)+"))");
						...
						try {
							...
							if (line.length>1) {
								if (i==0) {
									...
									writer.print(ext.formStr(line[2], maxindIDsize+1, true)+ext.formStr(line[3], maxindIDsize+1, true));
									writer.print((line[4].equals("1"))?"M":"F");
								}

								for (int j = 0; j<numMarkers[i]; j++) {
									if (line.length < 6+j*2+2) {
										System.err.println("Error - what's up with chromosome "+chrs[i]+" for "+line[0]+","+line[1]);
									}
									if (line[6+j*2+0].equals("0")||line[6+j*2+1].equals("0")) {
										writer.print("      ");
									} else {
										writer.print(" "+ext.formStr(line[6+j*2+0]+"/"+line[6+j*2+1], MAX_NUM_DIGITS_IN_ALLELE*2+1, true));
									}
								}
							}
						} catch (Exception ex2) {
							System.err.println("Error in processing the re_chrom"+ext.chrome(chrs[i])+".pre files");
							ex2.printStackTrace();
						}
					}
					writer.println();
	...
	public static void parseFiles(String dir, String genome, boolean filter) {
		...
		try {
			...
			do {
				...
				if (line.length>1&&(!filter||filterPairs(line[3], line[4]))) {
					...
					if (genome==null) {
						...
						within.put(line[0]+"\t"+line[1]+"\t"+line[2], line[3]+"\t"+line[4]+"\t"+line[5]);
					}
				}
			} while (line.length>1);
			...
			while (reader.ready()) {
				...
				if (!line[0].equals("NONE.") && (!filter||filterPairs(line[4], line[5]))) {
					...
					if (genome==null) {
						...
						between.put(line[0]+"\t"+line[1]+"\t"+line[2]+"\t"+line[3], line[4]+"\t"+line[5]+"\t"+line[6]);
					}
				}
			}
			if (genome==null) {
			...
			System.err.println("Error: file \""+dir+"relpair.out"+"\" not found in current directory");
			System.exit(1);
		} catch (IOException ioe) {
		...
		if (genome!=null) {
			...
			try {
				...
				while (reader.ready()) {
					...
					if (within.containsKey(line[0]+"\t"+line[1]+"\t"+line[3])) {
						withinWriter.println(line[0]+"\t"+line[1]+"\t"+line[3]+"\t"+line[5]+"\t"+line[6]+"\t"+line[7]+"\t"+line[8]+"\t"+line[9]+"\t"+within.remove(line[0]+"\t"+line[1]+"\t"+line[3]));
						withinWriter.flush();
					}

					if (between.containsKey(line[0]+"\t"+line[1]+"\t"+line[2]+"\t"+line[3])) {
						betweenWriter.println(line[0]+"\t"+line[1]+"\t"+line[2]+"\t"+line[3]+"\t"+line[6]+"\t"+line[7]+"\t"+line[8]+"\t"+line[9]+"\t"+between.remove(line[0]+"\t"+line[1]+"\t"+line[2]+"\t"+line[3]));
						betweenWriter.flush();
					}
				}
				reader.close();
			...
			for (int i = 0; i<keys.length; i++) {
				withinWriter.println(keys[i]+"\t"+Array.toStr(Array.stringArray(5, "."))+"\t"+within.get(keys[i]));
			}
			keys = HashVec.getKeys(between);
			for (int i = 0; i<keys.length; i++) {
				betweenWriter.println(keys[i]+"\t"+Array.toStr(Array.stringArray(4, "."))+"\t"+within.get(keys[i]));
			}
	...
	public static boolean filterPairs(String first, String second) {
		for (int i = 0; i<FILTER_PAIRS.length; i++) {
			if (FILTER_PAIRS[i][0].equalsIgnoreCase(first)&&FILTER_PAIRS[i][1].equalsIgnoreCase(second)) {
				return false;
			}
		}
		return true;
...
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\LOAD\\Family structure issues\\relpair\\";
//		String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\LOAD\\";
