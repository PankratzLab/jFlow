...
public class extractMarkers {
	...
	public extractMarkers(String filename) {
		...
		for (int i = 0; i<chrs.length; i++) {
			chrome = ext.chrome(Integer.parseInt(chrs[i]));
			markerNames = new LinkageMap("map"+chrome+".dat").getMarkerNames();
			markersPicked = Array.toStringArray(hash.get(chrs[i]));
			indices = ext.indexFactors(markersPicked, markerNames, false, false);

			if (Array.min(indices)<0) {
				System.err.println("Error - the following markers were not found in "+"map"+chrome+".dat");
			...
			try {
				...
				writer.println((markersPicked.length+1)+temp.substring(temp.split("[\\s]+")[0].length()));
				...
				for (int j = 0; j<line.length-3; j++) {
					...
					if (line[j].indexOf(".")>=0&&line[j].substring(line[j].indexOf(".")+1).length()>sigfigs) {
				...
				writer.println("  "+line[line.length-3]+" "+line[line.length-2]+" "+line[line.length-1]);
				writer.println(reader.readLine());
			...
			try {
				...
				while (reader.ready()) {
					...
					for (int j = 0; j<markerNames.length; j++) {
						if (ext.indexOfInt(j, indices)!=-1) {
							...
							count = Integer.parseInt(line[6+2*j+0])+Integer.parseInt(line[6+2*j+1])-2;
							if (count>=0) {
				...
				for (int j = 0; j<indices.length; j++) {
					writer.println(markerNames[indices[j]]+"\t"+ext.formDeci(AlleleFreq.computeHeterozygosity(genotypeCounts[j]), 3));
			...
			Cyrillic.format(dir+"re_chrom"+chrome+".pre", dir+"map"+chrome+".dat", dir+"cyrill"+chrome+".pre", dir+"cyrill"+chrome+".dat", Integer.parseInt(chrs[i]));
		}
