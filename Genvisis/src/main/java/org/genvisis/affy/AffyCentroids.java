...
public class AffyCentroids implements Serializable {
  ...
  private static float[][] getCentroid(double missingnessThreshold, double[] meanThetas,
      double[] meanRs, int[] counts, PrintWriter writer, String markerName, Logger log) {
    float[][] centroid;
  ...
  public static void parseCentroids(Project proj, boolean[] samplesToBeUsed,
    ...
    CentroidCompute.computeAndDumpCentroids(proj,
        new String[] {proj.CUSTOM_CENTROIDS_FILENAME.getValue()}, new CentroidBuilder[] {builder},
        2, 2);
  }
  ...
  public static void parseCentroidsFilteredSamples(Project proj, double missingnessThreshold,
    ...
    if (!proj.getSampleData(1, false).hasExcludedIndividuals()) {
      log.reportError(
          "Error - cannot exclude individuals for centroid computations , no factor named 'Exclude/CLASS=Exclude' in Sample Data");
      System.exit(1);
    } else {
  ...
  private static void processCentroid(float[][] centroid, double[] meanThetas, double[] meanRs,
      int[] counts, PrintWriter writer, String markerName, Logger log) {
    if (markerName.startsWith("CN_")) {
      ...
      centroid[0] = new float[] {(float) (meanThetas[3] - 0.3),
          getAltRs(centroid, meanRs, counts, 2, 3, 4, log)};
      centroid[1] = new float[] {(float) meanThetas[3], (float) meanRs[3]};
      centroid[2] = new float[] {(float) meanThetas[4], (float) meanRs[4]};
      ...
      if (counts[2] > 0 && counts[4] > 0) {
        centroid[1] = new float[] {(float) ((meanThetas[2] + meanThetas[4]) / 2),
            getAltRs(centroid, meanRs, counts, 3, 2, 4, log)};
      } else {
      ...
      centroid[0] = new float[] {(float) meanThetas[2], (float) meanRs[2]};
      centroid[1] = new float[] {(float) meanThetas[3], (float) meanRs[3]};
      centroid[2] = new float[] {(float) (meanThetas[3] + 0.3),
          getAltRs(centroid, meanRs, counts, 4, 3, 2, log)};
    } else {
      assignRegularCentroid(centroid, meanThetas, meanRs, counts, log);
    }
    if (centroid[2] != null
        && (centroid[0][0] > centroid[1][0] || centroid[2][0] < centroid[1][0])) {
      nullify(centroid);
    }
  }
