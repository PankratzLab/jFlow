...
public class Positions {
  ...
  public static final int[] CHROMOSOME_LENGTHS_B36_HG18 = {-1, 247300000, 243000000, 199600000,
      191300000, 180900000, 170900000, 158900000, 146300000, 140300000, 135400000, 134500000,
      132400000, 114200000, 106400000, 100400000, 88900000, 78800000, 76200000, 63900000, 62500000,
      47000000, 49700000, 155000000, 57800000, 155000000, 20000, -1};
  public static final int[] CHROMOSOME_LENGTHS_B37_HG19 = {-1, 249300000, 243200000, 198100000,
      191200000, 181000000, 171200000, 159200000, 146400000, 141300000, 135600000, 135100000,
      133900000, 115200000, 107400000, 102600000, 90400000, 81200000, 78100000, 59200000, 63100000,
      48200000, 51400000, 155300000, 59400000, 155300000, 20000, -1};
  public static final int[] CHROMOSOME_LENGTHS_B38_HG38 = {-1, 249000000, 242200000, 198300000,
      190300000, 181600000, 170900000, 159400000, 145200000, 138400000, 133800000, 135100000,
      133300000, 114400000, 107100000, 102000000, 90400000, 83300000, 80400000, 58700000, 64500000,
      46800000, 50900000, 156100000, 57300000, 156100000, 20000, -1};
  public static final int[] CHROMOSOME_LENGTHS_MAX = {-1, 249300000, 243200000, 199600000,
      191300000, 181600000, 171200000, 159400000, 146400000, 141300000, 135600000, 135100000,
      133900000, 115200000, 107400000, 102600000, 90400000, 83300000, 80400000, 63900000, 64500000,
      48200000, 51400000, 156100000, 59400000, 156100000, 20000, -1};
  ...
  public static byte chromosomeNumber(String chromosome, boolean verbose, Logger log) {
    ...
    if (chromosome.equals("XY") || chromosome.equals("PAR")) {
      ...
      try {
        ...
        if (chr < 0 || chr > 26) {
          log.reportError(
              "Error - chromosome number '" + chromosome + "' out of range for homo sapiens", true,
              true, 10);
        }
  ...
  public static Segment[] computeCentromereMidpoints(int[][] centromereBoundaries) {
    ...
    for (int i = 0; i < midpoints.length; i++) {
      midpoints[i] = new Segment("chr" + i + ":"
          + (int) Math.floor((centromereBoundaries[i][1] - centromereBoundaries[i][0]) / 2.0
  ...
  public static int[][] determineCentromereBoundariesFromMarkerSet(byte[] chrs, int[] positions,
    ...
    if (build == 36) {
      ...
      log.reportError(
          "Error - build needs to be either 36 or 37 at this point in time, not '" + build + "'");
      return null;
    }
    ...
    if (markerSetFilename == null || !Files.exists(markerSetFilename)) {
      log.reportError("Warning - since "
          + (markerSetFilename == null ? "markerSetFilename was set to null"
              : " the file '" + markerSetFilename + "' could not be found")
          + "; then the default centromere boundaries for build " + build + " will be used");
    } else {
  ...
  public static String getUCSCformat(int[] pos) {
    ...
    return "chr" + getChromosomeUCSC(pos[0], false)
        + (pos.length > 1 ? ":" + pos[1] + (pos.length == 3 ? "-" + pos[2] : "") : "");
  }
