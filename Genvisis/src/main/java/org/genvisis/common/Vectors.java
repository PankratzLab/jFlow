...
public final class Vectors {
	...
	/**
	 * Convert an {@link IntVector} to an {@code int[]}, using the values of the
	 * {@code order} array as indices into the vector.
	 * <p>
	 * FIXME: this will need to be rewritten for every primitive type. Instead
	 * of using primitive arrays directly, we should use array classes directly
	 * backed by primitive arrays.
	 * </p>
	 *
	 * @param v
	 *            Base vector to convert
	 * @param order
	 *            Each element {@code i} of the output array will have the value
	 *            {@code Vector.get(order[i])}.
	 * @return An ordered array, or {@code null} if the order array is not the
	 *         same size as the base vector.
	 */
	...
	private static boolean checkLength(Vector<?> v, int[] order) {
		if (v.size() != order.length) {
			Logger log = new Logger();
			log.reportError("Error - order array does not have the same number of elements ("+order.length+") as the target ("+v.size()+")");
			return false;
		}
		return true;
	}
