...
public class GEO_expression {
			...
			for (int i = 0; i<files.length; i++) {
				try {
		            ...
		            if (i==0) {
		            	...
		            	System.out.println("Successfully identified "+count+" of "+(data.length-1)+" transcripts");
		            }
		            reader.close();
	...
	public static void checkFileContents(String dir) {
        ...
        try {
	        ...
	        for (int i = 0; i<files.length; i++) {
	        	Zip.unzipFile(dir+files[i], dir);
	            contents = Files.list(dir, ".txt", false);
	            writer.print(files[i]);
	        	for (int j = 0; j<contents.length; j++) {
	        		try {
	                    reader = new BufferedReader(new FileReader(dir+contents[j]));
	                    line = reader.readLine().trim().split("[\\s]+");
	                    for (int k = 0; k<line.length; k++) {
	                    	if (line[k].startsWith("NA")) {
	    		        		writer.print("\t"+line[k]);
	                    	}
	                    }
	                    reader.close();
                    } catch (FileNotFoundException fnfe) {
	                    System.err.println("Error: file \""+dir+contents[j]+"\" not found in current directory");
	                    System.exit(1);
                    } catch (IOException ioe) {
	                    System.err.println("Error reading file \""+dir+contents[j]+"\"");
	                    System.exit(2);
                    }
	        		new File(dir+contents[j]).renameTo(new File(dest+contents[j]));
...
//	    String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\UMN\\Myron\\eQTLs\\GEO DataSet GSE6536 new\\unzipped\\";
//	    filename = dir + filename;
	    try {
		    parse(filename);
//			String dir = "C:\\Documents and Settings\\npankrat\\My Documents\\gwas\\followUp\\GAK\\eQTL\\Duke\\";
//			filename = dir + filename;
		    if (new File(ext.rootOf(filename, false)+"_transcripts.xln").exists()) { 
		    	regress(ext.rootOf(filename, false)+"_transcripts.xln");
		    } else {
		    	System.out.println("Error - file '"+ext.rootOf(filename, false)+"_transcripts.xln"+"' not found; nothing to parse");
		    }
