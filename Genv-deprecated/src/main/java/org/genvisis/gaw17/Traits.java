...
//	public static final String[] POPS = {"all697"};
	public static final String[] POPS = {"CEPH", "Denver", "Han", "Japanese", "Luhya", "Tuscan", "Yoruban"};
//	public static final String[] PHENOS = {"aff", "q1", "q2", "q4"};
	public static final String[] PHENOS = {"q1"};
//	public static final String[] PHENOS = {"affected", "q1", "q2", "q4", "AffResids", "Aff_minusQ1Q4", "Aff_minusQ1Q2Q4", "Q1resids", "Q1residsMinusQ2", "Q2residsQ1", "Q4resids"};
//	public static final String[] PHENOS = {"AffResids", "Aff_minusQ1Q4", "Aff_minusQ1Q2Q4", "Q1resids", "Q1residsMinusQ2", "Q2residsQ1", "Q4resids"};
//	public static final String[][] COVARS = {{"SEX", "AGE", "SMOKE", "C1", "C2"}, {"SEX", "AGE", "SMOKE", "C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9", "C10"}};
	public static final String[][] COVARS = {{"SEX", "SMOKE"}, {"SEX", "SMOKE"}, {}, {"SEX", "AGE", "SMOKE"}};
		...
		try {
			...
			for (int rep = 1; rep <= 200; rep++) {
				...
				try {
					...
					writer.print("\t"+Array.toStr(Array.subArray(new LogisticRegression(deps[0], indeps).getSigs(), 1, 4)));
					writer.print("\t"+Array.toStr(Array.subArray(new LeastSquares(deps[1], indeps).getSigs(), 1, 4)));
					writer.print("\t"+Array.toStr(Array.subArray(new LeastSquares(deps[2], indeps).getSigs(), 1, 4)));
					writer.print("\t"+Array.toStr(Array.subArray(new LeastSquares(deps[3], indeps).getSigs(), 1, 4)));
					writer.println();
					writer.flush();
					reader.close();
				} catch (FileNotFoundException fnfe) {
					System.err.println("Error: file \"" + "unr_phen."+rep + "\" not found in current directory");
					System.exit(1);
		...
		try {
			...
			for (int rep = 1; rep <= 200; rep++) {
				...
				try {
					...
					writer.print("\t"+Array.toStr(Array.subArray(new LogisticRegression(deps[0], indeps[0]).getSigs(), 1, 7)));
					writer.print("\t"+Array.toStr(Array.subArray(new LeastSquares(deps[1], indeps[1]).getSigs(), 1, 6)));
					writer.print("\t"+Array.toStr(Array.subArray(new LeastSquares(deps[2], indeps[2]).getSigs(), 1, 6)));
					writer.print("\t"+Array.toStr(Array.subArray(new LeastSquares(deps[3], indeps[3]).getSigs(), 1, 6)));
					writer.println();
					writer.flush();
					reader.close();
				} catch (FileNotFoundException fnfe) {
	...
	public static void convert(String pheno_dir) {
		...
		for (int rep = 1; rep <= 200; rep++) {
			try {
				...
				while (reader.ready()) {
					line = reader.readLine().trim().split(",");
					writer.println(line[0]+"\t"+line[0]+"\t"+line[1]+"\t"+line[2]+"\t"+line[3]+"\t"+line[4]+"\t"+line[5]+"\t"+line[6]+"\t"+(line[7].equals("1")?"2":"1"));
				}
				writer.close();
				reader.close();
			} catch (FileNotFoundException fnfe) {
				System.err.println("Error: file \"" + pheno_dir+"unr_phen."+rep + "\" not found in current directory");
				System.exit(1);
			} catch (IOException ioe) {
...
//					writer.println("plink --bfile ../"+POPS[i]+" --pheno "+pheno_dir+"unr_phen."+rep+".tab --pheno-name AFFECTED --covar "+pheno_dir+"unr_phen."+rep+".tab --covar-name SEX,SMOKE --logistic --ci 0.95 --out "+POPS[i]+".aff."+rep+"");
					writer.println("plink --bfile ../"+POPS[i]+" --pheno "+pheno_dir+"unr_phen."+rep+".tab --pheno-name Q1 --covar "+pheno_dir+"unr_phen."+rep+".tab --covar-name SEX,SMOKE --logistic --ci 0.95 --out "+POPS[i]+".q1."+rep+"");
//					writer.println("plink --bfile ../"+POPS[i]+" --pheno "+pheno_dir+"unr_phen."+rep+".tab --pheno-name Q2 --logistic --ci 0.95 --out "+POPS[i]+".q2."+rep+"");
//					writer.println("plink --bfile ../"+POPS[i]+" --pheno "+pheno_dir+"unr_phen."+rep+".tab --pheno-name Q4 --covar "+pheno_dir+"unr_phen."+rep+".tab --covar-name SEX,AGE,SMOKE --logistic --ci 0.95 --out "+POPS[i]+".q4."+rep+"");
				}
			}
			writer.close();
		} catch (Exception e) {
		...
		for (int i = 0; i < POPS.length; i++) {
			for (int pheno = 0; pheno < PHENOS.length; pheno++) {
				try {
					writer = new PrintWriter(new FileWriter(analysis_dir+"metal_"+POPS[i]+"_"+PHENOS[pheno]+".txt"));
					...
					writer.println("");
					for (int rep = 1; rep <= 200; rep++) {
						writer.println("PROCESS "+POPS[i]+".aff."+rep+".assoc."+(!PHENOS[pheno].startsWith("q")?"logistic":"linear"));
					}
					writer.println("");
					...
					System.err.println("Error writing to " + analysis_dir+"metal_"+POPS[i]+"_"+PHENOS[pheno]+".txt");
					e.printStackTrace();
				}
		...
		for (int pheno = 0; pheno < PHENOS.length; pheno++) {
			for (int rep = 1; rep <= 200; rep++) {
				try {
					writer = new PrintWriter(new FileWriter(analysis_dir+"metal_"+PHENOS[pheno]+"."+rep+".txt"));
					writer.println("MARKER SNP");
					writer.println("ALLELE A1 TEST");
					writer.println("EFFECT "+(!PHENOS[pheno].startsWith("q")?"OR":"BETA"));
					writer.println("STDERR SE");
					...
					writer.println("");
					for (int i = 0; i < POPS.length; i++) {
						writer.println("PROCESS "+POPS[i]+"."+PHENOS[pheno]+"."+rep+".assoc."+(!PHENOS[pheno].startsWith("q")?"logistic":"linear"));
					}
					writer.println("");
					...
					System.err.println("Error writing to " + analysis_dir+"metal_"+"_"+PHENOS[pheno]+"."+rep+".txt");
					e.printStackTrace();
				}
...
//				writer.println("GENOMICCONTROL ON");
		...
		for (int i = 0; i < POPS.length; i++) {
			hash = HashVec.loadFileToHashString(analysis_dir+POPS[i]+".frq", 1, new int[] {2,3}, "\t", true);
			...
			for (int rep = 1; rep <= 200; rep++) {
				...
				for (int pheno = 0; pheno < PHENOS.length; pheno++) {
					try {
						reader = new BufferedReader(new FileReader(analysis_dir+POPS[i]+"."+PHENOS[pheno]+"."+rep+".assoc."+(!PHENOS[pheno].startsWith("q")?"logistic":"linear")));
						writer = new PrintWriter(new FileWriter(analysis_dir+POPS[i]+"."+PHENOS[pheno]+"."+rep+".in"));
//						writer.println("SNP\tA1\tA2\tbeta\tse");
						...
						while (reader.ready()) {
							...
							if (!line[1].equals(snps[count])) {
								System.err.println("Error - out of sync in "+POPS[i]+".aff."+rep+".assoc."+(!PHENOS[pheno].startsWith("q")?"logistic":"linear")+" expecting "+snps[count]+" found "+line[1]);
							}
							if (!line[11].equals("NA")) {
								...
								if (alleles == null) {
									...
									System.err.println("Error - wrong A1 for "+line[1]+" found "+line[3]+" expecting "+alleles);
								}
...
//								writer.println(line[1]+"\t"+alleles+"\t"+line[6]+"\t"+line[11]+"\t"+(Double.parseDouble(line[6])<0?"-":"+")+"\t"+line[5]);
								writer.println(line[1]+"\t"+alleles+"\t"+line[6]+"\t"+line[11]+"\t"+"\t"+line[5]);
							}
						...
						System.err.println("Error: file \"" + analysis_dir+POPS[i]+"."+PHENOS[pheno]+"."+rep+".assoc."+(!PHENOS[pheno].startsWith("q")?"logistic":"linear") + "\" not found in current directory");
						System.exit(1);
					} catch (IOException ioe) {
						System.err.println("Error reading file \"" + analysis_dir+POPS[i]+"."+PHENOS[pheno]+"."+rep+".assoc."+(!PHENOS[pheno].startsWith("q")?"logistic":"linear") + "\"");
						System.exit(2);
					}
				}
			}
	...
	public static void addMDS(String pheno_dir, String mds_file, int numComps) {
		...
		for (int rep = 1; rep <= 200; rep++) {
//			Files.combine(HashVec.loadFileToStringArray(pheno_dir+"unr_phen."+rep, false, true, new int[] {0}, true, false, ","), new String[] {pheno_dir+"unr_phen."+rep+" 0 0=IID 4=Q1 1=SEX 2=AGE 3=SMOKE ,", mds_file+" 0 3=C1 4=C2"}, "FID", pheno_dir+"pheno_C2."+rep, new Logger(), false);
//			Files.combine(HashVec.loadFileToStringArray(pheno_dir+"unr_phen."+rep, false, true, new int[] {0}, true, false, ","), new String[] {pheno_dir+"unr_phen."+rep+" 0 0=IID 4=Q1 1=SEX 2=AGE 3=SMOKE ,", mds_file+" 0 3=C1 4=C2 5=C3 6=C4 7=C5 8=C6 9=C7 10=C8 11=C9 12=C10"}, "FID", pheno_dir+"pheno_C10."+rep, new Logger(), false);
			Files.combine(HashVec.loadFileToStringArray(pheno_dir+"unr_phen."+rep, false, true, new int[] {0}, true, false, ","), new String[] {pheno_dir+"unr_phen."+rep+" 0 0=IID 7=Affected 4=Q1 5=Q2 1=SEX 2=AGE 3=SMOKE ,", mds_file+comps}, "FID", pheno_dir+"pheno_C"+numComps+"."+rep, new Logger(), false);
		}
	}

	public static void normPheno(String pheno_dir, String output_dir, String filename_root, int column, int normType) {
		output_dir = ext.verifyDirFormat(output_dir);
		new File(output_dir).mkdirs();
		for (int rep = 1; rep <= 200; rep++) {
			Transformations.transformFile(pheno_dir+filename_root+"."+rep, output_dir+filename_root+"."+rep, true, column, false, true, normType, new Logger());
		}
	}
...
//		Files.qsub("C2", 1, 200, "/home/npankrat/bin/plink --bfile all697 --pheno phenos/pheno_C10.# --pheno-name Q1 --covar phenos/pheno_C10.# --covar-name SEX,AGE,SMOKE,C1,C2 --linear --ci 0.95 --out C2.#");
//		Files.qsub("C10", 1, 200, "/home/npankrat/bin/plink --bfile all697 --pheno phenos/pheno_C10.# --pheno-name Q1 --covar phenos/pheno_C10.# --covar-name SEX,AGE,SMOKE,C1,C2,C3,C4,C5,C6,C7,C8,C9,C10 --linear --ci 0.95 --out C10.#");
		Files.qsub("C"+numComps, 1, 200, "/home/npankrat/bin/plink --bfile all697 --pheno phenos/"+rootPheno+".# --pheno-name "+trait+" --covar phenos/"+rootPheno+".# --covar-name SEX,AGE,SMOKE"+comps+" --linear --ci 0.95 --out C"+numComps+".#");
	}
			...
			try {
				...
				while (reader.ready()) {
					...
					if (line[4].equals("ADD")) {
						if (!line[1].equals(snps[count])) {
							System.err.println("Error - out of sync in "+prefix+"."+rep+".assoc."+method+" expecting "+snps[count]+" found "+line[1]);
						}
						if (!line[11].equals("NA")) {
							if (Double.parseDouble(line[11]) < cutoff) {
								counts[count][0]++;
...
//						}
				...
				System.err.println("Error: file \"" + prefix+"."+rep+".assoc."+method+"\" not found in current directory");
//				System.exit(1);
			} catch (IOException ioe) {
...
//				System.exit(2);
		...
		try {
			...
			for (int j = 0; j < snps.length; j++) {
				writer.println(snps[j]+"\t"+counts[j][0]+"\t"+counts[j][1]+"\t"+ext.formDeci((double)counts[j][0]/(double)counts[j][1]*100, 1)+"%");
			}
			writer.close();
		} catch (Exception e) {
...
//					count++;
		...
		try {
			...
			for (int j = 0; j < snps.length; j++) {
				...
				writer.println(snps[j]+"\t"+(index == -1?0:counts[index])+"\t200\t"+ext.formDeci((double)(index == -1?0:counts[index])/(double)200*100, 1)+"%");
			}
			writer.close();
		} catch (Exception e) {
	...
	public static void splitPops(String analysis_dir) {
		for (int i = 0; i < POPS.length; i++) {
			CmdLine.run("plink --bfile all697 --keep data/"+POPS[i]+".txt --make-bed --out "+POPS[i], analysis_dir);
			CmdLine.run("plink --bfile "+POPS[i]+" --freq --out "+POPS[i], analysis_dir);
		}
	}
	
	public static void analyzeDistribution(String pheno_dir, String pheno_root, int col, boolean commaDelimited, double[] stddevThresholds, String output_file) {
		PrintWriter writer;
		String[] ids;
		double[] values;
		int[][] counts;
		...
		try {
			...
			writer.println();
			for (int i = 0; i < ids.length; i++) {
				writer.println(ids[i]+"\t"+Array.mean(matrix[i])+"\t"+Array.stdev(matrix[i])+"\t"+Array.toStr(counts[i]));
			}
			writer.close();
		} catch (Exception e) {
...
//			analyzeDistribution(pheno_dir, "unr_phen", 1, true, new double[] {1, 2, 3, 4, 5, 6, 7}, "Sex_distribution.xln");
//			analyzeDistribution(pheno_dir, "unr_phen", 2, true, new double[] {1, 2, 3, 4, 5, 6, 7}, "Age_distribution.xln");
			analyzeDistribution(pheno_dir, "unr_phen", 3, true, new double[] {1, 2, 3, 4, 5, 6, 7}, "Smoke_distribution.xln");
			analyzeDistribution(pheno_dir, "unr_phen", 4, true, new double[] {1, 2, 3, 4, 5, 6, 7}, "Q1_distribution.xln");
			analyzeDistribution(pheno_dir, "unr_phen", 5, true, new double[] {1, 2, 3, 4, 5, 6, 7}, "Q2_distribution.xln");
			analyzeDistribution(pheno_dir, "unr_phen", 6, true, new double[] {1, 2, 3, 4, 5, 6, 7}, "Q5_distribution.xln");
			analyzeDistribution(pheno_dir, "unr_phen", 7, true, new double[] {1, 2, 3, 4, 5, 6, 7}, "Aff_distribution.xln");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
