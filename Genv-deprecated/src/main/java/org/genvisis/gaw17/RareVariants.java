    ...
    for (int rep = 1; rep <= 1; rep++) {
      ...
      for (int j = 0; j < Traits.PHENOS.length; j++) {
        ...
        hash = HashVec.loadFileToHashString(analysis_dir + filename, new int[] {0, 1},
                                            ext.indexFactors(new String[] {Traits.PHENOS[j]},
        ...
        for (int i = 0; i < ids.length; i++) {
          ...
          if (trav == null) {
            System.err.println("Error - no pheno '" + Traits.PHENOS[j] + "' for indiviudal "
                               + ids[i][0] + "," + ids[i][1]);
            System.exit(1);
          }
        ...
        for (int k = 0; k < markerSets.length; k++) {
          try {
            ...
            System.err.println("Error writing to " + analysis_dir + pop + "." + rep + "."
                               + Traits.PHENOS[j] + ".maf" + maf + "." + markerSets[k]);
            e.printStackTrace();
          }
        }
      }
        ...
        for (int j = 0; j < Traits.PHENOS.length; j++) {
          for (int k = 0; k < markerSets.length; k++) {
            for (int i = 0; i < Traits.POPS.length; i++) {
              writer.println("mid_" + Traits.POPS[i] + "." + 1 + "." + Traits.PHENOS[j] + ".maf"
                             + MAFS[maf] + "." + markerSets[k] + " 0 3=" + Traits.POPS[i] + "." + 1
                             + "." + Traits.PHENOS[j] + ".maf" + MAFS[maf] + "." + markerSets[k]);
            }
          }
        }
      }
