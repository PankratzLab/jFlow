...
public class ExtractPlots {
  ...
  public static final String PLOT_HEADER =
      "Sample\tGC Score\tTheta\tR\tX\tY\tX Raw\tY Raw\tB Allele Freq\tLog R Ratio\tAlleleCalls\tAlleleCount";
  public static final String MARKERS_IN_INDEX_ORDER = "markersInIndexOrder.dat";
  public static final String[] PLOT_NEEDS =
      {"GC Score", "Theta", "R", "X", "Y", "X Raw", "Y Raw", "B Allele Freq", "Log R Ratio",
       "Allele1 - Forward", "Allele2 - Forward", "Allele1 - AB", "Allele2 - AB"};
  ...
  public static void extractPlotsInMemoryAndIndexed(String filename) {
    ...
    try {
      for (int i = 0; i < numSamples; i++) {
        try {
          ...
          for (int j = 0; j < numMarkers; j++) {
            ...
            if (ids[i].equals("")) {
              ids[i] =
                  pdgwas ? line[sampleNameIndex].substring(0, line[sampleNameIndex].indexOf("@"))
      ...
      for (int i = 0; i < numMarkers; i++) {
        writer =
            new PrintWriter(new FileWriter(translateName(markers.elementAt(markerNameIndices[i]))
                                           + "_plots.xls"));
