...
public class Incipient implements ActionListener, KeyListener, MenuListener {
  public class Word {
      ...
      for (int qORa = 0; qORa < 2; qORa++) {
        ...
        if (menuTaggedOption.getState() && score != 1) {
          ...
          if (menuHighErrOption.getState() && history.size() > 0
              && !(((TimePoint) history.elementAt(history.size() - 1)).sameDay(today)
                   || !((TimePoint) history.elementAt(history.size() - 1)).caughtUp())) {
    ...
    public String toString() {
      ...
      for (int i =
          (qHistory.size() > MAXHIST) ? qHistory.size() - MAXHIST : 0; i < qHistory.size(); i++) {
        stringToReturn += "|" + (TimePoint) qHistory.elementAt(i);
      }
      stringToReturn += "|<-Q and A->";
      for (int i =
          (aHistory.size() > MAXHIST) ? aHistory.size() - MAXHIST : 0; i < aHistory.size(); i++) {
  ...
  private int[] previousWordAction = {-1, -1, -1, -1, -1}; // {word's hashNum, skipped?, wrong?, in
                                                           // wrongHistory?}
  ...
  public void makeMenu(JFrame frame) {
    ...
    menuUndoItem = (JMenuItem) createMenuItem(editMenu, UNDO, KeyEvent.VK_U,
                                              "Backup one word and any undo changes to score",
                                              KeyEvent.VK_Z, false);
    ...
    menuRefreshOption =
        (JCheckBoxMenuItem) createMenuItem(optionsMenu, REFRESH, KeyEvent.VK_R,
                                           "Add 1 point per week not seen", KeyEvent.VK_R, true);
    ...
    menuBrandNewOption = (JCheckBoxMenuItem) createMenuItem(optionsMenu, BRANDNEW, KeyEvent.VK_B,
                                                            ...
                                                            KeyEvent.VK_B, true);
    menuTaggedOption =
        (JCheckBoxMenuItem) createMenuItem(optionsMenu, HIDETAGGED,
                                           KeyEvent.VK_T,
                                           ...
                                           KeyEvent.VK_T, true);
    menuBlenderOption =
        (JCheckBoxMenuItem) createMenuItem(optionsMenu, BLENDER, KeyEvent.VK_D,
                                           "Shuffles definitions", KeyEvent.VK_D, true);
    ...
    if (command.equals(SHOW_ANSWER)) {
      ...
      if (menuStatsOption.getState()) {
        if (wordStatsBox == null) {
          ...
          JLabel njl =
              new JLabel("Score = " + (currentQ2A ? currentWord.q_score : currentWord.a_score)
  ...
  public void saveData() {
    try {
      ...
      PrintWriter writer =
          new PrintWriter(new OutputStreamWriter(new FileOutputStream(DBFILE), "UnicodeLittle"));
      Word trav;
  ...
  public void loadData() {
    try {
      BufferedReader reader =
          new BufferedReader(new InputStreamReader(new FileInputStream(DBFILE), "UnicodeLittle"));
      ...
      while (reader.ready()) {
        ...
        if (trav.qHistory.size() == 0
            || (trav.qHistory.size() == 1
                && !((Word.TimePoint) trav.qHistory.elementAt(0)).caughtUp(RESET_Q_SCORE))) {
          brandNewQuestions = true;
        }
        if (trav.aHistory.size() == 0
            || (trav.aHistory.size() == 1
                && !((Word.TimePoint) trav.aHistory.elementAt(0)).caughtUp(RESET_A_SCORE))) {
  ...
  public void exportTo_iFlipr() {
    try {
      ...
      System.err.println("Saving database...");
      PrintWriter writer =
          new PrintWriter(new OutputStreamWriter(new FileOutputStream(DBFILE + ".out"),
                                                 "UnicodeLittle"));
      Word trav;
