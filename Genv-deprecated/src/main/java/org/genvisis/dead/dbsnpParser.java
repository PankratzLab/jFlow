	...
	public static Hashtable<String,String[]> getHash(String filename) {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (reader.ready()) {
					if (temp.equals("")) {
						hash.put(trav = reader.readLine().split("[\\s]+")[0], info = Array.stringArray(9, "-1"));
					}
					line = temp.split("\\|");
					...
					if (temp.indexOf("assembly=reference")>0||temp.indexOf("assembly=Celera")>0) {
						...
						if (line[2].startsWith(" chr=")) {
							...
							if (info[3+offset].equals("-1")&&!line[2].equals("?")) {
								...
								System.err.println("Error - chromosomes don't agree between Celera/reference for marker "+trav+" ("+info[3+offset]+" and "+line[2]+")");
							}
						} else {
							System.err.println("Strange assembly="+(offset==1?"Celera":"reference")+" line (no chr): '"+temp+"'");
						}
						offset = (temp.indexOf("assembly=Celera")>0?2:0)+(temp.indexOf("chr=Y")>0?1:0);
						if (line[3].startsWith(" chr-pos=")) {
							...
							if (info[5+offset].equals("-1")&&!line[3].equals("?")) {
								...
								System.err.println("Error - more than one "+(offset==1?"Celera":"reference")+" chr-position for marker "+trav+" ("+info[5+offset]+" and "+line[3]+")");
							}
						} else {
							System.err.println("Strange assembly="+(offset==1?"Celera":"reference")+" line (no chr-pos): '"+temp+"'");
						}
					}
				}
			}
			...
			System.err.print("Marker position data source \""+filename+"\" not found in current directory");
