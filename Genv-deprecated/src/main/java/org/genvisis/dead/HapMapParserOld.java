...
public class HapMapParserOld {
	...
	public static void parse(String dir, String filename, String famstruct) {
		...
		try {
			...
			writer = new PrintWriter(new FileWriter((new File(dir).exists()?dir:"")+root+".info"));
			while (reader.ready()) {
		...
		try {
			...
			while (reader.ready()) {
				...
				if (index==-2) {
					System.err.println("Error - Could not find sample "+trav+" from the pedigree file in the genotype file");
				}
