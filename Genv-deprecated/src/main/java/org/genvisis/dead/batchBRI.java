...
public class batchBRI {
	...
	public static final String[] SNPS = {"haplotype-111X", "haplotype-222X", "haplotype-X1X2",
																			 "haplotype-X2X1"};
	...
	public batchBRI(String jarPath, String genoPath) throws IOException {
		...
		for (int i = 0; i < SNPS.length; i++) {
			...
			for (String element : CUT3) {
				...
				writer.println("java -jar " + jarPath + " park.countBRI file=" + genoPath + "BRI3-"
											 + SNPS[i] + "-cases.dat" + opReps);
		...
		for (int i = 0; i < SNPS.length; i++) {
			writer.println("cd " + SNPS[i]
										 + SNP_POS[i].replaceAll("\\(", "\\\\(").replaceAll("\\)", "\\\\)"));
			writer.println("cd plates_1-10");
			for (String element : CUT3) {
				writer.println("cd " + element);
	...
	public static void main(String[] args) throws IOException {
		...
		for (String arg : args) {
			if (arg.equals("-h") || arg.equals("-help") || arg.equals("/h") || arg.equals("/help")) {
				...
				jarpath = arg.split("=")[1];
				numArgs--;
			} else if (arg.startsWith("genos=")) {
				genopath = arg.split("=")[1];
				numArgs--;
			}
		}
		if (numArgs != 0) {
		...
		System.out.println("Making a batch file utilizing " + jarpath + " and the files in "
											 + genopath);
		try {
