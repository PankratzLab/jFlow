...
public class createAssociation {
	public createAssociation(String filename) throws IOException {
		...
		try {
			...
			writer = new PrintWriter(new FileWriter(filename.substring(0, filename.indexOf("."))+".pre"));
			while (reader.ready()) {
				line = reader.readLine().split("[\\s]+");
				for (int i = 0; i<6; i++) {
		...
		writer = new PrintWriter(new FileWriter(filename.substring(0, filename.indexOf("."))+"-map.dat"));
		writer.println((markers.length+1)+" 0 0 5  << NO. OF LOCI, RISK LOCUS, SEXLINKED (IF 1) PROGRAM");
