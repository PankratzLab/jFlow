...
public class FindMarkersNearGenes {
  ...
  public static final String GENES_ALT =
  ...
  public static void findMarkers(String dir, String filename, String root, double mafCutoff,
    ...
    try {
      ...
      for (int i = 0; i < markerNames.length; i++) {
        ...
        writer.println(markerNames[i] + "\t" + markerPositions.get(markerNames[i]) + "\t"
                       + (trav == null ? "??" : trav) + "\t" + geneLookup.get(markerNames[i]));
      }
      writer.close();
    } catch (IOException ioe) {
    ...
    try {
      ...
      while (!done) {
        ...
        if (done || Integer.parseInt(line[2]) > chr
          ...
          if (!done) {
            writer =
                new PrintWriter(new FileWriter("chr" + chr + ext.getExcelColumn(suffix) + ".txt"));
            count = 0;
          }
        }
        if (!done) {
  ...
  public static void main(String[] args) {
    ...
    String usage =
        "\n" + "park.gwa.FindNearestGenes requires 0-1 arguments\n" + "   (1) directory (i.e. dir="
                   + dir + " (default))\n" + "   (2) list of genes (i.e. file=" + filename
                   + " (default))\n" + "   (3) plink root for hapmap data (i.e. root=" + root
