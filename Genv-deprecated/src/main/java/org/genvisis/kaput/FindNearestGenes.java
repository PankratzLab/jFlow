	...
	public static void findNearest(String filename) {
		...
		for (int j = 0; j<markers.length; j++) {
			if (poslar.containsKey(markers[j])) {
				...
				System.err.println("Error - "+markers[j]+" was listed in "+filename+" but was not found in the map loaded ("+MAP+")");
				System.exit(1);
			}
		}
		try {
			...
			while (reader.ready()) {
				...
				if (queuePoslar.size()>0&&queuePoslar.elementAt(queuePoslar.size()-1)[0]!=chr_start_stop[0]) {
					queuePoslar.clear();
					queueNames.clear();
				}

				for (int j = 0; j<markers.length; j++) {
					if (genePositions[j][0]==chr_start_stop[0]&&genePositions[j][1]<chr_start_stop[1]&&genes[j].equals("")) {
						if (queuePoslar.size()==0) {
							...
							nearestPosition = (genePositions[j][1]<queuePoslar.elementAt(queuePoslar.size()-1)[2]?0:-1*(genePositions[j][1]-queuePoslar.elementAt(queuePoslar.size()-1)[2]));
							for (int i = 0; i<queuePoslar.size(); i++) {
								genes[j] += (genes[j].equals("")?"":"|")+queueNames.elementAt(i)+"["+(queuePoslar.elementAt(i)[2]-queuePoslar.elementAt(i)[1])+"] ("+(genePositions[j][1]<queuePoslar.elementAt(i)[2]?"within":"-"+(genePositions[j][1]-queuePoslar.elementAt(i)[2]))+")";
								if (genePositions[j][1]-queuePoslar.elementAt(i)[2]<Math.abs(nearestPosition)) {
									nearestMarker = queueNames.elementAt(i);
									nearestPosition = (genePositions[j][1]<queuePoslar.elementAt(i)[2]?0:-1*(genePositions[j][1]-queuePoslar.elementAt(i)[2]));
								}
						...
						for (int i = 0; i<WINDOW; i++) {
							...
							if (genePositions[j][0]==trav[0]) {
								genes[j] += "|"+travLine[1]+"["+(trav[2]-trav[1])+"] (+"+(trav[1]-genePositions[j][1])+")";
							}
						...
						genes[j] = nearestMarker+"("+(nearestPosition==0?"within":(nearestPosition<0?"-":"+")+ext.prettyUpDistance(Math.abs(nearestPosition), 0))+")\t"+genes[j];
					}
				}
		...
		try {
			writer = new PrintWriter(new FileWriter(DIR+filename.substring(0, filename.indexOf("."))+"_described.out"));
