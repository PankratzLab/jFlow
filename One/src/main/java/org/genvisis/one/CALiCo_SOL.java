...
public class CALiCo_SOL {
	...
	public static Vector<String> generateScripts(String commandFullPath, String[] methods, String phenoDir, String phenoNameExtOrFullPath, String genoDir, String genoNameExtOrFullPath, String probMatrixDir, String probMatrixNameExtOrFullPath, String outDir, String batchDir, Logger log) {
		PrintWriter writer;
		File[] genoFiles = null;
			...
			for (int j = 0; j < genoFiles.length; j++) {
				for (int k = 0; k < methods.length; k++) {
					for (int l = 0; l < probMatrixFiles.length; l++) {
						resultFullPath = outDir + "Pheno_" + phenoLabels[i] + "_Geno_" + genoLabels[j] + "_Method_" + methods[k] + "_PMatrix_" + probMatrixLabels[l];
						script.add(commandFullPath + " --METHOD " + methods[k] + " --PHENO " + phenoFiles[i].getAbsolutePath() + " --GENO " + genoFiles[j].getAbsolutePath() + " --PROBMATRIX " + probMatrixFiles[l].getAbsolutePath() + " --OUT " + resultFullPath + ".out --LOG " + resultFullPath + ".log --CAF 0.01 --TRIM 0.02 --TRIM2 0.0004");
					}
				}
			}
		}
	...
	public static void generateQsubs(String commands, String root, String[][] iterations, Logger log) {
		Files.qsub(root, commands, iterations);
	}

//	public static void generateQsubMultiple(Vector<String> jobNamesWithAbsolutePaths, String batchDir, String modelName, int qsubMemInMBs, double qsubWalltimeInHours, Logger log) {
//		Files.qsubMultiple(jobNamesWithAbsolutePaths, null, batchDir, batchDir+"chunk_"+modelName, 8, true, null, -1, qsubMemInMBs, qsubWalltimeInHours);
//		Files.qsubMultiple(jobNamesWithAbsolutePaths, null, batchDir, batchDir+"chunkSB256_"+modelName, 16, true, "sb256", -1, qsubMemInMBs, qsubWalltimeInHours);
//		Files.qsubMultiple(jobNamesWithAbsolutePaths, null, batchDir, batchDir+"chunkSB_"+modelName, 16, true, "sb", -1, qsubMemInMBs, qsubWalltimeInHours);
//	}
	...
	public static void generateQsubMultiple(Vector<String> jobNamesWithAbsolutePaths, String batchDir, String modelName, int qsubMemInMBs, double qsubWalltimeInHours, Logger log) {
		...
		Files.qsubMultiple(jobNamesWithAbsolutePaths, jobSizes, batchDir, batchDir+"chunk_"+modelName, 8, true, null, -1, qsubMemInMBs, qsubWalltimeInHours);
		Files.qsubMultiple(jobNamesWithAbsolutePaths, jobSizes, batchDir, batchDir+"chunkSB256_"+modelName, 16, true, "sb256", -1, qsubMemInMBs, qsubWalltimeInHours);
		Files.qsubMultiple(jobNamesWithAbsolutePaths, jobSizes, batchDir, batchDir+"chunkSB_"+modelName, 16, true, "sb", -1, qsubMemInMBs, qsubWalltimeInHours);
	}

	/**
	 * List all the files in a directory with the file name extension specified.
	 * Alternatively, you can leave the directory as null, and specify a full path
	 * in filenameOrNameExtToList, then the method will check whether the file
	 * specified by exists.
	 * @param directory The directory to list files in.
	 * @param filenameOrNameExtToList There ways to use this parameter:
	 *   1) leave as null, then you will get list of all the files;
	 *   2) specify some file name extension, then you will get list of the files with the file name extension;
	 *   3) specify the full path of a file while leave directory as null, then you will know whether the file exists. 
	 * @param log
	 * @return
	 * @throws Elision 
	 */
	public static File[] getListOfFilesInADirectory(String directory, final String filenameOrNameExtToList, Logger log) throws Elision {
		File[] phenoFiles = null;
...
//				for (int j = 0; j < len; j++) {
//					isFound = false;
//					if(!(probMatrixName2.substring(j, j + strLength).equals(probMatrixName1.substring(j, j + strLength)))) {
//						if(i == 1) {
...
//				probMatrixLabels[i] = ext.rootOf(probMatrixFullPathes[i].getName()).toLowerCase();
			...
			for (int i = 0; i < (probMatrixLabels.length - 1); i++) {
				...
				for (int j = (i + 1); j < probMatrixLabels.length; j++) {
					...
					for (int k = 0; k < len; k++) {
						if(!(probMatrixLabels[i].substring(k, k + strLength).equals(probMatrixLabels[j].substring(k, k + strLength))) && k < start) {
							start = k;
							break;
						}
					}
			...
			for (int i = 0; i < probMatrixLabels.length; i++) {
				probMatrixLabels[i] = probMatrixLabels[i].substring(startMin, Math.min(startMax, probMatrixLabels[i].length()));
			}
		}
...
//		batchDir = "/home/pankrat2/shared/CALiCo_SOL/T2DM/batches/";
		...
		"   (2) methods of analysis (i.e. methods=" + methods[0] + ";" + methods[1] + " (add additional ones separated by ',' or ';' with no space) (default))\n" +
		"   (3) directory of the pheno files (i.e. phenodir=" + phenoDir + " (not default))\n" +
		"   (4) name extension of the pheno files, or full path of a specific pheno file (i.e. phenoext=" + phenoNameExtOrFullPath + " (not default))\n" +
		"   (5) directory of the pheno files (i.e. genodir=" + genoDir + " (not default))\n" +
		"   (6) name extension of the pheno files, or full path of a specific geno file (i.e. genoext=" + genoNameExtOrFullPath + " (not default))\n" +
		"   (7) directory of the probability matrix files (i.e. probmaxtrixdir=" + probMatrixDir + " (not default))\n" +
		"   (8) name extension of the probability matrix files, or full path of a specific probability matrix file (i.e. probmatrixext=" + probMatrixNameExtOrFullPath + " (not default))\n" +
		"   (9) directory of output files (i.e. outdir=" + outDir + " (not default))\n" +
		"   (10) directory of batch files (i.e. batchdir=" + batchDir + " (not default))\n" +
		"   (11) (optional) qsub memory size (i.e. qsubmem=" + qsubMemInMBs + " (default; in megabytes))\n" +
		"   (12) (optional) qsub walltime (i.e. qsubwalltime=" + qsubWalltimeInHours + " (default; in hours))\n" +
		"";
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equals("-h") || args[i].equals("-help") || args[i].equals("/h") || args[i].equals("/help")) {
				...
				numArgs--;
			} else if (args[i].startsWith("methods=")) {
				methods = args[i].split("=")[1].split("[,;]");
				numArgs--;
				...
				numArgs--;
			} else if (args[i].startsWith("genodir=")) {
				genoDir = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("genoext=")) {
				genoNameExtOrFullPath = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("probmatrixdir=")) {
				probMatrixDir = args[i].split("=")[1];
				numArgs--;
			} else if (args[i].startsWith("probmatrixext=")) {
				probMatrixNameExtOrFullPath = args[i].split("=")[1];
				numArgs--;
		...
		try {
			if(!new File(outDir).exists()) {
				if (! new File(outDir).mkdir()) {
					...
					log = new Logger(outDir + "Genvisis_CALiCo_SOL_" + (new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date())) + ".log");
					isExisted = true;
				}
			} else {
				log = new Logger(outDir + "Genvisis_CALiCo_SOL_" + (new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date())) + ".log");
			...
			jobNamesWithAbsolutePaths = generateScripts(commandFullPath, methods, phenoDir, phenoNameExtOrFullPath, genoDir, genoNameExtOrFullPath, probMatrixDir, probMatrixNameExtOrFullPath, outDir, batchDir, log);
			generateQsubMultiple(jobNamesWithAbsolutePaths, batchDir, "CALiCo_SOL_ver6", qsubMemInMBs, qsubWalltimeInHours, log);
